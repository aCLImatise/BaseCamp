...
inputs:
- id: in_info
  doc: Specify a semicolon-separated list of INFO IDs to output for each variant from
    the VCF file.
  type: File?
  inputBinding:
    prefix: --info
- id: in_genotype
  doc: Specify a colon-separated list of FORMAT IDs to output for each sample from
    the VCF file.
  type: File?
  inputBinding:
    prefix: --genotype
...
- id: in_pattern
  doc: "The pattern to use when generating output files. The default is \"%p-%s-%d\"\
    . Valid special patterns are:\n%p    Include the \"prefix\" here (either <filename>\
    \ or given in --prefix <prefix>\n%s    The name of the sample, taken from the\
