...
task Cdhitest {
  input {
    Boolean? input_filename_required
    ...
    Int? coverage_control_longer_set
    Float? coverage_shorter_set
    Int? coverage_control_shorter_set
  ...
  command <<<
    cd_hit_est \
      ~{if (input_filename_required) then "-i" else ""} \
      ...
      ~{if defined(coverage_control_longer_set) then ("-AL " +  '"' + coverage_control_longer_set + '"') else ""} \
      ~{if defined(coverage_shorter_set) then ("-aS " +  '"' + coverage_shorter_set + '"') else ""} \
      ~{if defined(coverage_control_shorter_set) then ("-AS " +  '"' + coverage_control_shorter_set + '"') else ""} \
      ...
      ~{if (do_do_alignmentsif) then "-r" else ""} \
  ...
  parameter_meta {
    input_filename_required: "input filename in fasta format, required"
    ...
    coverage_control_longer_set: "coverage control for the longer sequence, default 99999999\\nif set to 60, and the length of the sequence is 400,\\nthen the alignment must be >= 340 (400-60) residues"
    coverage_shorter_set: "coverage for the shorter sequence, default 0.0\\nif set to 0.9, the alignment must covers 90% of the sequence"
    coverage_control_shorter_set: "coverage control for the shorter sequence, default 99999999\\nif set to 60, and the length of the sequence is 400,\\nthen the alignment must be >= 340 (400-60) residues"
    ...
    do_do_alignmentsif: "1 or 0, default 1, by default do both +/+ & +/- alignments\\nif set to 0, only +/+ strand alignment"
