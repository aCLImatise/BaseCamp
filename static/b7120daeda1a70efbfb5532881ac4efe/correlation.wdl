...
task Correlation {
  ...
  command <<<
    correlation \
      ...
      ~{if (arg_pearson_correlation) then "-m" else ""} \
      ...
      ~{if (arg_methodscorestsvoutput_file) then "-o" else ""} \
  ...
  parameter_meta {
    ...
    arg_pearson_correlation: "[ --method ] arg (=pearson)        Correlation method"
    ...
    arg_methodscorestsvoutput_file: "[ --outfile ] arg (=<method>_scores.tsv)\\nOutput file path"
  ...
  output {
    ...
    File out_arg_methodscorestsvoutput_file = "${in_arg_methodscorestsvoutput_file}"
  }
}