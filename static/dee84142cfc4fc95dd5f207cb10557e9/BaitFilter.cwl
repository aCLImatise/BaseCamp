...
id: BaitFilter.cwl
inputs:
- id: in_input_bait_file_name
  doc: "(required)  Name of the input bait locus file. This is the bait file\nobtained\
    \ from the Bait-Fisher program."
  type: File?
  inputBinding:
    prefix: --input-bait-file-name
- id: in_output_bait_file_name
  doc: "Name of the output bait file. This is the file the contains the\nfiltered\
    \ bait loci and the baits."
  type: File?
  inputBinding:
    prefix: --output-bait-file-name
- id: in_convert
  doc: "Allows the user to produce the final output file for the bait\nproducing company.\
    \ In this mode, BaitFilter reads the input bait file\nand instead of doing a filtering\
    \ step, it produces a costumn bait file\nthat can be uploaded to the baits producing\
    \ company. In order to avoid\nconfuction a filtering step cannot be done in the\
    ...
    \ \"Agilent-Germany\".\nNew output formats can be added upon request. Please contact\
    \ the\nauthor: Christoph Mayer, Email: Mayer Christoph\n<c.mayer.zfmk@uni-bonn.de>"
  type: File?
  inputBinding:
    prefix: --convert
- id: in_mode
  doc: "Appart form the input file this is the most essential option. This\noption\
    \ specifies which filter mode Bait-Filter uses. (See the user\nmanual for more\
...
- id: in_blast_executable
  doc: "Name of or path+name to the blast executable. Default: nblast. Minimum\nversion\
    \ number: Blast+ 2.2.x"
  type: long?
  inputBinding:
    prefix: --blast-executable
- id: in_thinning_step_width
  doc: "Thin out the bait file by retaining only every Nth bait region. This\noption\
    \ specified the step width N. If one of the moded thin-b, thin-s\nis active, this\
    \ option is required, otherwise it is not allowed to set\nthis parameter."
  type: File?
  inputBinding:
...
- id: in_verbosity
  ...
  inputBinding:
    prefix: --verbosity
- id: in_stats
  doc: "Compute stats for the input file and report these. This mode is\nautomatically\
    \ used for all modes specified with -m or the conversion\nmode specified with\
    \ -c.The purpose of the -S option is to compute\nstats without having to filter\
    \ or convert the input file. In\nparticular, the -S mode does not require specifying\
...
- id: out_stdout
  ...
  type: stdout
- id: out_output_bait_file_name
  doc: "Name of the output bait file. This is the file the contains the\nfiltered\
    \ bait loci and the baits."
  type: File?
  outputBinding:
    glob: $(inputs.in_output_bait_file_name)
- id: out_ref_blast_db
  doc: "Base name to a blast data base file. This name is passed to the blast\ncommand.\
    \ This is the name of the fasta file of your reference genome.\nIMPORTANT: The\
    \ makeblastdb program has to be called before starting\nthe Bait-Filter program.\
...
- id: out_thinning_step_width
  doc: "Thin out the bait file by retaining only every Nth bait region. This\noption\
    \ specified the step width N. If one of the moded thin-b, thin-s\nis active, this\
    \ option is required, otherwise it is not allowed to set\nthis parameter."
  type: File?
  outputBinding:
    glob: $(inputs.in_thinning_step_width)
hints: []
cwlVersion: v1.1
baseCommand:
- BaitFilter
