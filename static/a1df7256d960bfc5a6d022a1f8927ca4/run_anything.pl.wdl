...
task RunAnythingpl {
  input {
    ...
    Boolean? verbose
    Boolean? version
    Boolean? print_product_continue
  ...
  command <<<
    run_anything_pl \
      ...
      ~{if defined(var_9) then ("-d " +  '"' + var_9 + '"') else ""} \
      ...
      ~{if (print_product_continue) then "-showversion" else ""} \
  ...
  parameter_meta {
    ...
    print_product_continue: "print product version to the error stream and continue"
