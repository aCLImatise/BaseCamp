...
task Cirdna {
  ...
  command <<<
    cirdna \
      ...
      ~{if (po_sticks) then "-posticks" else ""} \
      ~{if (pos_blocks) then "-posblocks" else ""} \
      ~{if (graph_out) then "-graphout" else ""} \
      ~{if (max_groups) then "-maxgroups" else ""} \
      ~{if (max_labels) then "-maxlabels" else ""} \
      ~{if (origin_angle) then "-originangle" else ""} \
  ...
  parameter_meta {
    ...
    pos_blocks: "selection  [In] Text inside or outside the blocks"
    ...
    origin_angle: "float      [90] Position in degrees of the molecule's\\norigin on the circle (Number from 0.000 to\\n360.000)"
    ...
    gap_size: "integer    [500] Interval between ticks in the ruler\\n(Integer 0 or more)"
    ...
    block_height: "float      [1.0] Height of blocks. Enter a number <1.0\\nor >1.0 to decrease or increase the size,\\nrespectively (Number 0.000 or more)"
