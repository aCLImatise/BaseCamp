...
task GtftkSplicingSite {
  input {
    Boolean? input_file
    File? output_file
    Boolean? exon_numbering_key
    Boolean? names
    Boolean? separator
    Boolean? verbosity
    File? no_date
  ...
  command <<<
    gtftk splicing_site \
      ...
      ~{if (exon_numbering_key) then "--exon-numbering-key" else ""} \
  ...
  parameter_meta {
    ...
    exon_numbering_key: "The name of the key containing the exon numbering (exon_number in ensembl) (default: exon_number)"
    names: "The key(s) that should be used as name. (default: exon_id,transcript_id,gene_id)"
