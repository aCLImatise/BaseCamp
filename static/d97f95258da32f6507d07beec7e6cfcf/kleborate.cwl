...
- id: in_kap_tive_k_outfile
  doc: "File for full Kaptive K locus output (default: do\nnot save Kaptive K locus\
    \ results to separate file)"
  type: File?
  inputBinding:
    prefix: --kaptive_k_outfile
- id: in_kap_tive_o_outfile
  doc: "File for full Kaptive O locus output (default: do\nnot save Kaptive O locus\
    \ results to separate file)"
  type: File?
  inputBinding:
    prefix: --kaptive_o_outfile
- id: in_min_identity
  doc: "Minimum alignment percent identity for main results\n(default: 90.0)"
  type: long?
  inputBinding:
    prefix: --min_identity
- id: in_min_coverage
  doc: "Minimum alignment percent coverage for main results\n(default: 80.0)"
  type: long?
  inputBinding:
    prefix: --min_coverage
- id: in_min_spurious_identity
  doc: "Minimum alignment percent identity for spurious\nresults (default: 80.0)"
  type: long?
  inputBinding:
    prefix: --min_spurious_identity
- id: in_min_spurious_coverage
  doc: "Minimum alignment percent coverage for spurious\nresults (default: 40.0)"
  type: long?
  inputBinding:
    prefix: --min_spurious_coverage
- id: in_min_kap_tive_confidence
  doc: "Minimum Kaptive confidence to call K/O loci -\nconfidence levels below this\
    \ will be reported as\nunknown (default: Good)"
  type: string?
  inputBinding:
    prefix: --min_kaptive_confidence
- id: in_force_index
  doc: "Rebuild the BLAST index at the start of execution\n(default: only build BLAST\
    \ indices when they are\nmissing)"
  type: boolean?
  inputBinding:
    prefix: --force_index
outputs:
...
- class: DockerRequirement
  dockerPull: quay.io/biocontainers/kleborate:2.0.4--py_0
cwlVersion: v1.1
baseCommand:
- kleborate
