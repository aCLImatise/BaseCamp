...
- id: in_set_variants_pass
  ...
  inputBinding:
    prefix: -S
- id: in_set_increase_stringency
  doc: 'If set, will increase stringency for candidate somatic: flag P0.01Likely and
    InDelLikely, and add filter P0.05'
  type: boolean?
  inputBinding:
    prefix: -M
...
- id: in_float_deprecatedthe_minimum
  doc: "float (0-1) # Deprecated\nThe minimum allele frequency difference between\
    \ two samples required in addition to p-value.  Not compatible\nwith -M option.\
...
- id: in_intthe_maximum_mean
  doc: "int\nThe maximum mean mismatches allowed.  Default: 5.25, or if a variant\
    \ is supported by reads with more than 5.25 mismatches, it'll be considered\n\
...
- id: in_minimum_allele_frequency_consider
  doc: "float\nThe minimum allele frequency to consider to be homozygous.  Default\
    ...
    below (-f) will be \"0/0\".\n-b  Path to the *.bed file which is used to generate\
    \ contigs in the header\n-G  Path to the *.fasta (*.fa) file which is used to\
    \ generate reference tag in the header"
  type: boolean?
  inputBinding:
    prefix: -F
outputs:
