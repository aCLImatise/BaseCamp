...
task Sortsnospl {
  ...
  command <<<
    sort_snos_pl \
      ...
      ~{if (same_start_bound) then "-r" else ""} \
  ...
  parameter_meta {
    ...
    same_start_bound: ":  same as -R, but only start bound must match to count as duplicate"
    ...
    sort__filter: ":  Don't sort -- just filter & output in same order"
    more_options: ""
    options: ""
    snorna_hits_file: ""
  }
