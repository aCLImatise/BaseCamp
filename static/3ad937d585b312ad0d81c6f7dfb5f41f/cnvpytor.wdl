...
task Cnvpytor {
  ...
  command <<<
    cnvpytor \
      ...
      ~{if defined(max_copy_number) then ("--max_copy_number " +  '"' + max_copy_number + '"') else ""} \
      ~{if defined(min_cell_fraction) then ("--min_cell_fraction " +  '"' + min_cell_fraction + '"') else ""} \
      ...
      ~{if (force_agg) then "--force_agg" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/cnvpytor:1.0--py38h3252c3a_1"
  }
  parameter_meta {
    ...
    min_count: "minimal count of haterozygous SNPs"
    ...
    max_copy_number: "maximal copy number"
    min_cell_fraction: "minimal cell fraction"
    ...
    force_agg: "Force Agg matplotlib backend"
