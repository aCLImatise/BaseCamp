...
task SOAPdenovo63merAll {
  ...
  parameter_meta {
    ...
    maxkmer_max_size: "maxKmer (max 63): maximum kmer size used for multi-kmer, [NO]"
    optional_merge_bubble: "(optional)  merge clean bubble before iterate, works only if -M is set when using multi-kmer, [NO]"
    max_kmer_size: "kmer_R2C(min 13, max 63): kmer size used for mapping reads to contigs, [K]"
