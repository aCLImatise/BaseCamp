...
- id: in_ref_path
  ...
  inputBinding:
    prefix: --ref-path
- id: in_input_folder
  doc: "Path to the folder containing the Raw GWASs summary\nstatistic files, must\
    \ end by '/'"
  type: Directory?
  inputBinding:
    prefix: --input-folder
- id: in_diagnostic_folder
  doc: "Path to the reporting information on the PreProcessing\nsuch as the SNPs sample\
    \ size distribution"
  type: Directory?
  inputBinding:
