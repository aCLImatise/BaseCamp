...
task RiboDensityAroundTripleteAAMotifs {
  input {
    File? bam_list_file
  ...
  command <<<
    RiboDensityAroundTripleteAAMotifs \
      ...
      ~{if defined(type_one) then ("--type1 " +  '"' + type_one + '"') else ""} \
      ~{if defined(type_two) then ("--type2 " +  '"' + type_two + '"') else ""} \
      ~{if defined(motif_list_one) then ("--motifList1 " +  '"' + motif_list_one + '"') else ""} \
      ~{if defined(motif_list_two) then ("--motifList2 " +  '"' + motif_list_two + '"') else ""} \
  ...
  parameter_meta {
    ...
    type_one: "The first motif you want to find. It must be a string\\nwith length equal to 2. e.g. PP,DD,KK motifs.\\ndefault=none."
    type_two: "The second motif you want to find. It must be a string\\nwith length equal to 3. e.g. PPP,DDD,KKK motifs.\\ndefault=none."
    motif_list_one: "The first motif list you want to find. Mutually\\nexclusive with '--type1' '--type2'. default=none"
    motif_list_two: "The second motif list you want to find. Mutually\\nexclusive with '--type1' '--type2'. default=none"
    ...
    transcript_fast_a: "Input the transcript fasta file used for motif\\ndetection and codon density calculation. cds sequences\\nrelated with coorFile (longest.trans.info.txt)\\ngenerated by GetProteinSequence.py"
