...
task Atoiindex {
  ...
  >>>
  parameter_meta {
    source_dir: "Directory where to read cmet index files (default is GMAP genome directory specified at compile time)"
    destdir: "Directory where to write cmet index files (default is value of -F, if provided; otherwise the value of the GMAP genome directory specified at compile time)"
    db: "Genome database"
    ...
    sampling: "Sampling to use in genome database.  If not specified, the program will find the smallest available sampling value in the genome database within selected basesize and k-mer size"
