...
task Testval {
  ...
  command <<<
    testval \
      ~{if (filename_input_optionaldefault) then "-i" else ""} \
  ...
  parameter_meta {
    ...
    filename_asn_optional: "Filename for asn.1 output [File Out]  Optional"
    ...
    continue_asn_error: "Continue on ASN.1 error? [T/F]  Optional\\ndefault = F"
  ...
  output {
    ...
    File out_filename_asn_optional = "${in_filename_asn_optional}"
  }
}