...
task RunDiscoSnp++ML.sh {
  input {
    String? path_compiled_directory
  ...
  command <<<
    run_discoSnp++_ML.sh \
      ~{if defined(path_compiled_directory) then ("-f " +  '"' + path_compiled_directory + '"') else ""} \
  ...
  >>>
  parameter_meta {
    path_compiled_directory: "path to compiled dsk (directory containing compiled dsk and dsk2ascii)"
