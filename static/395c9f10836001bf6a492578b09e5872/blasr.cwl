...
- id: no_split_sub_reads
  doc: Do not split subreads at adapters. This is typically only  useful when the
    ...
    sequence. For BAM input it reconstitutes full ZMW reads.
  type: boolean
  inputBinding:
    prefix: --noSplitSubreads
- id: polymerase
  doc: Instead of reconstituting ZMW reads, this option reconstitutes polymerase reads,
    omitting LQ regions. Polymerase reads are aligned, if at least one subread is
    present.
  type: boolean
  inputBinding:
    prefix: --polymerase
