...
task Gffcompare {
  input {
    ...
    String? matching_transfrags_outputie
    Int? discard_intronredundant_transfrags
    String? also_discard_contained
    String? discard_redundant_transfrag
  }
  command <<<
    gffcompare \
      ...
      ~{if defined(query_transfrags_ie) then ("-D " +  '"' + query_transfrags_ie + '"') else ""} \
      ...
      ~{if defined(generate_files_input) then ("-T " +  '"' + generate_files_input + '"') else ""} \
      ...
      ~{if defined(matching_transfrags_outputie) then ("-C " +  '"' + matching_transfrags_outputie + '"') else ""} \
      ~{if defined(discard_intronredundant_transfrags) then ("-A " +  '"' + discard_intronredundant_transfrags + '"') else ""} \
      ~{if defined(also_discard_contained) then ("-X " +  '"' + also_discard_contained + '"') else ""} \
      ~{if defined(discard_redundant_transfrag) then ("-K " +  '"' + discard_redundant_transfrag + '"') else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    query_transfrags_ie: "\\\"duplicate\\\" query transfrags (i.e. those with the same\\nintron chain) within a single sample (disable \\\"annotation\\\" mode)"
    ...
    generate_files_input: "not generate .tmap and .refmap files for each input file"
    ...
    matching_transfrags_outputie: "matching and \\\"contained\\\" transfrags in the GTF output\\n(i.e. collapse intron-redundant transfrags across all query files)"
    ...
    also_discard_contained: "also discard contained transfrags if transfrag ends stick out\\nwithin the container's introns"
    discard_redundant_transfrag: ", do NOT discard any redundant transfrag matching a reference"
  }
  output {
