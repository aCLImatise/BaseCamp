...
task Finestructure {
  input {
    ...
    Boolean? specifies_necessary_files
    Boolean? specifies_there_column
  ...
  command <<<
    finestructure \
      ...
      ~{if defined(hyperparameters_models_order) then ("-b " +  '"' + hyperparameters_models_order + '"') else ""} \
      ~{if defined(specify_type_accept) then ("-M " +  '"' + specify_type_accept + '"') else ""} \
      ...
      ~{if (specifies_necessary_files) then "-X" else ""} \
      ~{if (specifies_there_column) then "-Y" else ""} \
  ...
  parameter_meta {
    ...
    fix_number_set: "Fix the number of populations to whatever you started with.\\nThis would be set by '-I' or by an initial state file."
    ...
    specifies_necessary_files: "Specifies that there are row names in the data (not necessary for\\nChromoPainter or ChromoCombine style files.)"
    specifies_there_column: "Specifies that there are column names in the data file (as -X, not necessary.)"
