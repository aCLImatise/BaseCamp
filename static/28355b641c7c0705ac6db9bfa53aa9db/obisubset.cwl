...
id: ../../../../home/ubuntu/BiocondaCli/obisubset.cwl
inputs:
- id: skip
...
- id: sample
  ...
  inputBinding:
    prefix: --sample
- id: other_tag
  doc: Another tag to clean according to the sample subset
  type: string
  inputBinding:
    prefix: --other-tag
- id: sample_list
  doc: File containing the samples names (one sample id per line)
  type: File
  inputBinding:
    prefix: --sample-list
- id: sample_pattern
  doc: A regular expression pattern matching the sample ids to extract
  type: string
  inputBinding:
    prefix: --sample-pattern
- id: sample_name
  doc: A sample id to extract
  type: string
  inputBinding:
    prefix: --sample-name
outputs: []
