...
task Tn93 {
  ...
  command <<<
    tn93 \
      ...
      ~{true="-b" false="" bootstrap_alignment_columns} \
      ~{true="-r" false="" b_specified_supplied} \
      ~{true="-c" false="" only_count_pairs} \
      ~{true="-m" false="" compute_inter_} \
      ~{if defined(subsample_sequences_value) then ("-u " +  '"' + subsample_sequences_value + '"') else ""} \
      ~{true="-0" false="" report_distances_sequence} \
  ...
  parameter_meta {
    ...
    controls_format_set: "controls the format of the output unless -c is set (default=csv) csv: seqname1, seqname2, distance; csvn: 1, 2, distance; hyphy: {{d11,d12,..,d1n}...{dn1,dn2,...,dnn}}, where distances above THRESHOLD are set to 100;"
    ...
    sequence_name_form: "if sequence name is of the form 'somethingCOUNTS_IN_NAMEinteger' then treat the integer as a copy number when computing distance histograms (a character, default=':'):"
    ...
    bootstrap_alignment_columns: "bootstrap alignment columns before computing distances (default = false) when -s is supplied, permutes the assigment of sequences to files interacts with -r option"
    b_specified_supplied: "if -b is specified AND -s is supplied, using -r will bootstrap across sites instead of allocating sequences to 'compartments' randomly"
    only_count_pairs: "only count the pairs below a threshold, do not write out all the pairs "
    compute_inter_: "compute inter- and intra-population means suitable for FST caclulations only applied when -s is used to provide a second file"
    subsample_sequences_value: "subsample sequences with specified probability (a value between 0 and 1, default = 1.0)"
    report_distances_sequence: "report distances between each sequence and itself (as 0); this is useful to ensure every sequence in the input file appears in the output, e.g. for network construction to contrast clustered/unclustered"
