...
task Tally {
  ...
  command <<<
    tally \
      ...
      ~{if defined(gzipped_stream_default) then ("-o " +  '"' + gzipped_stream_default + '"') else ""} \
      ...
      ~{if defined(second_paired_end) then ("-j " +  '"' + second_paired_end + '"') else ""} \
      ...
      ~{if defined(turn_paired_end) then ("-v " +  '"' + turn_paired_end + '"') else ""} \
  ...
  parameter_meta {
    ...
    gzipped_stream_default: "(gzipped!) output stream (default out.tally.gz)"
