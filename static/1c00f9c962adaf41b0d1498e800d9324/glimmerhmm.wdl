...
task Glimmerhmm {
  ...
  command <<<
    glimmerhmm \
      ...
      ~{if defined(print_output_filename) then ("-o " +  '"' + print_output_filename + '"') else ""} \
      ~{if defined(print_top_best) then ("-n " +  '"' + print_top_best + '"') else ""} \
      ~{if (print_output_gff) then "-g" else ""} \
      ~{if (use_svm_predictions) then "-v" else ""} \
  ...
  parameter_meta {
    ...
    print_output_filename: "Print output in file_name; if n>1 for top best predictions, output is in file_name.1, file_name.2, ... , file_name.n f"
    print_top_best: "Print top n best predictions"
    print_output_gff: "Print output in gff format"
    use_svm_predictions: "Don't use svm splice site predictions"
