...
task SnpEff {
  ...
  command <<<
    snpEff \
      ...
      ~{if defined(fast_a_prot) then ("-fastaProt " +  '"' + fast_a_prot + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/snpeff:5.0--0"
  }
  parameter_meta {
    ...
    fast_a_prot: ": Create an output file containing the resulting protein sequences."
  ...
  output {
    ...
    File out_fast_a_prot = "${in_fast_a_prot}"
  }
}