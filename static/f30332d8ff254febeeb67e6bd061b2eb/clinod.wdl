...
task Clinod {
  input {
    File? in
    File? out
    Int? optional_defaults_medium
    Directory? optional_defaults_environmentalvariable
    Int? optional_defaults_number
    File? bm
    Boolean? non_ols
  ...
  command <<<
    clinod \
      ...
      ~{if defined(optional_defaults_environmentalvariable) then ("-d " +  '"' + optional_defaults_environmentalvariable + '"') else ""} \
      ~{if defined(optional_defaults_number) then ("-t " +  '"' + optional_defaults_number + '"') else ""} \
      ~{if defined(bm) then ("-bm " +  '"' + bm + '"') else ""} \
      ~{if (non_ols) then "-nonols" else ""} \
      ~{if (clean_sequence) then "-clean_sequence" else ""} \
