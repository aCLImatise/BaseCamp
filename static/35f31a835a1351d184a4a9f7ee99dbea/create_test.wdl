...
task CreateTest {
  ...
  command <<<
    create_test \
      ...
      ~{if (single_exe) then "--single-exe" else ""} \
      ...
      ~{if (allow_pnl) then "--allow-pnl" else ""} \
  ...
  parameter_meta {
    ...
    allow_pnl: "Do not pass skip-pnl to case.submit"
