...
task Bsmap {
  ...
  command <<<
    bsmap \
      ...
      ~{if (leave_output_bam) then "-U" else ""} \
  ...
  parameter_meta {
    ...
    leave_output_bam: "leave the output bam unsorted(reads will follow the order of the input fastq files), default=off"
