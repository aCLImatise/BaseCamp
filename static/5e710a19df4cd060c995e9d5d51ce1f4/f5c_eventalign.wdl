...
task F5cEventalign {
  input {
    ...
    File? summary
    Boolean? sam
    Boolean? print_read_names
    Boolean? scale_events
    Boolean? samples
    Boolean? signal_index
    Boolean? rna
    String? print_events
  ...
  command <<<
    f5c eventalign \
      ...
      ~{if (print_read_names) then "--print-read-names" else ""} \
      ~{if (scale_events) then "--scale-events" else ""} \
      ~{if (samples) then "--samples" else ""} \
      ~{if (signal_index) then "--signal-index" else ""} \
  ...
  parameter_meta {
    ...
    summary: "summarise the alignment of each read/strand in FILE"
    sam: "write output in SAM format"
    print_read_names: "print read names instead of indexes"
    scale_events: "scale events to the model, rather than vice-versa"
    samples: "write the raw samples for the event to the tsv output"
    signal_index: "write the raw signal start and end index values for the event to the tsv output"
