...
task SpacepharerPredictmatch {
  input {
    ...
    Float? tax_fdr
    Int? restrict_ranks_mode
    Int? rank_min_seq_ids
    Int? report_pam
    Int? perform_nucl_aln
    File? sub_mat
    Int? max_seq_len
    Int? db_load_mode
    Int? threads
  ...
  command <<<
    spacepharer predictmatch \
      ...
      ~{if defined(tax_fdr) then ("--tax-fdr " +  '"' + tax_fdr + '"') else ""} \
      ~{if defined(restrict_ranks_mode) then ("--restrict-ranks-mode " +  '"' + restrict_ranks_mode + '"') else ""} \
      ~{if defined(rank_min_seq_ids) then ("--rank-min-seq-ids " +  '"' + rank_min_seq_ids + '"') else ""} \
      ~{if defined(report_pam) then ("--report-pam " +  '"' + report_pam + '"') else ""} \
      ~{if defined(perform_nucl_aln) then ("--perform-nucl-aln " +  '"' + perform_nucl_aln + '"') else ""} \
  ...
  parameter_meta {
    ...
    kmer_length_set: "k-mer length (0: automatically set to optimum) [6]"
    k_score: "k-mer threshold for generating similar k-mer lists [2147483647]"
    alph_size: "Alphabet size (range 2-21) [nucl:5,aa:21]"
    max_seqs: "Maximum results per query sequence allowed to pass the prefilter (affects sensitivity) [0.000]"
    ...
    spaced_km_er_pattern: "User-specified spaced k-mer pattern [11011101]"
    ...
    add_string_convert: "Add backtrace string (convert to alignments with mmseqs convertalis module) [1]"
    ...
    list_matches_evalue: "List matches below this E-value (range 0.0-inf) [2.000E+02]"
    ...
    gap_open: "Gap open cost [nucl:10,aa:16]"
    gap_extend: "Gap extension cost [2]"
    ...
    min_length: "Minimum codon number in open reading frames [9]"
    ...
    lca_ranks: "Add column with specified ranks (',' separated) [superkingdom,phylum,class,order,family,genus,species]"
    ...
    tax_fdr: "FDR cutoff for taxonomy report [0.0, 1.0] [0.020]"
    restrict_ranks_mode: "0: disabled, 1: restrict taxonomic rank on target prediction by sequence identity [1]"
    ...
    report_pam: "Report protospacer adjacent motifs up and downstream of hits [1]"
    perform_nucl_aln: "0: perform only 6-frame translated search, 1:perform additional nucl-nucl alignment to the hits reported in 6-frame translated search, and update the E-value [1]"
    sub_mat: "Substitution matrix file [nucl:nucleotide.out,aa:VTML40.out]"
    max_seq_len: "Maximum sequence length [0.000]"
