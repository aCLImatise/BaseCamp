...
task HiliveOut {
  ...
  command <<<
    hilive-out \
      ~{true="-c" false="" path_hilive_config} \
      ...
      ~{true="-b" false="" arg_illuminas_basecalls} \
      ~{true="-t" false="" var_5} \
      ~{true="-T" false="" } \
      ~{true="-r" false="" example__reads} \
  ...
  >>>
  parameter_meta {
    path_hilive_config: "[ --config ] Path to a HiLive config file (in general, this should be 'hilive_config.ini' which is created in the temp directory of the respective run)"
    ...
    arg_illuminas_basecalls: "[ --bcl-dir ] arg              Illumina's BaseCalls directory which  contains the sequence information of the  reads."
    var_5: "[ --tiles ] arg                Specify the tiles to be considered for read alignment. [Default: [1-2][1-3][01-16] (96  tiles)]"
    ...
    example__reads: "[ --reads ] arg                Length and types of the read segments. Each segment is either a read ('R') or a barcode ('B'). Please give the segments in the  correct order as they are produced by the  sequencing machine. [REQUIRED] Example: --reads 101R,8B,8B,101R specifies paired-end sequencing with  2x101bp reads and 2x8bp barcodes."
    ...
    var_12: "[ --out-dir ] arg              Path to the directory that is used for the  output files. The directory will be created if it does not exist. [Default: ./out]"
    f: "[ --out-format ] arg           Format of the output files. Currently, SAM  and BAM format are supported. [Default:  BAM]"
    var_14: "[ --out-cycles ] arg           Cycles for that alignment output is  written. The respective temporary files are kept. [Default: write only after the last  cycle]"
    ...
    arg_path_hilive: "[ --index ] arg                Path to the HiLive index. Please use the  executable 'hilive-build' to create a new  HiLive index that is delivered with this  program. The index consists of several  files with the same prefix. Please include  the file prefix when specifying the index  location."
