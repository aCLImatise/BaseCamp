...
task CsvtkCsv2json {
  ...
  command <<<
    csvtk csv2json \
      ~{if (blanks) then "--blanks" else ""} \
      ~{if defined(indent) then ("--indent " +  '"' + indent + '"') else ""} \
      ~{if defined(key) then ("--key " +  '"' + key + '"') else ""} \
      ~{if defined(parse_num) then ("--parse-num " +  '"' + parse_num + '"') else ""} \
  ...
  parameter_meta {
    ...
    parse_num: "parse numeric values for nth column, multiple values are supported and \\\"a\\\"/\\\"all\\\" for all columns"
