...
task Happy {
  ...
  command <<<
    hap_py \
      ...
      ~{if (convert_gvc_f_truth) then "--convert-gvcf-truth" else ""} \
      ~{if (convert_gvc_f_query) then "--convert-gvcf-query" else ""} \
  ...
  parameter_meta {
    ...
    convert_gvc_f_truth: "Convert the truth set from genome VCF format to a VCF\\nbefore processing."
    convert_gvc_f_query: "Convert the query set from genome VCF format to a VCF\\nbefore processing."
