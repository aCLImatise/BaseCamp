...
task Tag2sort {
  ...
  command <<<
    tag2sort \
      ...
      ~{if defined(oslash_outdir) then ("-o/--outdir " +  '"' + oslash_outdir + '"') else ""} \
      ~{if (singletons) then "--singletons" else ""} \
      ~{if (hic) then "--hic" else ""} \
      ~{if (sam_tools_path) then "--samtools_path" else ""} \
      ~{if (conflicting_slash_weird) then "--conflicting/--weird" else ""}
  >>>
  parameter_meta {
    paired: "Paired-end mode. (Default: OFF)."
    oslash_outdir: "Write all output files into this directory. By default the output files will be written into\\nthe same folder as the input file(s)."
