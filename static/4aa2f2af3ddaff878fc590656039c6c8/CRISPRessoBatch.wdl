...
task CRISPRessoBatch {
  input {
    ...
    Boolean debugDebug
    Boolean noNoRerun
    String batchBatchSettings
    Boolean skipSkipFailed
    Int minMinReadsForInclusion
    String nNProcesses
    String batchBatchOutputFolder
  ...
  command <<<
    CRISPRessoBatch \
      ...
      ~{if defined(batchBatchSettings) then ("--batch_settings " +  '"' + batchBatchSettings + '"') else ""} \
      ~{true="--skip_failed" false="" skipSkipFailed} \
      ~{if defined(minMinReadsForInclusion) then ("--min_reads_for_inclusion " +  '"' + minMinReadsForInclusion + '"') else ""} \
      ~{if defined(nNProcesses) then ("--n_processes " +  '"' + nNProcesses + '"') else ""} \
      ~{if defined(batchBatchOutputFolder) then ("--batch_output_folder " +  '"' + batchBatchOutputFolder + '"') else ""} \
