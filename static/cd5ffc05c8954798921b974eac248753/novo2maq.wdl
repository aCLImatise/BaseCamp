...
task Novo2maq {
  ...
  command <<<
    novo2maq \
      ...
      ~{true="-r" false="" produces_short_report}
  >>>
  parameter_meta {
    ...
    in_dot_list: "is a list of reference sequence headers to be selected. This file servers to specify reference sequences to be selected for conversion to the MAQ map format and also allows translation of the header sequence. Each line in the file has format: <refheader> <tab> <replacement header> Any reference sequences not listed in this file will not be converted to MAQ map file. Use '-' rather than a filename to specify that all sequences are to be selected and converted."
