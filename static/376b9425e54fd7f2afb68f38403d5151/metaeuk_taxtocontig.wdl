...
task MetaeukTaxtocontig {
  input {
    ...
    Boolean taxTaxLineage
    Int lcaLcaMode
    Int taxTaxOutputMode
    Float majorityMajority
    String subSubMat
    Int maxMaxSeqLen
    Int dbDbLoadMode
    Int threadsThreads
  ...
  command <<<
    metaeuk taxtocontig \
      ...
      ~{if defined(lcaLcaRanks) then ("--lca-ranks " +  '"' + lcaLcaRanks + '"') else ""} \
      ~{if defined(blacklistBlacklist) then ("--blacklist " +  '"' + blacklistBlacklist + '"') else ""} \
      ~{if defined(taxTaxLineage) then ("--tax-lineage " +  '"' + taxTaxLineage + '"') else ""} \
      ~{if defined(lcaLcaMode) then ("--lca-mode " +  '"' + lcaLcaMode + '"') else ""} \
      ~{if defined(taxTaxOutputMode) then ("--tax-output-mode " +  '"' + taxTaxOutputMode + '"') else ""} \
      ~{if defined(majorityMajority) then ("--majority " +  '"' + majorityMajority + '"') else ""} \
