...
task Hmmemit {
  ...
  command <<<
    hmmemit \
      ...
      ~{if (emit_sequence_reqs) then "-C" else ""} \
      ~{if (sample_sequences_model) then "-p" else ""} \
  ...
  parameter_meta {
    ...
    emit_sequence_reqs: ": emit fancier consensus sequence (req's --minl, --minu)"
