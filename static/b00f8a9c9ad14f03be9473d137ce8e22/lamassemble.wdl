...
task Lamassemble {
  ...
  command <<<
    lamassemble \
      ...
      ~{if defined(seq_min) then ("--seq-min " +  '"' + seq_min + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/lamassemble:1.4.1--pyh3252c3a_0"
  }
  parameter_meta {
    ...
    seq_min: "omit consensus flanks with < S sequences (default=1)"
