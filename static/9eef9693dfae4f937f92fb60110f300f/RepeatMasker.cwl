...
inputs:
- id: in_pa
  doc: "(rallel) [number]\nThe number of sequence batch jobs [50kb minimum] to run\
    \ in parallel.\nRepeatMasker will fork off this number of parallel jobs, each\n\
    running the search engine specified. For each search engine\ninvocation ( where\
    \ applicable ) a fixed the number of cores/threads\nis used:\nRMBlast     4 cores\n\
    ABBlast     4 cores\nnhmmer      2 cores\ncrossmatch  1 core\nTo estimate the\
    \ number of cores a RepeatMasker run will use simply\nmultiply the -pa value by\
    \ the number of cores the particular search\nengine will use."
  type: boolean?
  inputBinding:
    prefix: -pa
...
- id: in_trf_prgm
  ...
  inputBinding:
    prefix: -trf_prgm
- id: in_sequence_dot
  doc: "-e(ngine) [crossmatch|wublast|abblast|ncbi|rmblast|hmmer]\nUse an alternate\
    \ search engine to the default. Note: 'ncbi' and\n'rmblast' are both aliases for\
    \ the rmblastn search engine engine.\nThe generic NCBI blastn program is not sensitive\
