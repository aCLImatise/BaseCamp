...
task SiteproBW {
  ...
  command <<<
    siteproBW \
      ...
      ~{true="--confid" false="" confid} \
      ~{if defined(name) then ("--name " +  '"' + name + '"') else ""} \
      ~{if defined(label) then ("--label " +  '"' + label + '"') else ""}
  >>>
  parameter_meta {
    bw: "input bigWIG file. Multiple bigWIG files can be given via -w (--bw) individually (eg -w WIG1.bw, -w WIG2.bw). WARNING! multiple bigwig and bed files are not allowed."
    ...
    confid: "If set, it will draw 95% confidence interval for each step."
    name: "Name of this run. If not given, the body of the bed file name will be used,"
    label: "Labels of the wig files. If given, they are used as the legends of the plot and in naming the TXT files of profile dumps; otherwise, the bigWIG file names will be used as the labels. Multiple labels can be given via -l (--label) individually (eg, -l LABEL1 -l LABEL2). WARNING! The number and order of the labels must be the same as the bigWIG files."
  }
}