...
task RunLSCpy {
  input {
    ...
    Int? foldername_required_inmode
    Directory? mode
  ...
  command <<<
    runLSC_py \
      ...
      ~{if defined(foldername_required_inmode) then ("--output " +  '"' + foldername_required_inmode + '"') else ""} \
  ...
  parameter_meta {
    ...
    foldername_required_inmode: "FOLDERNAME where output is to be written. Required in\\nmode 0 or 3. (default: None)"
