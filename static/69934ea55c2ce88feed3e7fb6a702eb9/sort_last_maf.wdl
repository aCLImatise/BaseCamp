...
task Sortlastmaf {
  ...
  command <<<
    sort_last_maf \
      ...
      ~{if (output_file_ok) then "--output" else ""} \
  ...
  parameter_meta {
    ...
    output_file_ok: "[string]                Output file (.gz ok, use 'stdout' for standard out). Default value: stdout."
    verbose: "Echo commandline options and be verbose. Default value: false."
  }
  output {
    File out_stdout = stdout()
    File out_reads_file = "${in_reads_file}"
    File out_output_file_ok = "${in_output_file_ok}"
  }
}