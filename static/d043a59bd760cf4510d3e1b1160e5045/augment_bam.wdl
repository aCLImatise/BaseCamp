...
task Augmentbam {
  ...
  command <<<
    augment_bam \
      ...
      ~{if (invoke_command_its) then "-c" else ""} \
  ...
  parameter_meta {
    ...
    invoke_command_its: "Invoke a command for its output rather than reading FILE"
  ...
  output {
    ...
    File out_invoke_command_its = "${in_invoke_command_its}"
