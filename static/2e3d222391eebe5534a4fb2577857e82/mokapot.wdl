...
task Mokapot {
  input {
    ...
    Boolean? override
    Boolean? save_models
    Float? open_modification_bin_size
    String? verbosity
    String psm_files
    String objects_dot
  }
  command <<<
    mokapot \
      ...
      ~{if defined(open_modification_bin_size) then ("--open_modification_bin_size " +  '"' + open_modification_bin_size + '"') else ""} \
      ~{if defined(verbosity) then ("--verbosity " +  '"' + verbosity + '"') else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/mokapot:0.6.2--py_0"
  }
  parameter_meta {
