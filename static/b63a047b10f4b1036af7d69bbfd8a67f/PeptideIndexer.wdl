...
task PeptideIndexer {
  ...
  command <<<
    PeptideIndexer \
      ...
      ~{if defined(unmatched_action) then ("-unmatched_action " +  '"' + unmatched_action + '"') else ""} \
  ...
  parameter_meta {
    ...
    fast_a: "Input sequence database in FASTA format. Leave empty for using the same DB as used for the input idXML (this might fail). Non-existing relative filenames are looked up via 'OpenMS.ini:id_db_dir' (valid formats: 'fasta')"
    ...
    unmatched_action: "If peptide sequences cannot be matched to any protein: 1) raise an error; 2) warn (unmatched PepHits will miss target/decoy annotation with downstream problems); 3) remove the hit. (default: 'error' valid: 'error', 'warn', 'remove')"
