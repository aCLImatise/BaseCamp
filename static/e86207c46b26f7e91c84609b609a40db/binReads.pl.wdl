...
task BinReadspl {
  input {
    ...
    Boolean? assume_be_mouse
    Boolean? assume_be_human
  ...
  command <<<
    binReads_pl \
      ...
      ~{if (assume_be_mouse) then "-M" else ""} \
      ~{if (assume_be_human) then "-H" else ""} \
  ...
  parameter_meta {
    ...
    assume_be_mouse: "Assume all bins should be on mouse chromosomes. This is the default."
    assume_be_human: "Assume all bins should be on human chromosomes."
