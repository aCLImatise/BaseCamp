...
- id: in_strict_duplicates
  doc: "by default reads are considered duplicates if they coincide in\ngenomic coordinates\
    \ and strand; with strict_duplicates enabled, we\nalso ask to consider read length\
    \ (WARNING: this option is called\nstrict, but it is more permissive)"
  type: boolean?
  inputBinding:
