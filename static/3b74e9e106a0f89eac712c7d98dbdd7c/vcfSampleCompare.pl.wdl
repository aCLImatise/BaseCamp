...
task VcfSampleComparepl {
  input {
    ...
    String? summary_suffix_appended
  ...
  command <<<
    vcfSampleCompare_pl \
      ...
      ~{if defined(summary_suffix_appended) then ("-u " +  '"' + summary_suffix_appended + '"') else ""} \
      ...
      ~{if defined(adequate_read_depth) then ("-x " +  '"' + adequate_read_depth + '"') else ""} \
  ...
  parameter_meta {
    ...
    summary_suffix_appended: "[STDOUT] Summary outfile suffix (appended to -i)."
    ...
    adequate_read_depth: "[20] Adequate read depth (DP)."
