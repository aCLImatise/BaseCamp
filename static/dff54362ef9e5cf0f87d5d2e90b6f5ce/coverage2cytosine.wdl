...
task Coverage2cytosine {
  ...
  command <<<
    coverage2cytosine \
      ...
      ~{if (gc_slash_gc_context) then "--gc/--gc_context" else ""} \
  ...
  >>>
  parameter_meta {
    oslash_output: "Name of the output file, mandatory."
  ...
  output {
    ...
    File out_gc_slash_gc_context = "${in_gc_slash_gc_context}"
