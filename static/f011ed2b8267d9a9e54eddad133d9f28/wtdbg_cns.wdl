...
task WtdbgCns {
  input {
    ...
    String? input_files_utgcns
    String? output_files
    Boolean? force_overwrite
    Int? expected_length_say
    Int? kmer_size_long
    Int? zcutoff_drop_score
    Int? wcutoff_drop_position
    Int? high_coverage_bonus
    Int? high_coverage_cutoff
  ...
  command <<<
    wtdbg-cns \
      ...
      ~{if defined(input_files_utgcns) then ("-i " +  '"' + input_files_utgcns + '"') else ""} \
      ...
      ~{if defined(wcutoff_drop_position) then ("-W " +  '"' + wcutoff_drop_position + '"') else ""} \
      ~{if defined(high_coverage_bonus) then ("-H " +  '"' + high_coverage_bonus + '"') else ""} \
      ~{if defined(high_coverage_cutoff) then ("-L " +  '"' + high_coverage_cutoff + '"') else ""} \
  ...
  parameter_meta {
    ...
    input_files_utgcns: "Input file(s) *.utg.cns from wtdbg, +, [STDIN]"
    ...
    wcutoff_drop_position: "W-cutoff, drop the lagger (position), [48] In DAG correction, -W set the bandwidth of alignment"
    high_coverage_bonus: "High coverage bonus, [1]"
    high_coverage_cutoff: "High coverage cutoff = avg_cov / <-L> [10]"
    candidate_strategy_bestkmers: "Candidate strategy, 0: best-kmers, 1: median length, 2: first (include), 3: first (exclude), 4: longest, 5, shortest, [0] In DAG correction, force to use strategy 2"
