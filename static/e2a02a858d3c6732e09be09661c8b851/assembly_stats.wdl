...
task Assemblystats {
  input {
    ...
    Boolean? print_tabdelimited_output_header
  ...
  command <<<
    assembly_stats \
      ...
      ~{if (print_tabdelimited_output_header) then "-u" else ""} \
  ...
  parameter_meta {
    ...
    print_tabdelimited_output_header: "Print tab-delimited output with no header line"
