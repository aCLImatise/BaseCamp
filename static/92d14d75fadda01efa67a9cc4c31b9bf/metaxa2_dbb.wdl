...
task Metaxa2Dbb {
  input {
    ...
    String? use_hmms_build
    String? taxonomy_file_containing
    String? id_first_sequence
    String? auto_rep
    String? cpu
    String? save_raw
    String? plus
    ...
    String? dna_fasta_file_containing_eukaryote_reference_sequences
    String? dna_fasta_file_containing_mitochondrial_sequences
    String? dna_fasta_file_containing_metazoan_sequences
    String? other
    String? full_length
    String? conservation_score_cutoff
    String? noise_cutoff_minimal
    String? autodetect_score_cutoff
    String? minimal_conserved_proportion
    String? lookahead_length_number
    String? minimal_conserved_region
  ...
  command <<<
    metaxa2_dbb \
      ...
      ~{if defined(use_hmms_build) then ("-p " +  '"' + use_hmms_build + '"') else ""} \
      ~{if defined(taxonomy_file_containing) then ("-t " +  '"' + taxonomy_file_containing + '"') else ""} \
      ~{if defined(id_first_sequence) then ("-r " +  '"' + id_first_sequence + '"') else ""} \
      ...
      ~{if defined(dna_fasta_file_containing_eukaryote_reference_sequences) then ("-e " +  '"' + dna_fasta_file_containing_eukaryote_reference_sequences + '"') else ""} \
      ~{if defined(dna_fasta_file_containing_mitochondrial_sequences) then ("-m " +  '"' + dna_fasta_file_containing_mitochondrial_sequences + '"') else ""} \
      ~{if defined(dna_fasta_file_containing_metazoan_sequences) then ("-n " +  '"' + dna_fasta_file_containing_metazoan_sequences + '"') else ""} \
      ...
      ~{if defined(autodetect_score_cutoff) then ("-A " +  '"' + autodetect_score_cutoff + '"') else ""} \
      ...
      ~{if defined(minimal_conserved_region) then ("-M " +  '"' + minimal_conserved_region + '"') else ""} \
  ...
  parameter_meta {
    ...
    use_hmms_build: ": Use HMMs from the specified directory instead of computing new ones (i.e. only build a new classification database), not used by default"
    ...
    id_first_sequence: ": ID of the sequence that should be used as the representative sequence of the gene\\nif blank, the first sequence in the input file is used"
    ...
    dna_fasta_file_containing_eukaryote_reference_sequences: ": DNA FASTA file containing eukaryote reference sequences to be used for classification (cannot be combined with the -i option)"
    dna_fasta_file_containing_mitochondrial_sequences: ": DNA FASTA file containing mitochondrial reference sequences to be used for classification (cannot be combined with the -i option)"
    dna_fasta_file_containing_metazoan_sequences: ": DNA FASTA file containing metazoan mitochondrial reference sequences to be used for classification (cannot be combined with the -i option)"
    ...
    autodetect_score_cutoff: ": Auto-detect conservation score cutoff, on (T) by default"
    ...
    minimal_conserved_region: ": Minimal conserved region length, 20 by default"
