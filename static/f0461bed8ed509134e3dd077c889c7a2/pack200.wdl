...
task Pack200 {
  input {
    ...
    Boolean? transmit_deflate_falseor
    Boolean? transmit_modtimes_latest
    Boolean? transmit_elements_unchanged
  ...
  command <<<
    pack200 \
      ...
      ~{if (transmit_deflate_falseor) then "-H" else ""} \
      ~{if (transmit_modtimes_latest) then "-m" else ""} \
      ~{if (transmit_elements_unchanged) then "-P" else ""} \
  ...
  parameter_meta {
    ...
    transmit_deflate_falseor: "{h}, --deflate-hint={h}       transmit deflate hint: true, false,\\nor keep (default)"
    transmit_modtimes_latest: "{V}, --modification-time={V}  transmit modtimes: latest or keep (default)"
    transmit_elements_unchanged: "{F}, --pass-file={F}          transmit the given input element(s) unchanged"
