...
task SansaAnnotate {
  input {
    ...
    Boolean? report_contained_genes
  ...
  command <<<
    sansa annotate \
      ...
      ~{if (arg_min_reciprocal) then "-r" else ""} \
      ~{if (s) then "-s" else ""} \
      ...
      ~{if (report_contained_genes) then "-c" else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/sansa:0.0.8--hf925130_1"
  }
  parameter_meta {
    ...
    arg_min_reciprocal: "[ --ratio ] arg (=0.800000012)     min. reciprocal overlap"
    ...
    report_contained_genes: "[ --contained ]                    report contained genes (useful for CNVs\\nbut potentially long list of genes)\\n"
    input_dot_bcf: ""
  }
  output {
