...
id: design.py_complete_targets_auto_from_file.cwl
inputs:
- id: in_guide_length
...
- id: in_write_input_seqs
  doc: "If set, write the sequences (accession.version) being\nused as input for design\
    \ to a file in OUT_TSV_DIR; the\nfilename is determined based on the label for\
    \ each\ntaxonomy"
  type: boolean?
  inputBinding:
    prefix: --write-input-seqs
- id: in_write_input_aln
  doc: "If set, write the alignments being used as input for\ndesign to a file in\
    \ OUT_TSV_DIR; the filename is\ndetermined based on the label for each taxonomy\
    \ (they\nare '[label].[cluster-number].fasta'\n"
  type: boolean?
  inputBinding:
    prefix: --write-input-aln
- id: in_in_tsv
  doc: "Path to input TSV. Each row gives the following\ncolumns, in order: (1) label\
    \ for the row (used for\nnaming output files; must be unique); (2) taxonomic\n\
  ...
  inputBinding:
    position: 0
- id: in_out_tsv_dir
  doc: "Path to directory in which to place output TSVs; each\noutput TSV corresponds\
    \ to a cluster for the taxon in a\nrow in the input"
  type: string
  inputBinding:
    position: 1
outputs:
