...
task RsatDyadanalysis {
  ...
  command <<<
    rsat dyad_analysis \
      ...
      ~{if defined(sequence_analyzed_canbe) then ("-i " +  '"' + sequence_analyzed_canbe + '"') else ""} \
  ...
  parameter_meta {
    ...
    o: ""
    sequence_analyzed_canbe: "The sequence to be analyzed. Multiple sequences can\\nbe entered at once with most sequence formats (see below)."
