...
task Spoa {
  input {
    ...
    Int? default_gap_opening_penalty_must
    Int? default_gap_extension_penalty_must
  ...
  command <<<
    spoa \
      ...
      ~{if defined(default_gap_opening_penalty_must) then ("-g " +  '"' + default_gap_opening_penalty_must + '"') else ""} \
      ~{if defined(default_gap_extension_penalty_must) then ("-e " +  '"' + default_gap_extension_penalty_must + '"') else ""} \
  ...
  parameter_meta {
    ...
    default_gap_opening_penalty_must: "default: -8\\ngap opening penalty (must be non-positive)"
    default_gap_extension_penalty_must: "default: -6\\ngap extension penalty (must be non-positive)"
