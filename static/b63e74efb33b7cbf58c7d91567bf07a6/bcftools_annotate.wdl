...
task BcftoolsAnnotate {
  ...
  command <<<
    bcftools annotate \
      ...
      ~{if defined(columns_file) then ("--columns-file " +  '"' + columns_file + '"') else ""} \
      ...
      ~{if defined(merge_logic) then ("--merge-logic " +  '"' + merge_logic + '"') else ""} \
      ...
      ~{if defined(rename_an_not_s) then ("--rename-annots " +  '"' + rename_an_not_s + '"') else ""} \
  ...
  parameter_meta {
    ...
    columns_file: "read -c columns from FILE, one name per row, with optional --merge-logic TYPE: NAME[ TYPE]"
    ...
    set_id: "[+]FORMAT       set ID column using a `bcftools query`-like expression, see man page for details"
    ...
    merge_logic: ":TYPE   merge logic for multiple overlapping regions (see man page for details), EXPERIMENTAL"
    ...
    regions_file: "restrict to regions listed in FILE"
    rename_an_not_s: "rename annotations: TYPE/old\\tnew, where TYPE is one of FILTER,INFO,FORMAT"
    rename_chrs: "rename sequences according to the mapping: old\\tnew"
