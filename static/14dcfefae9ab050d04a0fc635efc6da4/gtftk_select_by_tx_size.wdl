...
task GtftkSelectByTxSize {
  ...
  command <<<
    gtftk select_by_tx_size \
      ~{optional_arguments} \
      ~{if (input_file) then "--inputfile" else ""} \
      ~{if (output_file) then "--outputfile" else ""} \
      ~{if (min_size) then "--min-size" else ""} \
      ~{if (max_size) then "--max-size" else ""} \
  ...
  parameter_meta {
    ...
    min_size: "Minimum size. (default: 0)"
    max_size: "Maximum size. (default: 1000000000)"
