...
id: ../../../../home/ubuntu/BiocondaCli/gmapl.sse42.cwl
inputs:
- id: dir
  doc: Genome directory.  Default (as specified by --with-gmapdb to the configure
    program) is /tmp/tmpu5a6ivvl/share
  type: Directory
  inputBinding:
    prefix: --dir
- id: db
...
- id: gs_eg
  ...
  inputBinding:
    prefix: --gseg
- id: self_align
  doc: Align one sequence against itself in FASTA format via stdin (Useful for getting
    protein translation of a nucleotide sequence)
  type: boolean
  inputBinding:
    prefix: --selfalign
- id: pair_align
  doc: Align two sequences in FASTA format via stdin, first one being genomic and
...
- id: cmdline
  ...
  inputBinding:
    prefix: --cmdline
- id: part
  doc: Process only the i-th out of every n sequences e.g., 0/100 or 99/100 (useful
    for distributing jobs to a computer farm).
  type: long
  inputBinding:
    prefix: --part
...
- id: use_shared_memory
  doc: If 1, then allocated memory is shared among all processes on this node If 0
...
- id: compress
  ...
  inputBinding:
    prefix: --compress
- id: exons
  doc: Print exons ("cdna" or "genomic") Will also print introns with "cdna+introns"
    or "genomic+introns"
  type: string
  inputBinding:
    prefix: --exons
...
- id: format
  doc: 'Other format for output (also note the -A and -S options and other options
    listed under Output types): mask_introns, mask_utr_introns, psl (or 1) = PSL (BLAT)
    format, gff3_gene (or 2) = GFF3 gene format, gff3_match_cdna (or 3) = GFF3 cDNA_match
    format, gff3_match_est (or 4) = GFF3 EST_match format, splicesites (or 6) = splicesites
    output (for GSNAP splicing file), introns = introns output (for GSNAP splicing
    file), map_exons (or 7) = IIT FASTA exon map format, map_ranges (or 8) = IIT FASTA
    range map format, coords (or 9) = coords in table format, sampe = SAM format (setting
    paired_read bit in flag), samse = SAM format (without setting paired_read bit),
    bedpe = indels and gaps in BEDPE format'
  type: long
  inputBinding:
    prefix: --format
- id: n_paths
  doc: Maximum number of paths to show (default 5).  If set to 1, GMAP will not report
  ...
  inputBinding:
    prefix: --npaths
- id: suboptimal_score
  doc: Report only paths whose score is within this value of the best path. If specified
    between 0.0 and 1.0, then treated as a fraction of the score of the best alignment
    (matches minus penalties for mismatches and indels).  Otherwise, treated as an
    integer number to be subtracted from the score of the best alignment. Default
    value is 0.50.
  type: double
  inputBinding:
...
- id: gff_three_fast_a_annotation
  doc: 'Whether to include annotation from the FASTA header into the GFF3 output Values:
    0 (default): Do not include 1: Wrap all annotation as Annot="<header>" 2: Include
    key=value pairs, replacing brackets with quotation marks and replacing spaces
    between key=value pairs with semicolons'
  type: long
  inputBinding:
...
- id: check
  ...
  inputBinding:
    prefix: --check
- id: allocate_allocate_same
  doc: allocate        allocate     (same as 4)
  type: string
  inputBinding:
    position: 0
outputs: []
