...
task SmCounter2 {
  ...
  parameter_meta {
    ...
    min_mq: "minimum mapping quality allowed for analysis. If the bam is tagged with its mate's mapq, then the minimum of the R1 and R2 mapq will be used for comparison, if not each read is compared independently."
    ...
    mt_threshold: "threshold on read proportion to determine MT level consensus"
    rpb: "mean read pairs per UMI; default at 0 and will be calculated"
    is_rna: "RNAseq varinat calling only; default is DNAseq"
    primer_side: "read end that includes the primer; default is 1"
    minal_tumi: "minimum requirement of ALT UMIs; default is 3"
    maxalt_allele: "maximum ALT alleles that meet minAltUMI to be reported; default is 2 (tri-allelic variants)"
    ...
    ds: "down sample if number of UMIs greater than this value (RNA only)"
    bam_type: "raw (default): raw BAM file with UMIs; consensus: consensused BAM file"
