...
task RgttoolspyGtfToBed {
  ...
  command <<<
    rgt_tools_py gtf_to_bed \
      ...
      ~{if defined(define_the_feature) then ("-f " +  '"' + define_the_feature + '"') else ""} \
  ...
  parameter_meta {
    ...
    define_the_feature: "Define the feature {gene,transcript,exon,CDS,UTR,start_codon,sto\\np_codon,Selenocysteine,All}"
