...
task Medusa {
  input {
    ...
    Boolean? gex_f
    Boolean? targetgenome_option_iindicates
    Boolean? n_five_zero
  ...
  command <<<
    medusa \
      ...
      ~{if (targetgenome_option_iindicates) then "-i" else ""} \
  ...
  parameter_meta {
    ...
    targetgenome_option_iindicates: "<<targetGenome>>                   REQUIRED PARAMETER;The option *-i*\\nindicates the name of the target\\ngenome file."
