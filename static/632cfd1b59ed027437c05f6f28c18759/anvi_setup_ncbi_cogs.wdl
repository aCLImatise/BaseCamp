...
task Anvisetupncbicogs {
  ...
  command <<<
    anvi_setup_ncbi_cogs \
      ...
      ~{if defined(clusterizing_your_runs) then ("--if " +  '"' + clusterizing_your_runs + '"') else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    clusterizing_your_runs: "are clusterizing your runs, and asking for"
