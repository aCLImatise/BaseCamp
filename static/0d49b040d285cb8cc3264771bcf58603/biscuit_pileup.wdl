...
task BiscuitPileup {
  input {
    ...
    Boolean? redistribution_ambiguous_genotyping
  ...
  command <<<
    biscuit pileup \
      ...
      ~{if (somatic_mode_also) then "-S" else ""} \
      ...
      ~{if (redistribution_ambiguous_genotyping) then "-r" else ""} \
  ...
  parameter_meta {
    ...
    somatic_mode_also: "somatic mode, must also provide -T (tumor BAM) and -I (normal BAM) arguments [off]"
    ...
    redistribution_ambiguous_genotyping: "NO redistribution of ambiguous (Y/R) calls in SNP genotyping."
