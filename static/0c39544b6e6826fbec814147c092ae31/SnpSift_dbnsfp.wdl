...
task SnpSiftDbnsfp {
  input {
    Boolean? annotate_fields_even_database
    Boolean? collapse
    File? db
    String? _genome_version
    Boolean? no_collapse
    Boolean? annotate_fields_even_there
    Boolean? _invert_
  ...
  command <<<
    SnpSift dbnsfp \
      ...
      ~{d_bnsf_p} \
      ~{file_dot_vcf} \
      ~{if (annotate_fields_even_database) then "-a" else ""} \
      ...
      ~{if (annotate_fields_even_there) then "-m" else ""} \
  ...
  >>>
  parameter_meta {
    annotate_fields_even_database: ": Annotate fields, even if the database has an empty value (annotates using '.' for empty)."
    ...
    annotate_fields_even_there: ": Annotate fields even if there are no database matching entries (annotates using '.')."
