...
task AgatSqManageAttributespl {
  ...
  parameter_meta {
    ...
    value: "When a value is provided the attribute is taken into\\naccount only if the attribute contains (or match) a specific\\nvalue"
    strategy: "String. [Default equal]. Strategy to use when --value parameter\\nis in use. Can be equal or match. Equal => the attribute value\\nmust be identical. Match => the attribute value must match"
