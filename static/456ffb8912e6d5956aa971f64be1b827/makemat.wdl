...
task Makemat {
  input {
    ...
    Boolean? scaling_factor_matrix
    Boolean? print_help_overrides
  }
  command <<<
    makemat \
      ~{if (database_name_stdin) then "-P" else ""} \
      ...
      ~{if (scaling_factor_matrix) then "-S" else ""} \
  ...
  parameter_meta {
    database_name_stdin: "Database name for profile database [File In]\\ndefault = stdin"
    ...
    scaling_factor_matrix: "Scaling factor for  matrix outputs to avoid round-off problems [Real]\\ndefault = 100.0"
