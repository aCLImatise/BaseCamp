...
- id: cwl
  doc: Configure Galaxy for use with CWL tool. (this option is experimental and will
    be replaced when and if CWL support is merged into Galaxy).
  type: boolean
  inputBinding:
    prefix: --cwl
- id: cwl_galaxy_root
  doc: Root of development galaxy directory to execute command with (must be branch
    of Galaxy with CWL support, this option is experimental and will be replaced with
    --galaxy_root when and if CWL support is merged into Galaxy.
  type: Directory
  inputBinding:
    prefix: --cwl_galaxy_root
- id: output_directory
  doc: Where to store outputs of a 'run' task.
  type: Directory
  inputBinding:
    prefix: --output_directory
- id: output_json
  doc: Where to store JSON dictionary describing outputs of a 'run' task.
  type: File
  inputBinding:
    prefix: --output_json
- id: engine
