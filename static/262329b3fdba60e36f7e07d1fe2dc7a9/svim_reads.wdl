...
task SvimReads {
  ...
  parameter_meta {
    ...
    cores: "CPU cores to use for the alignment (default: 1)"
    aligner: "Tool for read alignment: ngmlr or minimap2 (default: ngmlr)"
    nano_pore: "Use Nanopore settings for read alignment (default: False)"
    ...
    reads: "Read file (FASTA, FASTQ, gzipped FASTA, gzipped FASTQ or file list). The read file has to have one of the following supported file endings: FASTA: .fa, .fasta, .FA, .fa.gz, .fa.gzip, .fasta.gz, .fasta.gzip FASTQ: .fq, .fastq, .FQ, .fq.gz, .fq.gzip, .fastq.gz, .fastq.gzip FILE LIST: .fa.fn, fq.fn"
