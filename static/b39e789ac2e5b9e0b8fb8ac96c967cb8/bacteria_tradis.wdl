...
task BacteriaTradis {
  input {
    ...
    Boolean? tag_search_optional
    Boolean? reference_genome_fasta
    Boolean? td
    Boolean? mm
    Boolean? mapping_quality_score
  ...
  parameter_meta {
    ...
    tag_search_optional: ": tag to search for (optional.  If not set runs bwa in tagless mode with no filtering.)"
    ...
    mapping_quality_score: ": mapping quality cutoff score (optional. default = 30)"
    ...
    set_defaults_m: ": set defaults for essentiality experiment (smalt_r = 0, -m = 0)"
