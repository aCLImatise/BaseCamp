...
task SalmIDpy {
  input {
    ...
    File? extension
    Directory? input_dir
    String? coverage_taxonomy_report
  ...
  command <<<
    SalmID_py \
      ...
      ~{if defined(coverage_taxonomy_report) then ("-r " +  '"' + coverage_taxonomy_report + '"') else ""} \
