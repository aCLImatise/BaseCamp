...
task BgtView {
  input {
    ...
    Int? process_intth_based
    Int? process_most_int
    File? variant_annotations_fmf
    ...
    Boolean? equivalent_overriding_b
    Boolean? output_sample_genotypes
    Boolean? write_acan_s
  ...
  command <<<
    bgt view \
      ...
      ~{if defined(process_intth_based) then ("-i " +  '"' + process_intth_based + '"') else ""} \
      ~{if defined(process_most_int) then ("-n " +  '"' + process_most_int + '"') else ""} \
      ~{if defined(variant_annotations_fmf) then ("-d " +  '"' + variant_annotations_fmf + '"') else ""} \
      ...
      ~{if (equivalent_overriding_b) then "-u" else ""} \
      ~{if (output_sample_genotypes) then "-G" else ""} \
      ~{if (write_acan_s) then "-C" else ""} \
  ...
  parameter_meta {
    ...
    process_intth_based: "process from the INT-th record (1-based) []"
    process_most_int: "process at most INT records []"
    variant_annotations_fmf: "variant annotations in FMF (to work with -a) []"
    ...
    equivalent_overriding_b: "equivalent to -bl0 (overriding -b and -l)"
    output_sample_genotypes: "don't output sample genotypes"
    write_acan_s: "write AC/AN to the INFO field (auto applied with -f or multipl -s)"
