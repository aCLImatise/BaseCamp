...
task CreateNewcase {
  ...
  command <<<
    create_newcase \
      ...
      ~{if (non_local) then "--non-local" else ""} \
      ~{if defined(extra_machines_dir) then ("--extra-machines-dir " +  '"' + extra_machines_dir + '"') else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    non_local: "Use when you've requested a machine that you aren't on. Will reduce errors for missing directories etc."
    extra_machines_dir: "Optional path to a directory containing one or more of:\\nconfig_machines.xml, config_compilers.xml, config_batch.xml.\\nIf provided, the contents of these files will be appended to\\nthe standard machine files (and any files in ~/.cime).\\n"
  }
  output {
