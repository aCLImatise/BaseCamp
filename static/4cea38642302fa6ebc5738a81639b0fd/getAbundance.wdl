...
task GetAbundance {
  input {
    ...
    Boolean? output_only_estimations
  ...
  command <<<
    getAbundance \
      ...
      ~{if (output_only_estimations) then "-a" else ""} \
  ...
  parameter_meta {
    ...
    output_only_estimations: "<minAbundance(%)>                     To output only estimations that are higher than a certain threshold, minAbundance.\\nminAbundance is a percentage value (between 0 and 100)."
