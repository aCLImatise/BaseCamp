...
- id: in_region_table
  doc: "Read in a read-region table in HDF format for masking portions of reads.\n\
    ...
    \ a region table is specified, any region table inside\nthe reads.plx.h5 or reads.bax.h5\
    \ files are ignored."
  type: File?
  inputBinding:
    prefix: -regionTable
...
- id: in_sam
  ...
  inputBinding:
    prefix: -sam
- id: in_clipping
  doc: "[none|hard|subread|soft] (none)\nUse no/hard/subread/soft clipping for SAM\
    \ output."
  type: boolean?
  inputBinding:
    prefix: -clipping
...
- id: in_hole_numbers
  doc: "When specified, only align reads whose ZMW hole numbers are in LIST.\nLIST\
    ...
    \ works when reads are in base or pulse h5 format."
  type: string?
  inputBinding:
    prefix: -holeNumbers
...
- id: in_concordant
  ...
  inputBinding:
    prefix: -concordant
- id: in_concordant_template
  doc: "(mediansubread)\nSelect a full pass subread of a zmw as template for concordant\
