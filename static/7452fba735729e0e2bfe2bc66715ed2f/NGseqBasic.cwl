...
- id: in_how_many_processors
  ...
  inputBinding:
    prefix: -p
- id: in_bowtie_one
  doc: (default is bowtie1 - decide if bowtie1 or bowtie2 is to be used. bowtie2 is
    better to long reads - read lenght more than 70b, fragment lenght more than 350b)
  type: boolean?
  inputBinding:
    prefix: --bowtie1
- id: in_chunk_mb
...
- id: in_flash
  ...
  inputBinding:
    prefix: --flash
- id: in_blacklist_filter
  doc: /noBlacklistFilter - supported genomes mm9 mm10 hg18 hg19 (filter/do-not-filter
    blacklisted regions out from the final bam file)
  type: File?
  inputBinding:
...
- id: in_next_era
  ...
  inputBinding:
    prefix: --nextera
- id: in_trim
  doc: /noTrim** (run/do-not-run TrimGalore for the data - Illumina PE standard adapter
...
- id: out_stdout
  ...
  type: stdout
- id: out_blacklist_filter
  doc: /noBlacklistFilter - supported genomes mm9 mm10 hg18 hg19 (filter/do-not-filter
    blacklisted regions out from the final bam file)
  type: File?
  outputBinding:
    glob: $(inputs.in_blacklist_filter)
