...
- id: in_j_place_path
  ...
  inputBinding:
    prefix: --jplace-path
- id: in_mass_norm
  doc: ":{absolute,relative}=absolute REQUIRED\nSet the per-sample normalization method.\
    \ 'absolute' does not change the masses, while 'relative' normalizes them to a\
    \ total mass of 1 per input jplace sample."
  type: long?
  inputBinding:
    prefix: --mass-norm
- id: in_point_mass
...
- id: in_edge_values
  doc: ":{both,imbalances,masses}=both\nValues per edge used to calculate the dispersion."
  type: string?
  inputBinding:
    prefix: --edge-values
- id: in_method
  doc: ":{all,cv,cv-log,sd,sd-log,var,var-log,vmr,vmr-log}=all\nMethod of dispersion.\
...
- id: in_mask_color
  doc: =#dfdfdf   Color used to indicate masked or invalid values.
  type: string?
  inputBinding:
    prefix: --mask-color
- id: in_out_dir
