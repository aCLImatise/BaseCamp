...
task CmphKeysfile {
  input {
    Boolean? value_determines_number
    Boolean? algorithm_valid_values
    Boolean? hash_function_used
    Boolean? print_version_number
    Boolean? increase_verbosity_used
    ...
    Boolean? set_number_function
    String keys_file
  }
  command <<<
    cmph keysfile \
      ~{keys_file} \
      ~{if (value_determines_number) then "-c" else ""} \
      ~{if (algorithm_valid_values) then "-a" else ""} \
      ~{if (hash_function_used) then "-f" else ""} \
      ~{if (print_version_number) then "-V" else ""} \
      ~{if (increase_verbosity_used) then "-v" else ""} \
