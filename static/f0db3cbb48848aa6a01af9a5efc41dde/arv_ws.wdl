...
task ArvWs {
  ...
  command <<<
    arv-ws \
      ...
      ~{true="--no-poll" false="" no_poll} \
      ~{if defined(pipeline) then ("--pipeline " +  '"' + pipeline + '"') else ""} \
      ~{if defined(job) then ("--job " +  '"' + job + '"') else ""}
  >>>
  parameter_meta {
    uuid: "Filter events on object_uuid"
    filters: "Arvados query filter to apply to log events (JSON encoded)"
    start_time: "Arvados query filter to fetch log events created at or after this time. This will be server time in UTC. Allowed format: YYYY-MM-DD or YYYY-MM-DD hh:mm:ss"
    id: "Start from given log id."
    poll_interval: "If websockets is not available, specify the polling interval, default is every 15 seconds"
    no_poll: "Do not poll if websockets are not available, just fail"
    pipeline: "Supply pipeline uuid, print log output from pipeline and its jobs"
