...
task DellyMerge {
  ...
  command <<<
    delly merge \
      ~{files} \
      ~{if (arg_merged_file) then "-o" else ""} \
  ...
  >>>
  parameter_meta {
    arg_merged_file: "[ --outfile ] arg (=\\\"sv.bcf\\\")      Merged SV BCF output file"
    ...
    arg_min_reciprocal: "[ --rec-overlap ] arg (=0.800000012)\\nmin. reciprocal overlap\\n"
    files: "-a [ --vaf ] arg (=0.150000006)       min. fractional ALT support"
  }
  output {
    File out_stdout = stdout()
    File out_arg_merged_file = "${in_arg_merged_file}"
  }
}