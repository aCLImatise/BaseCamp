...
task WhatshapPolyphase {
  ...
  command <<<
    whatshap polyphase \
      ...
      ~{if (include_haploid_sets) then "--include-haploid-sets" else ""} \
  ...
  parameter_meta {
    ...
    include_haploid_sets: "Include the phase set information for every single\\nhaplotype in a custom VCF format field 'HS'."
    ...
    threads: "Maximum number of CPU threads used (default: 1).\\n"
