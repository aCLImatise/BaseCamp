...
task CRISPRessoPooled {
  input {
    ...
    Boolean? skip_failed
    Boolean? skip_reporting_problematic_regions
    String? cris_presso_command
    Boolean? compile_post_run_references
    Int? compile_post_run_reference_allele_cut_off
    File? alternate_alleles
  }
  command <<<
    CRISPRessoPooled \
      ...
      ~{if (skip_reporting_problematic_regions) then "--skip_reporting_problematic_regions" else ""} \
      ~{if defined(cris_presso_command) then ("--crispresso_command " +  '"' + cris_presso_command + '"') else ""} \
      ~{if (compile_post_run_references) then "--compile_postrun_references" else ""} \
      ~{if defined(compile_post_run_reference_allele_cut_off) then ("--compile_postrun_reference_allele_cutoff " +  '"' + compile_post_run_reference_allele_cut_off + '"') else ""} \
      ~{if defined(alternate_alleles) then ("--alternate_alleles " +  '"' + alternate_alleles + '"') else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    skip_failed: "Continue with pooled analysis even if one sample fails\\n(default: False)"
    skip_reporting_problematic_regions: "Skip reporting of problematic regions. By default,\\nwhen both amplicons (-f) and genome (-x) are provided,\\nproblematic reads that align to the genome but to\\npositions other than where the amplicons align are\\nreported as problematic (default: False)"
    cris_presso_command: "CRISPResso command to call (default: CRISPResso)"
    compile_post_run_references: "If set, a file will be produced which compiles the\\nreference sequences of frequent amplicons. (default:\\nFalse)"
    compile_post_run_reference_allele_cut_off: "Only alleles with at least this percentage frequency\\nin the population will be reported in the postrun\\nanalysis. This parameter is given as a percent, so 30\\nis 30%. (default: 30)"
    alternate_alleles: "Path to tab-separated file with alternate allele\\nsequences for pooled experiments. This file has the\\ncolumns \\\"region_name\\\",\\\"reference_seqs\\\", and\\n\\\"reference_names\\\" and gives the reference sequences of\\nalternate alleles that will be passed to CRISPResso\\nfor each individual region for allelic analysis.\\nMultiple reference alleles and reference names for a\\ngiven region name are separated by commas (no spaces).\\n(default: )\\n"
  }
  output {
