...
task PhylophlanSetupDatabase {
  input {
    File? specify_path_folder
    String? get_core_proteins
    Boolean? database_update
    File? specify_path_output
    String? db_name
  ...
  command <<<
    phylophlan_setup_database \
      ~{if defined(specify_path_folder) then ("--input " +  '"' + specify_path_folder + '"') else ""} \
  ...
  >>>
  parameter_meta {
    specify_path_folder: "Specify the path to either the folder containing the\\nmarker files or the file of markers, in (multi-)fasta\\nformat (default: None)"
    ...
    specify_path_output: "Specify path to the output folder where to save the\\ndatabase (default: None)"
  ...
  output {
    ...
    File out_specify_path_output = "${in_specify_path_output}"
