...
task CompareMotifspl {
  input {
    ...
    File? known
    Boolean? p_value
    Boolean? remove_motifs_less_default
    Boolean? info
    Boolean? remove_motifs_background_default
    Boolean? mint
    Boolean? site_reduce
    String? stat
    Boolean? bits
  ...
  command <<<
    compareMotifs_pl \
      ...
      ~{if (remove_motifs_less_default) then "-F" else ""} \
      ~{if (info) then "-info" else ""} \
      ~{if (remove_motifs_background_default) then "-b" else ""} \
  ...
  parameter_meta {
    ...
    remove_motifs_less_default: "<#> (Remove Motifs with fold enrichment [target%/bg%] less than <#>, default: keep all)"
    info: "<#> (Remove Motifs with information content less than #, default: not used)"
    remove_motifs_background_default: "<#> (Remove Motifs with background percentage less than #, default: not used)"
