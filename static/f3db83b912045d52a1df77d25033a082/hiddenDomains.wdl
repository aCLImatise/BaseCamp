...
task HiddenDomains {
  input {
    ...
    Boolean? controlreadsa_bed_file
    Boolean? controlreadsbinsif_you_already
    Boolean? chrominfotxtif_you_get
    Boolean? outputprefixhiddendomains_generates_files
    Boolean? minposteriortoss_parts_minposterior
  ...
  command <<<
    hiddenDomains \
      ...
      ~{if (controlreadsa_bed_file) then "-c" else ""} \
      ~{if (controlreadsbinsif_you_already) then "-C" else ""} \
      ~{if (chrominfotxtif_you_get) then "-g" else ""} \
      ~{if (outputprefixhiddendomains_generates_files) then "-o" else ""} \
      ~{if (minposteriortoss_parts_minposterior) then "-p" else ""} \
