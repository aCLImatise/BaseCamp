...
- id: topic_target
  doc: '[use|admin-dev|instructors] Target audience for the topic'
  type: boolean
  inputBinding:
    prefix: --topic_target
- id: tutorial_name
  doc: Name (directory name) of the tutorial to create or to modify
  type: string
  inputBinding:
...
- id: galaxy_url
  ...
  inputBinding:
    prefix: --galaxy_url
- id: galaxy_api_key
  doc: API key on the Galaxy instance with the workflow
  type: string
  inputBinding:
    prefix: --galaxy_api_key
- id: workflow_id
  doc: ID of the workflow on the Galaxy instance
  type: string
  inputBinding:
    prefix: --workflow_id
- id: zeno_do_link
