...
task SOAPdenovo127merContig {
  input {
    ...
    Int? two_edges_arcs
    Int? max_k_using
    String? readsinfofilethe_file_contains
    Int? number_of_cpu
  ...
  command <<<
    SOAPdenovo-127mer contig \
      ...
      ~{if defined(max_k_using) then ("-m " +  '"' + max_k_using + '"') else ""} \
  ...
  parameter_meta {
    ...
    max_k_using: "max k when using multi-kmer, and the parameters below are used along with multi-kmer, [NO]"
