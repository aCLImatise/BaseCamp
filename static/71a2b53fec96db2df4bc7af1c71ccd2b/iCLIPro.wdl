...
task ICLIPro {
  input {
    ...
    Int? number_reads_default
    Int? flanking_distances_calculating
    Int? use_only_reads
    String? read_len_groups
    String? generate_read_overlap
  ...
  command <<<
    iCLIPro \
      ...
      ~{if defined(number_reads_default) then ("-r " +  '"' + number_reads_default + '"') else ""} \
      ~{if defined(flanking_distances_calculating) then ("-s " +  '"' + flanking_distances_calculating + '"') else ""} \
      ~{if defined(use_only_reads) then ("-q " +  '"' + use_only_reads + '"') else ""} \
      ~{if defined(read_len_groups) then ("-g " +  '"' + read_len_groups + '"') else ""} \
      ~{if defined(generate_read_overlap) then ("-p " +  '"' + generate_read_overlap + '"') else ""} \
  ...
  parameter_meta {
    ...
    number_reads_default: "number of reads required in bin (20..500, default: 50)"
    flanking_distances_calculating: "flanking distances when calculating start site overlap ratio (3..15, default: 5)"
    ...
    generate_read_overlap: "generate read overlap maps based on these comparisons (e.g.: \"A1-A3,A2-A3,A1-B,A2-B,A3-B,L-B,A-B\")"
