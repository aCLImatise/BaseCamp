...
task Estpostentropy {
  ...
  command <<<
    estpost_entropy \
      ...
      ~{if (number_additinal_mcmc) then "-b" else ""} \
  ...
  parameter_meta {
    ...
    number_additinal_mcmc: "Number of additinal MCMC samples to discard for burn-in [default = 0]"
