...
task IvarRemovereads {
  ...
  command <<<
    ivar removereads \
      ...
      ~{if (required_prefix_filtered) then "-p" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    required_prefix_filtered: "(Required) Prefix for the output filtered BAM file"
  }
  output {
    File out_stdout = stdout()
    File out_required_text_file = "${in_required_text_file}"
    File out_required_prefix_filtered = "${in_required_prefix_filtered}"
  }
}