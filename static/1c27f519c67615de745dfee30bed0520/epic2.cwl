...
- id: in_e_value
  ...
  inputBinding:
    prefix: --e-value
- id: in_required_flag
  doc: "(bam only.) Keep reads with these bits set in flag.\nSame as `samtools view\
    \ -f`. Default 0"
  type: long
  inputBinding:
    prefix: --required-flag
- id: in_filter_flag
  doc: "(bam only.) Discard reads with these bits set in flag.\nSame as `samtools\
