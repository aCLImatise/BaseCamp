...
task MatamCompareSamplespy {
  input {
    ...
    String? output_comparaison_taxonomy
  ...
  command <<<
    matam_compare_samples_py \
      ...
      ~{if defined(output_comparaison_taxonomy) then ("--ouput_comparaison_table " +  '"' + output_comparaison_taxonomy + '"') else ""}
  >>>
  parameter_meta {
    ...
    output_comparaison_taxonomy: "Output a comparaison table (taxonomy vs samples)\\n"
