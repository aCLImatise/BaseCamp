...
task Nosetests2.7 {
  input {
    ...
    Boolean withWithForcedGc
    String testTestTimeout
    Boolean withWithTestData
    File testTestDataFile
    Boolean testTestDataTable
    String attrAttr
    String evalEvalAttr
    Boolean noNoCapture
    ...
    Boolean withWithProfile
    String profileProfileSort
    File profileProfileStatsFile
    String profileProfileRestrict
    Boolean noNoSkip
    Boolean withWithId
    File idIdFile
  ...
  command <<<
    nosetests-2.7 \
      ...
      ~{true="--with-forced-gc" false="" withWithForcedGc} \
      ~{if defined(testTestTimeout) then ("--test-timeout " +  '"' + testTestTimeout + '"') else ""} \
      ~{true="--with-testdata" false="" withWithTestData} \
      ~{if defined(testTestDataFile) then ("--testdata-file " +  '"' + testTestDataFile + '"') else ""} \
      ~{true="--testdata-table" false="" testTestDataTable} \
      ...
      ~{true="--with-profile" false="" withWithProfile} \
      ~{if defined(profileProfileSort) then ("--profile-sort " +  '"' + profileProfileSort + '"') else ""} \
      ~{if defined(profileProfileStatsFile) then ("--profile-stats-file " +  '"' + profileProfileStatsFile + '"') else ""} \
      ~{if defined(profileProfileRestrict) then ("--profile-restrict " +  '"' + profileProfileRestrict + '"') else ""} \
