...
task Dialigntx {
  input {
    ...
    Boolean? translate_dna_end
    Boolean? compare_reading_framewarning
    Boolean? translate_dna_aminoacids
  ...
  command <<<
    dialign_tx \
      ...
      ~{if (sensitivity_mode_aligned) then "-l" else ""} \
      ...
      ~{if (translate_dna_end) then "-T" else ""} \
      ~{if (compare_reading_framewarning) then "-L" else ""} \
      ~{if (translate_dna_aminoacids) then "-O" else ""} \
  ...
  parameter_meta {
    ...
    threshold_low_alignment: "\\\"even\\\" threshold for low score for sequences alignment\\n[DEFAULT PROTEIN: 4]\\n[DEFAULT DNA: 0]"
    ...
    wether_overlap_calculated: "wether overlap weights are calculated or not [DEFAULT 0]"
    ...
    translate_dna_end: "translate DNA into aminoacids from begin to end (length will be cut to mod 3 = 0)\\nWARNING: Do not use -D with this option\\n(Default values for PROTEIN input will be loaded)"
    compare_reading_framewarning: "compare only longest Open Reading Frame\\nWARNING: Do not use -D with this option\\n(Default values for PROTEIN input will be loaded)"
    translate_dna_aminoacids: "translate DNA to aminoacids, reading frame for each sequence calculated due to its longest ORF\\nWARNING: Do not use -D with this option\\n(Default values for PROTEIN input will be loaded)"
