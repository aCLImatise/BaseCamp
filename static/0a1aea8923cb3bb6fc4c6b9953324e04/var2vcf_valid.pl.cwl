...
- id: in_amplicon_based_variant
  doc: amplicon based variant calling.  Variant not supported by all amplicons will
    be considered false positive, with filter set to "AMPBIAS".
  type: string?
  inputBinding:
    prefix: -a
...
- id: in_maximum_mean_mismatches
  doc: "double\nThe maximum mean mismatches allowed.  Default: 5.25, or if a variant\
    \ is supported by reads with more than 5.25 mean mismatches, it'll be considered\n\
...
- id: in_minimum_mean_position
  ...
  inputBinding:
    prefix: -p
- id: in_filter_variants_pstd
  doc: "0 or 1\nWhether to filter variants with pstd = 0.  Default: 1 or yes.  Set\
...
- id: in_minimum_number_split
  doc: "integer\nThe minimum number of split reads for SV.  Default: 1.  Change to\
    ...
    \ which is used to generate contigs in the header\n-G  Path to the *.fasta (*.fa)\
    \ file which is used to generate reference tag in the header"
  type: boolean?
  inputBinding:
    prefix: -T
outputs:
