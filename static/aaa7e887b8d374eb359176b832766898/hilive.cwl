...
- id: in_illuminas_basecalls_directory
  ...
  inputBinding:
    prefix: -b
- id: in_path_hilive_index
  doc: '[--index]          Path to the HiLive index.'
  type: boolean?
  inputBinding:
    prefix: -i
- id: in_length_types_read
  doc: '[--reads]          Length and types of the read segments.'
  type: boolean?
  inputBinding:
    prefix: -r
- id: in_print_license_information
  ...
  inputBinding:
    prefix: -l
- id: in_arg_path_config
  doc: "[ --config ] arg               Path to a config file. Config file is in\n\
...
- id: out_arg_path_config
  ...
  type: File?
  outputBinding:
    glob: $(inputs.in_arg_path_config)
