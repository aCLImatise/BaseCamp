...
task Anviestimatescgtaxonomy {
  ...
  command <<<
    anvi_estimate_scg_taxonomy \
      ...
      ~{if defined(per_scg_output_file) then ("--per-scg-output-file " +  '"' + per_scg_output_file + '"') else ""} \
  ...
  output {
    ...
    File out_per_scg_output_file = "${in_per_scg_output_file}"
