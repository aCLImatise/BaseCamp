...
task Extractbadreads {
  input {
    ...
    Boolean? arg_size_excludingreads
    Boolean? arg_maximum_excludingreads
  ...
  command <<<
    extract_bad_reads \
      ...
      ~{if (arg_size_excludingreads) then "-m" else ""} \
      ~{if (arg_maximum_excludingreads) then "-M" else ""} \
  ...
  parameter_meta {
    ...
    arg_size_excludingreads: "[ --min_insert_size ] arg (=-1)  Minimum internal segment size (excluding\\nreads) for a pair to be considered good\\n(and thus not be extracted)."
    arg_maximum_excludingreads: "[ --max_insert_size ] arg (=-1)  Maximum internal segment size (excluding\\nreads) for a pair to be considered good\\n(and thus not be extracted)."
    ...
    also_use_reads: "[ --use_hard_clipped ]           Also use hard clipped reads (default:\\nignore hard clipped reads).\\n"
