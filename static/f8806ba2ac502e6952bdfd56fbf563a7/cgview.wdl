...
task Cgview {
  ...
  command <<<
    cgview \
      ~{java} \
      ~{if defined(specifies_label_size) then ("-A " +  '"' + specifies_label_size + '"') else ""} \
      ~{if defined(specifies_base_center) then ("-c " +  '"' + specifies_base_center + '"') else ""} \
      ~{if defined(specifies_legend_size) then ("-D " +  '"' + specifies_legend_size + '"') else ""} \
      ...
      ~{if defined(format_output_svg) then ("-f " +  '"' + format_output_svg + '"') else ""} \
      ...
      ~{if defined(path_image_file) then ("-p " +  '"' + path_image_file + '"') else ""} \
      ...
      ~{if defined(specifies_sequence_size) then ("-U " +  '"' + specifies_sequence_size + '"') else ""} \
  ...
  >>>
  parameter_meta {
    specifies_label_size: "Specifies a label font size."
    specifies_base_center: "Specifies the base to center on when zooming."
    specifies_legend_size: "Specifies a legend font size."
    ...
    format_output_svg: "The format of the output: PNG, JPG, SVG, or SVGZ."
    ...
    path_image_file: "The path to the image file in the HTML file created using the -h option."
    ...
    specifies_sequence_size: "Specifies a sequence ruler font size."
