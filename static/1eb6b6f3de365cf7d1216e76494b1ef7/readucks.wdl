...
task Readucks {
  ...
  command <<<
    readucks \
      ...
      ~{if (report_alternate_call) then "--report_alternate_call" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/readucks:0.0.3--py_0"
  }
  parameter_meta {
    ...
    report_alternate_call: "Reports double/single barcode call to csv in\\nsingle/double barcoding mode (default: False)"
