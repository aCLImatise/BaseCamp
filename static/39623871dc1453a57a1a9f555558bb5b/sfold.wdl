...
task Sfold {
  input {
    ...
    Int? use_int_pins
    Int? use_predicted_pins
    Boolean? output_single_pair
    Boolean? execute_unconstrained_algorithm
  ...
  command <<<
    sfold \
      ...
      ~{if defined(use_predicted_pins) then ("-P " +  '"' + use_predicted_pins + '"') else ""} \
      ~{if (output_single_pair) then "-V" else ""} \
      ~{if (execute_unconstrained_algorithm) then "-f" else ""} \
  ...
  parameter_meta {
    ...
    use_predicted_pins: ": use <int> predicted pins"
    output_single_pair: ": output as single sequences rather than pair"
    execute_unconstrained_algorithm: ": execute full (unconstrained) algorithm"
