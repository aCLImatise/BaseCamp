...
- id: in_local_csq
  ...
  inputBinding:
    prefix: --local-csq
- id: in_nc_sq
  doc: maximum number of per-haplotype consequences to consider for each site [15]
  type: long?
  inputBinding:
    prefix: --ncsq
- id: in_phase
  doc: "|m|r|R|s           how to handle unphased heterozygous genotypes: [r]\na:\
    \ take GTs as is, create haplotypes regardless of phase (0/1 -> 0|1)\nm: merge\
    \ *all* GTs into a single haplotype (0/1 -> 1, 1/2 -> 1)\nr: require phased GTs,\
    \ throw an error on unphased het GTs\nR: create non-reference haplotypes if possible\
...
- id: in_output
  ...
  inputBinding:
    prefix: --output
- id: in_output_type
  doc: "|u|z|v|t     b: compressed BCF, u: uncompressed BCF, z: compressed VCF\nv:\
