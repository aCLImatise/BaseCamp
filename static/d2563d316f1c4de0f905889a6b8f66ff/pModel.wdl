...
task PModel {
  input {
    ...
    Boolean? print_parameters_scores
    Boolean? print_parameters_probs
    Boolean? linearize_output_valid
  ...
  command <<<
    pModel \
      ...
      ~{if (print_parameters_scores) then "-q" else ""} \
      ~{if (print_parameters_probs) then "-S" else ""} \
      ~{if (linearize_output_valid) then "-l" else ""} \
  ...
  parameter_meta {
    ...
    print_parameters_scores: ": Print parameters as scores (defaults to as probs)"
    print_parameters_probs: ": Print parameters as both probs and scores"
    linearize_output_valid: ": Linearize output for gnuplot (not valid with -q or -S)"
