...
task OmeroGroupAdd {
  ...
  command <<<
    omero group add \
      ...
      ~{omer_o_tmpdir} \
      ~{omer_o_password} \
      ~{true="--ignore-existing" false="" ignore_existing} \
  ...
  >>>
  parameter_meta {
    ignore_existing: "Do not fail if user already exists"
    perms: "Group permissions set as string, e.g. 'rw----' "
    type: "Group permissions set symbolically"
