...
inputs:
- id: in_dataset
  doc: "Labels for one or more target datasets; each can be a\nFASTA file, taxonomy\
    \ ID to download, or dataset. The\nformat is as specified for --dataset in design.py."
  type: string[]
  inputBinding:
    prefix: --dataset
- id: in_probes_fast_a
...
- id: in_write_probe_map_counts_to_tsv
  doc: "The file to which to write a TSV-formatted list of the\nnumber of sequences\
    \ each probe maps to. This\nexplicitly does not count reverse complements."
  type: long?
  inputBinding:
