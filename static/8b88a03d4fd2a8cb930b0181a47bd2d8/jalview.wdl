...
task Jalview {
  ...
  command <<<
    jalview \
      ...
      ~{if defined(jvm_mem_pc) then ("-jvmmempc " +  '"' + jvm_mem_pc + '"') else ""} \
      ~{if defined(jvm_mem_max) then ("-jvmmemmax " +  '"' + jvm_mem_max + '"') else ""} \
      ~{if (open) then "-open" else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/jalview:2.11.1.3--0"
  }
  parameter_meta {
    ...
    jvm_mem_pc: "Only available with standalone executable jar or jalview.bin.Launcher. Limit maximum heap size (memory) to PERCENT% of total physical memory detected. This defaults to 90 if total physical memory can be detected. See https://www.jalview.org/help/html/memory.html for more details."
    jvm_mem_max: "Only available with standalone executable jar or jalview.bin.Launcher. Limit maximum heap size (memory) to MAXMEMORY. MAXMEMORY can be specified in bytes, kilobytes(k), megabytes(m), gigabytes(g) or if you're lucky enough, terabytes(t). This defaults to 32g if total physical memory can be detected, or to 8g if total physical memory cannot be detected. See https://www.jalview.org/help/html/memory.html for more details."
    open: ""
    var_file: ""
    output_format: ""
    output_file: ""
