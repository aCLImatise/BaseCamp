...
- id: in_flag_followed_genomes
  doc: "This flag should be followed with embplant_pt\n(embryophyta plant plastome),\
    ...
    \ any of above organelle genomes split by\ncomma(s), which might be computationally\
    \ more\nintensive than separate runs), or anonym (uncertain\norganelle genome\
    \ type, with customized gene database\n('--genes'), which is suggested only when\
    \ the above\ndatabase is genetically distant from your sample,\ngenerally only\
    \ in some animal_mt and fungus_mt cases).\nFor easy usage and compatibility of\
...
- id: in_reverse_lsc
  doc: "For '-F embplant_pt' with complete circular result, by\ndefault, the direction\
    ...
    \ with less ORFs. Choose this option to\nreverse the direction of the starting\
