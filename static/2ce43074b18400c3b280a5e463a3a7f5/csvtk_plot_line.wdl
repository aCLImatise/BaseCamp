...
task CsvtkPlotLine {
  input {
    Int colorColorIndex
    String dataDataFieldX
    String dataDataFieldY
    Boolean legendLegendLeft
    Boolean legendLegendTop
    Float lineLineWidth
    Float pointPointSize
    Boolean scatterScatter
    Float axisAxisWidth
    Int chunkChunkSize
  ...
  command <<<
    csvtk plot line \
      ...
      ~{if defined(dataDataFieldX) then ("--data-field-x " +  '"' + dataDataFieldX + '"') else ""} \
      ~{if defined(dataDataFieldY) then ("--data-field-y " +  '"' + dataDataFieldY + '"') else ""} \
      ~{true="--legend-left" false="" legendLegendLeft} \
      ~{true="--legend-top" false="" legendLegendTop} \
      ~{if defined(lineLineWidth) then ("--line-width " +  '"' + lineLineWidth + '"') else ""} \
      ~{if defined(pointPointSize) then ("--point-size " +  '"' + pointPointSize + '"') else ""} \
      ~{true="--scatter" false="" scatterScatter} \
