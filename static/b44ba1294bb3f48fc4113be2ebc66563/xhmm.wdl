...
task Xhmm {
  input {
    ...
    String scores
    String analyzed
    String print_hmm_model
    String print_hmm_transition
  }
  command <<<
    xhmm \
      ...
      ~{if defined(g_atk_depths) then ("--GATKdepths " +  '"' + g_atk_depths + '"') else ""} \
      ...
      ~{if defined(merge_vcf) then ("--mergeVCF " +  '"' + merge_vcf + '"') else ""} \
      ...
      ~{if (var_70) then "--printHMM" else ""} \
      ~{if (var_71) then "--transition" else ""}
  >>>
  parameter_meta {
    ...
    prepare_targets: ""
    targets: "Input targets lists"
    merged_targets: "Output targets list  (default=`-')"
    merge_g_atk_depths: ""
    g_atk_depths: "GATK sample_interval_summary output file(s) to\\nbe merged [must have *IDENTICAL* target\\nlists]"
    ...
    var_49: "Matrix is read from --readDepths argument"
    ...
    var_53: "Matrix is read from --readDepths argument"
    ...
    merge_v_cfs: ""
    merge_vcf: "VCF file(s) to be merged [must have *IDENTICAL*\\ngenotyped intervals]"
    ...
    print_hmm_model: "Print HMM model parameters and exit"
    print_hmm_transition: "Print HMM transition matrix for user-requested genomic distances"
  }
  output {
    File out_stdout = stdout()
    File out_g_atk_depths = "${in_g_atk_depths}"
