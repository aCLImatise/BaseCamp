...
task Cdbyank {
  input {
    ...
    String? retrieval_indexeddatabase_had
    String? insensitive_query_expects
    String? query_key_surrounded
    String? q_use_char
    String? warnings_sent_stderr
    String? only_defline_sequence
    File? displays_positions_file
  ...
  command <<<
    cdbyank \
      ...
      ~{if defined(retrieval_indexeddatabase_had) then ("-x " +  '"' + retrieval_indexeddatabase_had + '"') else ""} \
      ...
      ~{if defined(warnings_sent_stderr) then ("-w " +  '"' + warnings_sent_stderr + '"') else ""} \
      ~{if defined(only_defline_sequence) then ("-F " +  '"' + only_defline_sequence + '"') else ""} \
      ~{if defined(displays_positions_file) then ("-P " +  '"' + displays_positions_file + '"') else ""} \
  ...
  parameter_meta {
    ...
    warnings_sent_stderr: "warnings (sent to stderr) when a key is not found"
    only_defline_sequence: "only the defline for each record (discard the sequence)"
    displays_positions_file: "displays the position(s) (file offset) within the\\ndatabase file, for the requested record(s)"
