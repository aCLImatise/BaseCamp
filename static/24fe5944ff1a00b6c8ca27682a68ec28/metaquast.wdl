...
task Metaquast.py {
  input {
    ...
    Boolean testTest
    Boolean testTestNoRef
  }
  command <<<
    metaquast.py \
      ~{true="--fungus" false="" fungusFungus} \
      ...
      ~{if defined(maxMaxRefNumber) then ("--max-ref-number " +  '"' + maxMaxRefNumber + '"') else ""} \
      ~{if defined(blastBlastDb) then ("--blast-db " +  '"' + blastBlastDb + '"') else ""} \
      ~{true="--use-input-ref-order" false="" useUseInputRefOrder} \
      ...
      ~{true="--unique-mapping" false="" uniqueUniqueMapping} \
      ...
      ~{true="--test-no-ref" false="" testTestNoRef}
  >>>
}