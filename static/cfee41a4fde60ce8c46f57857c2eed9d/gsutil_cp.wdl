...
task GsutilCp {
  input {
    ...
    Boolean? exclude_symlinks_specified
    File? outputs_manifest_file
    Boolean? noclobber_when_specified
    Boolean? causes_acls_preserved
    Boolean? causes_posix_attributes
    Boolean? r_r_options
    String? storage_class_destination
    Boolean? skip_objects_objects
    Boolean? requests_given_make
    Boolean? ext_applies_gzip
    Boolean? applies_gzip_contentencoding
    String metadata_dot
  ...
  command <<<
    gsutil cp \
      ...
      ~{if (exclude_symlinks_specified) then "-e" else ""} \
      ...
      ~{if defined(storage_class_destination) then ("-s " +  '"' + storage_class_destination + '"') else ""} \
      ~{if (skip_objects_objects) then "-U" else ""} \
      ~{if (requests_given_make) then "-v" else ""} \
      ~{if (ext_applies_gzip) then "-z" else ""} \
      ~{if (applies_gzip_contentencoding) then "-Z" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    exclude_symlinks_specified: "Exclude symlinks. When specified, symbolic links will not be"
