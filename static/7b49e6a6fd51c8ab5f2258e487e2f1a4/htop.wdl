...
task Htop {
  input {
    Boolean? emulate_hmm_hmmer
    Boolean? force_insert_scores
    Boolean? impose_limit_line
    Boolean? assume_input_be
    Boolean? implement_semiglobal_alignment
    Boolean? value_hmmer_base_default
    Boolean? valuelevel_zero_cutoff
    Boolean? value_score_default
    Boolean? value_only_effective
    Boolean? value_hmmer_specificscore_default
  ...
  command <<<
    htop \
      ~{if (emulate_hmm_hmmer) then "-f" else ""} \
      ~{if (force_insert_scores) then "-i" else ""} \
      ~{if (impose_limit_line) then "-l" else ""} \
      ~{if (assume_input_be) then "-o" else ""} \
      ~{if (implement_semiglobal_alignment) then "-s" else ""} \
      ~{if (value_hmmer_base_default) then "-B" else ""} \
      ~{if (valuelevel_zero_cutoff) then "-C" else ""} \
      ~{if (value_score_default) then "-F" else ""} \
      ~{if (value_only_effective) then "-H" else ""} \
      ~{if (value_hmmer_specificscore_default) then "-L" else ""} \
