...
- id: in_isoform
  doc: "=    Enables the isoform merging. All isoforms are united to a single entity\
    \ and treated as one. Extracting a group with an isoform will result in all isoforms.\n\
    ncbi -> if the word 'isoform' is found\nuniprot -> 'Isoform of XYZ' (You need\
    \ to add the *_additional.fasta files to the analysis)\ntrinity -> using '_iX'\
...
- id: in_self_blast
  ...
  inputBinding:
    prefix: -selfblast
- id: in_sim
  doc: "=        min. reciprocal similarity for additional hits (0..1) [default: 0.95]\n\
    1 : only the best reciprocal hits are reported\n0 : all possible reciprocal blast\
    \ matches (within the -evalue) are reported"
  type: boolean?
  inputBinding:
    prefix: -sim
- id: in_identity
