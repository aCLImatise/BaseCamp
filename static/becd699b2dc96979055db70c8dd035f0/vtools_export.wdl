...
task VtoolsExport {
  input {
    ...
    File? samples_exported_specified
    File? format
    Int? build
    Boolean? header
    Int? jobs
  ...
  command <<<
    vtools export \
      ...
      ~{if (samples_exported_specified) then "-s" else ""} \
  ...
  output {
    ...
    File out_samples_exported_specified = "${in_samples_exported_specified}"
  }
}