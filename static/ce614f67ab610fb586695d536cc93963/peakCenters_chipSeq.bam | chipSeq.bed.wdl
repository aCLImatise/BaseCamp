...
task PeakCentersChipSeqbamchipSeqbed {
  input {
    Boolean? hist
    Boolean? report_depth_based
    String? counts
    Boolean? mean
  ...
  command <<<
    peakCenters chipSeq_bam___chipSeq_bed \
      ~{bed_tools} \
      ~{coverage} \
      ~{if (hist) then "-hist" else ""} \
      ~{if (report_depth_based) then "-d" else ""} \
