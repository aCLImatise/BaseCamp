...
- id: in_gap_ext
  ...
  inputBinding:
    prefix: --gap-ext
- id: in_amb_strand
  doc: "ambiguous strand mode [False]\nfor each input sequence, try the reverse complement\
    \ if the current\nalignment score is too low, and pick the strand with a higher\
    \ score"
  type: boolean?
  inputBinding:
    prefix: --amb-strand
...
- id: in_result
  doc: "INT       output result mode [0]\n- 0: consensus (FASTA format)\n- 1: MSA\
    \ (PIR format)\n- 2: both 0 & 1\n- 3: graph (GFA format)\n- 4: graph with consensus\
    \ path (GFA format)"
  type: boolean?
  inputBinding:
    prefix: --result
- id: in_msa_header
  doc: add read ID as header of each sequence in MSA output [False]
  type: boolean?
  inputBinding:
    prefix: --msa-header
- id: in_out_pog
...
- class: DockerRequirement
  dockerPull: quay.io/biocontainers/abpoa:1.0.6--hed695b0_0
cwlVersion: v1.1
baseCommand:
- abpoa
