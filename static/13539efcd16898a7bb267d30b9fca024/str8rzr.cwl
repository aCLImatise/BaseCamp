...
- id: out_stdout
  ...
  type: stdout
- id: out_filters_type_autosomes
  doc: (This filters on Type, e.g. AUTOSOMES; ie, it restricts the output to STRs
    that have the same type as specified in column 2 of the config file)
  type: File?
  outputBinding:
    glob: $(inputs.in_filters_type_autosomes)
- id: out_writes_output_filename
  doc: (This writes the output to filename, as opposed to standard out)
  type: File?
  outputBinding:
    glob: $(inputs.in_writes_output_filename)
hints: []
cwlVersion: v1.1
baseCommand:
- str8rzr
