...
- id: in_skip_in_del_profiling
  doc: "The alignment of a read to a reference genome/sequence\ncan be imperfect,\
    \ such that the read exhibits\ninsertions or deletions relative to the reference.\n\
    Anvi'o normally stores this information in the profile\ndatabase since the time\
    \ taken and extra storage do not\namount to much, but if you insist on not having\
    \ this\ninformation, you can skip storing this information by\nproviding this\
    \ flag. Note: If --skip-SNV-profiling is\nprovided, --skip-INDEL-profiling will\
    \ automatically be\nenforced. (default: False)"
  type: boolean?
  inputBinding:
    prefix: --skip-INDEL-profiling
- id: in_profile_s_cvs
...
- id: in_min_percent_identity
  doc: "Ignore any reads with a percent identity to the\nreference less than this\
    \ number, e.g. 95. If not\nprovided, all reads in the BAM file will be used (and\n\
...
- id: in_queue_size
  doc: "The queue size for worker threads to store data to\ncommunicate to the main\
    ...
    \ not change this value. (default: 0)"
  type: long?
  inputBinding:
    prefix: --queue-size
