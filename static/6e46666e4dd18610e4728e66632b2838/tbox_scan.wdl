...
task Tboxscan {
  ...
  command <<<
    tbox_scan \
      ...
      ~{if defined(save_txt_file) then ("-l " +  '"' + save_txt_file + '"') else ""} \
      ~{if (model_search_using) then "-m" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/tbox-scan:1.0.2--0"
  }
  parameter_meta {
  ...
  output {
    ...
    File out_save_txt_file = "${in_save_txt_file}"
  }
}