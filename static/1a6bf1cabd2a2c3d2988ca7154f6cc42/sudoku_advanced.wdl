...
task Sudokuadvanced {
  ...
  parameter_meta {
    model: "(int, set, mixed) default: int\\nmodel variants\\nint: use integer constraints\\nset: use set constraints\\nmixed: use both integer and set constraints"
    propagation: "(none, same) default: none\\npropagation variants\\nnone: no additional constraints\\nsame: additional \\\"same\\\" constraint for integer model"
    ipl: "(def,val,bnd,dom,basic,advanced)\\ndefault: dom\\ninteger propagation level (comma-separated list)"
    branching: "(none, size, sizedeg, sizeafc, afc) default: sizeafc\\nbranching variants\\nnone: none\\nsize: min size\\nsizedeg: min size over degree\\nsizeafc: min size over afc\\nafc: maximum afc"
