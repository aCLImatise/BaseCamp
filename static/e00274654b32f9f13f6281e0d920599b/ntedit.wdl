...
task Ntedit {
  input {
    ...
    Boolean? should_missing_attempt
    Boolean? ratio_number_present
    Boolean? cap_number_base
    Boolean? controls_size_kmer
    Boolean? mode_editing_range
    Boolean? snv_mode_overrides
    Boolean? verbose_mode_v
  }
  command <<<
    ntedit \
      ...
      ~{if (should_missing_attempt) then "-X" else ""} \
      ~{if (ratio_number_present) then "-Y" else ""} \
      ~{if (cap_number_base) then "-c" else ""} \
      ~{if (controls_size_kmer) then "-j" else ""} \
      ~{if (mode_editing_range) then "-m" else ""} \
      ~{if (snv_mode_overrides) then "-s" else ""} \
      ~{if (verbose_mode_v) then "-v" else ""}
  >>>
  parameter_meta {
    ...
    should_missing_attempt: ",     ratio of number of kmers in the k subset that should be missing in order to attempt fix (higher=stringent), [default=0.5]"
    ...
    controls_size_kmer: ",     controls size of kmer subset. When checking subset of kmers, check every jth kmer, [default=3]"
    mode_editing_range: ",     mode of editing, range 0-2, [default=0]\\n0: best substitution, or first good indel\\n1: best substitution, or best indel\\n2: best edit overall (suggestion that you reduce i and d for performance)"
    ...
    verbose_mode_v: ",     verbose mode (-v 1 = yes, default = 0, no)"
  }
  output {
