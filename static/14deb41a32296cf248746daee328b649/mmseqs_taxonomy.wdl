...
task MmseqsTaxonomy {
  ...
  command <<<
    mmseqs taxonomy \
      ...
      ~{if defined(orf_filter) then ("--orf-filter " +  '"' + orf_filter + '"') else ""} \
      ~{if defined(orf_filter_e) then ("--orf-filter-e " +  '"' + orf_filter_e + '"') else ""} \
      ~{if defined(orf_filter_s) then ("--orf-filter-s " +  '"' + orf_filter_s + '"') else ""} \
  ...
  parameter_meta {
    ...
    sensitivity_faster_fast: "Sensitivity: 1.0 faster; 4.0 fast; 7.5 sensitive [2.000]"
    ...
    list_matches_evalue: "List matches below this E-value (range 0.0-inf) [1.000E+00]"
    ...
    max_rejected: "Maximum rejected alignments before alignment calculation for a query is stopped [5]"
    max_accept: "Maximum accepted alignments before alignment calculation for a query is stopped [30]"
    ...
    orf_filter: "Prefilter query ORFs with non-selective search\\nOnly used during nucleotide-vs-protein classification\\nNOTE: Consider disabling when classifying short reads [1]"
    orf_filter_e: "E-value threshold used for query ORF prefiltering [1.000E+02]"
    orf_filter_s: "Sensitivity used for query ORF prefiltering [2.000]"
    lca_mode: "LCA Mode 1: single search LCA , 2/3: approximate 2bLCA, 4: top hit [3]"
    tax_output_mode: "0: output LCA, 1: output alignment 2: output both [0]"
