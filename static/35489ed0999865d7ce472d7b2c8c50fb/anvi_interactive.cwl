...
- id: in_manual_mode
  doc: "Using this flag, you can run the interactive interface\nin an ad hoc manner\
    ...
    \ create an empty one for you. (default:\nFalse)"
  type: boolean?
  inputBinding:
    prefix: --manual-mode
- id: in_fast_a_file
...
- id: in_show_all_layers
  doc: "When declared, this flag tells the interface to show\nevery additional layer\
    \ even if there are no hits. By\ndefault, anvi'o doesn't show layers if there\
    \ are no\nhits for any of your items. (default: False)"
  type: boolean?
  inputBinding:
    prefix: --show-all-layers
