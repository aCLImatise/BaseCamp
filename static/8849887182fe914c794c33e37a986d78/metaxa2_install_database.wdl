...
task Metaxa2InstallDatabase {
  input {
    String? specify_name_specified
    String? directory_where_install
    String? repository_download_files
  }
  command <<<
    metaxa2_install_database \
      ~{if defined(specify_name_specified) then ("-g " +  '"' + specify_name_specified + '"') else ""} \
  ...
  parameter_meta {
    specify_name_specified: ": Specify the name of the database to install (usually a gene name)\\nIf not specified, the program will show a list of available database options"
