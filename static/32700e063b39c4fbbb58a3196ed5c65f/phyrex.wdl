...
task Phyrex {
  input {
    ...
    Boolean? multiple_mnbdatasetsmmnbdatasetsm_integer
    Boolean? or__parsm
    Boolean? bootstrap_mintmmintm_mintm
    Boolean? t_be
    Boolean? model_mmodelmmodelm_substitution
    Boolean? aa_rate_file
    Boolean? mem_mmm_mfafcfgftmmem
    Boolean? tstv_mtstvratiomtstvratiom_transitiontransversion
    Boolean? pinv_proportion_be
    Boolean? nclasses_mnbsubstcatmnbsubstcatm_number
    Boolean? free_rates
    Boolean? alpha_mgammamgammam_distribution
    Boolean? _search_operation
    Boolean? inputtree_musertreefilemusertreefilem_starting
    Boolean? option_focuses_optimisationmparamsmtlr
    Boolean? rand_start
  ...
  command <<<
    phyrex \
      ...
      ~{if (multiple_mnbdatasetsmmnbdatasetsm_integer) then "-n" else ""} \
      ~{if (or__parsm) then "-p" else ""} \
      ~{if (bootstrap_mintmmintm_mintm) then "-b" else ""} \
      ~{if (t_be) then "--tbe" else ""} \
      ~{if (model_mmodelmmodelm_substitution) then "-m" else ""} \
      ~{if (aa_rate_file) then "--aa_rate_file" else ""} \
      ~{if (mem_mmm_mfafcfgftmmem) then "-f" else ""} \
      ~{if (tstv_mtstvratiomtstvratiom_transitiontransversion) then "-t" else ""} \
      ~{if (pinv_proportion_be) then "-v" else ""} \
      ~{if (nclasses_mnbsubstcatmnbsubstcatm_number) then "-c" else ""} \
      ~{if (free_rates) then "--freerates" else ""} \
      ~{if (alpha_mgammamgammam_distribution) then "-a" else ""} \
      ~{if (_search_operation) then "-s" else ""} \
      ~{if (inputtree_musertreefilemusertreefilem_starting) then "-u" else ""} \
      ~{if (option_focuses_optimisationmparamsmtlr) then "-o" else ""} \
