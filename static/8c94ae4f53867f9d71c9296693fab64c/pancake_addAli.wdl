...
task PancakeAddAli {
  ...
  command <<<
    pancake addAli \
      ...
      ~{if defined(output_pangenome_panfile) then ("--output " +  '"' + output_pangenome_panfile + '"') else ""} \
      ~{if defined(min_len) then ("--min_len " +  '"' + min_len + '"') else ""} \
      ~{if (no_self_alignments) then "--no_self_alignments" else ""}
  >>>
  parameter_meta {
    pan_file: "Name of PanCake Data Object File (required)"
    output_pangenome_panfile: "output pangenome file (PAN_FILE will be unchanged)"
  ...
  output {
    ...
    File out_output_pangenome_panfile = "${in_output_pangenome_panfile}"
  }
}