...
task Pyrad {
  input {
    ...
    Boolean? creates_new_dtest_file
  }
  command <<<
    pyrad \
      ...
      ~{if (creates_new_empty_file) then "-n" else ""} \
      ~{if (creates_new_dtest_file) then "-D" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    creates_new_empty_file: "creates a new empty input params.txt file"
    creates_new_dtest_file: "creates a new empty Dtest input file"
  }
  output {
