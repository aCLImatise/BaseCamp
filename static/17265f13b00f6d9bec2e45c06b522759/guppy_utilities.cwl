...
id: ../../../../home/ubuntu/BiocondaCli/guppy_utilities.cwl
inputs:
- id: fat
...
- id: classification
  doc: classify                outputs classification information in a tabular or
    SQLite format classify_rdp            converts RDP output to something resembling
    guppy classify output to_rdp                  convert a reference package to a
    format RDP wants
  type: string
  inputBinding:
    position: 0
- id: utilities
  doc: adcl                    calculates ADCL for each pquery in a placefile check                   checks
    ...
    and Transform placefiles ograph                  finds the mass islands of one
    or more pqueries rarefy                  performs rarefaction on collections of
    placements redup                   restores duplicates to deduped placefiles round                   clusters
    the placements by rounding branch lengths to_csv                  turns a placefile
    into a csv file to_json                 converts old-style .place files to .jplace
    placement files trim                    trims placefiles down to only containing
    an informative subset of the mass
  type: string
  inputBinding:
    position: 1
outputs: []
