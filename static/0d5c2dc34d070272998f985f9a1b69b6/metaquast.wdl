...
task Metaquast.py {
  ...
  command <<<
    metaquast.py \
      ...
      ~{if defined(max_ref_number) then ("--max-ref-number " +  '"' + max_ref_number + '"') else ""} \
      ~{if defined(blast_db) then ("--blast-db " +  '"' + blast_db + '"') else ""} \
      ~{true="--use-input-ref-order" false="" use_input_ref_order} \
      ...
      ~{true="--unique-mapping" false="" unique_mapping} \
      ...
      ~{true="--test-no-ref" false="" test_no_ref}
  >>>
  parameter_meta {
    ...
    max_ref_number: "Maximum number of references (per each assembly) to download after looking in SILVA database. Set 0 for not looking in SILVA at all [default: 50]"
    blast_db: "Custom BLAST database (.nsq file). By default, MetaQUAST searches references in SILVA database"
    use_input_ref_order: "Use provided order of references in MetaQUAST summary plots (default order: by the best average value)"
    ...
    unique_mapping: "Disable --ambiguity-usage=all for the combined reference run, i.e. use user-specified or default ('one') value of --ambiguity-usage"
    ...
    test_no_ref: "Run MetaQUAST without references on the data from the test_data folder, output to quast_test_output. MetaQUAST will download SILVA 16S rRNA database (~170 Mb) for searching reference genomes Internet connection is required"
    python: ""
  }
}