...
task Obisplit {
  ...
  command <<<
    obisplit \
      ...
      ~{if defined(tag_name) then ("--tag-name " +  '"' + tag_name + '"') else ""} \
      ~{if defined(undefined) then ("--undefined " +  '"' + undefined + '"') else ""} \
  ...
  parameter_meta {
    ...
    prefix: "Prefix added to each subfile name"
    tag_name: "Attribute used to split the sequence file"
    undefined: "Name of the file where undefined sequenced are stored"
