...
task Pbbamify {
  ...
  command <<<
    pbbamify \
      ...
      ~{if (file_path_specified) then "--output" else ""} \
  ...
  parameter_meta {
    ...
    file_path_specified: "FILE  Path to the output BAM file. If not specified, output will be to the stdout."
  ...
  output {
    ...
    File out_file_path_specified = "${in_file_path_specified}"
  }
}