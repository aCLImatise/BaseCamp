...
task ConsensusFixer {
  ...
  command <<<
    ConsensusFixer \
      ~{java} \
      ~{options_dot_dot_dot} \
      ~{if defined(alignment_file_required) then ("-i " +  '"' + alignment_file_required + '"') else ""} \
      ...
      ~{if (remove_gaps_are) then "-d" else ""} \
      ~{if (mi) then "-mi" else ""} \
      ~{if (pi) then "-pi" else ""} \
  ...
  parameter_meta {
    alignment_file_required: ": Alignment file in BAM format (required)."
    ...
    remove_gaps_are: ": Remove gaps if they are >= pluralityN."
