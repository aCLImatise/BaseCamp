...
task Runbwamem {
  input {
    ...
    Int? read_type_pacbio
    Int? number_of_threads
    Boolean? apply_hla_typing
    Boolean? trim_pe_adapters
  ...
  command <<<
    run_bwamem \
      ...
      ~{if defined(read_type_pacbio) then ("-x " +  '"' + read_type_pacbio + '"') else ""} \
      ~{if defined(number_of_threads) then ("-t " +  '"' + number_of_threads + '"') else ""} \
      ~{if (apply_hla_typing) then "-H" else ""} \
      ~{if (trim_pe_adapters) then "-a" else ""} \
