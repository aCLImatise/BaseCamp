...
task ConsensusID {
  ...
  command <<<
    ConsensusID \
      ...
      ~{if (per_spectrum) then "-per_spectrum" else ""} \
  ...
  parameter_meta {
    in: "<file(s)>*                    Input file (valid formats: 'idXML', 'featureXML', 'consensusXML')"
    ...
    per_spectrum: "(only idXML) if set, mapping will be done based on exact matching of originating mzml file and spectrum_ref"
