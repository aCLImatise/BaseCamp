...
task VariantFilterpy {
  input {
    ...
    Boolean? optimise_p
    Int? random_seed
    String _cogfilter_apply
  }
  command <<<
    Variant_Filter_py \
      ~{_cogfilter_apply} \
      ~{if (binomial_loge_likelihood) then "-f" else ""} \
      ~{if (specifies_value_) then "-q" else ""} \
  ...
  parameter_meta {
    ...
    _cogfilter_apply: "-c, --cog_filter      whether to apply COG filtering default false"
  }
  output {
