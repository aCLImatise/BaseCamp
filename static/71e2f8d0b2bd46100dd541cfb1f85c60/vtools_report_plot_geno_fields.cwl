...
inputs:
- id: fields
  doc: A list of genotype fields that will be outputted.
  type: string
  inputBinding:
    position: 0
- id: variants
  doc: Limit value of fields to variant in specified variant table. Default to all
    variants.
  type: string
  inputBinding:
    prefix: --variants
- id: samples
  ...
  inputBinding:
    prefix: --samples
- id: genotypes
  doc: '[GENOTYPES [GENOTYPES ...]] Conditions based on which genotypes are selected.
