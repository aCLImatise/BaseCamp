...
task Bamsomaticsniper {
  input {
    ...
    String? normal_sample_i
    String? tumor_sample_i
  ...
  command <<<
    bam_somaticsniper \
      ...
      ~{if defined(filtering_snv_output) then ("-Q " +  '"' + filtering_snv_output + '"') else ""} \
      ...
      ~{if defined(normal_sample_i) then ("-n " +  '"' + normal_sample_i + '"') else ""} \
      ~{if defined(tumor_sample_i) then ("-t " +  '"' + tumor_sample_i + '"') else ""} \
  ...
  parameter_meta {
    ...
    filtering_snv_output: "filtering somatic snv output with somatic quality less than  INT [15]"
    ...
    normal_sample_i: "normal sample id (for VCF header) [NORMAL]"
    tumor_sample_i: "tumor sample id (for VCF header) [TUMOR]"
