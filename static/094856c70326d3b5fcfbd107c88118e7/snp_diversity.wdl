...
task SnpDiversity.py {
  ...
  command <<<
    snp_diversity.py \
      ...
      ~{true="--weight_by_depth" false="" weight_by_depth} \
      ~{if defined(rand_reads) then ("--rand_reads " +  '"' + rand_reads + '"') else ""} \
      ~{true="--replace_reads" false="" replace_reads} \
  ...
  parameter_meta {
    ...
    keep_samples: "comma-separated list of samples to use for computing diversity metrics. samples will still be subject to other filters"
    exclude_samples: "comma-separated list of samples to exclude from computing diversity metrics. samples will still be subject to other filters"
    site_list: "path to file containing newline-delimited list of genomic sites to include. other filters will still apply"
