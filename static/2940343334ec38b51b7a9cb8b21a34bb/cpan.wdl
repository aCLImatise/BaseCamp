...
task Cpan {
  input {
    ...
    Boolean? test_modules_install
    Boolean? upgrade_modules_breakthings
  ...
  command <<<
    cpan \
      ...
      ~{if (load_loading_paths) then "-I" else ""} \
      ~{if defined(load_file_cpan) then ("-j " +  '"' + load_file_cpan + '"') else ""} \
      ...
      ~{if (dry_run_actually) then "-n" else ""} \
      ...
      ~{if (test_modules_install) then "-T" else ""} \
      ~{if (upgrade_modules_breakthings) then "-u" else ""} \
  ...
  parameter_meta {
    ...
    load_loading_paths: "Load \\\"local::lib\\\" (think like \\\"-I\\\" for loading lib paths). Too bad\\n\\\"-l\\\" was already taken."
    load_file_cpan: "Load the file that has the CPAN configuration data. This should have\\nthe same format as the standard CPAN/Config.pm file, which defines\\n$CPAN::Config as an anonymous hash."
    ...
    dry_run_actually: "Do a dry run, but don't actually install anything. (unimplemented)"
    ...
    test_modules_install: "Do not test modules. Simply install them."
    upgrade_modules_breakthings: "Upgrade all installed modules. Blindly doing this can really break\\nthings, so keep a backup."
