...
- id: in_n_thread
  ...
  inputBinding:
    prefix: --nthread
- id: in_t_stat
  doc: "The number of threads for the statistical model. If\nnot set then the value\
    \ of --nthread is used"
  type: long?
  inputBinding:
    prefix: --tstat
...
- id: in_mel
  ...
  inputBinding:
    prefix: --mel
- id: in_allow_clipping
  doc: Allow alignments with soft or hard clipping to be used
  type: boolean?
  inputBinding:
    prefix: --allow-clipping
- id: in_fixed_event_set
  doc: "A directory containing fromGTF.[AS].txt files to be\nused instead of detecting\
    \ a new set of events\n"
  type: Directory?
  inputBinding:
