...
- id: query_2
  ...
  inputBinding:
    prefix: --query2
- id: output
  doc: 'BAM/SAM output file (default: SAM format)  [REQUIRED]'
  type: string
  inputBinding:
    prefix: --output
- id: fast
...
- id: errors_rate
  ...
  inputBinding:
    prefix: --errors-rate
- id: errors_per_base
  doc: 'alternative to errors-rate: number of errors  per base in the range [0,0.2[
    (equal to  1/errors-rate)'
  type: string
  inputBinding:
...
- id: print_all
  doc: print all possible alignments, see manual for  output syntax [only for single-end
    reads]
  type: boolean
  inputBinding:
    prefix: --print-all
- id: print_at_most
  doc: print at most the number of specified  alignments, see manual for output syntax
