...
- id: in_skip_tn_adjustments
  ...
  inputBinding:
    prefix: -D
- id: in_float_maximum_def
  doc: <float>      Maximum p-value (def. 0.01)
  type: boolean?
  inputBinding:
    prefix: -p
- id: in_float_fdradjusted_def
...
- id: out_stdout
  ...
  type: stdout
- id: out_file_peak_file
  doc: <file>       Output peak file (in ENCODE narrowPeak format)
  type: File?
  outputBinding:
    glob: $(inputs.in_file_peak_file)
- id: out_file_file_pq
  doc: <file>       Output bedgraph-ish file for p/q values
  type: File?
  outputBinding:
    glob: $(inputs.in_file_file_pq)
- id: out_file_file_pileups
  doc: <file>       Output bedgraph-ish file for pileups and p-values
  type: File?
  outputBinding:
    glob: $(inputs.in_file_file_pileups)
- id: out_file_bed_file
  doc: <file>       Output BED file for reads/fragments/intervals
  type: File?
  outputBinding:
    glob: $(inputs.in_file_bed_file)
