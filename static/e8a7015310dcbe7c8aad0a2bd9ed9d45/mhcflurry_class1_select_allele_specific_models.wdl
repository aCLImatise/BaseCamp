...
task Mhcflurryclass1selectallelespecificmodels {
  input {
    ...
    String? unselected_accuracy_scorer
    Int? unselected_accuracy_scorer_num_samples
  ...
  command <<<
    mhcflurry_class1_select_allele_specific_models \
      ...
      ~{if defined(unselected_accuracy_scorer) then ("--unselected-accuracy-scorer " +  '"' + unselected_accuracy_scorer + '"') else ""} \
      ~{if defined(unselected_accuracy_scorer_num_samples) then ("--unselected-accuracy-scorer-num-samples " +  '"' + unselected_accuracy_scorer_num_samples + '"') else ""} \
  ...
  parameter_meta {
    ...
    alleles_select_models: "Alleles to select models for. If not specified, all\\nalleles with enough measurements will be used."
