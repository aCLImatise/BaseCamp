...
task VtoolsEvaluate {
  ...
  >>>
  parameter_meta {
    call_vcf: "Path to VCF with calls to be evaluated [required]"
    positive_vcf: "Path to VCF with known calls  [required]"
    call_samples: "Sample(s) in call-vcf to consider. May be called multiple times  [required]"
    positive_samples: "Sample(s) in positive-vcf to consider. May be called multiple times  [required]"
