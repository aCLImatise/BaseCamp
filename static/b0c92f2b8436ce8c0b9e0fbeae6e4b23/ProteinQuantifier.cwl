...
- id: in_include_all
  ...
  inputBinding:
    prefix: -include_all
- id: in_best_charge_and_fraction
  doc: "Distinguish between fraction and charge states of a peptide. For peptides,\
    ...
    \ abundances will be computed based only on the most prevalent charge observed\
    \ of each peptide (over all fractions).\nBy default, abundances are summed over\
    \ all charge states."
  type: boolean?
  inputBinding:
