...
task NGseqBasic {
  input {
    ...
    Boolean? bowtie_one
    Int? chunk_mb
    Int? run_bowtie_parameter
    Int? max_ins
    String? no_bowtie
    Boolean? flash
    File? blacklist_filter_slash_no_blacklist_filter
    Boolean? footprint
    Boolean? peak_call
    Boolean? next_era
    Boolean? trim_slashnot_rim
    Boolean? trim_five
  ...
  parameter_meta {
    ...
    bowtie_one: "/ --bowtie2 (default is bowtie1 - decide if bowtie1 or bowtie2 is to be used. bowtie2 is better to long reads - read lenght more than 70b, fragment lenght more than 350b)"
    chunk_mb: "- memory allocated to Bowtie, defaults to 256mb - only affects bowtie1 run"
    run_bowtie_parameter: "run with bowtie parameter M=2 (if maps more than M times, report one alignment in random) - only affects bowtie1 run"
    ...
    blacklist_filter_slash_no_blacklist_filter: "- supported genomes mm9 mm10 hg18 hg19 (filter/do-not-filter blacklisted regions out from the final bam file)"
    ...
    trim_slashnot_rim: "** (run/do-not-run TrimGalore for the data - Illumina PE standard adapter filter, trims on 3' end)"
  ...
  output {
    ...
    File out_blacklist_filter_slash_no_blacklist_filter = "${in_blacklist_filter_slash_no_blacklist_filter}"
