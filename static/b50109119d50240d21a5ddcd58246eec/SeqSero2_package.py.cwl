...
- id: in_int_forseparated_pairedend
  ...
  inputBinding:
    prefix: -t
- id: in_string_algorithms_bwa
  doc: "<string>: algorithms for bwa mapping for allele mode; 'mem'\nfor mem, 'sam'\
    \ for samse/sampe; default=mem; optional; for\nnow we only optimized for default\
    \ 'mem' mode"
  type: string?
  inputBinding:
    prefix: -b
...
- id: in_flag_flagged_not
  doc: "<flag>: if '-s' was flagged, SeqSero2 will not output header\nin SeqSero_result.tsv"
  type: boolean?
  inputBinding:
    prefix: -s
- id: in_check
...
- id: in_d
  ...
  inputBinding:
    prefix: -d
- id: in_v_one_dot_two_dot_one
  doc: 'optional arguments:'
  type: double
  inputBinding:
    position: 0
- id: in_output
  doc: '-d D            <string>: optional, to specify an output directory name, if'
  type: string
  inputBinding:
    position: 1
- id: in_dependencies
