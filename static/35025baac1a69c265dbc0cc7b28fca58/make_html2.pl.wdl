...
task MakeHtml2pl {
  input {
    ...
    Boolean? be_stringent_assigning
  }
  command <<<
    make_html2_pl \
      ...
      ~{if (be_stringent_assigning) then "-l" else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    be_stringent_assigning: "be stringent when assigning miRNA-precursor connections like mmu-mir only is assigned to mmu-precursor"
  }
  output {
