...
- id: align_refining
  doc: "(=NO)            Apply <refn> refining to the alignment, one of available\
    \ values: NO    - Don't refine the alignment LIGHT - Refine any alignments with\
    \ few entries; glue alignments one more entry at a time HEAVY - Perform heavy\
    \ (slow) refining on the alignment, including when gluing alignments together\
    \ This can change the method of gluing alignments under --ssap-scores-infile and\
    \ --do-the-ssaps"
  type: string
  inputBinding:
...
- id: regions_context
  doc: '(=alone)     Show the alignment regions in the context <context>, one of available
    options: alone    - alone in_chain - within the chain(s) in which the regions
    appear in_pdb   - within the PDB in which the regions appear'
  type: string
  inputBinding:
