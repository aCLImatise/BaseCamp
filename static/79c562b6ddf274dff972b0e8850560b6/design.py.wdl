...
task Designpy {
  input {
    ...
    Int? write_probe_map_counts_to_tsv
  ...
  command <<<
    design_py \
      ...
      ~{if defined(write_taxid_acc) then ("--write-taxid-acc " +  '"' + write_taxid_acc + '"') else ""} \
      ...
      ~{if defined(write_probe_map_counts_to_tsv) then ("--write-probe-map-counts-to-tsv " +  '"' + write_probe_map_counts_to_tsv + '"') else ""} \
