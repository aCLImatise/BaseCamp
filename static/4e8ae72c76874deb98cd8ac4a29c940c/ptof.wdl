...
task Ptof {
  input {
    ...
    Boolean? valueinsert_score_default
    Boolean? valuestop_codon_penalty
    Boolean? valueintron_opening_default
  ...
  command <<<
    ptof \
      ...
      ~{if (valueinsert_score_default) then "-I" else ""} \
      ~{if (valuestop_codon_penalty) then "-X" else ""} \
      ~{if (valueintron_opening_default) then "-Y" else ""} \
