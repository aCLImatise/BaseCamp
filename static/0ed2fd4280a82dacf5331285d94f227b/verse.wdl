...
task Verse {
  input {
    File? give_name_annotation
    File? give_general_name
    Int? running_mode_default
    ...
    Boolean? specified_template_length
  ...
  command <<<
    verse \
      ~{input_file} \
      ~{if defined(give_name_annotation) then ("-a " +  '"' + give_name_annotation + '"') else ""} \
      ~{if defined(give_general_name) then ("-o " +  '"' + give_general_name + '"') else ""} \
      ~{if defined(running_mode_default) then ("-z " +  '"' + running_mode_default + '"') else ""} \
      ...
      ~{if defined(number_used_defaultnote) then ("-T " +  '"' + number_used_defaultnote + '"') else ""} \
      ...
      ~{if (specified_template_length) then "-P" else ""} \
  ...
  >>>
  parameter_meta {
    give_name_annotation: "Give the name of the annotation file. The program currently only\\nsupports GTF format."
