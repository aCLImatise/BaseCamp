...
task BwaSamConverterpl {
  ...
  command <<<
    bwa_sam_converter_pl \
      ...
      ~{if (collapsed_read_output) then "-o" else ""} \
  ...
  parameter_meta {
    ...
    collapsed_read_output: "collapsed read output file"
  ...
  output {
    ...
    File out_collapsed_read_output = "${in_collapsed_read_output}"
  }
}