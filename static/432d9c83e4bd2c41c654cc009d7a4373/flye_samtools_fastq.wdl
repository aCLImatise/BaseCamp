...
task FlyesamtoolsFastq {
  ...
  command <<<
    flye_samtools fastq \
      ...
      ~{if (compression_level_use) then "-c" else ""} \
  ...
  parameter_meta {
    ...
    compression_level_use: "compression level [0..9] to use when creating gz or bgzf fastq files"
