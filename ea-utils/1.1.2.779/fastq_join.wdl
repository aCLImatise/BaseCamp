...
task Fastqjoin {
  ...
  command <<<
    fastq_join \
      ...
      ~{if defined(verifies_files_probe) then ("-v " +  '"' + verifies_files_probe + '"') else ""} \
  ...
  parameter_meta {
    ...
    verifies_files_probe: "Verifies that the 2 files probe id's match up to char C\\nuse ' ' (space) for Illumina reads"
