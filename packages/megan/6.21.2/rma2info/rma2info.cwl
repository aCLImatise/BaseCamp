...
- id: in_in
  ...
  inputBinding:
    prefix: --in
- id: in_out
  doc: '[string]                   Output file (stdout or .gz ok). Default value:
    stdout'
  type: File?
  inputBinding:
    prefix: --out
...
- id: in_class_two_count
  doc: '[string(s)]      List class to count for named classification(s) (Possible
    values: EC EGGNOG GTDB INTERPRO2GO KEGG SEED Taxonomy).'
  type: boolean?
  inputBinding:
    prefix: --class2count
- id: in_read_two_class
  doc: '[string(s)]       List read to class assignments for named classification(s)
    (Possible values: EC EGGNOG GTDB INTERPRO2GO KEGG SEED Taxonomy).'
  type: boolean?
  inputBinding:
    prefix: --read2class
- id: in_names
...
- id: in_paths
  ...
  inputBinding:
    prefix: --paths
- id: in_ranks
  doc: 'When reporting taxonomy, report taxonomic rank using single letter (K for
    Kingdom, P for Phylum etc). Default value: false.'
  type: boolean?
  inputBinding:
    prefix: --ranks
