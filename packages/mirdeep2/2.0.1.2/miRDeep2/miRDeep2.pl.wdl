...
task MiRDeep2pl {
  ...
  command <<<
    miRDeep2_pl \
      ...
      ~{if defined(maximum_number_precursors) then ("-g " +  '"' + maximum_number_precursors + '"') else ""} \
      ...
      ~{if (output_list_exit) then "-u" else ""} \
  ...
  parameter_meta {
    ...
    maximum_number_precursors: "maximum number of precursors to analyze when automatic excision gearing is used.\\ndefault=50.000, if set to -1 all precursors will be analyzed"
    ...
    output_list_exit: "output list of UCSC browser species that are supported and exit"
