...
inputs:
- id: in_fields
  doc: key fields for grouping. e.g -f 1,2 or -f columnA,columnB (default "1")
  type: long?
  inputBinding:
    prefix: --fields
- id: in_fuzzy_fields
  doc: using fuzzy fields (only for key fields), e.g., -F -f "*name" or -F -f "id123*"
  type: boolean?
  inputBinding:
    prefix: --fuzzy-fields
- id: in_ignore_case
  ...
  inputBinding:
    prefix: --ignore-case
- id: in_separate_r
  doc: separater for folded values (default "; ")
  type: string?
  inputBinding:
    prefix: --separater
- id: in_v_field
  doc: value field for folding
  type: string?
  inputBinding:
    prefix: --vfield
- id: in_chunk_size
