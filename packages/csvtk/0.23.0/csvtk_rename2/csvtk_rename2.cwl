...
- id: in_pattern
  ...
  inputBinding:
    prefix: --pattern
- id: in_replacement
  doc: 'renamement. supporting capture variables.  e.g. $1 represents the text of
    the first submatch. ATTENTION: use SINGLE quote NOT double quotes in *nix OS or
    use the \ escape character. Ascending number is also supported by "{nr}".use ${1}
    instead of $1 when {kv} given!'
  type: long?
  inputBinding:
    prefix: --replacement
- id: in_start_num
  doc: starting number when using {nr} in replacement (default 1)
  type: long?
  inputBinding:
    prefix: --start-num
- id: in_chunk_size
