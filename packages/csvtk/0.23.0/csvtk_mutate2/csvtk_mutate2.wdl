...
task CsvtkMutate2 {
  ...
  command <<<
    csvtk mutate2 \
      ...
      ~{if (numeric_as_string) then "--numeric-as-string" else ""} \
  ...
  parameter_meta {
    digits: "number of digits after the decimal dot (default 2)"
    expression: "artithmetic/string expressions. e.g. \\\"'string'\\\", '\\\"abc\\\"', ' $a + \\\"-\\\" + $b ', '$1 + $2', '$a / $b', ' $1 > 100 ? \\\"big\\\" : \\\"small\\\" '"
    name: "new column name"
    numeric_as_string: "treat even numeric fields as strings to avoid converting big numbers into scientific notation"
