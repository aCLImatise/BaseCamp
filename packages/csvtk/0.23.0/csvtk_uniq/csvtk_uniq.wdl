...
task CsvtkUniq {
  ...
  command <<<
    csvtk uniq \
      ...
      ~{if defined(keep_n) then ("--keep-n " +  '"' + keep_n + '"') else ""} \
  ...
  parameter_meta {
    fields: "select these fields as keys. e.g -f 1,2 or -f columnA,columnB (default \\\"1\\\")"
    fuzzy_fields: "using fuzzy fields, e.g., -F -f \\\"*name\\\" or -F -f \\\"id123*\\\""
    ignore_case: "ignore case"
    keep_n: "keep at most N records for a key (default 1)"
