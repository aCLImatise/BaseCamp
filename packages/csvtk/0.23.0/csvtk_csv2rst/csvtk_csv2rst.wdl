...
task CsvtkCsv2rst {
  ...
  command <<<
    csvtk csv2rst \
      ...
      ~{if defined(horizontal_border) then ("--horizontal-border " +  '"' + horizontal_border + '"') else ""} \
      ~{if defined(padding) then ("--padding " +  '"' + padding + '"') else ""} \
      ~{if defined(vertical_border) then ("--vertical-border " +  '"' + vertical_border + '"') else ""} \
  ...
  parameter_meta {
    cross: "charactor of cross (default \\\"+\\\")"
    header: "charactor of separator between header row and data rowws (default \\\"=\\\")"
    horizontal_border: "charactor of horizontal border (default \\\"-\\\")"
    padding: "charactor of padding (default \\\" \\\")"
    vertical_border: "charactor of vertical border (default \\\"|\\\")"
