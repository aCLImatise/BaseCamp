...
task TOBIASPlotAggregate {
  ...
  command <<<
    TOBIAS PlotAggregate \
      ...
      ~{if (output_txt) then "--output-txt" else ""} \
  ...
  parameter_meta {
    ...
    path_output_default: "Path to output plot (default: TOBIAS_aggregate.pdf)"
    output_txt: "Path to output file for aggregates in .txt-format\\n(default: None)"
  ...
  output {
    ...
    File out_output_txt = "${in_output_txt}"
  }
}