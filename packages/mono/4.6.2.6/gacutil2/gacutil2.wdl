...
task Gacutil2 {
  input {
    ...
    File? il
    Float? uninstalls_assembly_cacheassemblydisplayname
    File? ul
    File? us
    Boolean? lists_contents_global
    Directory? package
    Directory? gac_dir
    Directory? root
    Boolean? check_refs
  ...
  command <<<
    gacutil2 \
      ...
      ~{if defined(uninstalls_assembly_cacheassemblydisplayname) then ("-u " +  '"' + uninstalls_assembly_cacheassemblydisplayname + '"') else ""} \
      ~{if defined(ul) then ("-ul " +  '"' + ul + '"') else ""} \
      ~{if defined(us) then ("-us " +  '"' + us + '"') else ""} \
      ~{if (lists_contents_global) then "-l" else ""} \
