...
task AmptkSRAsubmit {
  input {
    ...
    Boolean? rev_primer
    Boolean? append
    Boolean? reads_forward
    Boolean? reads_reverse
    Boolean? reads_index
  ...
  command <<<
    amptk SRA_submit \
      ...
      ~{if (reads_forward) then "--reads-forward" else ""} \
      ~{if (reads_reverse) then "--reads-reverse" else ""} \
      ~{if (reads_index) then "--reads-index" else ""} \
  ...
  parameter_meta {
    input_fastq_file: "Input FASTQ file or folder"
    ...
    platform: "Sequencing platform. Defalt: ion (ion, illumina, illumina3, 454)"
    ...
    reads_forward: "Input R1 reads (-p illumina3)"
    reads_reverse: "Input R2 reads (-p illumina3)"
    reads_index: "Input Index reads (-p illumina3)"
