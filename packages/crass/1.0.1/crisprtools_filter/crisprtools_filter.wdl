...
task CrisprtoolsFilter {
  ...
  command <<<
    crisprtools filter \
      ...
      ~{if defined(filter_based_number) then ("-s " +  '"' + filter_based_number + '"') else ""} \
  ...
  parameter_meta {
    ...
    filter_based_number: "Filter based on the number of spacers the spacers"
