...
task SeqkitHeadgenome {
  input {
    Int? mini_common_words
  ...
  command <<<
    seqkit head_genome \
      ~{if defined(mini_common_words) then ("--mini-common-words " +  '"' + mini_common_words + '"') else ""} \
  ...
  parameter_meta {
    mini_common_words: "minimal shared prefix words (default 1)"
