...
task MixcrExportAlignments {
  input {
    ...
    Boolean? cloneid
    Boolean? cloneid_with_mapping_type
    Boolean? d_alignment
    ...
    Boolean? descr_r_one
    Boolean? descr_r_two
    Boolean? descr_sr_one
    Boolean? descr_sr_two
    Boolean? d_families
    Boolean? d_family
    Boolean? d_gene
    Boolean? d_genes
  ...
  command <<<
    mixcr exportAlignments \
      ...
      ~{if (cloneid_with_mapping_type) then "-cloneIdWithMappingType" else ""} \
      ...
      ~{if (descr_r_one) then "-descrR1" else ""} \
      ~{if (descr_r_two) then "-descrR2" else ""} \
      ~{if (descr_sr_one) then "-descrsR1" else ""} \
      ~{if (descr_sr_two) then "-descrsR2" else ""} \
      ...
      ~{if (read_history) then "-readHistory" else ""} \
      ~{if (read_id) then "-readId" else ""} \
  ...
  parameter_meta {
    ...
    cloneid: "To which clone alignment was attached (make sure using .clna file as input\\nfor exportAlignments)"
    cloneid_with_mapping_type: "To which clone alignment was attached with additional info on mapping type\\n(make sure using .clna file as input for exportAlignments)"
    ...
    descr_r_one: "Export description line from initial .fasta or .fastq file (deprecated)"
    descr_r_two: "Export description line from initial .fasta or .fastq file (deprecated)"
    descr_sr_one: "Export description lines from initial .fasta or .fastq file for R1 reads\\n(only available if -OsaveOriginalReads=true was used in align command)"
    descr_sr_two: "Export description lines from initial .fastq file for R2 reads (only\\navailable if -OsaveOriginalReads=true was used in align command)"
    ...
    read_history: "Export read history"
    read_id: "Export id of read corresponding to alignment (deprecated)"
    read_ids: "Export id(s) of read(s) corresponding to alignment"
