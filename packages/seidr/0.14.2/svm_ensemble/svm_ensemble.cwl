...
- id: in_no_scale
  ...
  inputBinding:
    prefix: --no-scale
- id: in_y
  doc: '[ --type ] arg (=EPSILON_SVR)      SVM type [NU_SVR, EPSILON_SVR]'
  type: boolean?
  inputBinding:
    prefix: -y
- id: in_k
  doc: '[ --kernel ] arg (=LINEAR)         SVM kernel [LINEAR, POLY, RBF, SIGMOID]'
  type: boolean?
  inputBinding:
    prefix: -k
- id: in_arg_polynomial_degree
  doc: '[ --degree ] arg (=3)              Polynomial degree (for POLY kernel)'
  type: boolean?
  inputBinding:
    prefix: -d
- id: in_arg_kernel_coefficient
  doc: '[ --gamma ] arg (=0.01)            Kernel coefficient for POLY/RBF/SIGMOID'
  type: boolean?
  inputBinding:
    prefix: -G
- id: in_arg_value_nusvr
  doc: '[ --nu ] arg (=0.5)                nu value (for NU_SVR)'
  type: boolean?
  inputBinding:
    prefix: -n
- id: in_arg_penalty_c
...
- id: in__arg_epsilon
  ...
  inputBinding:
    prefix: -E
- id: in_arg_use_heuristic
  doc: "[ --shrinking ] arg (=1)           Whether to use the shrinking heuristic\n\
    [0: off, 1: on]"
  type: boolean?
  inputBinding:
    prefix: -S
...
- id: in_force_overwrite_output
  ...
  inputBinding:
    prefix: -f
- id: in_arg_svmscorestsvoutput_file
  doc: "[ --outfile ] arg (=svm_scores.tsv)\nOutput file path"
  type: File?
  inputBinding:
    prefix: -o
- id: in_save_resume
...
- id: in_print_program_version
  ...
  inputBinding:
    prefix: -V
- id: in_kernels
  doc: '-c [ --coef ] arg (=0.01)             Independent term in kernel function '
  type: string
  inputBinding:
    position: 0
- id: in_exists
...
- id: out_stdout
  ...
  type: stdout
- id: out_arg_svmscorestsvoutput_file
  doc: "[ --outfile ] arg (=svm_scores.tsv)\nOutput file path"
  type: File?
  outputBinding:
    glob: $(inputs.in_arg_svmscorestsvoutput_file)
hints:
