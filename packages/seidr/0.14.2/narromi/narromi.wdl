...
task Narromi {
  ...
  command <<<
    narromi \
      ...
      ~{if (arg_simplex_method) then "-m" else ""} \
      ~{if (arg_alpha_cutoff) then "-a" else ""} \
      ...
      ~{if (arg_narromiscorestsvoutput_file) then "-o" else ""} \
  ...
  parameter_meta {
    ...
    arg_simplex_method: "[ --algorithm ] arg (=simplex)     Method for linear programming\\noptimisaton. One of 'interior-point' or\\n'simplex'."
    arg_alpha_cutoff: "[ --alpha ] arg (=0.05)            Alpha cutoff for MI selection."
    ...
    arg_narromiscorestsvoutput_file: "[ --outfile ] arg (=narromi_scores.tsv)\\nOutput file path"
  ...
  output {
    ...
    File out_arg_narromiscorestsvoutput_file = "${in_arg_narromiscorestsvoutput_file}"
  }
}