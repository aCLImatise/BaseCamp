...
task MashDist {
  input {
    ...
    Boolean? table_output_report
    Int? maximum_pvalue_report
    Int? maximum_distance_report
    Boolean? show_comment_fields
  ...
  command <<<
    mash dist \
      ...
      ~{if (table_output_report) then "-t" else ""} \
      ...
      ~{if (show_comment_fields) then "-C" else ""} \
  ...
  parameter_meta {
    ...
    table_output_report: "Table output (will not report p-values, but fields will be blank if\\nthey do not meet the p-value threshold)."
    maximum_pvalue_report: "Maximum p-value to report. (0-1) [1.0]"
    maximum_distance_report: "Maximum distance to report. (0-1) [1.0]"
    show_comment_fields: "Show comment fields with reference/query names (denoted with ':').\\n(0-1) [1.0]"
