...
task MegalodonExtrasValidateResults {
  ...
  command <<<
    megalodon_extras validate results \
      ...
      ~{if defined(max_stats) then ("--max-stats " +  '"' + max_stats + '"') else ""} \
  ...
  parameter_meta {
    ...
    max_stats: "Maximum number of statistics to extract. Default:\\n1000000"
