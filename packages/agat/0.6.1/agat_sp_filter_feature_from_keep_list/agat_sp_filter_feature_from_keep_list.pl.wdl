...
task AgatSpFilterFeatureFromKeepListpl {
  input {
    File? ref_file
    Int? type
    String? keep_list
    ...
    String agat_sp_filter_feature_from_keep_list_do_tpl
  }
  command <<<
    agat_sp_filter_feature_from_keep_list_pl \
      ~{agat_sp_filter_feature_from_keep_list_do_tpl} \
      ~{if defined(ref_file) then ("--reffile " +  '"' + ref_file + '"') else ""} \
      ~{if defined(type) then ("--type " +  '"' + type + '"') else ""} \
      ~{if defined(keep_list) then ("--keep_list " +  '"' + keep_list + '"') else ""} \
  ...
  parameter_meta {
    ...
    keep_list: "Keep list. One value per line."
    ...
    agat_sp_filter_feature_from_keep_list_do_tpl: "Description:"
  }
  output {
