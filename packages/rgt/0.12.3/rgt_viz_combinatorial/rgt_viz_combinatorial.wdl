...
task RgtvizCombinatorial {
  input {
    ...
    Boolean? file_name_references
    Boolean? file_name_query
    Boolean? title_shown_thefolder
    String? group_data_default
  ...
  command <<<
    rgt_viz combinatorial \
      ...
      ~{if (file_name_references) then "-r" else ""} \
      ~{if (file_name_query) then "-q" else ""} \
      ~{if (title_shown_thefolder) then "-t" else ""} \
      ~{if defined(group_data_default) then ("-g " +  '"' + group_data_default + '"') else ""} \
  ...
  parameter_meta {
    ...
    file_name_references: "The file name of the reference Experimental Matrix.\\nMultiple references are acceptable. (default: None)"
    ...
    group_data_default: "Group the data by any optional column (for example,\\n'cell') of experimental matrix, or None. (default: None)"
