...
task SomaticQC {
  ...
  command <<<
    SomaticQC \
      ...
      ~{if defined(ref_cram) then ("-ref_cram " +  '"' + ref_cram + '"') else ""} \
  ...
  parameter_meta {
    tumor_bam: "Input tumor BAM/CRAM file."
    normal_bam: "Input normal BAM/CRAM file."
    ...
    ref_cram: "Reference genome for CRAM support (mandatory if CRAM is used). If set, it is used for tumor and normal file.\\nDefault value: ''"
