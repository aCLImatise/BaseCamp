...
task NGSDExportSamples {
  ...
  command <<<
    NGSDExportSamples \
      ...
      ~{if (match_external_names) then "-match_external_names" else ""} \
      ~{if (with_merged) then "-with_merged" else ""} \
      ~{if defined(species) then ("-species " +  '"' + species + '"') else ""} \
      ~{if defined(sender) then ("-sender " +  '"' + sender + '"') else ""} \
      ...
      ~{if (add_comments) then "-add_comments" else ""} \
  ...
  parameter_meta {
    ...
    sender: "Sample sender filter.\\nDefault value: ''"
    study: "Processed sample study filter.\\nDefault value: ''"
