...
- id: in_in
  ...
  inputBinding:
    prefix: -in
- id: in_fast_a
  doc: "Input sequence database in FASTA format. Leave empty for using the same DB\
    \ as used for the input idXML (this might fail). Non-existing relative filenames\
...
- id: in_unmatched_action
  doc: "If peptide sequences cannot be matched to any protein: 1) raise an error;\
    \ 2) warn (unmatched PepHits will miss target/decoy annotation with downstream\
    \ problems); 3) remove the hit. (default: 'error' valid: 'error', 'warn', 'remove')"
  type: long?
  inputBinding:
    prefix: -unmatched_action
- id: in_aaa_max
