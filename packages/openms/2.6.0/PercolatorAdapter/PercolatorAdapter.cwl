...
- id: in_in_osw
  ...
  inputBinding:
    prefix: -in_osw
- id: in_out
  doc: "*                          Output file (valid formats: 'idXML', 'mzid', 'osw')"
  type: File?
  inputBinding:
    prefix: -out
- id: in_out_type
...
- id: in_peptide_level_f_drs
  ...
  type: boolean?
  inputBinding:
    prefix: -peptide_level_fdrs
- id: in_protein_level_f_drs
  doc: Use the picked protein-level FDR to infer protein probabilities. Use the -fasta
    option and -decoy_pattern to set the Fasta file and decoy pattern.
  type: boolean?
  inputBinding:
    prefix: -protein_level_fdrs
- id: in_osw_level
