...
inputs:
- id: in_in
  doc: "<file(s)>*                    Input file (valid formats: 'idXML', 'featureXML',\
    \ 'consensusXML')"
  type: boolean?
  inputBinding:
    prefix: -in
...
- id: in_mz_delta
  ...
  inputBinding:
    prefix: -mz_delta
- id: in_per_spectrum
  doc: (only idXML) if set, mapping will be done based on exact matching of originating
    mzml file and spectrum_ref
  type: boolean?
  inputBinding:
    prefix: -per_spectrum
- id: in_algorithm
