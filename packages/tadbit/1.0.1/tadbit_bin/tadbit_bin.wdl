...
task TadbitBin {
  ...
  command <<<
    tadbit bin \
      ...
      ~{if defined(tad_def) then ("--tad_def " +  '"' + tad_def + '"') else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    chr_name: "[fasta header] chromosome name(s). Order of chromosomes in the\\noutput matrices."
    ...
    tad_def: "tsv file with tad definition, columns: # start end score density"
  }
  output {
