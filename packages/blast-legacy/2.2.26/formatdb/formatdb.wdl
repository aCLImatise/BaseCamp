...
task Formatdb {
  input {
    ...
    Boolean? input_files_optional
    File? logfile_name_optionaldefault
    Boolean? type_filet_proteinf
    Boolean? parse_optionst_seqid
  ...
  command <<<
    formatdb \
      ...
      ~{if (input_files_optional) then "-i" else ""} \
      ~{if (logfile_name_optionaldefault) then "-l" else ""} \
      ~{if (type_filet_proteinf) then "-p" else ""} \
      ~{if (parse_optionst_seqid) then "-o" else ""} \
      ~{if (input_file_database) then "-a" else ""} \
      ~{if (asn_database_binary) then "-b" else ""} \
  ...
  parameter_meta {
    ...
    input_files_optional: "Input file(s) for formatting [File In]  Optional"
    logfile_name_optionaldefault: "Logfile name: [File Out]  Optional\\ndefault = formatdb.log"
    type_filet_proteinf: "Type of file\\nT - protein\\nF - nucleotide [T/F]  Optional\\ndefault = T"
    parse_optionst_seqid: "Parse options\\nT - True: Parse SeqId and create indexes.\\nF - False: Do not parse SeqId. Do not create indexes."
    input_file_database: "Input file is database in ASN.1 format (otherwise FASTA is expected)\\nT - True,\\nF - False."
    asn_database_binary: "ASN.1 database in binary mode\\nT - binary,\\nF - text mode."
