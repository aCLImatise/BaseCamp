...
task Blastall {
  input {
    ...
    Boolean? x_dropoff_value_behaviorblastn
    Boolean? show_gis_f
    ...
    Boolean? threshold_megablast_default
  ...
  command <<<
    blastall \
      ...
      ~{if (x_dropoff_value_behaviorblastn) then "-X" else ""} \
      ...
      ~{if (threshold_megablast_default) then "-f" else ""} \
  ...
  parameter_meta {
    ...
    x_dropoff_value_behaviorblastn: "X dropoff value for gapped alignment (in bits) (zero invokes default behavior)\\nblastn 30, megablast 20, tblastx 0, all others 15 [Integer]\\ndefault = 0"
    ...
    threshold_megablast_default: "Threshold for extending hits, default if zero\\nblastp 11, blastn 0, blastx 12, tblastn 13\\ntblastx 13, megablast 0 [Real]\\ndefault = 0"
    ...
    psitblastn_checkpoint_file: "PSI-TBLASTN checkpoint file [File In]  Optional"
    ...
    frame_shift_penalty: "Frame shift penalty (OOF algorithm for blastx) [Integer]\\ndefault = 0"
    length_largest_intron: "Length of the largest intron allowed in a translated nucleotide sequence when linking multiple distinct alignments. (0 invokes default behavior; a negative value disables linking.) [Integer]\\ndefault = 0"
    number_concatenated_queries: "Number of concatenated queries, for blastn and tblastn [Integer]  Optional\\ndefault = 0"
    ...
    compute_locally_optimal: "Compute locally optimal Smith-Waterman alignments (This option is only\\navailable for gapped tblastn.) [T/F]\\ndefault = F\\n"
  }
  output {
