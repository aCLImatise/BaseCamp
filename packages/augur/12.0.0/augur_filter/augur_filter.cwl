...
- id: in_query
  ...
  inputBinding:
    prefix: --query
- id: in_min_date
  doc: "minimal cutoff for date, the cutoff date is inclusive;\nmay be specified as\
    \ an Augur-style numeric date (with\nthe year as the integer part) or YYYY-MM-DD\
    \ (default:\nNone)"
  type: long?
  inputBinding:
    prefix: --min-date
- id: in_max_date
  doc: "maximal cutoff for date, the cutoff date is inclusive;\nmay be specified as\
    \ an Augur-style numeric date (with\nthe year as the integer part) or YYYY-MM-DD\
    \ (default:\nNone)"
  type: long?
  inputBinding:
...
- id: in_no_probabilistic_sampling
  doc: "tab-delimited file with list of priority scores for\nstrains (e.g., \"<strain>\\\
    t<priority>\") and no header.\nWhen scores are provided, Augur converts scores\
    \ to\nfloating point values, sorts strains within each\nsubsampling group from\
    \ highest to lowest priority, and\nselects the top N strains per group where N\
