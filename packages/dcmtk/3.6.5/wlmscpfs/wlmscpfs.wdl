...
task Wlmscpfs {
  input {
    ...
    Boolean? request_file_path
    Boolean? request_file_format
  ...
  command <<<
    wlmscpfs \
      ...
      ~{if (request_file_path) then "--request-file-path" else ""} \
      ~{if (request_file_format) then "--request-file-format" else ""} \
      ...
      ~{if (sleep_before) then "--sleep-before" else ""} \
  ...
  parameter_meta {
    ...
    data_files_path: "[p]ath: string (default: .)\\npath to worklist data files"
    ...
    request_file_path: "[p]ath: string\\npath to store request files to"
    request_file_format: "[f]ormat: string (default: #t.dump)\\nrequest file name format"
