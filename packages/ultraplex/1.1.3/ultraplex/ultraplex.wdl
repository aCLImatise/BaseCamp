...
task Ultraplex {
  ...
  command <<<
    ultraplex \
      ...
      ~{if (minimum_length_reads) then "-l" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/ultraplex:1.1.3--py38h0213d0e_0"
  }
  parameter_meta {
    ...
    minimum_length_reads: "[MIN_LENGTH], --min_length [MIN_LENGTH]\\nminimum length of reads before any trimming takes\\nplace. Rememberthat this must include UMIs and\\nbarcodes, so should be fairly long!"
