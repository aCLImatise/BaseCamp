...
task Eslconstruct {
  ...
  command <<<
    esl_construct \
      ...
      ~{if (set_sscons_max) then "-x" else ""} \
      ~{if (remove_sscons_basepairs) then "-r" else ""} \
      ~{if (set_sscons_indi) then "-c" else ""} \
      ...
      ~{if defined(list_seqs_indi) then ("-l " +  '"' + list_seqs_indi + '"') else ""} \
  ...
  parameter_meta {
    ...
    set_sscons_max: ": set SS_cons as max set of non-conflicting bps from indi SSs"
    ...
    list_seqs_indi: ": list seqs w/> 0 indi bp that conflicts w/a SS_cons bp to file <f>"
