...
task EvalDualdecomppl {
  input {
    ...
    File? directory_input_augustus
    String? hist_iterations
    String? hist_errors
    Int? err_per_iter
    Int? threshold_evolution_helps
    Directory? outdir
  }
  command <<<
    eval_dualdecomp_pl \
      ...
      ~{if defined(threshold_evolution_helps) then ("--t " +  '"' + threshold_evolution_helps + '"') else ""} \
