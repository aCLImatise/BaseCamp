...
- id: in_names
  doc: "Substitute names for contigs, provided in a file.\nThe file can have two formats:\n\
    ...
    \ chromosome in the FASTA must have a corresponding line\nin the file.  This is\
    \ useful if you want to rename chromosomes\nwith a systematic numbering pattern.\n\
    2.  A file with two columns per line, separated by white\nspace.  In each line,\
    \ the original FASTA chromosome name\nshould be in column 1 and the desired chromosome\
    \ name\nwill be in column 2.\nThe meaning of file format 2 depends on whether\n\
    --limit-to-names is specified.  If so, the genome build will\nbe limited to those\
    \ chromosomes in this file.  Otherwise,\nall chromosomes in the FASTA file will\
    \ be included,\nbut only those chromosomes in this file will be re-named, which\n\
    provides an easy way to change just a few chromosome names.\nThis file can be\
    \ combined with the --sort=names option, in\nwhich the order of chromosomes is\
    \ that given in the file.  In\nthis case, every chromosome must be listed in the\
    \ file, and\nfor chromosome names that should not be changed, column 2 can\nbe\
    \ blank (or the same as column 1).  The option of a blank\ncolumn 2 is allowed\
    \ only when specifying --sort=names,\nbecause otherwise, the program cannot distinguish\
    \ between a\n1-column and 2-column names file."
  type: File?
  inputBinding:
    prefix: --names
- id: in_limit_to_names
  doc: "Determines whether to limit the genome build to the lines listed\nin the --names\
    \ file.  You can limit a genome build to certain\nchromosomes with this option,\
    \ plus a --names file that either\nrenames chromosomes, or lists the same names\
...
- id: in_circular
  doc: "Circular chromosomes (either a list of chromosomes separated\nby a comma,\
    ...
    \ --names feature, then you\nshould use the substitute name of the chromosome,\
