...
- id: in_start
  ...
  inputBinding:
    prefix: --start
- id: in_stop
  doc: "read stop structure(s) from stdin (otherwise use MFE)\n(default=off)"
  type: boolean?
  inputBinding:
    prefix: --stop
- id: in_met
...
- id: in_fpt
  ...
  inputBinding:
    prefix: --fpt
- id: in_rect
  doc: "compute recurrence time (of a start structure which is\ncontained in stop\
...
- id: in_lm_in
  ...
  inputBinding:
    prefix: --lmin
- id: in_cut
  doc: "only print structures with E <= MFE + <float> to stdout\n(default=`20')"
  type: double?
  inputBinding:
    prefix: --cut
outputs:
