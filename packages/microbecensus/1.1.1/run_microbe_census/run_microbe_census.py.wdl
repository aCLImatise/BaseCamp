...
task RunMicrobeCensuspy {
  ...
  command <<<
    run_microbe_census_py \
      ...
      ~{if defined(reads_trimmed_reads) then ("-l " +  '"' + reads_trimmed_reads + '"') else ""} \
      ...
      ~{if defined(max_percent_percent) then ("-u " +  '"' + max_percent_percent + '"') else ""} \
  ...
  parameter_meta {
    ...
    reads_trimmed_reads: "all reads trimmed to this length; reads shorter than\\nthis discarded (default = median read length)"
    ...
    max_percent_percent: "max percent of unknown bases per read (default = 100\\npercent; no filtering)\\n"
    var_10: ""
