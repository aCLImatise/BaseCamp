...
task GenHeadersAfterMergePriorsOneComparatepy {
  input {
    Directory? output_directory_complete
    String? ase_count_table_collection_identifiers
    String? ase_count_table_collection_filenames
    File? design
    Boolean? debug
    String collectionfilenames
    String design_design
  }
  command <<<
    gen_headers_after_merge_priors_one_comparate_py \
      ~{collectionfilenames} \
      ~{design_design} \
      ...
      ~{if defined(ase_count_table_collection_filenames) then ("--collection_filenames " +  '"' + ase_count_table_collection_filenames + '"') else ""} \
  ...
  parameter_meta {
    ...
    ase_count_table_collection_filenames: "ASE count table collection filenames"
    design: "Design file"
