...
task ParseSdrfConvertmaxquant {
  ...
  command <<<
    parse_sdrf convert_maxquant \
      ...
      ~{if defined(mq_confdir) then ("--mqconfdir " +  '"' + mq_confdir + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/sdrf-pipelines:0.0.13--py_0"
  }
  parameter_meta {
    sd_rf: "SDRF file  [required]"
    fast_a_file_path: "protein database file path  [required]"
    mq_confdir: "MaxQuant default configure path"
    ...
    raw_folder: "spectrum raw data folder  [required]"
