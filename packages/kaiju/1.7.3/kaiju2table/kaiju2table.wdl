...
task Kaiju2table {
  input {
    ...
    String? taxonomic_rank_must
    Float? number_denoting_percentage
  ...
  command <<<
    kaiju2table \
      ...
      ~{if defined(taxonomic_rank_must) then ("-r " +  '"' + taxonomic_rank_must + '"') else ""} \
      ~{if defined(number_denoting_percentage) then ("-m " +  '"' + number_denoting_percentage + '"') else ""} \
  ...
  parameter_meta {
    ...
    taxonomic_rank_must: "Taxonomic rank, must be one of: phylum, class, order, family, genus, species"
    number_denoting_percentage: "Number in [0, 100], denoting the minimum required percentage for the taxon (except viruses) to be reported (default: 0.0)"
