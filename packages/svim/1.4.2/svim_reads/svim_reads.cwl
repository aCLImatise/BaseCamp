...
- id: in_cores
  ...
  inputBinding:
    prefix: --cores
- id: in_aligner
  doc: "Tool for read alignment: ngmlr or minimap2 (default:\nngmlr)"
  type: string?
  inputBinding:
    prefix: --aligner
- id: in_nano_pore
  doc: "Use Nanopore settings for read alignment (default:\nFalse)"
  type: boolean?
  inputBinding:
    prefix: --nanopore
- id: in_min_mapq
...
- id: in_all_b_nds
  doc: "Output all rearrangements additionally in BND notation\n(default: False).\
    \ By default, SV signatures from the\nread alignments are used to detect complete\
    \ SVs, such\nas deletions, insertions and inversions. When this\noption is enabled,\
    \ all SVs are also output in breakend\n(BND) notation as defined in the VCF specs.\
    \ For\ninstance, a deletion gets two records in the VCF\noutput: 1. the normal\
    \ <DEL> record and 2. a <BND>\nrecord representing the novel adjacency between\
...
- id: in_working_dir
  ...
  inputBinding:
    position: 0
- id: in_reads
  doc: "Read file (FASTA, FASTQ, gzipped FASTA, gzipped FASTQ\nor file list). The\
    \ read file has to have one of the\nfollowing supported file endings: FASTA: .fa,\
    \ .fasta,\n.FA, .fa.gz, .fa.gzip, .fasta.gz, .fasta.gzip FASTQ:\n.fq, .fastq,\
    \ .FQ, .fq.gz, .fq.gzip, .fastq.gz,\n.fastq.gzip FILE LIST: .fa.fn, fq.fn"
  type: string
  inputBinding:
    position: 1
- id: in_genome
...
- class: DockerRequirement
  dockerPull: quay.io/biocontainers/svim:1.4.2--py_0
cwlVersion: v1.1
baseCommand:
- svim
- reads
