...
task CorrectorAR {
  input {
    Int? set_size_consecutive_defaultnote
    Int? set_cutoff_consecutive
    Int? set_size_space
    Int? set_size_space_defaultnote
  ...
  command <<<
    Corrector_AR \
      ...
      ~{reads_file_list} \
      ~{if defined(set_size_consecutive_defaultnote) then ("-k " +  '"' + set_size_consecutive_defaultnote + '"') else ""} \
      ~{if defined(set_cutoff_consecutive) then ("-l " +  '"' + set_cutoff_consecutive + '"') else ""} \
      ~{if defined(set_size_space) then ("-K " +  '"' + set_size_space + '"') else ""} \
      ~{if defined(set_size_space_defaultnote) then ("-s " +  '"' + set_size_space_defaultnote + '"') else ""} \
  ...
  >>>
  parameter_meta {
    set_size_consecutive_defaultnote: "Set size of consecutive kmer, default=17.\\nNote: this value must be set to be same as that used in Kmer_freq program."
