...
task Gffread {
  input {
    ...
    Boolean? only_parse_additional
    Boolean? use_description_field
    Boolean? process_also_gff
    Boolean? discard_having_codons
  ...
  command <<<
    gffread \
      ...
      ~{if (only_parse_additional) then "-G" else ""} \
      ~{if (use_description_field) then "-A" else ""} \
      ~{if (process_also_gff) then "-O" else ""} \
      ~{if (discard_having_codons) then "-V" else ""} \
  ...
  parameter_meta {
    ...
    coding_only_discard: "coding only: discard mRNAs that have no CDS feature"
    full_gff_attribute: "full GFF attribute preservation (all attributes are shown)"
    only_parse_additional: "only parse additional exon attributes from the first exon\\nand move them to the mRNA level (useful for GTF input)"
    use_description_field: "use the description field from <seq_info.fsize> and add it\\nas the value for a 'descr' attribute to the GFF record"
    process_also_gff: "process also non-transcript GFF records (by default non-transcript\\nrecords are ignored)"
    discard_having_codons: "discard any mRNAs with CDS having in-frame stop codons"
    ...
    m_slash_merge: ": cluster the input transcripts into loci, collapsing matching\\ntranscripts (those with the same exact introns and fully contained)"
    ...
    w_x_options: "for -w and -x options, also write for each fasta record the exon\\ncoordinates projected onto the spliced sequence"
    ...
    gff_records_will: "the \\\"filtered\\\" GFF records will be written to <outfile.gff>\\n(use -o- for printing to stdout)"
