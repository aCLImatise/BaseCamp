...
inputs:
- id: in_statistics
  doc: display total, unique, distnict, present number of the kmers on the screen.  accepts
    exactly one input.
  type: string
  inputBinding:
    position: 0
- id: in_histogram
  doc: display kmer frequency on the screen as 'frequency<tab>count'.  accepts exactly
    one input.
  type: string
  inputBinding:
    position: 1
...
- id: in_intersect_sum
  ...
  inputBinding:
    position: 7
- id: in_subtract
  doc: return kmers that occur in the first input, subtracting counts from the other
    inputs
  type: string
  inputBinding:
    position: 8
