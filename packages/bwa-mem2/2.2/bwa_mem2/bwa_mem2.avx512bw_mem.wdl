...
task Bwamem2avx512bwMem {
  ...
  command <<<
    bwa_mem2_avx512bw mem \
      ~{idx_base} \
      ~{in_one_dot_fq} \
      ~{in_two_dot_fq} \
      ~{if defined(output_sam_file) then ("-o " +  '"' + output_sam_file + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/bwa-mem2:2.2--he513fc3_0"
  }
  parameter_meta {
    output_sam_file: "Output SAM file name"
