...
- id: in_es_mode
  ...
  inputBinding:
    prefix: --esmode
- id: in_ep_mode
  doc: "Run ProtHint to generate protein hints (if\nnot already specified with --hints\
    \ option)\nand use the hints in GeneMark-EP+ to create\na training set for AUGUSTUS."
  type: boolean?
  inputBinding:
    prefix: --epmode
- id: in_etp_mode
  doc: "Use RNA-Seq and protein hints in GeneMark-ETP+\nto create a training set for\
...
- id: in_prot_hint_path
  doc: "=/path/to/           Set path to the directory with prothint.py.\n(if not\
    \ specified as PROTHINT_PATH\nenvironment variable). Has higher priority\nthan\
    \ environment variable."
  type: boolean?
  inputBinding:
...
- id: in_python_three_path
  ...
  inputBinding:
    prefix: --PYTHON3_PATH
- id: in_java_path
  doc: "=/path/to                Set path to java executable (if not\nspecified as\
    \ environment variable and if\nexecutable is not in your $PATH), only\nrequired\
    \ with flags --UTR=on and --addUTR=on"
  type: boolean?
  inputBinding:
    prefix: --JAVA_PATH
- id: in_gush_r_path
  doc: "=/path/to               Set path to gushr.py exectuable (if not\nspecified\
    \ as an environment variable and if\nexecutable is not in your $PATH), only required\n\
...
- id: in_skip_optimize
  ...
  inputBinding:
    prefix: --skipOptimize
- id: in_skip_iterative_prediction
  doc: "Skip iterative prediction in --epmode (does\nnot affect other modes, saves\
    \ a bit of runtime)"
  type: boolean?
  inputBinding:
...
- id: in_eval
  doc: "Reference set to evaluate predictions\nagainst (using evaluation scripts from\
    \ GaTech)"
  type: string?
  inputBinding:
    prefix: --eval
- id: in_eval_pseudo
  doc: "File with pseudogenes that will be excluded\nfrom accuracy evaluation (may\
    \ be empty file)"
  type: long?
  inputBinding:
    prefix: --eval_pseudo
...
- id: in_gm_max_intergenic
  doc: "Adjust maximum allowed size of intergenic\nregions in GeneMark-EX. If not\
