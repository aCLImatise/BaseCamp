...
- id: in_viral_gene_enrich_off
  doc: "turn off the requirement of more viral than\ncellular genes for calling full\
    \ sequence\nviral; this is useful when only using\nVirSorter2 to produce DRAMv\
    \ input with viral\nsequence identified from other tools, or those\ntrimmed by\
    \ checkV  [default: False]"
  type: boolean?
  inputBinding:
    prefix: --viral-gene-enrich-off
- id: in_seqname_suffix_off
  doc: "turn off adding suffix (||full, ||{i}_partial,\n||lt2gene) to sequence names;\
    \ note that this\nmight cause partial seqs from the same contig\nto have the same\
    \ name; this option is could be\nused when you are sure there is one partial\n\
...
- id: in_use_cond_a_off
  doc: "Stop using the conda envs (vs2.yaml) that come\nwith this package and use\
    ...
    \ dependencies on your own with your\nown prefer version; this option works with\
...
- class: DockerRequirement
  dockerPull: quay.io/biocontainers/virsorter:2.2.2--pyhdfd78af_0
cwlVersion: v1.1
baseCommand:
- virsorter
- run
