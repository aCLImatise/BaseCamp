...
task SpadesbwaAln {
  input {
    ...
    Boolean? input_illumina_fastqlike
    Boolean? input_read_file
    Boolean? use_singleend_reads
    Boolean? use_st_read
    Boolean? use_nd_read
  ...
  command <<<
    spades_bwa aln \
      ...
      ~{if defined(stop_searching_when) then ("-R " +  '"' + stop_searching_when + '"') else ""} \
  ...
  parameter_meta {
    ...
    stop_searching_when: "stop searching when there are >INT equally best hits [30]"
    ...
    input_illumina_fastqlike: "the input is in the Illumina 1.3+ FASTQ-like format"
    input_read_file: "the input read file is in the BAM format"
    use_singleend_reads: "use single-end reads only (effective with -b)"
