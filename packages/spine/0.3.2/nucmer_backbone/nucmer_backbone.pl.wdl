...
task NucmerBackbonepl {
  ...
  command <<<
    nucmer_backbone_pl \
      ...
      ~{if (number_genomes_section) then "-a" else ""} \
      ...
      ~{if (given_will_output) then "-o" else ""} \
      ~{if (output_file_position) then "-e" else ""} \
  ...
  parameter_meta {
    ...
    number_genomes_section: "number of genomes from which a section can be [A]bsent and still\\nbe included as core genome (default 0)"
  ...
  output {
    ...
    File out_output_file_position = "${in_output_file_position}"
  }
}