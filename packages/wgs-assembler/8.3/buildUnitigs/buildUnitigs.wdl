...
task BuildUnitigs {
  ...
  command <<<
    buildUnitigs \
      ...
      ~{if (enable_experimental_unitig) then "-U" else ""} \
      ~{if (enable_experimental_long) then "-J" else ""} \
      ~{if (break_promisciuous_unitigs) then "-b" else ""} \
      ~{if defined(break_unitig_region) then ("-m " +  '"' + break_unitig_region + '"') else ""} \
  ...
  parameter_meta {
    ...
    enable_experimental_unitig: "Enable EXPERIMENTAL short unitig merging (aka bubble popping)."
    ...
    break_unitig_region: "Break a unitig if a region has more than 7 bad mates"
