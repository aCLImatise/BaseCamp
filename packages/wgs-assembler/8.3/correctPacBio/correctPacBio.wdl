...
task CorrectPacBio {
  input {
    ...
    Int? maximum_uncorrected_gap
    Int? percentage_short_reads
    Int? percentage_used_estimate
  ...
  command <<<
    correctPacBio \
      ...
      ~{if defined(more_fraction_error) then ("-e " +  '"' + more_fraction_error + '"') else ""} \
      ...
      ~{if defined(maximum_uncorrected_gap) then ("-M " +  '"' + maximum_uncorrected_gap + '"') else ""} \
      ~{if defined(percentage_short_reads) then ("-m " +  '"' + percentage_short_reads + '"') else ""} \
      ~{if defined(percentage_used_estimate) then ("-S " +  '"' + percentage_used_estimate + '"') else ""} \
  ...
  parameter_meta {
    ...
    more_fraction_error: "no more than 0.015 fraction (1.5%) error"
    ...
    maximum_uncorrected_gap: "The maximum uncorrected PacBio gap that will be allowed. When there is no short-read coverage for a region, by default the pipeline will split a PacBio sequence. This option allows a number of PacBio sequences without short-read coverage to remain. For example, specifying 50, will mean 50bp can have no short-read coverage without splitting the PacBio sequence. Warning: this will allow more sequences that went through the SMRTportal to not be fixed."
    percentage_short_reads: "The percentage of short reads to use to recruit other PacBio sequences to fill coverage gaps. Must be a decimal value between 0 and 1. Higher values lead to a longer runtime but more gaps being recovered. The default is 0.010000"
    percentage_used_estimate: "The percentage of mates used to estimate the library insert size. Must be a decimal value between 0 and 1. For example, specifying 0.10 will use 10 percent of the mates to estimate the insert size. The default is 0.050000"
