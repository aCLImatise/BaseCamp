...
task FastqSimulate {
  ...
  command <<<
    fastqSimulate \
      ...
      ~{if defined(create_pairs_reads) then ("-n " +  '"' + create_pairs_reads + '"') else ""} \
  ...
  parameter_meta {
    ...
    create_pairs_reads: "Create 'n' reads (for -se) or 'n' pairs of reads (for -pe and -mp)."
