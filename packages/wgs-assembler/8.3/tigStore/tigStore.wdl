...
task TigStore {
  ...
  command <<<
    tigStore \
      ...
      ~{if defined(replace_multialign_one) then ("-R " +  '"' + replace_multialign_one + '"') else ""} \
      ...
      ~{if defined(output_files_will) then ("-o " +  '"' + output_files_will + '"') else ""}
  >>>
  parameter_meta {
    ...
    replace_multialign_one: "Replace a multialign with this one (type and id are from the layout)\\nThe multialign is replaced in version <v> from -t."
    ...
    output_files_will: "Output files will be written to 'prefix.*' in the current directory.\\n(defaults to 'tigStore' (the -t option) if not set.)"
  ...
  output {
    ...
    Directory out_output_files_will = "${in_output_files_will}"
  }
}