...
task GuppyRedup {
  input {
    ...
    Boolean? specified_redup_counts
    Boolean? as_mass
    Boolean? help
    String red_up
    String place_file
  ...
  command <<<
    guppy redup \
      ...
      ~{if (specified_redup_counts) then "-m" else ""} \
      ~{if (as_mass) then "--as-mass" else ""} \
  ...
  parameter_meta {
    ...
    specified_redup_counts: "If specified, redup with counts instead of a name list."
    as_mass: "If specified, add mass instead of names to each pquery."
