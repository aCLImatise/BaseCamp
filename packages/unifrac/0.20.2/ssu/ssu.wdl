...
task Ssu {
  ...
  command <<<
    ssu \
      ...
      ~{if (pco_a) then "--pcoa" else ""} \
      ~{if (disk_buf) then "--diskbuf" else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    the_method_: "The method, [unweighted | weighted_normalized | weighted_unnormalized | generalized | unweighted_fp32 | weighted_normalized_fp32 | weighted_unnormalized_fp32 | generalized_fp32]."
    ...
    pco_a: "[OPTIONAL] Number of PCoA dimensions to compute (default: 10, do not compute if 0)"
    disk_buf: "[OPTIONAL] Use a disk buffer to reduce memory footprint. Provide path to a fast partition (ideally NVMe)."
    method: ""
  }
  output {
