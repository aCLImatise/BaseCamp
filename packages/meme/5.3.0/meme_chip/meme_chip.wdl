...
task Memechip {
  input {
    ...
    Boolean? spam_o_skip
    Boolean? fimo_skip
    String dnarna_sequences_converted
  }
  command <<<
    meme_chip \
      ~{dnarna_sequences_converted} \
      ...
      ~{if (min_w) then "-minw" else ""} \
      ~{if (max_w) then "-maxw" else ""} \
      ...
      ~{if (s_treme_pvt) then "-streme-pvt" else ""} \
      ~{if (s_treme_n_motifs) then "-streme-nmotifs" else ""} \
  ...
  parameter_meta {
    ...
    min_w: "<num>   : minimum motif width; default: 6"
    max_w: "<num>   : maximum motif width; default: 15"
    ...
    meme_n_motifs: "<num>   : maximum number of motifs to find; default: 3\\n: if =0, MEME will not be run"
    ...
    s_treme_pvt: "<num>  : stop if hold-out set p-value greater than <num>"
    s_treme_n_motifs: "<num>  : maximum number of motifs to find; overrides -streme-pvt;\\n: if =0, STREME will not be run"
    ...
    dnarna_sequences_converted: "-dna2rna                  : input DNA sequences will be converted to RNA"
  }
  output {
