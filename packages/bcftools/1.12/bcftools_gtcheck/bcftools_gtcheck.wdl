...
task BcftoolsGtcheck {
  ...
  parameter_meta {
    ...
    n_matches: "Print only top INT matches for each sample (sorted by average score), 0 for unlimited.\\nUse negative value to sort by HWE probability rather than by discordance [0]"
    ...
    samples: "[qry|gt]:LIST        List of query or -g samples, \\\"-\\\" to select all samples (by default all samples are compared)"
