...
task Blocksplit {
  ...
  command <<<
    blocksplit \
      ...
      ~{if (arg_overlap_length) then "-w" else ""} \
      ~{if (arg_maximum_number_break) then "-b" else ""} \
  ...
  parameter_meta {
    ...
    arg_maximum_number_process: "[ --limit-records ] arg Maximum number of records to process"
    arg_print_message: "[ --message-every ] arg Print a message every N records."
    arg_overlap_length: "[ --window ] arg        Overlap window length."
