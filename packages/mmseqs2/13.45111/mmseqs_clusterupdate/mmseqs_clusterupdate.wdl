...
task MmseqsClusterupdate {
  ...
  command <<<
    mmseqs clusterupdate \
      ...
      ~{if defined(recover_deleted) then ("--recover-deleted " +  '"' + recover_deleted + '"') else ""} \
  ...
  parameter_meta {
    ...
    recover_deleted: "Find and recover deleted sequences during updating of clustering [0]"
