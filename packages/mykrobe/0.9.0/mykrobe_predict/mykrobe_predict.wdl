...
task MykrobePredict {
  ...
  command <<<
    mykrobe predict \
      ...
      ~{if defined(custom_lineage_json) then ("--custom_lineage_json " +  '"' + custom_lineage_json + '"') else ""} \
  ...
  parameter_meta {
    ...
    panel: "Name of panel to use. Ignored if species is 'custom'"
    custom_probe_set_path: "Required if species is 'custom'. Ignored otherwise.\\nFile path to fasta file from `mykrobe make-probes`."
    custom_variant_to_resistance_json: "For use with `--panel custom`. Ignored otherwise. File\\npath to JSON with key,value pairs of variant names and\\ninduced drug resistance."
    custom_lineage_json: "For use with `--panel custom`. Ignored otherwise. File\\npath to JSON made by --lineage option of make-probes"
    format: "Choose output format. Default: csv.\\n"
    sample: "sample id"
    species: "species name, or 'custom' to use custom data, in which\\ncase --custom_probe_set_path is required"
    threads: "-m MEMORY, --memory MEMORY"
    stdout_dot: "--panels_dir DIRNAME  Name of directory that contains panel data. Default:"
  }
  output {
