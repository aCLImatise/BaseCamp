...
task RtgVcfeval {
  ...
  command <<<
    rtg vcfeval \
      ...
      ~{if defined(sample_ploidy) then ("--sample-ploidy " +  '"' + sample_ploidy + '"') else ""} \
      ...
      ~{if defined(roc_expr) then ("--roc-expr " +  '"' + roc_expr + '"') else ""} \
      ~{if defined(roc_regions) then ("--roc-regions " +  '"' + roc_regions + '"') else ""} \
      ~{if defined(roc_subset) then ("--roc-subset " +  '"' + roc_subset + '"') else ""} \
  ...
  parameter_meta {
    ...
    sample_ploidy: "expected ploidy of samples (Default is 2)"
  ...
  output {
    ...
    File out_roc_expr = "${in_roc_expr}"
    File out_roc_regions = "${in_roc_regions}"
    File out_roc_subset = "${in_roc_subset}"
  }
}