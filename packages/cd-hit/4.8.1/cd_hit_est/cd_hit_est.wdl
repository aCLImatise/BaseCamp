...
task Cdhitest {
  input {
    ...
    Boolean? input_filename_fastafastq
  ...
  command <<<
    cd_hit_est \
      ...
      ~{if (input_filename_fastafastq) then "-j" else ""} \
  ...
  parameter_meta {
    ...
    input_filename_fastafastq: "input filename in fasta/fastq format for R2 reads if input are paired end (PE) files"
