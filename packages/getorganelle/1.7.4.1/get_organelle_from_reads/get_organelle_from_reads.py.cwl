...
- id: in_flag_followed_be
  doc: "This flag should be followed with embplant_pt\n(embryophyta plant plastome),\
    ...
    \ label databases ('-s' and '--genes'). For easy\nusage and compatibility of old\
...
- id: in_genes
  doc: "Followed with a customized database (a fasta file or\nthe base name of a blast\
    ...
    \ on the organelle types chosen (-F). The\ndefault value become invalid when '--genes'\
    \ or '--ex-\ngenes' is used."
  type: long?
  inputBinding:
    prefix: --genes
...
- id: in_which_blast
  doc: "Assign the path to BLAST binary files if not added to\nthe path. Default:\
    \ try \"/usr/local/lib/python3.9/site-\npackages/GetOrganelleDep/linux/ncbi-blast\"\
    \ first, then\n$PATH"
  type: File?
  inputBinding:
    prefix: --which-blast
- id: in_which_bowtie_two
  doc: "Assign the path to Bowtie2 binary files if not added\nto the path. Default:\
    \ try\n\"/usr/local/lib/python3.9/site-\npackages/GetOrganelleDep/linux/bowtie2\"\
    \ first, then\n$PATH"
  type: long?
  inputBinding:
...
- id: in_which_spades
  doc: "Assign the path to SPAdes binary files if not added to\nthe path. Default:\
    \ try \"/usr/local/lib/python3.9/site-\npackages/GetOrganelleDep/linux/SPAdes\"\
    \ first, then\n$PATH"
  type: File?
  inputBinding:
    prefix: --which-spades
