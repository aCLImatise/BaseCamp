...
task NetReg {
  input {
    ...
    Boolean? arg_penalization_parameter_affinitymatrix_gx
    Boolean? arg_penalization_parameter_affinitymatrix_gy
  ...
  command <<<
    netReg \
      ...
      ~{if (affinity_matrix_gx) then "-u" else ""} \
      ~{if (affinity_matrix_gy) then "-v" else ""} \
      ~{if (arg_penalization_parameter_affinitymatrix_gx) then "-s" else ""} \
      ~{if (arg_penalization_parameter_affinitymatrix_gy) then "-x" else ""} \
      ...
      ~{if (arg_number_crossvalidation) then "-n" else ""} \
  ...
  parameter_meta {
    ...
    affinity_matrix_gx: "[ --gx ] arg                      Filename of the affinity matrix GX for"
    affinity_matrix_gy: "[ --gy ] arg                      Filename of the affinity matrix GY for"
    arg_penalization_parameter_affinitymatrix_gx: "[ --psi ] arg (=0)                Penalization parameter for affinity\\nmatrix GX."
    arg_penalization_parameter_affinitymatrix_gy: "[ --xi ] arg (=0)                 Penalization parameter for affinity\\nmatrix GY."
    ...
    arg_number_crossvalidation: "[ --nfolds ] arg (=10)            The number of cross-validation folds.\\nThis can be maximal the number of rows\\nof X/Y and minimal 3."
