...
- id: in_output_file
  ...
  inputBinding:
    prefix: --output-file
- id: in_per_scg_output_file
  doc: "A more detailed output file that will describe\ntaxonomy of each scg in a\
    \ single bin. When consensus\ntaxonomy is generated per bin or genome, taxonomy\
    \ for\neach underlying item is not reported. This additional\noptional output\
    \ file will elucidate things. (default:\nNone)"
  type: File?
  inputBinding:
    prefix: --per-scg-output-file
...
- id: out_per_scg_output_file
  ...
  type: File?
  outputBinding:
    glob: $(inputs.in_per_scg_output_file)
