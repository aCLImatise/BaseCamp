...
- id: in_min_percent_identity
  doc: "The defualt value for this is 90.0%, and in an ideal\nworld you sholdn't really\
    ...
    \ So, when you are getting no tRNA hits\nwhatsoever, only then you may want to\
...
- id: in_max_num_target_sequences
  doc: "This parameter is used to determine how many hits from\nthe database that\
    ...
    \ transfer RNA gene sequence. The default is\n100, which has been quite reasonable\
...
- id: in_trna_taxonomy
  doc: '🍺 More on `anvi-run-trna-taxonomy`:'
  type: string
  inputBinding:
    position: 0
outputs:
