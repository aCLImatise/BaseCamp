...
task Anvigenvariabilityprofile {
  input {
    ...
    Boolean? include_additional_data
  ...
  command <<<
    anvi_gen_variability_profile \
      ...
      ~{if (include_additional_data) then "--include-additional-data" else ""} \
  ...
  parameter_meta {
    ...
    output_file: "File path to store results. (default: variability.txt)"
    ...
    variability_profile_txt: "🍺 More on `anvi-gen-variability-profile`:"
  }
  output {
