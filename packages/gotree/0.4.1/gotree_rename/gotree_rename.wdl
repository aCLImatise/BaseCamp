...
task GotreeRename {
  ...
  command <<<
    gotree rename \
      ...
      ~{if (add_quotes) then "--add-quotes" else ""} \
      ...
      ~{if (rm_quotes) then "--rm-quotes" else ""} \
  ...
  parameter_meta {
    ...
    add_quotes: "Add quotes arround tip/node names"
    ...
    rm_quotes: "Remove quotes arround tip/node names (priority over --rm-quotes)"
