...
task Snifflesdebug {
  ...
  command <<<
    sniffles_debug \
      ...
      ~{if (not_report_seq) then "--not_report_seq" else ""} \
      ~{if (report_seq) then "--report-seq" else ""} \
      ~{if (ignore_sd) then "--ignore_sd" else ""} \
      ~{if (ccs_reads) then "--ccs_reads" else ""} \
      ~{if (report_str) then "--report_str" else ""} \
