...
task MetaeukEasypredict {
  input {
    ...
    Float? start_sens
    Int? sens_steps
    Float? meta_e_uk_eval
    Float? meta_e_uk_t_cov
    Int? max_intron
    Int? min_intron
    Int? min_exon_aa
    Int? max_overlap
  ...
  command <<<
    metaeuk easy_predict \
      ...
      ~{if defined(meta_e_uk_eval) then ("--metaeuk-eval " +  '"' + meta_e_uk_eval + '"') else ""} \
      ~{if defined(meta_e_uk_t_cov) then ("--metaeuk-tcov " +  '"' + meta_e_uk_t_cov + '"') else ""} \
      ~{if defined(max_intron) then ("--max-intron " +  '"' + max_intron + '"') else ""} \
      ~{if defined(min_intron) then ("--min-intron " +  '"' + min_intron + '"') else ""} \
      ~{if defined(min_exon_aa) then ("--min-exon-aa " +  '"' + min_exon_aa + '"') else ""} \
      ~{if defined(max_overlap) then ("--max-overlap " +  '"' + max_overlap + '"') else ""} \
      ~{if defined(set_gap_open) then ("--set-gap-open " +  '"' + set_gap_open + '"') else ""} \
      ~{if defined(set_gap_extend) then ("--set-gap-extend " +  '"' + set_gap_extend + '"') else ""} \
  ...
  parameter_meta {
    ...
    meta_e_uk_eval: "maximal combined evalue of an optimal set [0.0, inf] [0.001]"
    meta_e_uk_t_cov: "minimal length ratio of combined set to target [0.0, 1.0] [0.500]"
    max_intron: "Maximal allowed intron length [10000]"
    min_intron: "Minimal allowed intron length [15]"
    min_exon_aa: "Minimal allowed exon length in amino acids [11]"
    max_overlap: "Maximal allowed overlap of consecutive exons in amino acids [10]"
    set_gap_open: "Gap open penalty (negative) for missed target amino acids between exons [-1]"
    set_gap_extend: "Gap extend penalty (negative) for missed target amino acids between exons [-1]"
