...
- id: in_full
  ...
  inputBinding:
    prefix: --full
- id: in_random_seed
  doc: "Generate a new random seed number (instead of the\ndefault of 0)"
  type: boolean?
  inputBinding:
    prefix: --random-seed
...
- id: in_scoring
  doc: "Scoring file to use. Mikado provides the following:\nplant.yaml, mammalian.yaml,\n\
    HISTORIC/hsapiens_scoring.yaml,\nHISTORIC/athaliana_scoring.yaml, HISTORIC/plants.yaml,\n\
    HISTORIC/plant.yaml, HISTORIC/scerevisiae.yaml,\nHISTORIC/insects.yaml, HISTORIC/worm.yaml,\n\
    HISTORIC/human.yaml, HISTORIC/celegans_scoring.yaml,\nHISTORIC/mammalian.yaml,\n\
...
- id: in_report_all_external_metrics
  doc: "Boolean switch. If activated, Mikado will report all\navailable external metrics,\
    \ not just those requested\nfor in the scoring configuration. This might affect\n\
