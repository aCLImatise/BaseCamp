...
task AlfredAnnotate {
  ...
  command <<<
    alfred annotate \
      ...
      ~{if (arg_annotated_output) then "-o" else ""} \
      ...
      ~{if (q) then "-q" else ""} \
      ~{if (arg_gzipped_file) then "-p" else ""} \
  ...
  parameter_meta {
    ...
    arg_annotated_output: "[ --outfile ] arg (=\\\"anno.bed\\\")    annotated peaks output"
    ...
    arg_gzipped_file: "[ --position ] arg                 gzipped output file of motif hits"
