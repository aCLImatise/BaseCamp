...
task AlfredQc {
  input {
    ...
    File? arg_gzipped_json_file
    File? arg_gzipped_tsv_file
  ...
  command <<<
    alfred qc \
      ...
      ~{if (arg_gzipped_json_file) then "-j" else ""} \
      ~{if (arg_gzipped_tsv_file) then "-o" else ""} \
  ...
  parameter_meta {
    ...
    arg_gzipped_json_file: "[ --jsonout ] arg     gzipped json output file"
    arg_gzipped_tsv_file: "[ --outfile ] arg     gzipped tsv output file"
  ...
  output {
    ...
    File out_arg_gzipped_json_file = "${in_arg_gzipped_json_file}"
    File out_arg_gzipped_tsv_file = "${in_arg_gzipped_tsv_file}"
  }
}