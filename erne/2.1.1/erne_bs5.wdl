...
task ErneBs5 {
  ...
  >>>
  parameter_meta {
    reference: "reference file to use (in our format and with  .ebm extension). [REQUIRED]"
    ...
    bam_output_file: "BAM output file (other files are created  depending on other options) [REQUIRED]"
    ...
    print_all: "print all possible alignments [only for  single-end reads]"
    ...
    cl: "Search the hash collision lists up to this depth (value 0 means no limit). Default: 500."
  }
}