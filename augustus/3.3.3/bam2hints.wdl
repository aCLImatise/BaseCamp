...
task Bam2hints {
  ...
  parameter_meta {
    ...
    exon_hints: "-x   compute exonpart, exon and splice site hints in addition to intron hints (set to 0=Off)\\nYou should generate exonpart hints from RNA-Seq using wiggle (.wig) input to wig2hints."
    ...
    introns_only: "-I   only retrieve intron hints (e.g. because the exon(part) hints are retrieved by converting to a wig track, set to 1=On)\\ndeprecated as this is the default now"
    ...
    max_coverage: "-C   maximal number of hints at a given position (0: filtering deactivated). A high value causes long running time of\\nAUGUSTUS in regions with thousands of cDNA alignments. (set to 0)"
    ...
    max_gene_len: "-G   alignments of the same clone are considered to be of the same gene if not separated by more than this (set to 400000)\\nAlignments that span more than this are ignored, but better filter long introns through an alignment program."
    out: ""
    in: ""
  }
  output {
