...
task OptimizeAugustus.pl {
  input {
    ...
    String? matrix_constraints
    String? utr
    String? aug_exec_dir
    String? train_only_utr
    String? not_rain_pars
    String? sens_spec_bias
    String? nice
    String? cleanup
    String? stop_codon_excluded_from_cds
  ...
  >>>
  parameter_meta {
    meta_pars: "metapars.cfg contains the names and their ranges of the meta parameters that are subject to optimization. (default: generic_metapars.cfg)"
    rounds: "r is the number of rounds (default: 5)"
    cpus: "n is the number of CPUs to use (default: 1)"
    only_train: "an optional genbank file that is used in addition to train.gb but only for etrain not for intermediate evaluation of accuracy. These genes may e.g. be incomplete."
    k_fold: "Make a k-fold cross validation (default: 8)"
    p_step: "For integer and floating parameters start with p tests equidistributed in the allowed range of values (default: 5)"
    ...
    nice: "execute all time consuming system calls with bash nice (default nice level) Default value for execution of optmize_augustus.pl is --nice=0."
    ...
    eval_against: "s is the species identifier to which 'eval.gtf' belongs to. Caution, if not specified, the reference species in the alignment (first s-line in Maf block) is assumed."
    chunksize: "when more than 1 CPU is used, the alignment is split into multiple smaller alignments each approximately covering a sequence range of length n (in the reference genome). The prediction step is executed in parallel on these chunks (default: 5000000)."
    dbaccess: "retrieve genomes either from a MySQL or from an SQLITE database. In the SQLITE case, 'db' is a database file with extension .db, e.g. --dbaccess=vertebrates.db. In the MySQL case, 'db' is a string that contains the connection information, e.g. --dbaccess=dbname,host,user,passwd (the parameter --speciesfilenames is not required, here)."
    jg: "Use this option, if you want to filter out duplicates from the prediction with the external tool 'joingenes' (default: 0, however --jg=1 is recommended). The tool 'joingenes' is part of the augustus package and can be found in the 'auxprogs' folder."
    ...
    genomes_dot_tbl: "a text file containing the locations of the genomes. Each line starts with the species identifier followed by the location of the corresponding genome file, e.g. hg19 /path/to/human/genome.fa mm9  /path/to/mouse/genome.fa ..."
