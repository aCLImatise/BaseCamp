...
task EqtlbmaBf {
  ...
  command <<<
    eqtlbma_bf \
      ...
      ~{if (g_coord) then "--gcoord" else ""} \
      ~{if (anchor) then "--anchor" else ""} \
      ~{if (cis) then "--cis" else ""} \
      ~{if (in_ss) then "--inss" else ""} \
  ...
  parameter_meta {
    ...
    q_norm: "quantile-normalize the exp levels to a N(0,1)"
    ...
    fit_err: "param used when estimating the variance of the errors (if --analys join, only with 'mvlr' or 'hybrid')\\ndefault=0.5 but can be between 0 (null model) and 1 (full model)"
