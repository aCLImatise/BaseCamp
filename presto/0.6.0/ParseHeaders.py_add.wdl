...
task ParseHeaderspyAdd {
  input {
    ...
    Boolean? fast_a
    String? delim
    Array[String] list_add_default
    Array[String] list_values_add
  }
  command <<<
    ParseHeaders_py add \
      ...
      ~{if defined(list_add_default) then ("-f " +  '"' + list_add_default + '"') else ""} \
      ~{if defined(list_values_add) then ("-u " +  '"' + list_values_add + '"') else ""}
  >>>
  parameter_meta {
    ...
    list_add_default: "List of fields to add. (default: None)"
    list_values_add: "List of values to add for each field. (default: None)\\n"
  }
  output {
