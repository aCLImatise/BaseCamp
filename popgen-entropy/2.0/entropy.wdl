...
task Entropy {
  input {
    ...
    Int? intra__ancestry
    Int? includes_population_frequencies
  ...
  command <<<
    entropy \
      ...
      ~{if defined(level_single_number) then ("-n " +  '"' + level_single_number + '"') else ""} \
      ...
      ~{if defined(includes_population_frequencies) then ("-w " +  '"' + includes_population_frequencies + '"') else ""} \
  ...
  parameter_meta {
    ...
    includes_population_frequencies: "includes population allele frequencies [default = 0]"
