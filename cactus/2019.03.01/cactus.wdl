...
task Cactus {
  input {
    String experimentExperiment
    Boolean buildBuildAvgS
    Boolean buildBuildReference
    Boolean buildBuildHal
    Boolean buildBuildFastA
    Int intermediateIntermediateResultsUrl
    String databaseDatabase
    String configConfigFile
    String rootRoot
    Boolean latestLatest
    String containerContainerImage
    String binariesBinariesMode
  ...
  command <<<
    cactus \
      ~{seqSeqFile} \
      ~{jobJobStore} \
      ~{if defined(experimentExperiment) then ("--experiment " +  '"' + experimentExperiment + '"') else ""} \
      ~{true="--buildAvgs" false="" buildBuildAvgS} \
      ~{true="--buildReference" false="" buildBuildReference} \
      ~{true="--buildHal" false="" buildBuildHal} \
      ~{true="--buildFasta" false="" buildBuildFastA} \
      ~{if defined(intermediateIntermediateResultsUrl) then ("--intermediateResultsUrl " +  '"' + intermediateIntermediateResultsUrl + '"') else ""} \
      ...
      ~{if defined(containerContainerImage) then ("--containerImage " +  '"' + containerContainerImage + '"') else ""} \
      ~{if defined(binariesBinariesMode) then ("--binariesMode " +  '"' + binariesBinariesMode + '"') else ""} \
