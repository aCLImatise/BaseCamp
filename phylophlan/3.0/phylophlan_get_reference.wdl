...
task PhylophlanGetReference {
  ...
  command <<<
    phylophlan_get_reference \
      ...
      ~{if defined(specify_path_save) then ("--output " +  '"' + specify_path_save + '"') else ""} \
  ...
  parameter_meta {
    ...
    specify_path_save: "Specify path to the output folder where to save the\\nfiles, required when -g/--get is specified (default:\\nNone)"
  ...
  output {
    ...
    File out_specify_path_save = "${in_specify_path_save}"
  }
}