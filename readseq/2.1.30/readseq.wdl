...
task Readseq {
  ...
  command <<<
    readseq \
      ...
      ~{if defined(combine_features_files) then ("-pair " +  '"' + combine_features_files + '"') else ""} \
  ...
  parameter_meta {
    ...
    combine_features_files: "* combine features (fff,gff) and sequence files to one output"
