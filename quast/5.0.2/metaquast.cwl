...
- id: operons
  ...
  inputBinding:
    prefix: --operons
- id: max_ref_number
  doc: 'Maximum number of references (per each assembly) to download after looking
    in SILVA database. Set 0 for not looking in SILVA at all [default: 50]'
  type: long
  inputBinding:
    prefix: --max-ref-number
- id: blast_db
  doc: Custom BLAST database (.nsq file). By default, MetaQUAST searches references
    in SILVA database
  type: File
  inputBinding:
    prefix: --blast-db
- id: use_input_ref_order
  doc: 'Use provided order of references in MetaQUAST summary plots (default order:
    by the best average value)'
  type: boolean
  inputBinding:
    prefix: --use-input-ref-order
...
- id: unique_mapping
  doc: Disable --ambiguity-usage=all for the combined reference run, i.e. use user-specified
...
- id: test_no_ref
  doc: Run MetaQUAST without references on the data from the test_data folder, output
    to quast_test_output. MetaQUAST will download SILVA 16S rRNA database (~170 Mb)
    for searching reference genomes Internet connection is required
  type: boolean
  inputBinding:
    prefix: --test-no-ref
outputs: []
