...
task Nosetests2.7 {
  input {
    ...
    Boolean? with_forced_gc
    String? test_timeout
    Boolean? with_test_data
    File? test_data_file
    Boolean? test_data_table
    String? attr
    String? eval_attr
    Boolean? no_capture
  ...
  command <<<
    nosetests-2.7 \
      ...
      ~{true="--with-forced-gc" false="" with_forced_gc} \
      ~{if defined(test_timeout) then ("--test-timeout " +  '"' + test_timeout + '"') else ""} \
      ~{true="--with-testdata" false="" with_test_data} \
      ~{if defined(test_data_file) then ("--testdata-file " +  '"' + test_data_file + '"') else ""} \
      ~{true="--testdata-table" false="" test_data_table} \
  ...
  parameter_meta {
    ...
    with_forced_gc: "Enable plugin ForcedGC: Force calls to the Python garbage collector before and after each test. [NOSE_WITH_FORCED_GC]"
    test_timeout: "A per-test timeout (in seconds). [NOSE_TEST_TIMEOUT]"
    with_test_data: "Enable plugin TestData: This plugin archives test data in standard CSV format. [NOSE_WITH_TESTDATA]"
    test_data_file: "Path to CSV file to store the test data. Default is testdata.csv in the working directory [NOSE_TESTDATA_FILE]"
    test_data_table: "If this option is specified, then the CSV file is formatted as a table.  By default, the format is a sparse list."
