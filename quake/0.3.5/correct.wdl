...
task Correct {
  input {
    ...
    File? file_containing_saved
    Int? separate_trusteduntrusted_kmers_cutoff
  ...
  command <<<
    correct \
      ...
      ~{if defined(file_containing_saved) then ("-b " +  '"' + file_containing_saved + '"') else ""} \
      ~{if defined(separate_trusteduntrusted_kmers_cutoff) then ("-c " +  '"' + separate_trusteduntrusted_kmers_cutoff + '"') else ""} \
  ...
  parameter_meta {
    ...
    file_containing_saved: "File containing saved bithash."
    separate_trusteduntrusted_kmers_cutoff: "Separate trusted/untrusted kmers at cutoff <num>"
