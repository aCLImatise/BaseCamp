...
task Commetpy {
  input {
    ...
    Directory? _outputdirectorydirectory_results
    Int? kmer_size
  ...
  command <<<
    Commet_py \
      ...
      ~{if (__binariesdirectory) then "-b" else ""} \
      ~{if (_outputdirectorydirectory_results) then "-o" else ""} \
  ...
  parameter_meta {
    ...
    _outputdirectorydirectory_results: ", --output_directory\\ndirectory in which results will be output [default:\\n\\\"output_commet\\\"]"
  ...
  output {
    ...
    Directory out__outputdirectorydirectory_results = "${in__outputdirectorydirectory_results}"
  }
}