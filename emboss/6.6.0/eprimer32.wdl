...
task Eprimer32 {
  ...
  command <<<
    eprimer32 \
      ...
      ~{if (ok_left_region) then "-okleftregion" else ""} \
      ~{if (ok_right_region) then "-okrightregion" else ""} \
      ...
      ~{if (s_correction) then "-scorrection" else ""} \
  ...
  parameter_meta {
    ...
    ok_left_region: "range      [(full sequence)] Possible left primer of\\npair location"
    ok_right_region: "range      [(full sequence)] Possible right primer of\\npair location"
