...
- id: pos_wt
  ...
  inputBinding:
    prefix: -posWt
- id: width_wt
  doc: 'integer    For -posWt 1 or 2, width of central sequence region with large
    EM weights for PWM optimization (default: 50). This argument is ignored when -posWt
    is 0 (uniform prior).'
  type: boolean
  inputBinding:
    prefix: -widthWt
- id: ev
  doc: 'decimal    ln(E-value) cutoff for selecting MOTIFS (default: 0.0). If a seeded
    analysis fails to identify the expected motif, run GADEM with -verbose 1 to show
    motif ln(E-value)s on screen, then re-run with a larger ln(E-value) cutoff. This
    can help in identifying short and/or low abundance motifs, for which the default
    log(E-value) threshold may be too low. A larger value (e.g., 10000) can return
    groups of closely related motifs (possible  variants) whereas a smaller value
    (<0) returns fewer (but conserved) motifs. To identify potential motif variants,
  ...
  inputBinding:
    prefix: -ev
- id: pv
  doc: "decimal    P-value cutoff for declaring BINDING SITES (default: 0.00025).\
  ...
  inputBinding:
    prefix: -pv
- id: minn
  doc: 'integer    Minimal number of sites required for a motif to be reported (default:
    numSeq/20).'
  type: boolean
  inputBinding:
    prefix: -minN
- id: fpwm0
  doc: "string     File name for the seed PWM, when a 'seeded' approach is used, in\
    ...
    \ focused, robust  to noise, and much faster than 'unseeded' de novo discovery.\
  ...
  inputBinding:
    prefix: -fpwm0
- id: b_file
  doc: 'string     File name for background model. The current version only supports
    0-th Markov background model Format: #monomer frequencies (this line is optional)
    a        0.2834 c        0.2215 g        0.2333 t        0.2618'
  type: boolean
  inputBinding:
    prefix: -bfile
- id: gen
  doc: 'integer    Number of genetic algorithm (GA) generations (default: 5).'
  type: boolean
  inputBinding:
    prefix: -gen
- id: pop
  doc: 'integer    GA population size (default: 100). Both default settings should
    work well for most datasets (ChIP-chip and ChIP-seq). The above two arguments
...
- id: full_scan
  ...
  inputBinding:
    prefix: -fullScan
- id: mask_r
  doc: "0 or 1     Mask repetitive, low-complexity sequences as below(default: 0-no\
    \ masking,1-masking): a) 'aaaaaaaa', 'tttttttt', 'cacacaca', 'tgtgtgtg', or 'tatatatat',\
    \ each of which is at least 8 nucleotides long. b) 'ggaggaggagga','gaggaggaggag','agaagaagaaga','ctcctcctcctc','tcctcctcctcc',\
  ...
  inputBinding:
    prefix: -maskR
- id: em
  doc: 'integer    Number of EM steps (default: 40). One might want to set it to a
    larger value (e.g. 80) in a seeded run, because such runs are fast. If you wish
    to scan the sequences with a PWM, set -em to 0.'
  type: boolean
  inputBinding:
    prefix: -em
- id: fem
  doc: 'decimal    Fraction of sequences used in EM to obtain PWMs in an unseeded
  ...
  inputBinding:
    prefix: -fEM
- id: ext_trim
  doc: '1 or 0     Base extension and trimming (1 -yes, 0 -no) (default: 1).'
  type: boolean
  inputBinding:
    prefix: -extTrim
- id: n_motifs
  doc: 'integer    Maximal number of motifs sought (default: 25).'
  type: boolean
  inputBinding:
    prefix: -nmotifs
- id: maxw3
...
- id: maxw5
  ...
  inputBinding:
    prefix: -maxw5
- id: min_gap
  doc: 'integer    Minimal number of unspecified nucleotides in spaced dyads (default:
    0).'
  type: boolean
  inputBinding:
    prefix: -mingap
- id: max_gap
  doc: 'integer    Maximal number of unspecified nucleotides in spaced dyads (default:
    10). -mingap and -maxgap control the lengths of spaced dyads, and, with -extrim,
    control motif lengths. Longer motifs can be discovered by setting -maxgap to larger
    values (e.g. 50). To identify short motifs (6-10 bps), set both -maxgap and -maxw5
    to 0'
  type: boolean
  inputBinding:
    prefix: -maxgap
- id: use_score
  ...
  inputBinding:
    prefix: -useScore
- id: f_pwm
  doc: 'string     Name of output PWM file in STAMP format (http://www.benoslab.pitt.edu/stamp).
  ...
  inputBinding:
    prefix: -fpwm
- id: f_out
  doc: 'string     Name of main GADEM output file (see documentation for description)
    (default: gadem.txt).'
  type: boolean
  inputBinding:
    prefix: -fout
- id: nbs
  doc: 'integer    Number of sets of randomly simulated sequences (default: 10) using
    the [a,c,g,t] frequencies in the input sequences, with length matched between
    the two sets. Those sequences are used as the random sequences for assessing motif
    enrichment in the input data.'
  type: boolean
  inputBinding:
    prefix: -nbs
- id: verbose
  doc: 1 or 0     Print immediate results on screen [1-yes (default), 0-no]. These
    results include the motif consensus sequence, number of sites (in the subset of
    sequences subjected to EM optimization, see -fEM, above), and ln(E-value).
  type: boolean
  inputBinding:
    prefix: -verbose
outputs: []
