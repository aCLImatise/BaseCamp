...
task Sc3Sc3.R {
  input {
    ...
    String? ks
    String? gene_filter
    String? pct_dropout_min
    String? pct_dropout_max
    String? d_region_min
    String? d_region_max
    String? svm_num_cells
    String? svm_train_inds
    String? svm_max
    String? n_cores
    String? k_means_n_start
    String? k_means_iter_max
    String? k_estimator
    String? biology
    String? rand_seed
  ...
  command <<<
    sc3-sc3.R \
      ...
      ~{if defined(k_estimator) then ("--k_estimator " +  '"' + k_estimator + '"') else ""} \
      ~{if defined(biology) then ("--biology " +  '"' + biology + '"') else ""} \
  ...
  parameter_meta {
    ...
    k_estimator: "A boolean variable which defines whether to estimate an optimal number of clusters. If user has already defined the ks parameter the estimation does not affect the user's paramater."
    biology: "A boolean variable which defines whether to compute differentially expressed genes, marker genes and cell outliers."
