...
task ClosestBed {
  input {
    ...
    Boolean? report_closest_feature
    Boolean? io
    Boolean? iu
    Boolean? id
    Boolean? fu
  ...
  command <<<
    closestBed \
      ...
      ~{if (report_closest_feature) then "-D" else ""} \
      ~{if (io) then "-io" else ""} \
      ~{if (iu) then "-iu" else ""} \
      ~{if (id) then "-id" else ""} \
      ~{if (fu) then "-fu" else ""} \
      ...
      ~{if (require_query_hit) then "-N" else ""} \
  ...
  parameter_meta {
    ...
    require_query_hit: "Require that the query and the closest hit have different names.\\nFor BED, the 4th column is compared."
