...
task Art454 {
  ...
  command <<<
    art_454 \
      ...
      ~{if defined(output_aln_file) then ("-a " +  '"' + output_aln_file + '"') else ""} \
      ~{if defined(output_sam_file) then ("-s " +  '"' + output_sam_file + '"') else ""} \
      ~{if defined(warning_messages_debugging) then ("-d " +  '"' + warning_messages_debugging + '"') else ""} \
      ~{if defined(simulate_reads_builtin) then ("-t " +  '"' + simulate_reads_builtin + '"') else ""} \
  ...
  parameter_meta {
    ...
    output_aln_file: "to output the ALN alignment file"
    output_sam_file: "to output the SAM alignment file"
    warning_messages_debugging: "out warning messages for debugging"
    simulate_reads_builtin: "to simulate reads from the built-in GS FLX Titanium profile [default: GS FLX profile]"
  ...
  output {
    ...
    File out_output_aln_file = "${in_output_aln_file}"
