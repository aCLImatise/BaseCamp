...
- doc: "Don't rerun CRISPResso2 if a run using the same parameters has already been\
  ...
  inputBinding:
    prefix: --no_rerun
  type: boolean
- doc: 'Amplicons description file. This file is a tab- delimited text file with up
    to 5 columns (2 required): AMPLICON_NAME: an identifier for the amplicon (must
    be unique) AMPLICON_SEQUENCE: amplicon sequence used in the experiment sgRNA_SEQUENCE
    (OPTIONAL): sgRNA sequence used for this amplicon without the PAM sequence. Multiple
    guides can be given separated by commas and not spaces. If not available enter
    NA. EXPECTED_AMPLICON_AFTER_HDR (OPTIONAL): expected amplicon sequence in case
    of HDR. If not available enter NA. CODING_SEQUENCE (OPTIONAL): Subsequence(s)
    of the amplicon corresponding to coding sequences. If more than one separate them
    by commas and not spaces. If not available enter NA. (default: )'
  id: amplicons_file
  inputBinding:
    prefix: --amplicons_file
  type: string
- doc: 'Gene Annotation Table from UCSC Genome Browser Tables (http://genome.ucsc.edu/cgi-
    bin/hgTables?command=start), please select as table "knownGene", as output format
    "all fields from selected table" and as file returned "gzip compressed" (default:
    )'
  id: gene_annotations
  inputBinding:
    prefix: --gene_annotations
  type: string
- doc: 'Specify the number of processes to use for Bowtie2. Please use with caution
    since increasing this parameter will increase significantly the memory required
    to run CRISPResso. (default: 1)'
  id: n_processes
  inputBinding:
    prefix: --n_processes
  type: string
...
- doc: 'Override options for the Bowtie2 alignment command (default: -k 1 --end-to-end
  ...
  type: string
- doc: 'Minimum number of reads that align to a region to perform the CRISPResso analysis
    (default: 1000)'
  id: min_reads_to_use_region
  inputBinding:
    prefix: --min_reads_to_use_region
  type: long
- doc: 'Continue with pooled analysis even if one sample fails (default: False)'
  id: skip_failed
  inputBinding:
    prefix: --skip_failed
  type: boolean
