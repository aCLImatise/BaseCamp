...
task CRISPRessoPooled {
  input {
    ...
    Boolean? debug
    Boolean? no_rerun
    String? amplicons_file
    String? gene_annotations
    String? n_processes
    String? bowtie_two_index
    String? bowtie_two_options_string
  ...
  parameter_meta {
    ...
    amplicons_file: "Amplicons description file. This file is a tab- delimited text file with up to 5 columns (2 required): AMPLICON_NAME: an identifier for the amplicon (must be unique) AMPLICON_SEQUENCE: amplicon sequence used in the experiment sgRNA_SEQUENCE (OPTIONAL): sgRNA sequence used for this amplicon without the PAM sequence. Multiple guides can be given separated by commas and not spaces. If not available enter NA. EXPECTED_AMPLICON_AFTER_HDR (OPTIONAL): expected amplicon sequence in case of HDR. If not available enter NA. CODING_SEQUENCE (OPTIONAL): Subsequence(s) of the amplicon corresponding to coding sequences. If more than one separate them by commas and not spaces. If not available enter NA. (default: )"
    ...
    n_processes: "Specify the number of processes to use for Bowtie2. Please use with caution since increasing this parameter will increase significantly the memory required to run CRISPResso. (default: 1)"
    bowtie_two_index: "Basename of Bowtie2 index for the reference genome (default: )"
    bowtie_two_options_string: "Override options for the Bowtie2 alignment command (default: -k 1 --end-to-end -N 0 --np 0 )"
