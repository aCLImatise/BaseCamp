...
- id: from_table
  ...
  inputBinding:
    position: 0
- id: condition
  doc: Conditions by which variants are excluded. Multiple arguments are automatically
    joined by 'AND' so 'OR' conditions should be provided by a single argument with
    conditions joined by 'OR'. If unspecified, all variants (except those excluded
    by parameter --samples) will be excluded.
  type: string
  inputBinding:
    position: 1
- id: s
  doc: "[COND [COND ...]], --samples [COND [COND ...]] Limiting variants from samples\
    ...
    \ (e.g. 'aff=1', 'filename like \"MG%\"'). Variants with homozygous wildtype genotype\
    \ are not selected if the sample contains genotype information."
  type: boolean
  inputBinding:
    prefix: -s
- id: t
  doc: '[TABLE [DESC ...]], --to_table [TABLE [DESC ...]] Destination variant table.'
  type: boolean
  inputBinding:
    prefix: -t
- id: count
  doc: Output number of variant, which is a shortcut to '-- output count(1)'.
  type: boolean
  inputBinding:
    prefix: --count
- id: o
  doc: '[FIELDS [FIELDS ...]], --output [FIELDS [FIELDS ...]] A list of fields that
    will be outputted. SQL- compatible expressions or functions such as "pos-1", "count(1)"
    or "sum(num)" are also allowed.'
  type: boolean
  inputBinding:
    prefix: -o
- id: v
