...
task Megablast {
  input {
    ...
    Boolean? maximal_number_positions
    ...
    Boolean? identity_percentage_cutoff
    ...
    Boolean? make_megablast_generate
    Boolean? use_nongreedy_extension
    Boolean? type_two_default
  ...
  command <<<
    megablast \
      ...
      ~{if (maximal_number_positions) then "-P" else ""} \
      ...
      ~{if (identity_percentage_cutoff) then "-p" else ""} \
      ...
      ~{if (make_megablast_generate) then "-g" else ""} \
      ~{if (use_nongreedy_extension) then "-n" else ""} \
      ~{if (type_two_default) then "-N" else ""} \
  ...
  parameter_meta {
    ...
    maximal_number_positions: "Maximal number of positions for a hash value (set to 0 to ignore) [Integer]\\ndefault = 0"
    ...
    identity_percentage_cutoff: "Identity percentage cut-off [Real]\\ndefault = 0"
    ...
    make_megablast_generate: "Make discontiguous megablast generate words for every base of the database (mandatory with the current BLAST engine) [T/F]  Optional\\ndefault = T"
    use_nongreedy_extension: "Use non-greedy (dynamic programming) extension for affine gap scores [T/F]  Optional\\ndefault = F"
    type_two_default: "Type of a discontiguous word template (0 - coding, 1 - optimal, 2 - two simultaneous [Integer]\\ndefault = 0"
