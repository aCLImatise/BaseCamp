...
task Fastacmd {
  input {
    ...
    Boolean? retrieve_sequences_optional
  }
  command <<<
    fastacmd \
      ~{if (database_optionaldefault_nr) then "-d" else ""} \
      ~{if (type_fileg_guess) then "-p" else ""} \
      ~{if (commadelimited_search_stringsgis) then "-s" else ""} \
      ~{if (input_file_gisaccessionsloci) then "-i" else ""} \
      ~{if (retrieve_duplicate_accessions) then "-a" else ""} \
      ~{if (line_length_sequence) then "-l" else ""} \
      ~{if (definition_line_should) then "-t" else ""} \
      ...
      ~{if (retrieve_sequences_optional) then "-P" else ""}
  >>>
  parameter_meta {
    database_optionaldefault_nr: "Database [String]  Optional\\ndefault = nr"
    ...
    input_file_gisaccessionsloci: "Input file with GIs/accessions/loci for batch\\nretrieval [String]  Optional"
    ...
    definition_line_should: "Definition line should contain target gi only [T/F]  Optional\\ndefault = F"
    ...
    retrieve_sequences_optional: "Retrieve sequences with this PIG [Integer]  Optional"
  }
  output {
