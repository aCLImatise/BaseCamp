...
task BismarkMethylationExtractor {
  input {
    ...
    File? cx_slash_cx_context
    Int? buffer_size
    Boolean? scaffolds_slash_gazillion
  ...
  command <<<
    bismark_methylation_extractor \
      ...
      ~{if (s_slash_single_end) then "-s/--single-end" else ""} \
      ~{if (p_slash_paired_end) then "-p/--paired-end" else ""} \
      ...
      ~{if (cx_slash_cx_context) then "--CX/--CX_context" else ""} \
      ~{if defined(buffer_size) then ("--buffer_size " +  '"' + buffer_size + '"') else ""} \
      ~{if (scaffolds_slash_gazillion) then "--scaffolds/--gazillion" else ""} \
  ...
  parameter_meta {
    ...
    s_slash_single_end: "Input file(s) are Bismark result file(s) generated from single-end\\nread data. If neither -s nor -p is set the type of experiment will\\nbe determined automatically."
    p_slash_paired_end: "Input file(s) are Bismark result file(s) generated from paired-end\\nread data. If neither -s nor -p is set the type of experiment will\\nbe determined automatically."
    ...
    oslash_output: "Allows specification of a different output directory (absolute or relative\\npath). If not specified explicitly, the output will be written to the current directory."
    ...
    h_slash_help: "Displays this help file and exits."
  ...
  output {
    ...
    File out_cx_slash_cx_context = "${in_cx_slash_cx_context}"
