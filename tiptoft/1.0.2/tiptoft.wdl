...
task Tiptoft {
  ...
  command <<<
    tiptoft \
      ...
      ~{true="--no_hc_compression" false="" no_hc_compression} \
      ~{true="--no_gene_filter" false="" no_gene_filter} \
  ...
  >>>
  parameter_meta {
    plasmid_data: "FASTA file containing plasmid data from downloader script, defaults to bundled database (default: None)"
    ...
    print_interval: "Print results every this number of reads (default: None)"
    verbose: "Turn on debugging [False]"
    no_hc_compression: "Turn off homoploymer compression of k-mers (default: False)"
    no_gene_filter: "Dont filter out lower coverage genes from same group (default: False)"
    max_gap: "Maximum gap for blocks to be contigous, measured in multiples of the k-mer size (default: 3)"
    max_km_er_count: "Exclude k-mers which occur more than this number of times in a sequence (default: 10)"
    margin: "Flanking region around a block to use for mapping (default: 10)"
    min_block_size: "Minimum block size in bases (default: 50)"
    min_fast_a_hits: "Minimum No. of kmers matching a read (default: 8)"
    min_perc_coverage: "Minimum percentage coverage of typing sequence to report (default: 85)"
    min_km_ers_for_one_x_pass: "Minimum No. of kmers matching a read in 1st pass (default: 5)"
  }
}