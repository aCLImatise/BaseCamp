...
id: arb_repair_outputdatabase.cwl
inputs:
- id: author
...
- id: escape
  ...
  inputBinding:
    prefix: --escape
- id: block_size
  doc: with -l, scale sizes by SIZE when printing them; e.g., '--block-size=M'; see
    SIZE format below
  type: long
  inputBinding:
    prefix: --block-size
...
- id: hide
  ...
  inputBinding:
    prefix: --hide
- id: hyperlink
  doc: "[=WHEN]     hyperlink file names; WHEN can be 'always' (default if omitted),\
    \ 'auto', or 'never'"
  type: boolean
  inputBinding:
    prefix: --hyperlink
...
- id: ignore
  ...
  inputBinding:
    prefix: --ignore
- id: kib_i_bytes
  doc: default to 1024-byte blocks for disk usage; used only with -s and per directory
    totals
  type: boolean
  inputBinding:
    prefix: --kibibytes
