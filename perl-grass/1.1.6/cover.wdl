...
task Cover {
  input {
    ...
    String? delete_uncover_able_point
    Boolean? clean_uncover_able_points
    Boolean? uncover_able_file
  }
  command <<<
    cover \
      ...
      ~{if (clean_uncover_able_points) then "-clean_uncoverable_points" else ""} \
      ~{if (uncover_able_file) then "-uncoverable_file" else ""}
  >>>
  parameter_meta {
    ...
    delete_uncover_able_point: ""
    clean_uncover_able_points: ""
    uncover_able_file: "file"
  }
  output {
