...
task GatbH5dump {
  input {
    ...
    String anyAnyPath
    Boolean aA
    String vdsVdsViewFirstMissing
    String vdsVdsGapSize
    Boolean iI
    Boolean pP
  ...
  command <<<
    gatb-h5dump \
      ...
      ~{true="-A" false="" aA} \
      ~{if defined(vdsVdsViewFirstMissing) then ("--vds-view-first-missing " +  '"' + vdsVdsViewFirstMissing + '"') else ""} \
      ~{if defined(vdsVdsGapSize) then ("--vds-gap-size " +  '"' + vdsVdsGapSize + '"') else ""} \
      ~{true="-i" false="" iI} \
      ~{true="-p" false="" pP} \
