...
task Obiannotate {
  input {
    ...
    Boolean seqSeqRank
    Boolean rR
    String deleteDeleteTag
    Boolean sS
    File tagTagList
    String setSetIdentifier
    String runRun
    String setSetSequence
    String setSetDefinition
    Boolean onlyOnlyValidPython
    Boolean clearClear
    String keepKeep
  ...
  command <<<
    obiannotate \
      ~{if defined(withWithTaxOnAtRank) then ("--with-taxon-at-rank " +  '"' + withWithTaxOnAtRank + '"') else ""} \
      ...
      ~{true="--seq-rank" false="" seqSeqRank} \
      ~{true="-R" false="" rR} \
      ~{if defined(deleteDeleteTag) then ("--delete-tag " +  '"' + deleteDeleteTag + '"') else ""} \
      ~{true="-S" false="" sS} \
      ~{if defined(tagTagList) then ("--tag-list " +  '"' + tagTagList + '"') else ""} \
      ~{if defined(setSetIdentifier) then ("--set-identifier " +  '"' + setSetIdentifier + '"') else ""} \
      ~{if defined(runRun) then ("--run " +  '"' + runRun + '"') else ""} \
      ~{if defined(setSetSequence) then ("--set-sequence " +  '"' + setSetSequence + '"') else ""} \
      ~{if defined(setSetDefinition) then ("--set-definition " +  '"' + setSetDefinition + '"') else ""} \
      ~{true="--only-valid-python" false="" onlyOnlyValidPython} \
      ~{true="--clear" false="" clearClear} \
      ~{if defined(keepKeep) then ("--keep " +  '"' + keepKeep + '"') else ""} \
      ~{true="--length" false="" lengthLength} \
      ~{if defined(withWithTaxOnAtRank) then ("--with-taxon-at-rank " +  '"' + withWithTaxOnAtRank + '"') else ""} \
      ~{if defined(mclMcl) then ("--mcl " +  '"' + mclMcl + '"') else ""} \
      ~{true="--uniq-id" false="" uniqUniqId} \
