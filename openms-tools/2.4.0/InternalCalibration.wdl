...
task InternalCalibration {
  ...
  parameter_meta {
    ...
    ppm_match_tolerance: "<delta m/z in [ppm]>  Finding calibrants in raw data uses this tolerance (for lock masses and ID's). (default: '25')"
    ms_level: "MS levels to apply the transformation onto. Does not affect calibrant collection. (default: '[1 2 3]')"
    rt_chunking: "<RT window in [sec]>          RT window (one-sided, i.e. left->center, or center->right) around an MS scan in which calibrants are collected to build a model. Set to -1 to use ALL calibrants for all scans, i.e. a global model. (default: '300')"
