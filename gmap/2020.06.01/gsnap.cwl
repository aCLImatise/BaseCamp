...
id: ../../../../home/ubuntu/BiocondaCli/gsnap.sse42.cwl
inputs:
- id: dir
...
- id: max_mismatches
  doc: Maximum number of mismatches allowed (if not specified, then GSNAP tries to
    ...
    number of mismatches (including indel and splicing penalties). Default is to be
    unspecified (to find the best possible match)
  type: double
  inputBinding:
...
- id: max_middle_insertions
  doc: Maximum number of middle insertions allowed (default is 0.2) If specified between
    0.0 and 1.0, then treated as a fraction of each read length.  Otherwise, treated
    as an integral number of base pairs
  type: double
  inputBinding:
    prefix: --max-middle-insertions
- id: max_middle_deletions
  doc: Maximum number of middle deletions allowed (default 0.2) If specified between
    0.0 and 1.0, then treated as a fraction of each read length.  Otherwise, treated
    as an integral number of base pairs
  type: double
  inputBinding:
