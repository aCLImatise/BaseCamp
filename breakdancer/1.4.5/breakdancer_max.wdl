...
task BreakdancerMax {
  ...
  command <<<
    breakdancer-max \
      ...
      ~{if defined(minimum_number_required) then ("-r " +  '"' + minimum_number_required + '"') else ""} \
      ...
      ~{true="-t" false="" only_detect_rearrangement} \
      ...
      ~{true="-l" false="" analyze_insert_library} \
      ~{true="-a" false="" print_copy_number} \
  ...
  parameter_meta {
    ...
    minimum_number_required: "minimum number of read pairs required to establish a connection [2]"
