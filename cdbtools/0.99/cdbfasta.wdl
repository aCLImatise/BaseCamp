...
task Cdbfasta {
  ...
  command <<<
    cdbfasta \
      ...
      ~{fast_a_file} \
      ~{argument} \
      ~{if defined(index_file_will) then ("-o " +  '"' + index_file_will + '"') else ""} \
      ...
      ~{if defined(compressed_file_compresseddbbefore) then ("-z " +  '"' + compressed_file_compresseddbbefore + '"') else ""} \
      ~{if defined(extraneous_characters_space) then ("-s " +  '"' + extraneous_characters_space + '"') else ""} \
      ~{if (option_create_entries) then "-m" else ""} \
      ~{if defined(same_only_takes) then ("-n " +  '"' + same_only_takes + '"') else ""} \
      ~{if defined(space_delimited_fields) then ("-f " +  '"' + space_delimited_fields + '"') else ""} \
      ~{if defined(exclude_indexing_words) then ("-w " +  '"' + exclude_indexing_words + '"') else ""} \
  ...
  >>>
  parameter_meta {
    index_file_will: "index file will be named <index_file>; if not given,\\nthe index filename is database name plus the suffix '.cidx'"
    ...
    same_only_takes: "same as -m, but only takes the first <numkeys>\\ntokens from the defline"
    ...
    exclude_indexing_words: "exclude from indexing all the words found\\nin the file <stopwordslist> (for options -m, -n and -k)"
