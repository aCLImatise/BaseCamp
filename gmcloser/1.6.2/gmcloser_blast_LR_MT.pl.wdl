...
task GmcloserblastLRMTpl {
  ...
  command <<<
    gmcloser_blast_LR_MT_pl \
      ...
      ~{if defined(query_index) then ("--query_index " +  '"' + query_index + '"') else ""} \
  ...
  parameter_meta {
    ...
    query_index: "basename of bowtie2 index files for query long reads [optional]"
    ...
    min_match_len: "minimum overlap length to be filtered for BLASTn alignments.\\nContig-alignments that satisfy both the values specified with -mm and -mi are selected, irrespective of any mapping rates of PE-reads. [>49, default: 300]"
    min_identity: "minimum overlap identity (%) to be filtered for BLASTn alignments. Alignments are selected by combination with -mm option. [95~100, default: 99]"
    min_len_local: "minimum overlap length for merging between neighbor subcontigs with YASS aligner [>14, default: 20]"
    min_subcon: "minimum length of subcontigs, to be used for gapclosing [default: 150]"
