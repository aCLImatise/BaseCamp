...
task PairtoolsRestrict {
  ...
  command <<<
    pairtools restrict \
      ...
      ~{if defined(output_pairspairsam_file) then ("--output " +  '"' + output_pairspairsam_file + '"') else ""} \
  ...
  output {
    ...
    File out_output_pairspairsam_file = "${in_output_pairspairsam_file}"
  }
}