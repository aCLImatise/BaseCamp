...
task KinSimRiboswitch {
  input {
    ...
    Int? use_monomer_basin
    Float? ligand_concentration_molar
  ...
  command <<<
    kinSimRiboswitch \
      ...
      ~{if defined(ligand_concentration_molar) then ("-L " +  '"' + ligand_concentration_molar + '"') else ""} \
      ~{if defined(bonus_energy_binding) then ("-B " +  '"' + bonus_energy_binding + '"') else ""} \
      ~{if (folding_rate_rna) then "-F" else ""} \
      ~{if (dimerization_rate_m) then "-D" else ""} \
