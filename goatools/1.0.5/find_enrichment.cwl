...
- id: alpha
  ...
  inputBinding:
    prefix: --alpha
- id: pval
  doc: 'Only print results with uncorrected p-value < PVAL. Print all results, significant
    and otherwise, by setting --pval=1.0 (default: 0.05)'
  type: string
  inputBinding:
    prefix: --pval
- id: pval_field
...
- id: ratio
  ...
  inputBinding:
    prefix: --ratio
- id: prt_study_gos_only
  doc: 'Print GO terms only if they are associated with study genes. This is useful
    if printng all GO results regardless of their significance (--pval=1.0). (default:
    False)'
  type: boolean
  inputBinding:
...
- id: relationships
  doc: '[RELATIONSHIPS [RELATIONSHIPS ...]] Propagate counts up user-specified relationships,
    which include: part_of regulates negatively_regulates positively_regulates (default:
    None)'
  type: boolean
  inputBinding:
