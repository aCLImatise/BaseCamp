...
task RunMidas.pySnps {
  input {
    ...
    Int? species_top_n
    String? species_id
    String? fastafastq_file_containing_st_mate
    String? fastafastq_file_containing_nd_mate
    Boolean? interleaved
  ...
  command <<<
    run_midas.py snps \
      ~{outdir} \
      ~{true="--remove_temp" false="" remove_temp} \
      ~{true="--build_db" false="" build_db} \
      ...
      ~{true="--baq" false="" baq} \
      ~{true="--adjust_mq" false="" adjust_mq}
  >>>
  parameter_meta {
    remove_temp: "Remove intermediate files generated by MIDAS (False). Useful to reduce disk space of MIDAS output"
    ...
    fastafastq_file_containing_st_mate: "FASTA/FASTQ file containing 1st mate if using paired-end reads. Otherwise FASTA/FASTQ containing unpaired reads. Can be gzip'ed (extension: .gz) or bzip2'ed (extension: .bz2)"
    fastafastq_file_containing_nd_mate: "FASTA/FASTQ file containing 2nd mate if using paired-end reads. Can be gzip'ed (extension: .gz) or bzip2'ed (extension: .bz2)"
    ...
    outdir: "Path to directory to store results.  Directory name should correspond to sample identifier"
  }
}