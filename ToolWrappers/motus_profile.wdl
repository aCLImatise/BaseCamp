...
task MotusProfile {
  input {
    ...
    File? provide_mgc_file
    File? output_file_name
    File? save_result_bwa
    File? save_mgc_reads
  ...
  command <<<
    motus profile \
      ...
      ~{if defined(provide_mgc_file) then ("-m " +  '"' + provide_mgc_file + '"') else ""} \
      ~{if defined(output_file_name) then ("-o " +  '"' + output_file_name + '"') else ""} \
      ~{if defined(save_result_bwa) then ("-I " +  '"' + save_result_bwa + '"') else ""} \
      ...
      ~{if (print_full_taxonomy) then "-q" else ""} \
  ...
  parameter_meta {
    ...
    provide_mgc_file: "provide a mgc reads count file"
    output_file_name: "output file name [stdout]"
    save_result_bwa: "save the result of bwa in bam format (intermediate step) [None]"
