...
task Svmtrain {
  ...
  command <<<
    svm_train \
      ~{training_set_file} \
      ~{model_file} \
      ~{if defined(set_type_svm) then ("-s " +  '"' + set_type_svm + '"') else ""} \
      ~{if defined(set_type_uv) then ("-t " +  '"' + set_type_uv + '"') else ""} \
      ...
      ~{if defined(set_parameter_nu) then ("-n " +  '"' + set_parameter_nu + '"') else ""} \
  ...
  >>>
  parameter_meta {
    set_type_svm: ": set type of SVM (default 0)\\n0 -- C-SVC              (multi-class classification)\\n1 -- nu-SVC             (multi-class classification)\\n2 -- one-class SVM\\n3 -- epsilon-SVR        (regression)\\n4 -- nu-SVR             (regression)"
    set_type_uv: ": set type of kernel function (default 2)\\n0 -- linear: u'*v\\n1 -- polynomial: (gamma*u'*v + coef0)^degree\\n2 -- radial basis function: exp(-gamma*|u-v|^2)\\n3 -- sigmoid: tanh(gamma*u'*v + coef0)\\n4 -- precomputed kernel (kernel values in training_set_file)"
    ...
    set_parameter_nu: ": set the parameter nu of nu-SVC, one-class SVM, and nu-SVR (default 0.5)"
    set_epsilon_loss: ": set the epsilon in loss function of epsilon-SVR (default 0.1)"
    set_cache_size: ": set cache memory size in MB (default 100)"
    ...
    nfold_cross_validation: ": n-fold cross validation mode"
