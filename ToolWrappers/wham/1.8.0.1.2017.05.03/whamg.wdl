...
task Whamg {
  input {
    ...
    Boolean? string_region_seqidstartend
    Boolean? int_mapping_quality
    Boolean? string_split_read
  ...
  command <<<
    whamg \
      ...
      ~{if (string_region_seqidstartend) then "-r" else ""} \
      ~{if (int_mapping_quality) then "-m" else ""} \
      ~{if (string_split_read) then "-i" else ""} \
  ...
  parameter_meta {
    ...
    string_region_seqidstartend: "<STRING>  Region in format: seqid:start-end [whole genome]"
    int_mapping_quality: "<INT>     Mapping quality filter [20]."
    string_split_read: "<STRING>  non standard split read tag [SA]"
