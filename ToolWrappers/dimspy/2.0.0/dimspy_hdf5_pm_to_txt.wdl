...
task DimspyHdf5pmtotxt {
  input {
    ...
    File? directory_peaklists_towrite
  ...
  command <<<
    dimspy hdf5_pm_to_txt \
      ...
      ~{if defined(directory_peaklists_towrite) then ("--output " +  '"' + directory_peaklists_towrite + '"') else ""} \
  ...
  parameter_meta {
    ...
    directory_peaklists_towrite: "Directory (peaklists) or text file (peak matrix) to\\nwrite to."
