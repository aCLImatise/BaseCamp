...
task DimspyProcessscans {
  input {
    File? directory_mzml_peaklistfiles
    File? hdf_file_save
    File? file_list
  ...
  command <<<
    dimspy process_scans \
      ~{ringing} \
      ~{if defined(directory_mzml_peaklistfiles) then ("--input " +  '"' + directory_mzml_peaklistfiles + '"') else ""} \
  ...
  >>>
  parameter_meta {
    directory_mzml_peaklistfiles: "Directory (*.raw, *.mzml or tab-delimited peaklist\\nfiles), single *.mzml/*.raw file or zip archive\\n(*.mzml only)"
