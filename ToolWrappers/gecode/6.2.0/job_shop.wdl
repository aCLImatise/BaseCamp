...
task Jobshop {
  ...
  command <<<
    job_shop \
      ...
      ~{if (branching) then "-branching" else ""} \
      ...
      ~{if (probes) then "-probes" else ""} \
      ~{if (fail_probe) then "-fail-probe" else ""} \
      ~{if (time_probe) then "-time-probe" else ""} \
      ~{if (time_adjust) then "-time-adjust" else ""} \
      ~{if (time_solve) then "-time-solve" else ""} \
      ~{if (tbf) then "-tbf" else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/gecode:6.2.0--hb70a6f5_0"
  }
  parameter_meta {
    propagation: "(order, unary) default: unary\\npropagation variants"
    ipl: "(def,val,bnd,dom,basic,advanced)\\ndefault: def\\ninteger propagation level (comma-separated list)"
    branching: "(afc, action, chb) default: afc\\nbranching variants"
    ...
    restart_base: "(double) default: 1.3\\nbase for geometric restart sequence"
    restart_scale: "(unsigned int) default: 5000\\nscale factor for restart sequence"
    no_goods: "(optional: false, 0, true, 1) default: true\\nwhether to use no-goods from restarts"
    ...
    verbose: "(optional: false, 0, true, 1) default: false\\nwhether to print schedule"
    probes: "(unsigned int) default: 50\\nhow many probes to perform"
    fail_probe: "(unsigned int) default: 10000\\nfailure limit per probe"
    time_probe: "(unsigned int) default: 30000\\ntime-out for probing (in milliseconds)"
    time_adjust: "(unsigned int) default: 30000\\ntime-out for adjusting (in milliseconds)"
    time_solve: "(unsigned int) default: 60000\\ntime-out for solving (in milliseconds)"
    tbf: "(double) default: 0\\ntie-breaking factor"
    disabled: "Options for JobShop:"
  }
  output {
