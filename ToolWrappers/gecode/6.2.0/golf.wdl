...
task Golf {
  ...
  command <<<
    golf \
      ...
      ~{if (int_default_number_weeks) then "-w" else ""} \
      ~{if (int_default_number_groups) then "-g" else ""} \
      ~{if (int_default_number_players) then "-s" else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/gecode:6.2.0--hb70a6f5_0"
  }
  parameter_meta {
    model: "(none, symmetry) default: none\\nmodel variants\\nnone: no symmetry breaking\\nsymmetry: static symmetry breaking"
    propagation: "(set, int, mixed) default: set\\npropagation variants\\nset: Use set intersection cardinality for pair play constraints\\nint: Use integer distinct for pair play constraints\\nmixed: Use set interesection cardinality and integer distinct for pair play constraints"
    ...
    int_default_number_weeks: "(int) default: 9\\nnumber of weeks"
    int_default_number_groups: "(int) default: 8\\nnumber of groups"
    int_default_number_players: "(int) default: 4\\nnumber of players per group\\n"
    disabled: "Options for Golf:"
  }
  output {
