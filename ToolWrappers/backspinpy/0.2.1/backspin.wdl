...
task Backspin {
  input {
    ...
    Boolean? controls_rate_parametersmaller
    Boolean? minimal_number_genes_group_contain
    Boolean? minimal_number_cells_group_contain
    Boolean? minimum_score_has
    Boolean? difference_average_expression
    Boolean? run_normal_spin
    Boolean? verbose_print_details
  }
  command <<<
    backspin \
      ~{if (i) then "-i" else ""} \
      ~{if (path_cef_formatted) then "--input" else ""} \
      ~{if (o) then "-o" else ""} \
      ...
      ~{if (controls_rate_parametersmaller) then "-S" else ""} \
      ~{if (minimal_number_genes_group_contain) then "-g" else ""} \
      ~{if (minimal_number_cells_group_contain) then "-c" else ""} \
      ~{if (minimum_score_has) then "-k" else ""} \
      ~{if (difference_average_expression) then "-r" else ""} \
      ~{if (run_normal_spin) then "-b" else ""} \
      ~{if (verbose_print_details) then "-v" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    verbose_print_details: "Verbose. Print  to the stdoutput extra details of what is happening\\n"
  }
  output {
