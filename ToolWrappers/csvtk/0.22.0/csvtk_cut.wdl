...
task CsvtkCut {
  ...
  command <<<
    csvtk cut \
      ...
      ~{if (uniq_column) then "--uniq-column" else ""} \
  ...
  parameter_meta {
    fields: "select only these fields. type \\\"csvtk cut -h\\\" for examples"
    fuzzy_fields: "using fuzzy fields, e.g., -F -f \\\"*name\\\" or -F -f \\\"id123*\\\""
    ignore_case: "ignore case (column name)"
    uniq_column: "deduplicate columns matched by multiple fuzzy column names"
