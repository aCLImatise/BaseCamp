...
task CsvtkFreq {
  ...
  command <<<
    csvtk freq \
      ...
      ~{if (sort_by_freq) then "--sort-by-freq" else ""} \
      ~{if (sort_by_key) then "--sort-by-key" else ""} \
  ...
  parameter_meta {
    fields: "select these fields as the key. e.g -f 1,2 or -f columnA,columnB (default \\\"1\\\")"
    fuzzy_fields: "using fuzzy fields, e.g., -F -f \\\"*name\\\" or -F -f \\\"id123*\\\""
    ignore_case: "ignore case"
    reverse: "reverse order while sorting"
    sort_by_freq: "sort by frequency"
