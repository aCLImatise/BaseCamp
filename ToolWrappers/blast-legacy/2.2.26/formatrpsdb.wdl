...
task Formatrpsdb {
  ...
  command <<<
    formatrpsdb \
      ...
      ~{if (logfile_name_formatrpsdblog) then "-l" else ""} \
      ...
      ~{if (base_name_same) then "-n" else ""} \
      ~{if (scoremats_contain_frequencies) then "-S" else ""} \
  ...
  parameter_meta {
    ...
    logfile_name_formatrpsdblog: "Logfile name [File Out]  Optional\\ndefault = formatrpsdb.log"
    ...
    base_name_same: "Base name of output database (same as input file if not specified) [String]  Optional"
    scoremats_contain_frequencies: "For scoremats that contain only residue frequencies, the scaling factor to apply when creating PSSMs [Real]  Optional\\ndefault = 100.0"
  ...
  output {
    ...
    File out_logfile_name_formatrpsdblog = "${in_logfile_name_formatrpsdblog}"
  }
}