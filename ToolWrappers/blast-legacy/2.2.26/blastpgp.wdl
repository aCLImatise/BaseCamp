...
task Blastpgp {
  input {
    ...
    File? output_file_psiblast_checkpointing_optional
  ...
  command <<<
    blastpgp \
      ...
      ~{if (constant_uses_method) then "-c" else ""} \
      ...
      ~{if (output_file_psiblast_checkpointing_optional) then "-C" else ""} \
      ~{if (input_file_optional) then "-R" else ""} \
  ...
  parameter_meta {
    ...
    threshold_extending_hits: "Threshold for extending hits [Integer]\\ndefault = 11"
    ...
    constant_uses_method: "Constant in pseudocounts for multipass version; 0 uses entropy method; otherwise a value near 30 is recommended [Integer]\\ndefault = 0"
    ...
    output_file_psiblast_checkpointing_optional: "Output File for PSI-BLAST Checkpointing [File Out]  Optional"
    input_file_optional: "Input File for PSI-BLAST Restart [File In]  Optional"
    ...
    hit_file_hitfile: "Hit File for PHI-BLAST [File In]\\ndefault = hit_file"
    produce_html_output: "Produce HTML output [T/F]\\ndefault = F"
    output_file_psiblast_matrix_optional: "Output File for PSI-BLAST Matrix in ASCII [File Out]  Optional"
    ...
    use_based_adjustment: "Use composition based score adjustment"
  ...
  output {
    ...
    File out_output_file_psiblast_checkpointing_optional = "${in_output_file_psiblast_checkpointing_optional}"
    File out_output_file_psiblast_matrix_optional = "${in_output_file_psiblast_matrix_optional}"
  }
}