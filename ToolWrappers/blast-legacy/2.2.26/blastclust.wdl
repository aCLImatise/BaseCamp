...
task Blastclust {
  ...
  command <<<
    blastclust \
      ...
      ~{if (require_coverage_neighbours) then "-b" else ""} \
      ...
      ~{if (restrict_reclustering_i) then "-l" else ""} \
      ~{if (input_proteins_default) then "-p" else ""} \
      ~{if (enable_i_parsing) then "-e" else ""} \
      ~{if (configuration_file_optional) then "-c" else ""} \
  ...
  parameter_meta {
    ...
    require_coverage_neighbours: "Require coverage on both neighbours? [T/F]\\ndefault = TRUE"
    ...
    print_progress_messages: "Print progress messages (verbose mode) [File Out]\\ndefault = stdout"
    complete_unfinished_clustering: "Complete unfinished clustering [T/F]\\ndefault = FALSE"
    restrict_reclustering_i: "Restrict reclustering to id list [File In]  Optional"
    input_proteins_default: "Is input proteins? [T/F]\\ndefault = TRUE"
    enable_i_parsing: "Enable id parsing in database formatting? [T/F]\\ndefault = TRUE"
    configuration_file_optional: "Configuration file with advanced options [File In]  Optional"
  ...
  output {
    ...
    File out_print_progress_messages = "${in_print_progress_messages}"
  }
}