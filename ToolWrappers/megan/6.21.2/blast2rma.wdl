...
task Blast2rma {
  ...
  command <<<
    blast2rma \
      ...
      ~{if (blast_mode) then "--blastMode" else ""} \
  ...
  parameter_meta {
    ...
    reads_files_fasta: "[string(s)]              Reads file(s) (fasta or fastq, .gz ok)."
    ...
    i__mandatory: "-i, --in [string(s)]                 Input BLAST file[s] (.gz ok). Mandatory option."
