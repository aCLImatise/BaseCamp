...
task Cgw {
  ...
  command <<<
    cgw \
      ...
      ~{if (strongly_enforce_flag) then "-F" else ""} \
      ...
      ~{if defined(set_coverage_stat) then ("-j " +  '"' + set_coverage_stat + '"') else ""} \
      ...
      ~{if defined(restart_checkpoint_location) then ("-N " +  '"' + restart_checkpoint_location + '"') else ""} \
      ...
      ~{if defined(place_frags_mean) then ("-S " +  '"' + place_frags_mean + '"') else ""} \
      ...
      ~{if (turn_check_repeat) then "-z" else ""}
  >>>
  parameter_meta {
    ...
    strongly_enforce_flag: "strongly enforce unique/repeat flag set in unitig, default if not set is to still\\nallow those marked unique to be demoted due to Repeat Branch Pattern or being\\ntoo small"
    ...
    set_coverage_stat: "Set min coverage stat for definite uniqueness"
    ...
    restart_checkpoint_location: "restart from checkpoint location 'ckp' (see the timing file)"
    ...
    turn_check_repeat: "Turn on Check for Repeat Branch Pattern (demotes some unique unitigs to repeat)"
  }
  output {
