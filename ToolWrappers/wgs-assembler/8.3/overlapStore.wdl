...
task OverlapStore {
  input {
    ...
    Boolean? report_overlap_exists
    Boolean? dump_picture_overlaps
    Boolean? estimate_genome_length
    Boolean? dump_store_create
    String? dump_only_overlaps_error
    Int? dump_only_overlaps_are
    Boolean? d_five
    Boolean? d_three
  ...
  command <<<
    overlapStore \
      ...
      ~{if (report_overlap_exists) then "-q" else ""} \
      ~{if (dump_picture_overlaps) then "-p" else ""} \
      ~{if (estimate_genome_length) then "-G" else ""} \
      ~{if (dump_store_create) then "-B" else ""} \
      ~{if defined(dump_only_overlaps_error) then ("-E " +  '"' + dump_only_overlaps_error + '"') else ""} \
      ~{if defined(dump_only_overlaps_are) then ("-L " +  '"' + dump_only_overlaps_are + '"') else ""} \
  ...
  parameter_meta {
    ...
    report_overlap_exists: "report the a,b overlap, if it exists."
    ...
    dump_store_create: "Dump the store as binary, suitable for input to create a new store."
    dump_only_overlaps_error: "Dump only overlaps <= erate error."
    dump_only_overlaps_are: "Dump only overlaps that are larger than L bases (only for -p picture mode)."
