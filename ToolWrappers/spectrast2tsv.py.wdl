...
task Spectrast2tsvpy {
  input {
    ...
    Boolean? allowedfrgz_fragment_ion
  ...
  command <<<
    spectrast2tsv_py \
      ...
      ~{if (massmodifs_list_allowed) then "-g" else ""} \
      ~{if (labelingfile_file_containing) then "-i" else ""} \
      ~{if (outputkey_select_output) then "-k" else ""} \
      ...
      ~{if (float_maximum_error) then "-p" else ""} \
      ...
      ~{if (allowedfrgz_fragment_ion) then "-x" else ""} \
  ...
  parameter_meta {
    ...
    massmodifs_list_allowed: "mass_modifs   List of allowed fragment mass modifications. Useful for phosphorylation and neutral losses. Example: -g -79.97,-97.98,-17.03,-18.01"
    ...
    outputkey_select_output: "output_key    Select the output provided. Keys available: openswath, peakview. Default: peakview"
    ...
    float_maximum_error: "float         Maximum error allowed at the annotation of a fragment ion. Default: 0.05"
    ...
    allowedfrgz_fragment_ion: "allowed_frg_z Fragment ion charge states allowed. Default: 1,2"
