...
task Spadespy {
  input {
    ...
    Boolean? isolate
    Boolean? sc
    Boolean? flag_required_metagenomic
    Boolean? bio
    Boolean? corona
    Boolean? rna
    Boolean? plasmid
    Boolean? meta_viral
    Boolean? meta_plasmid
    Boolean? rna_viral
    Boolean? ion_torrent
    Boolean? test
  ...
  command <<<
    spades_py \
      ...
      ~{if (sc) then "--sc" else ""} \
      ~{if (flag_required_metagenomic) then "--meta" else ""} \
      ~{if (bio) then "--bio" else ""} \
      ~{if (corona) then "--corona" else ""} \
      ~{if (rna) then "--rna" else ""} \
      ~{if (plasmid) then "--plasmid" else ""} \
      ~{if (meta_viral) then "--metaviral" else ""} \
      ~{if (meta_plasmid) then "--metaplasmid" else ""} \
      ~{if (rna_viral) then "--rnaviral" else ""} \
  ...
  parameter_meta {
    ...
    flag_required_metagenomic: "this flag is required for metagenomic data"
    ...
    meta_viral: "runs metaviralSPAdes pipeline for virus detection"
    meta_plasmid: "runs metaplasmidSPAdes pipeline for plasmid detection in metagenomic datasets (equivalent for --meta --plasmid)"
    rna_viral: "this flag enables virus assembly module from RNA-Seq data"
