...
- id: in_arg_file_containing
  ...
  inputBinding:
    prefix: -t
- id: in_arg_spaceseparated_list_may
...
- id: in_arg_haplotypes_posterior
...
- id: in_arg_indel_heterozygosity
...
- id: out_stdout
  ...
  type: stdout
- id: out_arg_file_written
  doc: "[ --output ] arg                   File to where output is written. If\nunspecified,\
    \ calls are written to\nstdout"
  type: File?
  outputBinding:
    glob: $(inputs.in_arg_file_written)
- id: out_re_genotype
  doc: "VCF file specifying calls to\nregenotype, only sites in this files\nwill appear\
    \ in the final output"
  type: File?
