...
task Raxmlng {
  ...
  command <<<
    raxml_ng \
      ...
      ~{if (site_weights) then "--site-weights" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/raxml-ng:1.0.1--h7447c1b_0"
  }
  parameter_meta {
    ...
    site_lh: "print per-site log-likelihood values"
    ...
    site_weights: "FILE                     file with MSA column weights (positive integers only!)"
