...
task Quantifierpl {
  ...
  command <<<
    quantifier_pl \
      ...
      ~{if (specify_option_mirna) then "-P" else ""} \
      ...
      ~{if (eg_mouse_mmuif) then "-t" else ""} \
      ~{if (optional_otherwise_generating) then "-y" else ""} \
      ~{if (parameter_given_pdfs) then "-d" else ""} \
      ~{if (parameter_given_reads) then "-o" else ""} \
      ~{if (also_considers_allowed) then "-k" else ""} \
      ~{if (file_conversion_again) then "-n" else ""} \
      ~{if (mapping_precursor_again) then "-x" else ""} \
      ~{if (number_when_reads) then "-g" else ""} \
      ~{if (number_nucleotides_upstream_consider_default) then "-e" else ""} \
      ~{if (number_nucleotides_downstream_consider_default) then "-f" else ""} \
  ...
  parameter_meta {
    ...
    also_considers_allowed: "also considers precursor-mature mappings that have different ids, eg let7c\\nwould be allowed to map to pre-let7a"
    ...
    number_nucleotides_upstream_consider_default: "[int]     number of nucleotides upstream of the mature sequence to consider, default 2"
    number_nucleotides_downstream_consider_default: "[int]     number of nucleotides downstream of the mature sequence to consider, default 5"
