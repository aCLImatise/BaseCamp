...
task OptiTypePipelinepy {
  input {
    Array[String] fastq_files_fished
    Boolean? rna
    Boolean? dna
    Float? beta
    Int? enumerate
  ...
  command <<<
    OptiTypePipeline_py \
      ~{opti_type} \
      ~{if defined(fastq_files_fished) then ("--input " +  '"' + fastq_files_fished + '"') else ""} \
