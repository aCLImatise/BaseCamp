...
- id: in_groovy
  ...
  inputBinding:
    prefix: -groovy
- id: in_jvm_mem_pc
  doc: Only available with standalone executable jar or jalview.bin.Launcher. Limit
    maximum heap size (memory) to PERCENT% of total physical memory detected. This
    defaults to 90 if total physical memory can be detected. See https://www.jalview.org/help/html/memory.html
    for more details.
  type: long?
  inputBinding:
    prefix: -jvmmempc
...
- class: DockerRequirement
  dockerPull: quay.io/biocontainers/jalview:2.11.1.3--0
cwlVersion: v1.1
baseCommand:
- jalview
