...
task Iqtree2 {
  input {
    ...
    Int? date_ci
    Int? clock_sd
    Boolean? date_no_out_group
    Int? date_outlier
    Boolean? date_options
    String? dating
    Int? sup_min
    Int? burnin
    Boolean? con_tree
  ...
  command <<<
    iqtree2 \
      ...
      ~{if (redo_tree) then "--redo-tree" else ""} \
      ~{if (undo) then "--undo" else ""} \
      ...
      ~{if (cf_quartet) then "--cf-quartet" else ""} \
      ~{if defined(date_tip) then ("--date-tip " +  '"' + date_tip + '"') else ""} \
      ~{if defined(date_root) then ("--date-root " +  '"' + date_root + '"') else ""} \
      ~{if defined(date_ci) then ("--date-ci " +  '"' + date_ci + '"') else ""} \
      ~{if defined(clock_sd) then ("--clock-sd " +  '"' + clock_sd + '"') else ""} \
      ~{if (date_no_out_group) then "--date-no-outgroup" else ""} \
      ~{if defined(date_outlier) then ("--date-outlier " +  '"' + date_outlier + '"') else ""} \
      ~{if (date_options) then "--date-options" else ""} \
      ~{if defined(dating) then ("--dating " +  '"' + dating + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/iqtree:2.1.2--h56fc30b_0"
  }
  parameter_meta {
    ...
    redo_tree: "Restore ModelFinder and only redo tree search"
    undo: "Revoke finished run, used when changing some options"
    cp_time: "Minimum checkpoint interval (default: 60 sec and adapt)"
    ...
    cf_quartet: "Write sCF for all resampled quartets to .cf.quartet"
    date_tip: "Tip dates as a real number or YYYY-MM-DD"
    date_root: "Root date as a real number or YYYY-MM-DD"
    date_ci: "Number of replicates to compute confidence interval"
    clock_sd: "Std-dev for lognormal relaxed clock (default: 0.2)"
    date_no_out_group: "Exclude outgroup from time tree"
    date_outlier: "Z-score cutoff to remove outlier tips/nodes (e.g. 3)"
    date_options: "\\\"..\\\"  Extra options passing directly to LSD2"
