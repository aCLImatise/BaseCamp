...
task Cdhitest {
  ...
  command <<<
    cd_hit_est \
      ...
      ~{if (input_filename_paired) then "-j" else ""} \
      ...
      ~{if (var_13) then "-AL" else ""} \
      ~{if (alignment_coverage_shorter) then "-aS" else ""} \
      ~{if (var_15) then "-AS" else ""} \
  ...
  parameter_meta {
    ...
    input_filename_paired: "input filename in fasta/fastq format for R2 reads if input are paired end (PE) files"
    ...
    var_13: "alignment coverage control for the longer sequence, default 99999999\\nif set to 60, and the length of the sequence is 400,\\nthen the alignment must be >= 340 (400-60) residues"
    alignment_coverage_shorter: "alignment coverage for the shorter sequence, default 0.0\\nif set to 0.9, the alignment must covers 90% of the sequence"
    var_15: "alignment coverage control for the shorter sequence, default 99999999\\nif set to 60, and the length of the sequence is 400,\\nthen the alignment must be >= 340 (400-60) residues"
