...
task MungeSumstatspy {
  input {
    ...
    Int? a_one
    Int? a_two
    String? name_pvalue_name_ldscunderstands_nb_case_insensitive
    String? frq
  ...
  command <<<
    munge_sumstats_py \
      ...
      ~{if defined(name_pvalue_name_ldscunderstands_nb_case_insensitive) then ("--p " +  '"' + name_pvalue_name_ldscunderstands_nb_case_insensitive + '"') else ""} \
  ...
  parameter_meta {
    ...
    name_pvalue_name_ldscunderstands_nb_case_insensitive: "Name of p-value column (if not a name that ldsc\\nunderstands). NB: case insensitive."
