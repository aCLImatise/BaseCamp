...
task Lr2genepl {
  input {
    ...
    Float? pvalue_ttest_when_consecutive
    Float? when_minimum_number
    Float? when_minimum_differences
    Float? pvalue_ttest_when_breakpoint
  ...
  command <<<
    lr2gene_pl \
      ...
      ~{if defined(minimum_ratio_amplified) then ("-A " +  '"' + minimum_ratio_amplified + '"') else ""} \
      ...
      ~{if defined(pvalue_ttest_when_consecutive) then ("-p " +  '"' + pvalue_ttest_when_consecutive + '"') else ""} \
      ...
      ~{if defined(pvalue_ttest_when_breakpoint) then ("-P " +  '"' + pvalue_ttest_when_breakpoint + '"') else ""} \
  ...
  parameter_meta {
    ...
    minimum_ratio_amplified: "Minimum log2 ratio for a whole gene to be considered amplified.  Default: 1.50"
