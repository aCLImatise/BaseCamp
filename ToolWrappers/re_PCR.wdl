...
task RePCR {
  ...
  command <<<
    re_PCR \
      ...
      ~{if defined(set_default_size) then ("-d " +  '"' + set_default_size + '"') else ""} \
      ...
      ~{if defined(set_output_name) then ("-o " +  '"' + set_output_name + '"') else ""} \
  ...
  parameter_meta {
    ...
    set_default_size: "Set default STS size"
    ...
    set_output_name: "Set output file name"
  ...
  output {
    ...
    File out_set_output_name = "${in_set_output_name}"
  }
}