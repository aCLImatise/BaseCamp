...
task PhyloFlashBarplotR {
  ...
  command <<<
    phyloFlash_barplot_R \
      ...
      ~{if defined(csv_file_containing) then ("--file " +  '"' + csv_file_containing + '"') else ""} \
  ...
  parameter_meta {
    ...
    csv_file_containing: "CSV file containing three columns: Taxon, sample, and counts"
