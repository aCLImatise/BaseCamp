...
task Findscu {
  ...
  command <<<
    findscu \
      ...
      ~{if (extract_xml) then "--extract-xml" else ""} \
      ~{if (extract_xml_single) then "--extract-xml-single" else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    extract: "extract responses to DICOM file (rsp0001.dcm...)"
    extract_xml: "extract responses to XML file (rsp0001.xml...)"
    extract_xml_single: "[f]ilename: string\\nextract all responses to given XML file f\\n"
