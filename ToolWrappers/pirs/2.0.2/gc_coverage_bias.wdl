...
task GcCoverageBias {
  input {
    ...
    File? designated_chromosome_file
  ...
  command <<<
    gc_coverage_bias \
      ...
      ~{if defined(designated_chromosome_file) then ("-c " +  '"' + designated_chromosome_file + '"') else ""} \
  ...
  parameter_meta {
    ...
    designated_chromosome_file: "the designated chromosome id list file, one id per line, without settings, processing all the reference sequence"
