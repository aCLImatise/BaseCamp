...
- id: in_sensitivity_faster_fast
  doc: 'Sensitivity: 1.0 faster; 4.0 fast; 7.5 sensitive [5.700]'
  type: double?
  inputBinding:
    prefix: -s
- id: in_kmer_length_set
  doc: 'k-mer length (0: automatically set to optimum) [6]'
  type: long?
  inputBinding:
    prefix: -k
- id: in_k_score
...
- id: in_spaced_km_er_pattern
  doc: User-specified spaced k-mer pattern [11011101]
  type: long?
  inputBinding:
...
- id: in_list_matches_evalue
  doc: List matches below this E-value (range 0.0-inf) [2.000E+02]
  type: double?
  inputBinding:
    prefix: -e
- id: in_min_seq_id
...
- id: in_gap_open
  doc: Gap open cost [nucl:10,aa:16]
  type: long?
  inputBinding:
    prefix: --gap-open
- id: in_gap_extend
...
- id: in_min_length
  doc: Minimum codon number in open reading frames [9]
  type: long?
  inputBinding:
    prefix: --min-length
- id: in_max_length
...
- id: in_fmt
  ...
  inputBinding:
    prefix: --fmt
- id: in_lca_ranks
  doc: Add column with specified ranks (',' separated) [superkingdom,phylum,class,order,family,genus,species]
  type: string?
  inputBinding:
    prefix: --lca-ranks
- id: in_tax_lineage
...
- id: in_tax_fdr
  doc: FDR cutoff for taxonomy report [0.0, 1.0] [0.020]
  type: double?
  inputBinding:
    prefix: --tax-fdr
- id: in_restrict_ranks_mode
  doc: '0: disabled, 1: restrict taxonomic rank on target prediction by sequence identity
    [1]'
  type: long?
  inputBinding:
...
- id: in_report_pam
  doc: Report protospacer adjacent motifs up and downstream of hits [1]
  type: long?
  inputBinding:
    prefix: --report-pam
- id: in_perform_nucl_aln
  doc: '0: perform only 6-frame translated search, 1:perform additional nucl-nucl
    alignment to the hits reported in 6-frame translated search, and update the E-value
    [1]'
  type: long?
  inputBinding:
    prefix: --perform-nucl-aln
- id: in_sub_mat
  doc: Substitution matrix file [nucl:nucleotide.out,aa:VTML40.out]
  type: File?
  inputBinding:
    prefix: --sub-mat
- id: in_max_seq_len
  doc: Maximum sequence length [0.000]
  type: long?
  inputBinding:
    prefix: --max-seq-len
- id: in_db_load_mode
