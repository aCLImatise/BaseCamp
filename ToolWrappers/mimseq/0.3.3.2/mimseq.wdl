...
task Mimseq {
  input {
    ...
    Boolean? no_cca_analysis
    Boolean? double_cca
    Boolean? local_modo_mics
    Int? max_mismatches
    Int? remap_mismatches
    Boolean? no_snp_tolerance
  ...
  command <<<
    mimseq \
      ...
      ~{if (no_cca_analysis) then "--no-cca-analysis" else ""} \
      ...
      ~{if (no_snp_tolerance) then "--no-snp-tolerance" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/mimseq:0.3.3.2--pyhdfd78af_0"
  }
  parameter_meta {
    ...
    no_snp_tolerance: "Disable GSNAP SNP-tolerant read alignment, where known\\nmodifications from Modomics are mapped as SNPs.\\nDefault is enabled."
