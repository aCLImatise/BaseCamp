...
- id: in_clean_env
  doc: "clean environment before running container\n-c, --contain                use\
    ...
    \  enable VM support\n--vm-cpu string          number of CPU cores to allocate\
    \ to Virtual\nMachine (implies --vm) (default \"1\")\n--vm-err               \
    \  enable attaching stderr from VM\n--vm-ip string           IP Address to assign\
    ...
    \ non persistent data (with\noverlay support only)"
  type: boolean
  inputBinding:
    prefix: --cleanenv
- id: in_directory_slash
  doc: "sandbox format. Directory containing a valid root file \nsystem and optionally\
