...
task DesignpySlidingwindowAutofromfile {
  input {
    Int? guide_length
    String? obj
    ...
    Boolean? write_input_seqs
    Boolean? write_input_aln
    String var_45
    String in_tsv
    String out_tsv_dir
  }
  command <<<
    design_py sliding_window auto_from_file \
      ~{var_45} \
      ~{in_tsv} \
      ~{out_tsv_dir} \
      ...
      ~{if defined(path_mafft_used) then ("--mafft-path " +  '"' + path_mafft_used + '"') else ""} \
      ...
      ~{if (write_input_seqs) then "--write-input-seqs" else ""} \
      ~{if (write_input_aln) then "--write-input-aln" else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    path_mafft_used: "Path to mafft executable, used for generating\\nalignments"
