...
- id: in_specify_column_name
  doc: "[-a weightFileName] [-A secondaryStructureSubstModel]\n[-b bootstrapRandomNumberSeed]\
    ...
    \ [-E excludeFileName]\n[-f a|A|b|B|c|C|d|D|e|E|F|g|G|h|H|i|I|j|J|k|m|n|N|o|p|P|q|r|R|s|S|t|T|u|v|V|w|W|x|y]\
    \ [-F]\n[-g groupingFileName] [-G placementThreshold] [-h] [-H]\n[-i initialRearrangementSetting]\
    \ [-I autoFC|autoMR|autoMRE|autoMRE_IGN]\n[-j] [-J MR|MR_DROP|MRE|STRICT|STRICT_DROP|T_<PERCENT>]\
    ...
    \ distinct rate catgories for RAxML when model of rate heterogeneity\nis set to\
    ...
    \ for the \"-L\" and \"-f i\" options. This will produce more, as well as more\
    \ verbose output files\nDEFAULT: OFF\n-d      start ML optimization from random\
    \ starting tree\nDEFAULT: OFF\n-D      ML search convergence criterion. This will\
    \ break off ML searches if the relative\nRobinson-Foulds distance between the\
    \ trees obtained from two consecutive lazy SPR cycles\nis smaller or equal to\
    \ 1%. Usage recommended for very large datasets in terms of taxa.\nOn trees with\
    \ more than 500 taxa this will yield execution time improvements of approximately\
    \ 50%\nWhile yielding only slightly worse trees.\nDEFAULT: OFF\n-e      set model\
    \ optimization precision in log likelihood units for final\noptimization of tree\
    \ topology\nDEFAULT: 0.1   for models not using proportion of invariant sites\
    ...
    100-100\", if you use a mixed model, an appropriately adapted model file\nwill\
    \ be written.\n-f      select algorithm:\n\"-f a\": rapid Bootstrap analysis and\
    \ search for best-scoring ML tree in one program run\n\"-f A\": compute marginal\
    \ ancestral states on a ROOTED reference tree provided with \"-t\"\n\"-f b\":\
    \ draw bipartition information on a tree provided with \"-t\" based on multiple\
    \ trees\n(e.g., from a bootstrap) in a file specified by \"-z\"\n\"-f B\": optimize\
    \ br-len scaler and other model parameters (GTR, alpha, etc.) on a tree provided\
    \ with \"-t\".\nThe tree needs to contain branch lengths. The branch lengths will\
    \ not be optimized, just scaled by a single common value.\n\"-f c\": check if\
    \ the alignment can be properly read by RAxML\n\"-f C\": ancestral sequence test\
    \ for Jiajie, users will also need to provide a list of taxon names via -Y separated\
    \ by whitespaces\n\"-f d\": new rapid hill-climbing\nDEFAULT: ON\n\"-f D\": rapid\
    \ hill-climbing with RELL bootstraps\n\"-f e\": optimize model+branch lengths\
    \ for given input tree under GAMMA/GAMMAI only\n\"-f E\": execute very fast experimental\
    \ tree search, at present only for testing\n\"-f F\": execute fast experimental\
    \ tree search, at present only for testing\n\"-f g\": compute per site log Likelihoods\
    \ for one or more trees passed via\n\"-z\" and write them to a file that can be\
    ...
    \ hill climbing\n-F      enable ML tree searches under CAT model for very large\
    \ trees without switching to\nGAMMA in the end (saves memory).\nThis option can\
    \ also be used with the GAMMA models in order to avoid the thorough optimization\n\
    of the best-scoring ML tree in the end.\nDEFAULT: OFF\n-g      specify the file\
    \ name of a multifurcating constraint tree\nthis tree does not need to be comprehensive,\
    \ i.e. must not contain all taxa\n-G      enable the ML-based evolutionary placement\
    ...
    \ for the frequency-based criterion\n\"-I autoMR\" for the majority-rule consensus\
    \ tree criterion\n\"-I autoMRE\" for the extended majority-rule consensus tree\
    \ criterion\n\"-I autoMRE_IGN\" for metrics similar to MRE, but include bipartitions\
    \ under the threshold whether they are compatible\nor not. This emulates MRE but\
    \ is faster to compute.\nYou also need to pass a tree file containg several bootstrap\
    \ replicates via \"-z\"\n-j      Specifies that intermediate tree files shall\
    \ be written to file during the standard ML and BS tree searches.\nDEFAULT: OFF\n\
    -J      Compute majority rule consensus tree with \"-J MR\" or extended majority\
    ...
    -K      Specify one of the multi-state substitution models (max 32 states) implemented\
    \ in RAxML.\nAvailable models are: ORDERED, MK, GTR\nDEFAULT: GTR model\n-L  \
    \   Compute consensus trees labelled by IC supports and the overall TC value as\
    ...
    \ UNROOTED trees via \"-z\"!\n-m      Model of Binary (Morphological), Nucleotide,\
    \ Multi-State, or Amino Acid Substitution:\nBINARY:\n\"-m BINCAT[X]\"       :\
    ...
    \ Final tree might be evaluated\nautomatically under BINGAMMA, depending on the\
    ...
    \ ASC prefix willl correct the likelihood for ascertainment bias.\nWith the optional\
    \ \"X\" appendix you can specify a ML estimate of base frequencies.\n\"-m BINGAMMAI[X]\"\
    ...
    NUCLEOTIDES:\n\"-m GTRCAT[X]\"       : GTR + Optimization of substitution rates\
    \ + Optimization of site-specific\nevolutionary rates which are categorized into\
    \ numberOfCategories distinct\nrate categories for greater computational efficiency.\
    \  Final tree might be evaluated\nunder GTRGAMMA, depending on the tree search\
    \ option.\nWith the optional \"X\" appendix you can specify a ML estimate of base\
    \ frequencies.\n\"-m GTRCATI[X]\"      : GTR + Optimization of substitution rates\
    \ + Optimization of site-specific\nevolutionary rates which are categorized into\
    \ numberOfCategories distinct\nrate categories for greater computational efficiency.\
    \  Final tree might be evaluated\nunder GTRGAMMAI, depending on the tree search\
    \ option.\nWith the optional \"X\" appendix you can specify a ML estimate of base\
    \ frequencies.\n\"-m ASC_GTRCAT[X]\"   : GTR + Optimization of substitution rates\
    \ + Optimization of site-specific\nevolutionary rates which are categorized into\
    \ numberOfCategories distinct\nrate categories for greater computational efficiency.\
    \  Final tree might be evaluated\nunder GTRGAMMA, depending on the tree search\
    \ option.\nWith the optional \"X\" appendix you can specify a ML estimate of base\
    \ frequencies.\nThe ASC prefix willl correct the likelihood for ascertainment\
    \ bias.\n\"-m GTRGAMMA[X]\"     : GTR + Optimization of substitution rates + GAMMA\
    \ model of rate\nheterogeneity (alpha parameter will be estimated).\nWith the\
    \ optional \"X\" appendix you can specify a ML estimate of base frequencies.\n\
    \"-m ASC_GTRGAMMA[X]\" : GTR + Optimization of substitution rates + GAMMA model\
    \ of rate\nheterogeneity (alpha parameter will be estimated).\nThe ASC prefix\
    ...
    \     : GAMMA model of rate heterogeneity (alpha parameter will be estimated).\n\
    With the optional \"X\" appendix you can specify a ML estimate of base frequencies.\n\
    \"-m ASC_MULTIGAMMA[X]\" : GAMMA model of rate heterogeneity (alpha parameter\
    \ will be estimated).\nThe ASC prefix willl correct the likelihood for ascertainment\
    \ bias.\nWith the optional \"X\" appendix you can specify a ML estimate of base\
    \ frequencies.\n\"-m MULTIGAMMAI[X]\"    : Same as MULTIGAMMA, but with estimate\
    \ of proportion of invariable sites.\nWith the optional \"X\" appendix you can\
    ...
    \ 0, 1, 2, 3, 4, 5 to encode these.\nThe substitution model for the multi-state\
    \ regions can be selected via the \"-K\" option\nAMINO ACIDS:\n\"-m PROTCATmatrixName[F|X]\"\
    ...
    \ a ML estimate of base frequencies.\n\"-m PROTCATImatrixName[F|X]\"      : specified\
    \ AA matrix + Optimization of substitution rates + Optimization of site-specific\n\
    evolutionary rates which are categorized into numberOfCategories distinct\nrate\
    \ categories for greater computational efficiency.   Final tree might be evaluated\n\
    automatically under PROTGAMMAImatrixName[F|X], depending on the tree search option.\n\
    With the optional \"X\" appendix you can specify a ML estimate of base frequencies.\n\
    \"-m ASC_PROTCATmatrixName[F|X]\"   : specified AA matrix + Optimization of substitution\
    \ rates + Optimization of site-specific\nevolutionary rates which are categorized\
    \ into numberOfCategories distinct\nrate categories for greater computational\
    \ efficiency.   Final tree might be evaluated\nautomatically under PROTGAMMAmatrixName[F|X],\
    \ depending on the tree search option.\nWith the optional \"X\" appendix you can\
    \ specify a ML estimate of base frequencies.\nThe ASC prefix willl correct the\
    \ likelihood for ascertainment bias.\n\"-m PROTGAMMAmatrixName[F|X]\"     : specified\
    \ AA matrix + Optimization of substitution rates + GAMMA model of rate\nheterogeneity\
    ...
    \ single outgroup or a comma-separated list of outgroups, eg \"-o Rat\"\nor \"\
    ...
    \ must not contain all taxa\n-R      Specify the file name of a binary model parameter\
    \ file that has previously been generated\nwith RAxML using the -f e tree evaluation\
    \ option. The file name should be:\nRAxML_binaryModelParameters.runID\n-s    \
    \  Specify the name of the alignment data file in PHYLIP format\n-S      Specify\
    \ the name of a secondary structure file. The file can contain \".\" for\nalignment\
    \ columns that do not form part of a stem and characters \"()<>[]{}\" to define\n\
    stem regions and pseudoknots\n-t      Specify a user starting tree file name in\
    \ Newick format\n-T      PTHREADS VERSION ONLY! Specify the number of threads\
    \ you want to run.\nMake sure to set \"-T\" to at most the number of CPUs you\
    ...
    \ heterogeneity\nDEFAULT: OFF\n-U      Try to save memory by using SEV-based implementation\
    \ for gap columns on large gappy alignments\nThe technique is described here:\
    \ http://www.biomedcentral.com/1471-2105/12/470\nThis will only work for DNA and/or\
    \ PROTEIN data and only with the SSE3 or AVX-vextorized version of the code.\n\
    -v      Display version information\n-V      Disable rate heterogeneity among\
    \ sites model and use one without rate heterogeneity instead.\nOnly works if you\
    \ specify the CAT model of rate heterogeneity.\nDEFAULT: use rate heterogeneity\n\
    -w      FULL (!) path to the directory into which RAxML shall write its output\
    \ files\nDEFAULT: current directory\n-W      Sliding window size for leave-one-out\
    \ site-specific placement bias algorithm\nonly effective when used in combination\
    \ with \"-f S\"\nDEFAULT: 100 sites\n-x      Specify an integer number (random\
    \ seed) and turn on rapid bootstrapping\nCAUTION: unlike in version 7.0.4 RAxML\
    \ will conduct rapid BS replicates under\nthe model of rate heterogeneity you\
    \ specified via \"-m\" and not by default under CAT\n-X      Same as the \"-y\"\
    \ option below, however the parsimony search is more superficial.\nRAxML will\
    \ only do a randomized stepwise addition order parsimony tree reconstruction\n\
    ...
    DEFAULT: OFF\n-y      If you want to only compute a parsimony starting tree with\
    \ RAxML specify \"-y\",\nthe program will exit after computation of the starting\
    \ tree\nDEFAULT: OFF\n-Y      Pass a quartet grouping file name defining four\
    \ groups from which to draw quartets\nThe file input format must contain 4 groups\
    ...
    \nDEFAULT: 1 single analysis\n--mesquite Print output files that can be parsed\
    ...
    \ for large phylogenomic alignments.\nBefore using this, make sure to check the\
    \ alignment using the \"-f c\" option!\nDEFAULT: Off\n--no-bfgs Disables automatic\
    \ usage of BFGS method to optimize GTR rates on unpartitioned DNA datasets\nDEFAULT:\
    \ BFGS on\n--asc-corr Allows to specify the type of ascertainment bias correction\
    \ you wish to use. There are 3\ntypes available:\n--asc-corr=lewis: the standard\
    \ correction by Paul Lewis\n--asc-corr=felsenstein: a correction introduced by\
    \ Joe Felsenstein that allows to explicitely specify\nthe number of invariable\
    \ sites (if known) one wants to correct for.\n--asc-corr=stamatakis: a correction\
    \ introduced by myself that allows to explicitely specify\nthe number of invariable\
    \ sites for each character (if known) one wants to correct for.\n--flag-check\
    \ When using this option, RAxML will only check if all command line flags specifed\
    ...
    \ When using automatic protein model selection you can chose the criterion for\
    \ selecting these models.\nRAxML will test all available prot subst. models except\
    \ for LG4M, LG4X and GTR-based models, with and without empirical base frequencies.\n\
    ...
    \ values printed will also depend on the settings for --epa-prob-threshold=threshold\
    \ !\nDEFAULT: 7\n--epa-prob-threshold=threshold specify a percent threshold for\
    \ including potential placements of a read depending on the\nmaximum placement\
    \ weight for this read. If you set this value to 0.01 placements that have a placement\
    \ weight of 1 per cent of\nthe maximum placement will still be printed to file\
    \ if the setting of --epa-keep-placements allows for it\nDEFAULT: 0.01\n--epa-accumulated-threshold=threshold\
    \ specify an accumulated likelihood weight threshold for which different placements\
    ...
    \ with --epa-keep-placements!\n--JC69 specify that all DNA partitions will evolve\
    \ under the Jukes-Cantor model, this overrides all other model specifications\
    \ for DNA partitions.\nDEFAULT: Off\n--K80 specify that all DNA partitions will\
    \ evolve under the K80 model, this overrides all other model specifications for\
    \ DNA partitions.\nDEFAULT: Off\n--HKY85 specify that all DNA partitions will\
    \ evolve under the HKY85 model, this overrides all other model specifications\
    \ for DNA partitions.\nDEFAULT: Off\n--bootstop-perms=number specify the number\
    \ of permutations to be conducted for the bootstopping/bootstrap convergence test.\n\
    The allowed minimum number is 100!\nDEFAULT: 100\n--quartets-without-replacement\
    \ specify that quartets are randomly subsampled, but without replacement.\nDEFAULT:\
    \ random sampling with replacements\n--print-identical-sequences specify that\
    \ RAxML shall automatically generate a .reduced alignment with all\nundetermined\
