...
task Snakemake {
  input {
    ...
    Int? preemption_default
    Array[String] preemptible_rules
    Boolean? config
    ...
    Boolean? scheduler
    Boolean? wms_monitor
    Boolean? wms_monitor_arg
    String? scheduler_ilp_solver
    Boolean? no_sub_workflows
    Array[String] groups
    Array[String] group_components
    Boolean? report
    File? report_stylesheet
    File? edit_notebook
    Int? notebook_listen
    Boolean? lint
    Boolean? generate_unit_tests
    Boolean? container_ize
    File? export_cwl
    Boolean? list
    ...
    Int? tes
    Boolean? use_cond_a
    Boolean? cond_a_not_block_search_path_env_vars
  ...
  command <<<
    snakemake \
      ...
      ~{if defined(set_threads) then ("--set-threads " +  '"' + set_threads + '"') else ""} \
      ~{if defined(set_scatter) then ("--set-scatter " +  '"' + set_scatter + '"') else ""} \
      ~{if (default_resources) then "--default-resources" else ""} \
      ~{if defined(preemption_default) then ("--preemption-default " +  '"' + preemption_default + '"') else ""} \
      ~{if defined(preemptible_rules) then ("--preemptible-rules " +  '"' + preemptible_rules + '"') else ""} \
      ...
      ~{if (scheduler) then "--scheduler" else ""} \
      ~{if (wms_monitor) then "--wms-monitor" else ""} \
      ~{if (wms_monitor_arg) then "--wms-monitor-arg" else ""} \
      ~{if defined(scheduler_ilp_solver) then ("--scheduler-ilp-solver " +  '"' + scheduler_ilp_solver + '"') else ""} \
      ~{if (no_sub_workflows) then "--no-subworkflows" else ""} \
      ~{if defined(groups) then ("--groups " +  '"' + groups + '"') else ""} \
      ~{if defined(group_components) then ("--group-components " +  '"' + group_components + '"') else ""} \
      ...
      ~{if (generate_unit_tests) then "--generate-unit-tests" else ""} \
      ~{if (container_ize) then "--containerize" else ""} \
      ...
      ~{if (drop_metadata) then "--drop-metadata" else ""} \
      ...
      ~{if defined(max_inventory_time) then ("--max-inventory-time " +  '"' + max_inventory_time + '"') else ""} \
      ...
      ~{if defined(tes) then ("--tes " +  '"' + tes + '"') else ""} \
      ~{if (use_cond_a) then "--use-conda" else ""} \
      ~{if (cond_a_not_block_search_path_env_vars) then "--conda-not-block-search-path-envvars" else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/snakemake-minimal:6.1.1--pyhdfd78af_0"
  }
  parameter_meta {
    ...
    tes: "Send workflow tasks to GA4GH TES server specified by\\nurl. (default: None)"
