...
id: glam2scan.cwl
inputs:
- id: in_output_directory_clobber
  doc: ': output directory; will not clobber existing files'
  type: Directory?
  inputBinding:
    prefix: -o
- id: in_output_directory_allow
...
- id: out_stdout
  ...
  type: stdout
- id: out_output_directory_clobber
  doc: ': output directory; will not clobber existing files'
  type: Directory?
  outputBinding:
    glob: $(inputs.in_output_directory_clobber)
- id: out_output_directory_allow
  doc: ': output directory (glam2scan_out); allow clobbering'
  type: Directory?
  outputBinding:
    glob: $(inputs.in_output_directory_allow)
hints: []
cwlVersion: v1.1
baseCommand:
- glam2scan
