...
task Quakepy {
  input {
    ...
    Boolean? no_jelly
    Boolean? no_count
    Boolean? count_kmers_integers
    Int? hash_size
    Boolean? no_cut
    Int? ratio
    Int? return_only_reads
    Boolean? output_error_reads
    String? use_bwalike_trim
    Boolean? headers
    Boolean? log
  }
  command <<<
    quake_py \
      ...
      ~{if (count_kmers_integers) then "--int" else ""} \
      ...
      ~{if defined(use_bwalike_trim) then ("-t " +  '"' + use_bwalike_trim + '"') else ""} \
  ...
  parameter_meta {
    ...
    count_kmers_integers: "Count kmers as integers w/o the use of quality values\\n[default: False]"
    ...
    use_bwalike_trim: "Use BWA-like trim parameter <trim_par>"
