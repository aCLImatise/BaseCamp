...
task CountQmers {
  ...
  command <<<
    count_qmers \
      ...
      ~{if defined(gigabyte_limit_limited) then ("-l " +  '"' + gigabyte_limit_limited + '"') else ""} \
  ...
  parameter_meta {
    ...
    gigabyte_limit_limited: "Gigabyte limit on RAM. If limited, the output will\\ncontain redundancies"
