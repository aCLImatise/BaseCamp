...
task TEpy {
  input {
    ...
    File? tab_delimited_text
    Int? allow_different_dispersions
    Int? set_sum_normalized
  ...
  command <<<
    TE_py \
      ...
      ~{if defined(tab_delimited_text) then ("-o " +  '"' + tab_delimited_text + '"') else ""} \
      ~{if defined(allow_different_dispersions) then ("-d " +  '"' + allow_different_dispersions + '"') else ""} \
      ~{if defined(set_sum_normalized) then ("-s " +  '"' + set_sum_normalized + '"') else ""} \
