...
task Julietflow {
  input {
    ...
    Boolean? maximal_number_default
  ...
  command <<<
    julietflow \
      ...
      ~{if (maximal_number_default) then "-e" else ""} \
      ...
      ~{if (only_report_positions) then "-k" else ""} \
  ...
  parameter_meta {
    ...
    maximal_number_default: "[arg]  Maximal number of re-align iterations. Default=1"
    ...
    only_report_positions: "Only report known DRM positions."
