...
task KodojaBuildpy {
  input {
    ...
    Boolean? no_download
    Boolean? list_extra_files
    Boolean? list_taxid_extra
    Boolean? all_viruses
    File? kraken_tax
    Int? kraken_km_er
  ...
  command <<<
    kodoja_build_py \
      ...
      ~{if (list_extra_files) then "-e" else ""} \
      ~{if (list_taxid_extra) then "-x" else ""} \
  ...
  parameter_meta {
    ...
    list_extra_files: "[EXTRA_FILES [EXTRA_FILES ...]], --extra_files [EXTRA_FILES [EXTRA_FILES ...]]\\nList of extra files added to \\\"extra\\\" dir"
    list_taxid_extra: "[EXTRA_TAXIDS [EXTRA_TAXIDS ...]], --extra_taxids [EXTRA_TAXIDS [EXTRA_TAXIDS ...]]\\nList of taxID of extra files"
