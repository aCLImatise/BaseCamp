...
task PlanemoTrainingInit {
  input {
    Directory? topic_name
    String? topic_title
    String? topic_summary
    Boolean? topic_target
    Directory? tutorial_name
    String? tutorial_title
    Boolean? hands_on
    Boolean? slides
    File? workflow_tutorial_locally
    String? galaxy_url
    String? galaxy_api_key
    String? zeno_do_link
    File? data_types
    ...
    String _shedinstall_noshedinstall
  }
  command <<<
    planemo training_init \
      ...
      ~{_shedtoolconf_text} \
      ~{executing_dot} \
      ~{_shedinstall_noshedinstall} \
      ~{if defined(topic_name) then ("--topic_name " +  '"' + topic_name + '"') else ""} \
      ~{if defined(topic_title) then ("--topic_title " +  '"' + topic_title + '"') else ""} \
      ~{if defined(topic_summary) then ("--topic_summary " +  '"' + topic_summary + '"') else ""} \
      ~{if (topic_target) then "--topic_target" else ""} \
      ~{if defined(tutorial_name) then ("--tutorial_name " +  '"' + tutorial_name + '"') else ""} \
      ~{if defined(tutorial_title) then ("--tutorial_title " +  '"' + tutorial_title + '"') else ""} \
      ~{if (hands_on) then "--hands_on" else ""} \
      ~{if (slides) then "--slides" else ""} \
      ~{if defined(workflow_tutorial_locally) then ("--workflow " +  '"' + workflow_tutorial_locally + '"') else ""} \
      ~{if defined(galaxy_url) then ("--galaxy_url " +  '"' + galaxy_url + '"') else ""} \
      ~{if defined(galaxy_api_key) then ("--galaxy_api_key " +  '"' + galaxy_api_key + '"') else ""} \
  ...
  parameter_meta {
    topic_name: "Name (directory name) of the topic to create\\nor in which a tutorial should be created or\\nupdates  [required]"
    ...
    _text_id: "--workflow_id TEXT              ID of the workflow on the Galaxy instance"
    ...
    executing_dot: "--skip_client_build             Do not build Galaxy client when serving"
    _shedinstall_noshedinstall: "--shed_install / --no_shed_install"
  }
  output {
