...
- id: in_number_parallel_analysis
  doc: Number of parallel analysis threads
  type: long?
  inputBinding:
    prefix: -a
- id: in_input_dna_sequences
  doc: Input is DNA sequences
  type: boolean?
  inputBinding:
    prefix: -d
...
- id: in_sequence_search_program
  doc: "Sequence search program [Default = diamond]\nOptions: blast, diamond, diamond_ultra_sens,\
    \ blast_gz, mmseqs, blast_nucl"
  type: string?
  inputBinding:
    prefix: -S
...
- id: in_add_species_names
  ...
  inputBinding:
    prefix: -X
- id: in_split_paralogous_clades
  doc: Split paralogous clades below root of a HOG into separate HOGs
  type: boolean?
  inputBinding:
    prefix: -y
- id: in_trim_min_length
  doc: Don't trim MSAs (columns>=90% gap, min. alignment length 500)
  type: boolean?
  inputBinding:
    prefix: -z
