...
task GoalignSort {
  ...
  command <<<
    goalign sort \
      ~{if defined(sorted_alignment_output) then ("--output " +  '"' + sorted_alignment_output + '"') else ""} \
  ...
  output {
    ...
    File out_sorted_alignment_output = "${in_sorted_alignment_output}"
  }
}