...
task Kissplice {
  ...
  command <<<
    kissplice \
      ...
      ~{if defined(maximal_length_shorter) then ("-l " +  '"' + maximal_length_shorter + '"') else ""} \
      ~{if defined(minimum_length_) then ("-m " +  '"' + minimum_length_ + '"') else ""} \
      ~{if defined(maximum_length_exons) then ("-M " +  '"' + maximum_length_exons + '"') else ""} \
      ...
      ~{if defined(number_cores_be) then ("-t " +  '"' + number_cores_be + '"') else ""} \
      ~{if defined(changes_types_be) then ("-s " +  '"' + changes_types_be + '"') else ""} \
      ...
      ~{if defined(estimated_genometranscriptome_size) then ("-z " +  '"' + estimated_genometranscriptome_size + '"') else ""} \
  ...
  parameter_meta {
    ...
    maximal_length_shorter: "maximal length of the shorter path (default: 2k+1)"
    minimum_length_: "minimum length of the shorter path (default 2k-8)"
    maximum_length_exons: "maximum length of the longest path (default 1000000),\\nskipped exons longer than UL_MAX are not reported"
    ...
    percentage_edges_relative: "a percentage from [0,1), edges with relative coverage\\nbelow this number are removed (default 0.05)"
    estimated_genometranscriptome_size: "estimated genome/transcriptome size (default = 1G)"
