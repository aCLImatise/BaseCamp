...
task RunMapsembler2Pipelinesh {
  input {
    ...
    String? out_files_start
    Int? set_length_used
    Int? set_minimal_coverage
    Int? set_number_reads
    Int? estimated_genome_size
    Int? set_process_example
  ...
  command <<<
    run_mapsembler2_pipeline_sh \
      ...
      ~{if defined(out_files_start) then ("-p " +  '"' + out_files_start + '"') else ""} \
      ...
      ~{if defined(set_process_example) then ("-f " +  '"' + set_process_example + '"') else ""} \
      ~{if defined(set_nodes_length) then ("-x " +  '"' + set_nodes_length + '"') else ""} \
      ~{if defined(set_graph_depth) then ("-y " +  '"' + set_graph_depth + '"') else ""} \
  ...
  parameter_meta {
    ...
    out_files_start: "All out files will start with this prefix. Example: -p my_prefix"
    ...
    set_process_example: "Set the process of search in the graph (1=Breadth  and 2=Depth). Default=1. Example: -f 1"
    set_nodes_length: "Set the maximal nodes length . Default=40. Example: -x 40"
    set_graph_depth: "Set the maximal graph depth . Default=10000. Example: -y 10000"
