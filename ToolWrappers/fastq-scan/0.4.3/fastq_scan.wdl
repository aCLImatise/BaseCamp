...
task Fastqscan {
  ...
  command <<<
    fastq_scan \
      ...
      ~{if defined(genome_size_default) then ("-g " +  '"' + genome_size_default + '"') else ""} \
  ...
  parameter_meta {
    genome_size_default: "Genome size for calculating estimated sequencing coverage. (Default 1)"
