...
task TransIndelCallpy {
  ...
  command <<<
    transIndel_call_py \
      ...
      ~{if (minimal_mapq_call) then "-m" else ""} \
      ~{if (limit_analysis_string) then "-t" else ""} \
  ...
  parameter_meta {
    ...
    limit_analysis_string: ":Limit analysis to targets listed in the BED-format FILE or a samtools region string"
    o: ""
    i: ""
  }
  output {
