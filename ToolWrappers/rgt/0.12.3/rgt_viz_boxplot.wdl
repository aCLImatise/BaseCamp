...
task RgtvizBoxplot {
  input {
    ...
    Boolean? title_shown_also
    Boolean? group_data_header
    Boolean? color_data_header
    Boolean? sort_data_none
    Boolean? scol
    Boolean? n_log
    Boolean? color
    Boolean? pw
  ...
  command <<<
    rgt_viz boxplot \
      ...
      ~{if (title_shown_also) then "-t" else ""} \
      ~{if (group_data_header) then "-g" else ""} \
      ~{if (color_data_header) then "-c" else ""} \
      ~{if (sort_data_none) then "-s" else ""} \
  ...
  parameter_meta {
    ...
    title_shown_also: "The title shown on the top of the plot and also the folder name.\\n(default: boxplot)"
    group_data_header: "Group the data by reads(needs 'factor' column), regions(needs\\n'factor' column), another name of column (for example, 'cell')in\\nthe header of experimental matrix, or None. (default: reads)"
    color_data_header: "Color the data by reads(needs 'factor' column), regions(needs\\n'factor' column), another name of column (for example, 'cell')in\\nthe header of experimental matrix, or None. (default: regions)"
    sort_data_none: "Sort the data by reads(needs 'factor' column), regions(needs\\n'factor' column), another name of column (for example, 'cell')in\\nthe header of experimental matrix, or None. (default: None)"
