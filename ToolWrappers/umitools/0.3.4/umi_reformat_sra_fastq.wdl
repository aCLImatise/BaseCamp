...
task UmiReformatSraFastq {
  input {
    ...
    File? output_fastq_file_containing_reads_notduplicates
    File? pcr_duplicate
  ...
  command <<<
    umi_reformat_sra_fastq \
      ...
      ~{if defined(output_fastq_file_containing_reads_notduplicates) then ("--output " +  '"' + output_fastq_file_containing_reads_notduplicates + '"') else ""} \
  ...
  parameter_meta {
    ...
    output_fastq_file_containing_reads_notduplicates: "the output fastq file containing reads that are not\\nduplicates (default: None)"
  ...
  output {
    ...
    File out_output_fastq_file_containing_reads_notduplicates = "${in_output_fastq_file_containing_reads_notduplicates}"
