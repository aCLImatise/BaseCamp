...
task SnapalignerPaired {
  input {
    ...
    File? sid
    Boolean? pro
    Boolean? min_max_spacing
    Boolean? fs
    Boolean? max_hits_intersecting
    Int? mcp
    Boolean? ku
    String index_dir
  }
  command <<<
    snap_aligner paired \
      ...
      ~{if (min_max_spacing) then "-s" else ""} \
      ~{if (fs) then "-fs" else ""} \
      ~{if (max_hits_intersecting) then "-H" else ""} \
      ~{if defined(mcp) then ("-mcp " +  '"' + mcp + '"') else ""} \
      ~{if (ku) then "-ku" else ""}
  >>>
  parameter_meta {
    ...
    maximum_edit_distance: "maximum edit distance allowed per read or pair (default: 15)"
    ...
    min_max_spacing: "min and max spacing to allow between paired ends (default: 50 1000)."
    fs: "force spacing to lie between min and max."
    max_hits_intersecting: "max hits for intersecting aligner (default: 2000)."
    mcp: "the maximum candidate pool size (An internal data structure.\\nOnly increase this if you get an error message saying to do so. If you're running\\nout of memory, you may want to reduce it.  Default: 1000000)"
