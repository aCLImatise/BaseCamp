...
- id: in_aligned
  ...
  inputBinding:
    prefix: --aligned
- id: in_alignments
  doc: / -a               output aligned sequences pairwise.
  type: boolean
  inputBinding:
    prefix: --alignments
- id: in_difference
  doc: "/ -i               output difference between the Jukes-Cantor (JC)\ndistance\
    \ and proportion different sites."
  type: boolean
  inputBinding:
    prefix: --difference
- id: in_distances
  doc: "/ -d                output proportion different sites, JC distance,\nand diference\
    \ between the two."
  type: boolean
  inputBinding:
    prefix: --distances
- id: in_format
  ...
  inputBinding:
    prefix: --format
- id: in_group
  doc: "/ -g                    this option will cluster sequences that are\nsimilar\
    \ and/or find the most inclusive taxa in a\nhierarchy that are alignable according\
    \ to MAD\n(Smith et al. 2009, BMC evol. Biol. 9:37). It\nneed the taxonomy given\
    \ after a (the first) | in\nthe sequence name or in a separate file. Each\ntaxa\
    \ in the hierarchy should be separated by a\nsemicolon, with the highest rank\
  ...
  inputBinding:
    prefix: --group
- id: in_jc_distance
  doc: / -j              output Jukes-Cantor (JC) distance.
  type: boolean
  inputBinding:
    prefix: --jc_distance
- id: in_matrix
  doc: "/ -m                   output in the form of a space separated\nleft-upper\
    \ triangular matrix."
  type: boolean
  inputBinding:
    prefix: --matrix
- id: in_names
  doc: "/ -n                    output sequence names (if outputting alignments\n\
    then in fasta format)."
  type: boolean
  inputBinding:
    prefix: --names
- id: in_proportion_difference
  doc: / -p    output proportion sites that are different.
  type: boolean
  inputBinding:
    prefix: --proportion_difference
- id: in_similarity
  doc: "/ -s               output similarity between sequences (1-proportion\ndifferent)."
  type: boolean
  inputBinding:
    prefix: --similarity
- id: in_threads
  doc: "/ -T [number]         set the number of threads additional to the\ncontrolling\
