...
task FilterSeqpyRepeats {
  input {
    ...
    Boolean? fast_a
    Int? nproc
    Int? threshold_fraction_repeating
    Boolean? missing
    Boolean? inner
  }
  command <<<
    FilterSeq_py repeats \
      ...
      ~{if defined(threshold_fraction_repeating) then ("-n " +  '"' + threshold_fraction_repeating + '"') else ""} \
  ...
  parameter_meta {
    ...
    threshold_fraction_repeating: "Threshold for fraction of repeating nucleotides.\\n(default: 15)"
