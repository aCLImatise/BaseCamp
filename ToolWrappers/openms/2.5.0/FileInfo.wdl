...
task FileInfo {
  ...
  command <<<
    FileInfo \
      ...
      ~{if (check_corrupt_data) then "-c" else ""} \
      ~{if (validate_file_mzxml) then "-v" else ""} \
  ...
  parameter_meta {
    ...
    validate_file_mzxml: "Validate the file only (for mzML, mzData, mzXML, featureXML, idXML, consensusXML, pepXML)"
