...
task PercolatorAdapter {
  ...
  command <<<
    PercolatorAdapter \
      ...
      ~{if (peptide_level_f_drs) then "-peptide_level_fdrs" else ""} \
      ~{if (protein_level_f_drs) then "-protein_level_fdrs" else ""} \
  ...
  parameter_meta {
    ...
    out: "*                          Output file (valid formats: 'idXML', 'mzid', 'osw')"
    ...
    protein_level_f_drs: "Use the picked protein-level FDR to infer protein probabilities. Use the -fasta option and -decoy_pattern to set the Fasta file and decoy pattern."
