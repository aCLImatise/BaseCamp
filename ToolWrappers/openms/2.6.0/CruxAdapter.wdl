...
task CruxAdapter {
  ...
  command <<<
    CruxAdapter \
      ...
      ~{if defined(decoy_format) then ("-decoy_format " +  '"' + decoy_format + '"') else ""} \
      ~{if defined(keep_terminal_aminos) then ("-keep_terminal_aminos " +  '"' + keep_terminal_aminos + '"') else ""} \
  ...
  parameter_meta {
    ...
    enzyme: "The enzyme used for peptide digestion. (default: 'trypsin' valid: 'custom-enzyme', 'cyanogen-bromide', 'clostripain', 'elastase-trypsin-chymotrypsin', 'trypsin', 'staph-protease', 'proline-endopeptidase', 'chymotrypsin', 'trypsin/p', 'asp-n', 'glu-c', 'pepsin-a', 'elastase', 'iodosobenzoate', 'no-enzyme', 'arg-c', 'lys-c', 'lys-n')"
