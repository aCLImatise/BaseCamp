...
task Cutpl {
  input {
    ...
    String? specify_columns_ranges
    File? get_column_names
  ...
  command <<<
    cut_pl \
      ...
      ~{if defined(change_input_output) then ("-d " +  '"' + change_input_output + '"') else ""} \
      ...
      ~{if defined(specify_columns_ranges) then ("-n " +  '"' + specify_columns_ranges + '"') else ""} \
      ~{if defined(get_column_names) then ("-file " +  '"' + get_column_names + '"') else ""} \
  ...
  parameter_meta {
    ...
    change_input_output: ": Change the input and output delimiter to DELIM (default <tab>)."
