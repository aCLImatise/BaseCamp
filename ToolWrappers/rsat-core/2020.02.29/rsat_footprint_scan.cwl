...
- id: in_all_genes
  ...
  inputBinding:
    prefix: -all_genes
- id: in_matrix_suffix
  doc: "whereas if your matrix files contains all the matrices from the\nRegulonDB\
    \ database, you can specify\n-matrix_suffix RegulonDB\nThe matrix suffix will\
    \ be concatenated to the output prefix, in\norder to maintain separate output\
    \ files for distinct analyses\nperformed on the same promoter sequences. For example,\
    \ if you run\nsuccessively the analysis with the matrix LexA, and then with the\n\
    matrix CRP, you don't want to loose the results of the first\nscanning when running\
    \ the second scanning."
  type: string
  inputBinding:
    prefix: -matrix_suffix
- id: in_tf
  doc: "If the option -matrix_table is used instead of the name of an\nspecific TF\
    \ specify the names are in the file using:\n-tf file"
  type: File
  inputBinding:
    prefix: -tf
- id: in_org_list_dot
