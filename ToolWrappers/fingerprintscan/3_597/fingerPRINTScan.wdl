...
task FingerPRINTScan {
  ...
  command <<<
    fingerPRINTScan \
      ...
      ~{if (use_conventional_format) then "-n" else ""} \
  ...
  parameter_meta {
    ...
    explicitly_in_fasta: "Explicitly state the format of the provided sequence is in the fasta format\\n(Currently default)"
    ...
    use_conventional_format: "Use the conventional database format (Slow when computing pvals)"
