...
task RunCQPMUnstrandedsh {
  input {
    File? file_name_genome
    Int? file_aligned_recommended
    ...
    String? predict_genes_is
  }
  command <<<
    run_CQ_PM_unstranded_sh \
      ~{if defined(file_name_genome) then ("-f " +  '"' + file_name_genome + '"') else ""} \
      ~{if defined(file_aligned_recommended) then ("-t " +  '"' + file_aligned_recommended + '"') else ""} \
      ...
      ~{if defined(predict_genes_is) then ("-h " +  '"' + predict_genes_is + '"') else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    file_aligned_recommended: "file of aligned transcripts (recommended)"
    ...
    predict_genes_is: "not predict genes in soft-masked regions (that is, hard-mask these regions)"
  }
  output {
