...
task Taxknife {
  ...
  command <<<
    taxknife \
      ...
      ~{if (set_nodes_will) then "-a" else ""} \
      ~{if (arg_replace_taxids_invalid) then "-c" else ""} \
      ~{if (_arg_name) then "-s" else ""} \
      ~{if (arg_select_ranks) then "-n" else ""} \
      ~{if (arg_consider_ranks) then "-q" else ""} \
  ...
  parameter_meta {
    ...
    set_nodes_will: "[ --allnodes ]                 if set, all nodes will be used, not only at\\nselected ranks"
    arg_replace_taxids_invalid: "[ --set-invalid-annotate ] arg replace all taxids that are invalid by this\\ngiven value"
    ...
    arg_consider_ranks: "[ --tree-ranks ] arg             consider these taxonomic ranks (ordered\\nspace separated list from low to high,\\ne.g. species family phylum)"
