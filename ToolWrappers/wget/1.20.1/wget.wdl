...
task Wget {
  ...
  command <<<
    wget \
      ...
      ~{if defined(retry_on_http_error) then ("--retry-on-http-error " +  '"' + retry_on_http_error + '"') else ""} \
      ~{if defined(output_document) then ("--output-document " +  '"' + output_document + '"') else ""} \
      ~{if (no_clobber) then "--no-clobber" else ""} \
      ~{if (no_netrc) then "--no-netrc" else ""} \
      ...
      ~{if defined(use_askpass) then ("--use-askpass " +  '"' + use_askpass + '"') else ""} \
  ...
  parameter_meta {
    ...
    retry_on_http_error: "comma-separated list of HTTP errors to retry"
    output_document: "write documents to FILE"
    no_clobber: "skip downloads that would download to\\nexisting files (overwriting them)"
    no_netrc: "don't try to obtain credentials from .netrc"
    ...
    use_askpass: "specify credential handler for requesting\\nusername and password.  If no COMMAND is\\nspecified the WGET_ASKPASS or the SSH_ASKPASS\\nenvironment variable is used."
