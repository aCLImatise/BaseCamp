...
task CLARK {
  ...
  command <<<
    CLARK \
      ...
      ~{if defined(directory_name_text) then ("-D " +  '"' + directory_name_text + '"') else ""} \
      ...
      ~{if defined(mode_default_express) then ("-m " +  '"' + mode_default_express + '"') else ""} \
      ...
      ~{if defined(gap_number_nonoverlapping) then ("-g " +  '"' + gap_number_nonoverlapping + '"') else ""} \
  ...
  parameter_meta {
    ...
    directory_name_text: ",       directory name for the database (to load/save database files):  text."
    ...
    mode_default_express: ",               mode of execution: 0 (full), 1 (default), 2 (express) and 3 (spectrum).\\nFor CLARK-S, the full and default mode are the same."
    ...
    gap_number_nonoverlapping: ",          gap or number of non-overlapping k-mers to pass for the database creation (for CLARK-l only). The default value is 4."
