...
- id: in_flank_distance
  doc: distance of flank from gap [0]
  type: long?
  inputBinding:
...
- id: in_min_frag
  ...
  inputBinding:
    prefix: --min-frag
- id: in_max_frag
  doc: "max fragment size in base pairs\nNote: --max-frag was formerly used to determine\
    \ the maximum gap\nsize that abyss-sealer would attempt to close, according to\
    \ the formula\nmax_gap_size = max_frag - 2 * flank_length, where flank_length\
    \ is\ndeteremined by the -L option.  --max-frag is kept only for backwards\ncompatibility\
    \ and is superceded by the more intuitive -G (--max-gap-length)\noption. Similarly,\
