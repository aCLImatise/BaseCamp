...
task Qfilt {
  ...
  command <<<
    qfilt \
      ...
      ~{if defined(minimum_quality_score) then ("-q " +  '"' + minimum_quality_score + '"') else ""} \
      ...
      ~{if (tolerate_low_regions) then "-p" else ""} \
      ...
      ~{if defined(prefix_supplied_prefix) then ("-t " +  '"' + prefix_supplied_prefix + '"') else ""} \
  ...
  parameter_meta {
    ...
    minimum_quality_score: "minimum per-base quality score below which a read will be split\\nor truncated (default=20)"
    ...
    prefix_supplied_prefix: "if PREFIX is supplied, prefix matching tolerates at most\\nMISMATCH mismatches (default=0)"
