...
task TracyAssemble {
  input {
    Boolean? arg_referenceguided_optional
  ...
  command <<<
    tracy assemble \
      ~{generic} \
      ~{if (arg_referenceguided_optional) then "-r" else ""} \
      ...
      ~{if (include_consensus_fasta) then "-i" else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/tracy:0.5.8--hf3ca161_0"
  }
  parameter_meta {
    arg_referenceguided_optional: "[ --reference ] arg              reference-guided assembly (optional)"
