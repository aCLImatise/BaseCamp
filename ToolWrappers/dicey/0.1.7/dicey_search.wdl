...
task DiceySearch {
  ...
  command <<<
    dicey search \
      ...
      ~{if (arg_prune_primer) then "-q" else ""} \
      ~{if (use_hamming_neighborhood) then "-n" else ""} \
      ~{if (arg_min_primer) then "-c" else ""} \
      ~{if (arg_max_pcr) then "-l" else ""} \
  ...
  parameter_meta {
    ...
    arg_min_primer: "[ --cutTemp ] arg (=45)            min. primer melting temperature"
