...
task Bseqc2 {
  input {
    ...
    Boolean? arg_number_first
    Boolean? arg_rscript_defaultbindirbseqcmbiasplotr
    String required_dot
  }
  command <<<
    bseqc2 \
      ...
      ~{if (arg_number_first) then "-n" else ""} \
      ~{if (arg_rscript_defaultbindirbseqcmbiasplotr) then "-s" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    arg_number_first: "[ --numreads ] arg (=20000000) Number of reads. First `n` reads will be\\nexamined. Be aware of extremely low CpG\\nmethylation levels when chrM is the first\\nchromosome. Default: 20000000."
    arg_rscript_defaultbindirbseqcmbiasplotr: "[ --rscript ] arg              Rscript for mbias plot. Default:\\n`$bindir/bseqc2mbiasplot.R`."
