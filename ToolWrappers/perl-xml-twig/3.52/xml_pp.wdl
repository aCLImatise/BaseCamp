...
task XmlPp {
  input {
    ...
    Boolean? loads_documents_memory
    File? read_list_process
    Boolean? verbose_list_processed
    String twig
  }
  command <<<
    xml_pp \
      ...
      ~{if (loads_documents_memory) then "-l" else ""} \
      ~{if defined(read_list_process) then ("-f " +  '"' + read_list_process + '"') else ""} \
      ~{if (verbose_list_processed) then "-v" else ""}
  >>>
  parameter_meta {
    ...
    loads_documents_memory: "loads the documents in memory instead of outputting them as they are\\nbeing parsed.\\nThis prevents a bug (see BUGS) but uses more memory"
    read_list_process: "read the list of files to process from <file>, one per line"
    verbose_list_processed: "verbose (list the current file being processed)"
    twig: "OPTIONS"
  }
  output {
