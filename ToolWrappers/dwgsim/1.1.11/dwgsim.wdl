...
task Dwgsim {
  ...
  command <<<
    dwgsim \
      ...
      ~{if defined(mean_coverage_available) then ("-C " +  '"' + mean_coverage_available + '"') else ""} \
      ...
      ~{if defined(fraction_mutations_are) then ("-R " +  '"' + fraction_mutations_are + '"') else ""} \
      ...
      ~{if defined(maximum_number_allowed) then ("-n " +  '"' + maximum_number_allowed + '"') else ""} \
      ~{if defined(generate_reads_torrent) then ("-c " +  '"' + generate_reads_torrent + '"') else ""} \
      ~{if defined(generate_reads_default) then ("-S " +  '"' + generate_reads_default + '"') else ""} \
      ~{if defined(flow_order_ion) then ("-f " +  '"' + flow_order_ion + '"') else ""} \
      ~{if (use_perbase_rate) then "-B" else ""} \
  ...
  parameter_meta {
    ...
    mean_coverage_available: "mean coverage across available positions (-1 to disable) [100.00]"
    ...
    frequency_simulate_mutations: "frequency of given mutation to simulate low fequency somatic mutations [0.5000]\\nNB: freqeuncy F refers to the first strand of mutation, therefore mutations\\non the second strand occour with a frequency of 1-F"
    fraction_mutations_are: "fraction of mutations that are indels [0.10]"
    ...
    maximum_number_allowed: "maximum number of Ns allowed in a given read [0]"
    generate_reads_torrent: "generate reads for [0]:\\n0: Illumina\\n1: SOLiD\\n2: Ion Torrent"
