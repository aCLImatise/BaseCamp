...
- id: in_number_of_bins
  ...
  inputBinding:
    prefix: --numberOfBins
- id: in_transform
  doc: "Type of transformation for the matrix. The options are\n\"none\", \"total-counts\"\
    , \"z-score\" and \"obs/exp\". If\ntotal counts are selected, the sub-matrix values\
    \ are\ndivided by the total counts for normalization. If\nz-score or obs/exp are\
    \ selected, the Hi-C matrix is\nconverted into a z-score or observed / expected\n\
    matrix."
  type: string
  inputBinding:
    prefix: --transform
