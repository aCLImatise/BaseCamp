...
task Biohansel {
  ...
  command <<<
    biohansel \
      ...
      ~{if defined(min_km_er_frac) then ("--min-kmer-frac " +  '"' + min_km_er_frac + '"') else ""} \
  ...
  >>>
  runtime {
    docker: "quay.io/biocontainers/bio_hansel:2.6.1--py_0"
  }
  parameter_meta {
    ...
    min_km_er_frac: "Proportion of k-mer required for detection (0.0 - 1)"
