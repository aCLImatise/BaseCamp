...
task Bcool {
  ...
  command <<<
    bcool \
      ...
      ~{if defined(kmer_size_auto) then ("-k " +  '"' + kmer_size_auto + '"') else ""} \
      ~{if defined(kmers_present_less) then ("-s " +  '"' + kmers_present_less + '"') else ""} \
      ~{if defined(unitig_coverage_auto) then ("-S " +  '"' + unitig_coverage_auto + '"') else ""} \
