...
task LefseformatInputpy {
  ...
  command <<<
    lefse_format_input_py \
      ...
      ~{if defined(set_value_default) then ("-o " +  '"' + set_value_default + '"') else ""} \
  ...
  parameter_meta {
    ...
    set_value_default: "set the normalization value (default -1.0 meaning no\\nnormalization)"
