...
task Mbuffer {
  input {
    ...
    Boolean? lock_buffer_memory
    Boolean? use_blocksize_device
    Int? assumed_default_infiniteautodetect
    Int? start_writing_filled
    Int? start_reading_buffer
    File? use_file_input
  ...
  command <<<
    mbuffer \
      ...
      ~{if (lock_buffer_memory) then "-L" else ""} \
      ~{if (use_blocksize_device) then "-d" else ""} \
      ~{if defined(assumed_default_infiniteautodetect) then ("-D " +  '"' + assumed_default_infiniteautodetect + '"') else ""} \
      ~{if defined(start_writing_filled) then ("-P " +  '"' + start_writing_filled + '"') else ""} \
      ~{if defined(start_reading_buffer) then ("-p " +  '"' + start_reading_buffer + '"') else ""} \
      ...
      ~{if defined(issue_command_cmd) then ("-A " +  '"' + issue_command_cmd + '"') else ""} \
  ...
  parameter_meta {
    ...
    lock_buffer_memory: ": lock buffer in memory (unusable with file based buffers)"
    use_blocksize_device: ": use blocksize of device for output"
    assumed_default_infiniteautodetect: ": assumed output device size (default: infinite/auto-detect)"
    start_writing_filled: ": start writing after buffer has been filled more than <num>%"
    start_reading_buffer: ": start reading after buffer has been filled less than <num>%"
    ...
    issue_command_cmd: ": issue command <cmd> to request new volume"
