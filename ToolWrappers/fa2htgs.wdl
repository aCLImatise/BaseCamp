...
task Fa2htgs {
  input {
    Boolean? filename_fasta_input
    Boolean? filename_seqsubmit_template
    ...
    Boolean? genbank_accession_optional
    ...
    Boolean? filename_quality_scores
    Boolean? whole_genome_shotgun
    Boolean? arbitrary_keyword_optional
  }
  command <<<
    fa2htgs \
      ~{if (filename_fasta_input) then "-i" else ""} \
      ~{if (filename_seqsubmit_template) then "-t" else ""} \
      ...
      ~{if (genbank_accession_optional) then "-a" else ""} \
      ...
      ~{if (filename_phrap_input) then "-T" else ""} \
      ~{if (contigs_use_separate) then "-P" else ""} \
      ~{if (filename_accession_list) then "-A" else ""} \
      ~{if (coordinates_resulting_sequence) then "-X" else ""} \
      ~{if (htgsdraft_sequence_optionaldefault) then "-D" else ""} \
      ...
      ~{if (filename_phrap_contig) then "-L" else ""} \
      ...
      ~{if (filename_quality_scores) then "-Q" else ""} \
      ~{if (whole_genome_shotgun) then "-w" else ""} \
      ~{if (arbitrary_keyword_optional) then "-k" else ""}
  >>>
  parameter_meta {
    filename_fasta_input: "Filename for fasta input [File In]  Optional\\ndefault = stdin"
    filename_seqsubmit_template: "Filename for Seq-submit template [File In]\\ndefault = template.sub"
    ...
    genbank_accession_optional: "GenBank accession (if an update) [String]  Optional"
    ...
    filename_phrap_input: "Filename for phrap input [File In]  Optional"
    contigs_use_separate: "Contigs to use, separate by commas if multiple [String]  Optional"
    filename_accession_list: "Filename for accession list input [File In]  Optional"
    coordinates_resulting_sequence: "Coordinates are on the resulting sequence ? [T/F]  Optional\\ndefault = F"
    htgsdraft_sequence_optionaldefault: "HTGS_DRAFT sequence? [T/F]  Optional\\ndefault = F"
    ...
    filename_phrap_contig: "Filename for phrap contig order [File In]  Optional"
    ...
    filename_quality_scores: "Filename for quality scores [File In]  Optional"
