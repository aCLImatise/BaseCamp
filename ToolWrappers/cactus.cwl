...
inputs:
- id: in_experiment
  doc: "The file containing a link to the experiment\nparameters"
  type: File
  inputBinding:
    prefix: --experiment
- id: in_build_avg_s
...
- id: in_build_hal
  ...
  inputBinding:
    prefix: --buildHal
- id: in_build_fast_a
  doc: "Build a fasta file of the input sequences (and\nreference sequence, used with\
    \ hal output)"
  type: boolean
  inputBinding:
    prefix: --buildFasta
- id: in_intermediate_results_url
  doc: "URL prefix to save intermediate results like DB dumps\nto (e.g. prefix-dump-caf,\
    \ prefix-dump-avg, etc.)"
  type: string
  inputBinding:
    prefix: --intermediateResultsUrl
- id: in_database
  doc: "Database type: tokyo_cabinet or kyoto_tycoon [default:\nkyoto_tycoon]"
  type: string
  inputBinding:
    prefix: --database
- id: in_config_file
  ...
  inputBinding:
    prefix: --configFile
- id: in_root
  doc: "Name of ancestral node (which must appear in NEWICK\ntree in <seqfile>) to\
    \ use as a root for the alignment.\nAny genomes not below this node in the tree\
    \ may be\nused as outgroups but will never appear in the output.\nIf no root is\
    \ specifed then the root of the tree is\nused."
  type: string
  inputBinding:
    prefix: --root
- id: in_latest
  doc: "Use the latest version of the docker container rather\nthan pulling one matching\
    \ this version of cactus"
  type: boolean
  inputBinding:
    prefix: --latest
- id: in_container_image
  doc: "Use the the specified pre-built containter image\nrather than pulling one\
    \ from quay.io"
  type: string
  inputBinding:
