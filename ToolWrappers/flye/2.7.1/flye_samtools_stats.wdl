...
task FlyesamtoolsStats {
  ...
  command <<<
    flye_samtools stats \
      ...
      ~{if (cov_threshold) then "--cov-threshold" else ""} \
  ...
  parameter_meta {
    ...
    read_length: "Include in the statistics only reads with the given read length []"
    ...
    cov_threshold: "Only bases with coverage above this value will be included in the target percentage computation."
