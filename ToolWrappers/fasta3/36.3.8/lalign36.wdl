...
task Lalign36 {
  input {
    ...
    Boolean? show_identity_alignment
    Boolean? _number_shuffles
    Boolean? maximum_number_nonintersecting
  ...
  command <<<
    lalign36 \
      ...
      ~{if (show_identity_alignment) then "-J" else ""} \
      ~{if (_number_shuffles) then "-k" else ""} \
      ~{if (maximum_number_nonintersecting) then "-K" else ""} \
  ...
  parameter_meta {
    ...
    show_identity_alignment: "show identity alignment"
    _number_shuffles: ":  number of shuffles"
    maximum_number_nonintersecting: ":  maximum number of non-intersecting alignments"
