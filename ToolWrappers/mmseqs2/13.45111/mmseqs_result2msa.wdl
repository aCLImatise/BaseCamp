...
task MmseqsResult2msa {
  ...
  command <<<
    mmseqs result2msa \
      ...
      ~{if defined(msa_format_mode) then ("--msa-format-mode " +  '"' + msa_format_mode + '"') else ""} \
      ~{if defined(sub_mat) then ("--sub-mat " +  '"' + sub_mat + '"') else ""} \
      ~{if defined(threads) then ("--threads " +  '"' + threads + '"') else ""} \
      ~{if defined(compressed) then ("--compressed " +  '"' + compressed + '"') else ""} \
      ~{if defined(verbosity_level_errors) then ("-v " +  '"' + verbosity_level_errors + '"') else ""} \
      ~{if defined(summary_prefix) then ("--summary-prefix " +  '"' + summary_prefix + '"') else ""} \
  ...
  parameter_meta {
    ...
    filter_msa: "Filter msa: 0: do not filter, 1: filter [0]"
    ...
    msa_format_mode: "Format MSA as: 0: binary cA3M DB\\n1: binary ca3m w. consensus DB\\n2: aligned FASTA DB\\n3: aligned FASTA w. header summary\\n4: STOCKHOLM flat file [0]"
    ...
    summary_prefix: "Set the cluster summary prefix [cl]"
