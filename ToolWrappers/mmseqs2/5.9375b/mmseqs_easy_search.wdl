...
task MmseqsEasysearch {
  ...
  command <<<
    mmseqs easy_search \
      ...
      ~{if (format_mode) then "--format-mode" else ""} \
      ~{if (db_output) then "--db-output" else ""} \
      ~{if (overlap) then "--overlap" else ""} \
      ~{if (greedy_best_hits) then "--greedy-best-hits" else ""} \
  ...
  parameter_meta {
    ...
    sensitivity_faster_fast: "5.700           sensitivity: 1.0 faster; 4.0 fast default; 7.5 sensitive [1.0,7.5]"
    ...
    format_mode: "0               output format 0: BLAST-TAB, 1: PAIRWISE, 2: BLAST-TAB + query/db length"
    db_output: "false           Output a result db instead of a text file"
    overlap: "0.000           maximum overlap"
    greedy_best_hits: "false           Choose the best hits greedily to cover the query."
