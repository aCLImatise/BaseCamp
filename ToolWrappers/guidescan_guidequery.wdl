...
task GuidescanGuidequery {
  ...
  command <<<
    guidescan_guidequery \
      ...
      ~{if defined(amount_optimal_desired) then ("-n " +  '"' + amount_optimal_desired + '"') else ""} \
  ...
  parameter_meta {
    ...
    amount_optimal_desired: "amount of optimal sgRNAs desired from the --select\\nparameter. Ignored if --select not used. Default is 3."
