...
task FilterReads {
  ...
  command <<<
    filter_reads \
      ...
      ~{if defined(given_string_written) then ("-c " +  '"' + given_string_written + '"') else ""} \
  ...
  parameter_meta {
    ...
    given_string_written: ": the given string will be written in the header of the output file. [default=command line]"
    prints_version_number: ": prints the version number."
  }
  output {
    File out_stdout = stdout()
    File out_given_string_written = "${in_given_string_written}"
  }
}