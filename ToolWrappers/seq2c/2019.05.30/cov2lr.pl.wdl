...
task Cov2lrpl {
  input {
    ...
    File? output_frozenfile_parameters
    File? var_7
  }
  command <<<
    cov2lr_pl \
      ...
      ~{if defined(output_frozenfile_parameters) then ("-Z " +  '"' + output_frozenfile_parameters + '"') else ""} \
      ~{if defined(var_7) then ("-z " +  '"' + var_7 + '"') else ""}
  >>>
  parameter_meta {
    ...
    output_frozenfile_parameters: "to output the frozen_file and all parameters into file Seq2C.frozen.txt"
    var_7: ""
  }
  output {
    File out_stdout = stdout()
    File out_output_frozenfile_parameters = "${in_output_frozenfile_parameters}"
  }
}