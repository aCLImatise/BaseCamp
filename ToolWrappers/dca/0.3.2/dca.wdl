...
task Dca {
  input {
    ...
    Boolean? check_counts
    Boolean? no_check_counts
  ...
  command <<<
    dca \
      ...
      ~{if (check_counts) then "--checkcounts" else ""} \
      ~{if (no_check_counts) then "--nocheckcounts" else ""} \
      ~{if defined(de_noise_subset) then ("--denoisesubset " +  '"' + de_noise_subset + '"') else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/dca:0.3.2--py_0"
  }
  parameter_meta {
    ...
    check_counts: "Check if the expression matrix has raw (unnormalized)\\ncounts (default: True)"
    no_check_counts: "Do not check if the expression matrix has raw\\n(unnormalized) counts"
