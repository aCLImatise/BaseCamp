...
task Paragraph {
  input {
    ...
    Boolean? arg_output_alignments_was
  ...
  command <<<
    paragraph \
      ...
      ~{if defined(used_output_file) then ("-O " +  '"' + used_output_file + '"') else ""} \
      ...
      ~{if (arg_output_alignments_was) then "-A" else ""} \
  ...
  parameter_meta {
    ...
    used_output_file: "used, output file names"
    ...
    arg_output_alignments_was: "[ --output-filtered-alignments ] arg (=0) Output alignments for every read\\neven when it was filtered\\n(larger)."
  ...
  output {
    ...
    File out_used_output_file = "${in_used_output_file}"
