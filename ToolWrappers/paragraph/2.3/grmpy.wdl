...
task Grmpy {
  input {
    Directory? arg_output_folder_alignments
  ...
  command <<<
    grmpy \
      ...
      ~{alignment} \
      ~{arguments} \
      ~{if (arg_output_folder_alignments) then "-A" else ""} \
      ...
      ~{if (arg_output_folder_path) then "-O" else ""} \
  ...
  output {
    ...
    Directory out_arg_output_folder_alignments = "${in_arg_output_folder_alignments}"
