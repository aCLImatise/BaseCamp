...
- id: in_minimum_diversity_cv
  ...
  inputBinding:
    prefix: -d
- id: in_minimum_agreement_weighted
...
- id: in_running_bam_turn
  ...
  inputBinding:
    prefix: -B
- id: in_homopolymer_repeat_filtering
...
- id: in_read_statistics_params
  ...
  inputBinding:
    prefix: -S
- id: in_calculate_intarget_requires
...
- id: out_stdout
  ...
  type: stdout
- id: out_list_file_types
  doc: List of file types to output (var, varsum, eav, vcf)
  type: File
  outputBinding:
    glob: $(inputs.in_list_file_types)
cwlVersion: v1.1
baseCommand:
- varcall
