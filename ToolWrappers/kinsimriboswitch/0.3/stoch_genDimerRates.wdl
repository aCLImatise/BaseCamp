...
task StochGenDimerRates {
  input {
    ...
    Int? folding_temperature_celsius
  ...
  command <<<
    stoch_genDimerRates \
      ...
      ~{if defined(folding_temperature_celsius) then ("-T " +  '"' + folding_temperature_celsius + '"') else ""} \
  ...
  parameter_meta {
    ...
    folding_temperature_celsius: ":  Folding temperature in deg Celsius [37]"
