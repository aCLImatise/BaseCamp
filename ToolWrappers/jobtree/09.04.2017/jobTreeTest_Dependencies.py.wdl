...
task JobTreeTestDependenciespy {
  input {
    Int? sleep_time
    String? tree
    Int? size
  ...
  command <<<
    jobTreeTest_Dependencies_py \
      ~{if defined(sleep_time) then ("--sleepTime " +  '"' + sleep_time + '"') else ""} \
      ~{if defined(tree) then ("--tree " +  '"' + tree + '"') else ""} \
      ~{if defined(size) then ("--size " +  '"' + size + '"') else ""} \
      ~{if defined(cpus_per_job) then ("--cpusPerJob " +  '"' + cpus_per_job + '"') else ""} \
  ...
  parameter_meta {
    ...
    size: "tree size (for comb or star) [default=10]"
