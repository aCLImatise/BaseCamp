...
task Hilive {
  ...
  command <<<
    hilive \
      ...
      ~{if (illuminas_basecalls_directory) then "-b" else ""} \
      ~{if (path_hilive_index) then "-i" else ""} \
      ~{if (length_types_read) then "-r" else ""} \
  ...
  output {
    ...
    File out_arg_path_config = "${in_arg_path_config}"
