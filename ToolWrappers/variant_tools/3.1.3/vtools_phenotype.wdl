...
task VtoolsPhenotype {
  input {
    ...
    Boolean? set
    Boolean? from_stat
    Boolean? list_phenotype_outputted
    String? jobs
    Boolean? limit_operation_genotypes
    Boolean? update_phenotype_use
    String? verbosity
    Boolean? header
    String? delimiter
    String? na
  ...
  command <<<
    vtools phenotype \
      ...
      ~{if (list_phenotype_outputted) then "--output" else ""} \
      ~{if defined(jobs) then ("--jobs " +  '"' + jobs + '"') else ""} \
      ~{if (limit_operation_genotypes) then "-g" else ""} \
      ~{if (update_phenotype_use) then "-s" else ""} \
