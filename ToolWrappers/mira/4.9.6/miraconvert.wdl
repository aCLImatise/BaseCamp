...
task Miraconvert {
  input {
    String? load_type_project
    String? write_sequencesassembly_allowed
    ...
    File? when_given_reads
    File? n_sorts_output
    Boolean? when_n_used
    File? t_fastq_quality
    String? string_mira_parameters
    File? set_default_quality
    String? rename_contigssingletsreads_given
    String? namescheme_renaming_reads
    Boolean? when_converting_single
    Boolean? print_version_number
    Int? yield_max_clippedpadded
    Boolean? extract_contigs_consensus
    Boolean? recalculate_consensus_cons
    File? split_output_multiple
    Boolean? fill_holes_genome
    Int? defines_minimum_quality
    Int? defines_minimum_coverage
    Int? minimum_contig_unclipped
    Int? similar_applies_reads
    Int? minimum_average_contig
    Int? minimum_number_contigs
    Int? text_html_number
    String? text_html_character
    String var_32
    String var_33
    ...
    String wig
    String gc_wig
    String sequences_consensus_fasta
  ...
  command <<<
    miraconvert \
      ...
      ~{masked_fast_a} \
      ~{s_caf} \
      ~{if defined(load_type_project) then ("-f " +  '"' + load_type_project + '"') else ""} \
      ~{if defined(write_sequencesassembly_allowed) then ("-t " +  '"' + write_sequencesassembly_allowed + '"') else ""} \
      ...
      ~{if defined(when_given_reads) then ("-n " +  '"' + when_given_reads + '"') else ""} \
      ~{if defined(n_sorts_output) then ("-N " +  '"' + n_sorts_output + '"') else ""} \
      ~{if (when_n_used) then "-i" else ""} \
      ~{if defined(t_fastq_quality) then ("-o " +  '"' + t_fastq_quality + '"') else ""} \
      ~{if defined(string_mira_parameters) then ("-P " +  '"' + string_mira_parameters + '"') else ""} \
      ~{if defined(set_default_quality) then ("-q " +  '"' + set_default_quality + '"') else ""} \
      ~{if defined(rename_contigssingletsreads_given) then ("-R " +  '"' + rename_contigssingletsreads_given + '"') else ""} \
      ~{if defined(namescheme_renaming_reads) then ("-S " +  '"' + namescheme_renaming_reads + '"') else ""} \
      ~{if (when_converting_single) then "-T" else ""} \
      ...
      ~{if (recalculate_consensus_cons) then "-r" else ""} \
      ~{if (split_output_multiple) then "-s" else ""} \
      ~{if (fill_holes_genome) then "-u" else ""} \
      ...
      ~{if defined(minimum_contig_unclipped) then ("-x " +  '"' + minimum_contig_unclipped + '"') else ""} \
      ...
      ~{if defined(minimum_number_contigs) then ("-z " +  '"' + minimum_number_contigs + '"') else ""} \
      ~{if defined(text_html_number) then ("-l " +  '"' + text_html_number + '"') else ""} \
      ~{if defined(text_html_character) then ("-c " +  '"' + text_html_character + '"') else ""}
  >>>
  parameter_meta {
    load_type_project: "load this type of project files, where fromtype is:"
    write_sequencesassembly_allowed: "write the sequences/assembly to this type (multiple\\nmentions of -t are allowed):"
    ...
    minimum_number_contigs: "Minimum number of reads in contig\\nWhen loading, discard all contigs with a\\nnumber of reads less than this value.\\nDefault: 0 (=switched off)"
    text_html_number: "when output as text or HTML: number of bases shown in\\none alignment line. Default: 60."
    text_html_character: "when output as text or HTML: character used to pad\\nendgaps. Default: ' ' (blank)"
    ...
    sequences_consensus_fasta: "sequences or consensus to FASTA file (qualities to\\n.qual)"
