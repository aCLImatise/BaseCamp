...
task Mirabait {
  input {
    ...
    File? load_paired_sequences
    Int? kmer_length_default
    Int? minimum_number_kmer
    Boolean? use_kmers_see
    ...
    Boolean? number_use_cpu
    Boolean? case_change_denote
    Int? length_line_unlimited
    File? save_kmer_statistics
    String? change_prefix_used
  ...
  command <<<
    mirabait \
      ...
      ~{if defined(load_paired_sequences) then ("-P " +  '"' + load_paired_sequences + '"') else ""} \
      ...
      ~{if (number_use_cpu) then "-t" else ""} \
      ~{if (case_change_denote) then "-c" else ""} \
      ~{if defined(length_line_unlimited) then ("-l " +  '"' + length_line_unlimited + '"') else ""} \
      ~{if defined(save_kmer_statistics) then ("-K " +  '"' + save_kmer_statistics + '"') else ""} \
      ~{if defined(change_prefix_used) then ("-N " +  '"' + change_prefix_used + '"') else ""} \
  ...
  parameter_meta {
    ...
    use_kmers_see: "Do not use kmers with microrepeats (DUST-like, see also -D)"
    ...
    number_use_cpu: "Number of threads to use (default=0 -> up to 4 CPU cores)"
    case_change_denote: "No case change of sequence to denote bait hits"
    length_line_unlimited: "length of a line (FASTA only, default 0=unlimited)"
    save_kmer_statistics: "Save kmer statistics to 'file' (see also -B)"
    change_prefix_used: "Change the prefix 'bait' to <name>\\nHas no effect if -o/-O is used and targets are not\\ndirectories"
