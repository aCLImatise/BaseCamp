...
task Cdscan {
  input {
    ...
    Boolean? output_format_optionaldefault
  ...
  command <<<
    cdscan \
      ...
      ~{if (output_format_optionaldefault) then "-f" else ""} \
  ...
  parameter_meta {
    ...
    output_format_optionaldefault: "Output format: GenBank (1) or FASTA (2) [Integer]  Optional\\ndefault = 1\\nrange from 1 to 2"
