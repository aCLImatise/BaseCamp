...
task BlandAltmanPlotpy {
  ...
  command <<<
    bland_altman_plot_py \
      ...
      ~{if defined(prop_feature) then ("--prop_feature " +  '"' + prop_feature + '"') else ""} \
      ~{if defined(prop_sample) then ("--prop_sample " +  '"' + prop_sample + '"') else ""} \
  ...
  parameter_meta {
    ...
    prop_feature: "Name of the output TSV for proportion of features."
    prop_sample: "Name of the output TSV for proportion of samples."
