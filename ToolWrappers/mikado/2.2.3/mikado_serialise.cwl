...
- id: in_start_method
  ...
  inputBinding:
    prefix: --start-method
- id: in_shm
  doc: Use /dev/shm (if available) for faster database
  type: boolean?
  inputBinding:
    prefix: --shm
- id: in_no_force
  doc: "Flag. If set, do not drop the contents of an existing\nMikado DB before beginning\
    \ the serialisation."
  type: boolean?
  inputBinding:
    prefix: --no-force
...
- id: in_log_level
  ...
  inputBinding:
    prefix: --log-level
- id: in_blast_loading_debug
  doc: "Flag. If set, Mikado will switch on the debug mode for\nthe XML/TSV loading."
  type: boolean?
  inputBinding:
...
- id: in_building_dot
  doc: --no-shm              Force building the database on its final location,
  type: string
  inputBinding:
    position: 0
- id: in_db
