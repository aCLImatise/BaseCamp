...
task Spinepl {
  input {
    File? file_list_input
    Int? pct_core
    Int? max_dist
    Boolean? license_print_license
    Boolean? nucpath_full_path
    Int? refs
    Boolean? mini
    Boolean? pan_genome
    String? prefix
  ...
  command <<<
    spine_pl \
      ~{if defined(file_list_input) then ("--file " +  '"' + file_list_input + '"') else ""} \
      ~{if defined(pct_core) then ("--pctcore " +  '"' + pct_core + '"') else ""} \
      ~{if defined(max_dist) then ("--maxdist " +  '"' + max_dist + '"') else ""} \
      ~{if (license_print_license) then "-l" else ""} \
      ~{if (nucpath_full_path) then "-m" else ""} \
  ...
  parameter_meta {
    ...
    license_print_license: "or --license  print license information and quit"
    nucpath_full_path: "or --nucpath  full path to folder containing MUMmer scripts and\\nexecutables, i.e. /home/applications/MUMmer/bin\\n(default: tries to find MUMmer in your PATH)"
