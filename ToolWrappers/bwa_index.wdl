...
task BwaIndex {
  ...
  command <<<
    bwa index \
      ...
      ~{if defined(block_size_effective) then ("-b " +  '"' + block_size_effective + '"') else ""} \
  ...
  parameter_meta {
    ...
    block_size_effective: "block size for the bwtsw algorithm (effective with -a bwtsw) [10000000]"
