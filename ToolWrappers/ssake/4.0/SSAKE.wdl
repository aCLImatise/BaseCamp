...
task SSAKE {
  ...
  command <<<
    SSAKE \
      ...
      ~{if (minimum_depth_allowed) then "-w" else ""} \
  ...
  parameter_meta {
    ...
    minimum_depth_allowed: "Minimum depth of coverage allowed for contigs (e.g. -w 1 = process all reads [v3.7 behavior], required, recommended -w 5)\\n*The assembly will stop when 50+ contigs with coverage < -w have been seen.*"
