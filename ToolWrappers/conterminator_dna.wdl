...
task ConterminatorDna {
  ...
  >>>
  parameter_meta {
    comp_bias_corr: "correct for locally biased amino acid composition (range 0-1) [0]"
    ...
    sensitivity_faster_fast: "sensitivity: 1.0 faster; 4.0 fast default; 7.5 sensitive (range 1.0-7.5) [5.700]"
    kmer_size_range: "k-mer size in the range (0: set automatically to optimum) [15]"
    ...
    diag_score: "Use ungapped diagonal scoring during prefilter"
    exact_km_er_matching: "only exact k-mer matching (range 0-1) [1]"
    ...
    min_un_gapped_score: "accept only matches with ungapped alignment score above this threshold [25]"
    ...
    add_string_convert: "add backtrace string (convert to alignments with mmseqs convertalis utility) [1, set to 0 to disable]"
    ...
    min_aln_len: "minimum alignment length (range 0-INT_MAX) [100]"
    ...
    re_score_mode: "Rescore diagonal with: 0: Hamming distance, 1: local alignment (score only), 2: local alignment, 3: global alignment or 4: longest alignment fullfilling window quality criterion [2]"
    ...
    forward_frames: "comma-seperated list of ORF frames on the forward strand to be extracted [1]"
    reverse_frames: "comma-seperated list of ORF frames on the reverse strand to be extracted [1]"
    ...
    strand: "Strand selection only works for DNA/DNA search 0: reverse, 1: forward, 2: both [2]"
  }
  output {
