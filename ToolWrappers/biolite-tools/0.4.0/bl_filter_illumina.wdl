...
task Blfilterillumina {
  input {
    ...
    Boolean? filter_reads_have
    Boolean? limit_output_pass
  ...
  command <<<
    bl_filter_illumina \
      ...
      ~{if (filter_reads_have) then "-q" else ""} \
      ~{if (limit_output_pass) then "-n" else ""} \
  ...
  parameter_meta {
    ...
    filter_reads_have: "filter out reads that have a mean quality less than QUALITY"
    limit_output_pass: "limit output to only the first NREADS that pass"
