...
- id: in_read_one_files
  doc: "list of all the read 1 fastq files you would like to\nrun through the pipeline.\
    \ Note that globbing is\nsupported here (i.e., you can use * in your paths)\n\
    (default: None)"
  type: long[]
  inputBinding:
    prefix: --read1-files
- id: in_read_two_files
  doc: "list of all the read 2 fastq files you would like to\nrun through the pipeline.\
    \ Note that globbing is\nsupported here (i.e., you can use * in your paths)\n\
    (default: None)"
  type: long[]
  inputBinding:
...
- id: in_sample
  ...
  inputBinding:
    prefix: --sample
- id: in_forward_ref
  doc: "string indicating the path to the forward strand\nreference created by build_ref\
    \ (default: None)"
  type: File?
  inputBinding:
    prefix: --forward-ref
- id: in_reverse_ref
  doc: "string indicating the path to the reverse strand\nreference created by build_ref\
    \ (default: None)"
  type: File?
  inputBinding:
    prefix: --reverse-ref
- id: in_string_indicating_path
...
- id: in_merge_by_max_mapq
  doc: "Boolean indicates whether to merge alignment results\nfrom two converted genomes\
    \ by MAPQ score. Be default,\nwe only keep read pairs that are mapped to only\
    \ one of\nthe two converted genomes. If this option is set to\nTrue, for a read\
    \ pair that could be mapped to both\nconverted genomes, the alignment that achieves\
...
- id: in_adapter_seq_read_one
  doc: "sequence of an adapter that was ligated to the 3' end\nof read 1. The adapter\
    \ itself and anything that\nfollows is trimmed. (default:\nAGATCGGAAGAGCACACGTCTGAAC)"
  type: long?
  inputBinding:
    prefix: --adapter-seq-read1
- id: in_adapter_seq_read_two
  doc: "sequence of an adapter that was ligated to the 3' end\nof read 2. The adapter\
