...
task PhenixpyFilterVcf {
  input {
    ...
    Float? filters
    String? config
    String? location_filtered_written
    Float? reference
    Boolean? only_good
  }
  command <<<
    phenix_py filter_vcf \
      ...
      ~{if defined(location_filtered_written) then ("--output " +  '"' + location_filtered_written + '"') else ""} \
  ...
  parameter_meta {
    ...
    location_filtered_written: "Location for filtered VCF to be written."
