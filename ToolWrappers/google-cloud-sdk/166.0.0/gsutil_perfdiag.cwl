...
- id: in_adds_metadata_file
  ...
  inputBinding:
    prefix: -m
- id: in_writes_results_diagnostic
...
- id: out_adds_metadata_file
  ...
  type: File
  outputBinding:
    glob: $(inputs.in_adds_metadata_file)
- id: out_writes_results_diagnostic
  doc: "Writes the results of the diagnostic to an output file. The output\nis a JSON\
    \ file containing system information and performance\ndiagnostic results. The\
    \ file can be read and reported later using\nthe -i option."
  type: File
  outputBinding:
    glob: $(inputs.in_writes_results_diagnostic)
