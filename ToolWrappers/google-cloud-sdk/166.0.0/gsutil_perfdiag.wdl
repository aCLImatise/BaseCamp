...
task GsutilPerfdiag {
  input {
    Boolean? sets_number_objects
    Boolean? sets_number_running
    ...
    File? adds_metadata_m
    File? writes_results_file
    File? reads_json_file
  }
  command <<<
    gsutil perfdiag \
      ~{if (sets_number_objects) then "-n" else ""} \
      ~{if (sets_number_running) then "-c" else ""} \
      ~{if (sets_number_use) then "-k" else ""} \
      ~{if (sets_type_used) then "-p" else ""} \
      ~{if (sets_number_divide) then "-y" else ""} \
      ~{if (sets_size_n) then "-s" else ""} \
      ~{if (sets_directory_store) then "-d" else ""} \
      ~{if (sets_list_perform) then "-t" else ""} \
      ~{if (adds_metadata_m) then "-m" else ""} \
      ~{if (writes_results_file) then "-o" else ""} \
  ...
  output {
    ...
    File out_adds_metadata_m = "${in_adds_metadata_m}"
    File out_writes_results_file = "${in_writes_results_file}"
