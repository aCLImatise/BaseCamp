...
task GtftkGetAttrValueList {
  ...
  command <<<
    gtftk get_attr_value_list \
      ...
      ~{if (print_key_name) then "--print-key-name" else ""} \
  ...
  parameter_meta {
    ...
    key_name: "The comma separated list of key name. (default: None)"
    separator: "The separator used if -c or -p are true. (default: )"
    count: "Add the counts for each key (separator will be set to ' ' by default). (default: False)"
    print_key_name: "Also print the key name in the first column. (default: False)"
