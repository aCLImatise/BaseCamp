...
task Stripit {
  ...
  command <<<
    strip_it \
      ...
      ~{if defined(specifies_file_extracted) then ("--output " +  '"' + specifies_file_extracted + '"') else ""} \
  ...
  output {
    ...
    File out_specifies_file_extracted = "${in_specifies_file_extracted}"
  }
}