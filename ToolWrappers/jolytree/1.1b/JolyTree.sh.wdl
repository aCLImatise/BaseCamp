...
task JolyTreesh {
  ...
  command <<<
    JolyTree_sh \
      ...
      ~{if defined(least_one_estimated) then ("-c " +  '"' + least_one_estimated + '"') else ""} \
  ...
  parameter_meta {
    ...
    least_one_estimated: "if at least one of the estimated p-distances is above this specified\\ncutoff, then a F81 correction is performed (default: 0.1)"
    bme_tree_inference: "no BME tree inference (only pairwise distance estimation)"
    number_when_performing: "number of steps  when performing the  ratchet-based  BME tree search\\n(default: 100)"
