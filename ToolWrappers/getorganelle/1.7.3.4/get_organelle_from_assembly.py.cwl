...
- id: in_flag_fungusmtfungus_rna
  doc: "This flag should be followed with embplant_pt\n(embryophyta plant plastome),\
    ...
    \ or embplant_mt,other_pt,fungus_mt (the\ncombination of any of above organelle\
    \ genomes split by\ncomma(s), which might be computationally more\nintensive than\
    \ separate runs), or anonym (uncertain\norganelle genome type, with customized\
    \ gene database\n('--genes'), which is suggested only when the above\ndatabase\
    \ is genetically distant from your sample) or\nraw (disentangle the raw graph\
    \ directly without\ntagging)."
  type: string?
  inputBinding:
    prefix: -F
