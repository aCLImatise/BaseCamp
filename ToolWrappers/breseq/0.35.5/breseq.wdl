...
task Breseq {
  ...
  command <<<
    breseq \
      ...
      ~{if defined(genbank_field_for_seq_id) then ("--genbank-field-for-seq-id " +  '"' + genbank_field_for_seq_id + '"') else ""} \
  ...
  parameter_meta {
    ...
    genbank_field_for_seq_id: "Which GenBank header field will be used to assign sequence IDs. Valid choices are LOCUS, ACCESSION, and VERSION. The default is to check those fields, in that order, for the first one that exists. If you override the default, you will need to use the converted reference file (data/reference.gff) for further breseq and gdtools operations on breseq output! (DEFAULT=AUTOMATIC)"
  ...
  output {
    ...
    File out_genbank_field_for_seq_id = "${in_genbank_field_for_seq_id}"
