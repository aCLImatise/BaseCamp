...
task Diamond {
  ...
  command <<<
    diamond \
      ...
      ~{if (k_parameter_custom) then "--K" else ""} \
      ...
      ~{if (rank_ratio) then "--rank-ratio" else ""} \
  ...
  parameter_meta {
    ...
    max_hsps: "maximum number of HSPs per subject sequence to report for each query (default=unlimited)"
    ...
    k_parameter_custom: "K parameter for custom matrix"
    comp_based_stats: "enable composition based statistics (0/1=default)"
    ...
    rank_ratio: "include subjects within this ratio of last hit"
