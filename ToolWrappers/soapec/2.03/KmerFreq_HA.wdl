...
task KmerFreqHA {
  input {
    Int? set_kmer_size
    Int? set_read_length
    Int? set_ignored_length_beginning_defaultnote_set_option
    Int? set_ignored_length_end_defaultnote_set_option
    Int? set_total_number
    File? set_input_read
    String? set_output_prefix
    Int? set_initial_size
    Int? set_thread_number
    Int? set_maximum_length
    Int? set_use_filter
    Int? set_bloom_size
  ...
  command <<<
    KmerFreq_HA \
      ...
      ~{if defined(set_ignored_length_beginning_defaultnote_set_option) then ("-a " +  '"' + set_ignored_length_beginning_defaultnote_set_option + '"') else ""} \
      ~{if defined(set_ignored_length_end_defaultnote_set_option) then ("-d " +  '"' + set_ignored_length_end_defaultnote_set_option + '"') else ""} \
      ...
      ~{if defined(set_maximum_length) then ("-L " +  '"' + set_maximum_length + '"') else ""} \
  ...
  parameter_meta {
    ...
    set_ignored_length_beginning_defaultnote_set_option: "Set ignored length of the beginning of a read, default=0.\\nNote: set this option when sequencing quality is low in the beginning of reads."
    set_ignored_length_end_defaultnote_set_option: "Set ignored length of the end of a read, default=0\\nNote: set this option when sequencing quality is low in the end of reads."
