...
task SamtoolsView {
  input {
    ...
    File? only_include_reads_read_group
    File? only_include_reads_read_name
    Int? only_include_reads_tag
    ...
    String? only_include_reads_matching
  ...
  command <<<
    samtools view \
      ...
      ~{if defined(only_include_reads_read_group) then ("-R " +  '"' + only_include_reads_read_group + '"') else ""} \
      ~{if defined(only_include_reads_read_name) then ("-N " +  '"' + only_include_reads_read_name + '"') else ""} \
      ~{if defined(only_include_reads_tag) then ("-d " +  '"' + only_include_reads_tag + '"') else ""} \
      ...
      ~{if defined(only_include_reads_matching) then ("-e " +  '"' + only_include_reads_matching + '"') else ""} \
  ...
  parameter_meta {
    ...
    only_include_reads_read_group: "only include reads with read group listed in FILE [null]"
    only_include_reads_read_name: "only include reads with read name listed in FILE [null]"
    only_include_reads_tag: "[:STR2]\\nonly include reads with tag STR1 and associated value STR2 [null]\\nThe value can be omitted, in which case only the tag is considered"
    ...
    only_include_reads_matching: "only include reads matching the filter expression [null]"
