...
task Vardictjava {
  input {
    ...
    Boolean? chimeric
    Boolean? debug
    String? delimiter_split_regioninfo
    Boolean? del_dup_var
  ...
  command <<<
    vardict_java \
      ...
      ~{if defined(delimiter_split_regioninfo) then ("-d " +  '"' + delimiter_split_regioninfo + '"') else ""} \
      ...
      ~{if defined(reference_fasta_should) then ("-G " +  '"' + reference_fasta_should + '"') else ""} \
      ...
      ~{if defined(set_reads_mapping) then ("-Q " +  '"' + set_reads_mapping + '"') else ""} \
  ...
  parameter_meta {
    ...
    delimiter_split_regioninfo: "The delimiter for split region_info, default to tab \\\"\\t\\\""
    ...
    crispr_mode_amount: "In CRISPR mode, the minimum amount in bp that a read needs to overlap with cutting site.  If a read does\\nnot meet the criteria,\\nit will not be used for variant calling, since it is likely just a partially amplified PCR.  Default: not\\nset, or no filtering"
    ...
    set_reads_mapping: "If set, reads with mapping quality less than INT will be filtered and ignored"
