...
task Sparse {
  ...
  command <<<
    sparse \
      ...
      ~{if defined(in_del_loop) then ("--indel-loop " +  '"' + in_del_loop + '"') else ""} \
      ~{if defined(in_del_opening) then ("--indel-opening " +  '"' + in_del_opening + '"') else ""} \
      ~{if defined(in_del_opening_loop) then ("--indel-opening-loop " +  '"' + in_del_opening_loop + '"') else ""} \
      ...
      ~{if (special_gap_symbols) then "--special-gap-symbols" else ""} \
      ~{if (stopwatch) then "--stopwatch" else ""} \
      ~{if defined(min_prob) then ("--min-prob " +  '"' + min_prob + '"') else ""} \
      ~{if defined(prob_unpaired_in_loop_threshold) then ("--prob-unpaired-in-loop-threshold " +  '"' + prob_unpaired_in_loop_threshold + '"') else ""} \
      ~{if defined(prob_base_pair_in_loop_threshold) then ("--prob-basepair-in-loop-threshold " +  '"' + prob_base_pair_in_loop_threshold + '"') else ""} \
  ...
  parameter_meta {
    ...
    constraints_dot: "Report bugs to <miladim (at) informatik.uni-freiburg.de>."
  }
  output {
