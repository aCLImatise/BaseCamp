...
task GappaAnalyzePhylogenetickmeans {
  input {
    File? j_place_path
  ...
  command <<<
    gappa analyze phylogenetic_kmeans \
  ...
  parameter_meta {
    ...
    bins: "=0               Bin the masses per-branch in order to save time and memory, with only minor differences in the cluster assignments. Default is 0, that is, no binning. If set, we recommend to use 50 bins or more."
    ...
    file_prefix: "=pkmeans_ File prefix for output files"
