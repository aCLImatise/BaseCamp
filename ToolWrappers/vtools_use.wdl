...
task VtoolsUse {
  input {
    String? verbosity
    String? alternative_name_linked
    Boolean? list_fields_used
    String? an_no_type
    Boolean? linked_fields
    Boolean? list_specified_vtools
    Boolean? rebuild
    Int? jobs
    String? field
  }
  command <<<
    vtools use \
      ...
      ~{if defined(alternative_name_linked) then ("--as " +  '"' + alternative_name_linked + '"') else ""} \
