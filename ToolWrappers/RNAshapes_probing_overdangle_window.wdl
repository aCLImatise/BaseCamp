...
task RNAshapesProbingOverdangleWindow {
  input {
    Int? specify_window_size
    Int? specify_window_incrementdefault
    Float? rescale_energy_parameters
  ...
  command <<<
    RNAshapes_probing_overdangle_window \
      ~{if defined(specify_window_size) then ("-w " +  '"' + specify_window_size + '"') else ""} \
      ~{if defined(specify_window_incrementdefault) then ("-i " +  '"' + specify_window_incrementdefault + '"') else ""} \
      ~{if defined(rescale_energy_parameters) then ("-T " +  '"' + rescale_energy_parameters + '"') else ""} \
      ...
      ~{if defined(var_23) then ("-A " +  '"' + var_23 + '"') else ""} \
  ...
  >>>
  parameter_meta {
    specify_window_size: "Specify the window size."
    specify_window_incrementdefault: "Specify the window increment.\\nDefault is 1. Use with -w."
    rescale_energy_parameters: "Rescale energy parameters to a temperature of <float-value> C.\\nDefault is 37C."
    ...
    var_23: "sets the 'slope' for the RNAstructure inspired formula\\nof how to combine free energies and reactivities [1.8]"
