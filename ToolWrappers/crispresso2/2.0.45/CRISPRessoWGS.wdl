...
task CRISPRessoWGS {
  ...
  command <<<
    CRISPRessoWGS \
      ...
      ~{if defined(fasta_format_reference) then ("--reference_file " +  '"' + fasta_format_reference + '"') else ""} \
  ...
  parameter_meta {
    ...
    fasta_format_reference: "A FASTA format reference file (for example hg19.fa for\\nthe human genome) (default: )"
    ...
    n_processes: "The number of processes to use for the quantification.\\nPlease use with caution since increasing this\\nparameter will increase the memory required to run\\nCRISPResso. Can be set to 'max'. (default: 1)"
    cris_presso_command: "CRISPResso command to call (default: CRISPResso)\\n"
    var_82: "[--min_reads_to_use_region MIN_READS_TO_USE_REGION]"
  }
  output {
