...
task NwDisplay {
  ...
  command <<<
    nw_display \
      ~{if defined(rotate_labels_default) then ("-a " +  '"' + rotate_labels_default + '"') else ""} \
      ~{if defined(rotate_leftside_labels) then ("-A " +  '"' + rotate_leftside_labels + '"') else ""} \
      ~{if defined(css_branch_default) then ("-b " +  '"' + css_branch_default + '"') else ""} \
      ~{if defined(use_specified_file_css) then ("-c " +  '"' + use_specified_file_css + '"') else ""} \
      ~{if defined(css_tree_nodes) then ("-d " +  '"' + css_tree_nodes + '"') else ""} \
      ~{if defined(graph_style_text) then ("-e " +  '"' + graph_style_text + '"') else ""} \
      ~{if defined(css_inner_default) then ("-i " +  '"' + css_inner_default + '"') else ""} \
      ~{if defined(sets_position_inner) then ("-I " +  '"' + sets_position_inner + '"') else ""} \
      ~{if defined(css_leaf_default) then ("-l " +  '"' + css_leaf_default + '"') else ""} \
      ~{if defined(add_number_pixels) then ("-n " +  '"' + add_number_pixels + '"') else ""} \
      ~{if defined(use_specified_file_works) then ("-o " +  '"' + use_specified_file_works + '"') else ""} \
      ~{if defined(integer_is_use) then ("-R " +  '"' + integer_is_use + '"') else ""} \
      ~{if (draw_radial_default) then "-r" else ""} \
      ~{if (output_graph_svg) then "-s" else ""} \
      ~{if (suppress_scale_bar) then "-S" else ""} \
      ~{if (set_zero_is) then "-t" else ""} \
      ~{if defined(string_scale_bar) then ("-u " +  '"' + string_scale_bar + '"') else ""} \
  ...
  parameter_meta {
    ...
    css_tree_nodes: ": CSS for 'plain' tree nodes (i.e., unless overridden by -c)\\n[only SVG]\\nDefault: stroke:black;fill:none;stroke-width:1;stroke-linecap:round"
