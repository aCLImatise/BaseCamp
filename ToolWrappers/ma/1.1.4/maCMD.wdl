...
task MaCMD {
  input {
    ...
    Int? mean_distance_of_paired_reads
    ...
    Int? maximal_number_of_socs
    Int? minimal_number_of_socs
    Int? fixed_soc_width
  ...
  command <<<
    maCMD \
      ...
      ~{if defined(mean_distance_of_paired_reads) then ("--Mean_Distance_of_Paired_Reads " +  '"' + mean_distance_of_paired_reads + '"') else ""} \
      ...
      ~{if defined(maximal_number_of_socs) then ("--Maximal_Number_of_SoCs " +  '"' + maximal_number_of_socs + '"') else ""} \
      ~{if defined(minimal_number_of_socs) then ("--Minimal_Number_of_SoCs " +  '"' + minimal_number_of_socs + '"') else ""} \
      ...
      ~{if defined(use_m_in_cigar) then ("--Use_M_in_CIGAR " +  '"' + use_m_in_cigar + '"') else ""} \
      ~{if defined(output_long_cigars_in_cg_tag) then ("--Output_long_cigars_in_CG_tag " +  '"' + output_long_cigars_in_cg_tag + '"') else ""} \
      ~{if defined(soft_clip) then ("--Soft_clip " +  '"' + soft_clip + '"') else ""} \
      ...
      ~{if defined(minimizer_window_size) then ("--Minimizer_Window_Size " +  '"' + minimizer_window_size + '"') else ""}
  >>>
  runtime {
    docker: "quay.io/biocontainers/ma:1.1.4--py37h8270d21_1"
  }
  parameter_meta {
    ...
    maximal_number_of_socs: "[30]        Consider the <val> best scored SoCs merely. 0 =\\nConsider all SoCs."
    minimal_number_of_socs: "[1]         Always consider the first <val> SoCs no matter the\\nHeuristic optimizations. Increasing this parameter\\nmight improve the quality of supplementary\\nalignments."
    ...
    use_m_in_cigar: "[true]                 Disabled: Distinguish matches and mismatches in\\nCIGARs using '=' and 'X' operations. Enabled: Use\\nthe 'M' operation in CIGARs."
    ...
    disable_all_heuristics: "[false]        Disables all runtime heuristics. (For debugging\\npurposes)"
    ...
    bandwidth_for_extensions: "[512]         Bandwidth used in the context of the extension of\\nan alignment towards the endpoints of its read.\\n(See 'Padding')"
