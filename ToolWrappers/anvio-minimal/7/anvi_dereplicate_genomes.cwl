...
- id: in_skip_fast_a_report
  doc: "By default, if any sequence source is provided, FASTA\nfiles of non-redundant\
    \ genomes are reported. With this\nflag, no FASTA files are reported. (default:\
    \ False)"
  type: boolean?
  inputBinding:
...
- id: in_cluster_method
  doc: "Currently, genomes are clustered based on a simple\ngreedy algorithm. Let's\
    ...
    \ the developers with your ideas.\n(default: simple_greedy)"
  type: string?
  inputBinding:
...
- id: in_just_do_it
  ...
  inputBinding:
    prefix: --just-do-it
- id: in_skip_checking_genome_hashes
  doc: "Use this flag if you would like anvi'o to skip\nchecking genome hashes. This\
    \ is only relevant if you\nmay have genomes in your internal or external genomes\n\
    files that have identical sequences with different\nnames AND if you are OK with\
    \ it. You may be OK with\nit, for instance, if you are using `anvi-dereplicate-\n\
    genomes` program to dereplicate genomes desribed in\nmultiple collections in an\
  ...
  type: boolean?
  inputBinding:
    prefix: --skip-checking-genome-hashes
- id: in_log_file
  ...
  inputBinding:
    prefix: --log-file
- id: in_fast_a
  doc: '🍺 More on `anvi-dereplicate-genomes`:'
  type: string
  inputBinding:
    position: 0
outputs:
