...
- id: in_output_db_path
  doc: "Output file path for the new database. (default:\nCONTIGS.db)"
  type: File?
  inputBinding:
...
- id: in_skip_mindful_splitting
  doc: "By default, anvi'o attempts to prevent soft-splitting\nlarge contigs by cutting\
    ...
    \ the projects that rely on a contigs\ndatabase that is initiated this way). (default:\
    \ False)"
  type: boolean?
  inputBinding:
...
- id: in_external_gene_calls
  doc: "A TAB-delimited file to define external gene calls.\nThe file must have these\
...
- id: in_ignore_internal_stop_codons
  doc: "This is only relevant when you have an external gene\ncalls file. If anvi'o\
    ...
    \ failures at the first place\n(smiley). (default: False)"
  type: File?
  inputBinding:
    prefix: --ignore-internal-stop-codons
- id: in_skip_predict_frame
  doc: "When you have provide an external gene calls file,\nanvi'o will predict the\
...
- id: in_contigs_db
  doc: '🍺 More on `anvi-gen-contigs-database`:'
  type: string
  inputBinding:
    position: 0
outputs:
