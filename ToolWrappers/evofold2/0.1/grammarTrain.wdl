...
task GrammarTrain {
  ...
  command <<<
    grammarTrain \
      ~{grammars_dot} \
      ~{if (arg_file_used) then "-t" else ""} \
      ~{if (arg_name_use) then "-n" else ""} \
      ~{if (arg_defines_number) then "-p" else ""} \
