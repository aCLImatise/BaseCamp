...
task LtrFinder {
  input {
    ...
    Int? max_gap_joined
    Int? max_gap_rt
    Int? min_subdomains_found
    Int? threshold_join_sequence
    Int? threshold_split_existed
    Int? output_limit_default
    Int? min_ltr_similarity
    Int? boundary_alignment_sharpness_threshold_higher
    Int? boundary_alignment_sharpness_threshold_oneboth
    Int? pbs_detecting_length
    Int? output_format_full
    Int? output_alignment_lengthonly
    String? seqids_only_calculate
    File? trna_sequence_format
    File? data_file_used
    String? use_psscan_predict
    Boolean? output_html_format
    Boolean? ltr_have_edge
    Boolean? detect_repeat_regions
  ...
  command <<<
    ltr_finder \
      ...
      ~{if defined(min_subdomains_found) then ("-T " +  '"' + min_subdomains_found + '"') else ""} \
      ~{if defined(threshold_join_sequence) then ("-j " +  '"' + threshold_join_sequence + '"') else ""} \
      ~{if defined(threshold_split_existed) then ("-J " +  '"' + threshold_split_existed + '"') else ""} \
      ~{if defined(output_limit_default) then ("-S " +  '"' + output_limit_default + '"') else ""} \
      ~{if defined(min_ltr_similarity) then ("-M " +  '"' + min_ltr_similarity + '"') else ""} \
      ~{if defined(boundary_alignment_sharpness_threshold_higher) then ("-B " +  '"' + boundary_alignment_sharpness_threshold_higher + '"') else ""} \
      ~{if defined(boundary_alignment_sharpness_threshold_oneboth) then ("-b " +  '"' + boundary_alignment_sharpness_threshold_oneboth + '"') else ""} \
      ~{if defined(pbs_detecting_length) then ("-r " +  '"' + pbs_detecting_length + '"') else ""} \
      ~{if defined(output_format_full) then ("-w " +  '"' + output_format_full + '"') else ""} \
      ~{if defined(output_alignment_lengthonly) then ("-O " +  '"' + output_alignment_lengthonly + '"') else ""} \
      ~{if defined(seqids_only_calculate) then ("-P " +  '"' + seqids_only_calculate + '"') else ""} \
      ~{if defined(trna_sequence_format) then ("-s " +  '"' + trna_sequence_format + '"') else ""} \
      ...
      ~{if (ltr_have_edge) then "-E" else ""} \
      ~{if (detect_repeat_regions) then "-C" else ""} \
  ...
  parameter_meta {
    ...
    min_subdomains_found: "Min sub-domains found in a RT domain, default is 4"
    threshold_join_sequence: "Threshold for join new sequence in existed alignment\\nnew alignment similarity higher than this will be joined,\\ndefault is 0.70"
    ...
    output_limit_default: "output Score limit, default is 6.00, [0,10]"
    min_ltr_similarity: "min LTR similarity threshold, default is 0.00, [0,1]"
    boundary_alignment_sharpness_threshold_higher: "Boundary alignment sharpness threshold, higher one.\\none of the two edge's sharpness must higher than\\nthis threshold, default is 0.400, [0,1]"
    ...
    pbs_detecting_length: "PBS detecting threshold, min tRNA match length: 14, [1,18]"
    output_format_full: "output format: [0]-full, 1-summary, 2-table."
    output_alignment_lengthonly: "output alignment length(only affect -w0), default is 40"
    seqids_only_calculate: "SeqIDs, will only calculate matched SeqID\\nPOSIX style regular express is supported."
    trna_sequence_format: "tRNA sequence file(FASTA format)"
    ...
    ltr_have_edge: "LTR must have edge signal\\n(at least two of PBS,PPT,TSR)"
    detect_repeat_regions: "detect Centriole, delete highly repeat regions"
