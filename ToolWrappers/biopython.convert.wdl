...
task Biopythonconvert {
  ...
  command <<<
    biopython_convert \
      ...
      ~{if defined(select_records_must) then ("-q " +  '"' + select_records_must + '"') else ""} \
  ...
  parameter_meta {
    ...
    select_records_must: "to select records. Must return list of SeqIO records. Root is list of input SeqIO records."
