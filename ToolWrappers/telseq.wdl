...
task Telseq {
  ...
  command <<<
    telseq \
      ...
      ~{if (ignore_read_groups) then "-u" else ""} \
      ...
      ~{if (consider_bams_speicfied) then "-w" else ""}
  >>>
  parameter_meta {
    ...
    ignore_read_groups: "ignore read groups. Treat all reads in BAM as if they were from a same read group."
    ...
    consider_bams_speicfied: ",                      consider BAMs in the speicfied bamlist as one single BAM. This is useful when\\nthe initial alignemt is separated for some reason, such as one for mapped and one for ummapped reads."
    in_dot_one_dot_bam: ""
    in_dot_two_dot_bam: ""
  }
  output {
