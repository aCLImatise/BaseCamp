...
task Bcov {
  ...
  command <<<
    bcov \
      ...
      ~{if defined(read_strand_boundaries) then ("-s " +  '"' + read_strand_boundaries + '"') else ""} \
  ...
  parameter_meta {
    ...
    read_strand_boundaries: ":        Read beta strand boundaries from file. REQUIRED."
