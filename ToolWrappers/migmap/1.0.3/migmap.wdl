...
task Migmap {
  input {
    ...
    Int? threshold_average_nregions
    Boolean? chain_receptor_gene
    File? report
    String? species_allowed_rabbitrhesusmonkey
    Boolean? unmapped
    String mapping_dot
    File file_dot
  }
  command <<<
    migmap \
      ...
      ~{if defined(species_allowed_rabbitrhesusmonkey) then ("-S " +  '"' + species_allowed_rabbitrhesusmonkey + '"') else ""} \
  ...
  parameter_meta {
    ...
    number_use_default: "Number of cores to use. [default =\\nall available processors]"
    threshold_average_nregions: "Threshold for average quality of\\nmutations and N-regions of CDR3\\n[default = 25]"
    ...
    species_allowed_rabbitrhesusmonkey: "Species. Allowed values: [human,\\nmouse, rat, rabbit,\\nrhesus_monkey]. [required]"
