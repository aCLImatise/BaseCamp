...
task DUDespy {
  ...
  command <<<
    DUDes_py \
      ...
      ~{if defined(keep_reads_default) then ("-m " +  '"' + keep_reads_default + '"') else ""} \
  ...
  parameter_meta {
    ...
    keep_reads_default: "Keep reads up to this number/percentile of matches (0:\\noff / 0-1: percentile / >=1: match count). Default: 0"
