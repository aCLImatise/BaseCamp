...
task Snntest {
  input {
    ...
    Int? get_pvalues_nperms
    Boolean? only_analyze_considers
  }
  command <<<
    snntest \
      ...
      ~{if (only_analyze_considers) then "-b" else ""}
  >>>
  runtime {
    docker: "None"
  }
  parameter_meta {
    ...
    only_analyze_considers: ": only analyze bi-allelic sites (only considers the ingroup!)"
  }
  output {
