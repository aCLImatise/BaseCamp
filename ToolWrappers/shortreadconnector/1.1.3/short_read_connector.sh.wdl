...
task ShortReadConnectorsh {
  input {
    ...
    String? kmers_present_least
    Boolean? incompatible_srccounter_output
  }
  command <<<
    short_read_connector_sh \
      ~{if defined(bankexample_b_datacfastagz) then ("-b " +  '"' + bankexample_b_datacfastagz + '"') else ""} \
      ...
      ~{if defined(see_option_windows) then ("-w " +  '"' + see_option_windows + '"') else ""} \
      ...
      ~{if defined(kmers_present_least) then ("-A " +  '"' + kmers_present_least + '"') else ""} \
  ...
  parameter_meta {
    bankexample_b_datacfastagz: "for bank\\nExample: -b data/c1.fasta.gz"
    ...
    see_option_windows: "window_size. See option -s. If the windows size is zero (default value), then the full read is considered. Default=0"
    ...
    kmers_present_least: "kmers present at least 'kmer_abundance_min' times in the bank AND in the queries."
