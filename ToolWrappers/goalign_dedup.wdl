...
task GoalignDedup {
  input {
    File? log
    File? deduplicated_output_alignment
    Boolean? unaligned
    File? align
    Boolean? auto_detect
    Boolean? clustal
  ...
  command <<<
    goalign dedup \
      ...
      ~{if defined(deduplicated_output_alignment) then ("--output " +  '"' + deduplicated_output_alignment + '"') else ""} \
  ...
  output {
    ...
    File out_log = "${in_log}"
    File out_deduplicated_output_alignment = "${in_deduplicated_output_alignment}"
  }
}