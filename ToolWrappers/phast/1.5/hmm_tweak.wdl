...
task HmmTweak {
  input {
    ...
    File? required_assume_given
    String? use_from_states
    String? use_to_states
  ...
  command <<<
    hmm_tweak \
      ...
      ~{if defined(required_assume_given) then ("-u " +  '"' + required_assume_given + '"') else ""} \
      ~{if defined(use_from_states) then ("-F " +  '"' + use_from_states + '"') else ""} \
      ~{if defined(use_to_states) then ("-T " +  '"' + use_to_states + '"') else ""} \
  ...
  parameter_meta {
    ...
    required_assume_given: "(Required with -i) Assume given tree topology (.nh file)."
    ...
    use_to_states: "(For use with -i) Operate on transitions to states corresp.\\nto the specified gap-pattern numbers (ANDed with -t)."
