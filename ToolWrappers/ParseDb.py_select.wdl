...
task ParseDbpySelect {
  input {
    Array[String] list_tab_defaultnone
    Array[String] explicit_output_file
    File? outdir
    File? out_name
    Array[String] name_fields_check
    Array[String] values_defining_select
    String? logic
    Boolean? regex
  }
  command <<<
    ParseDb_py select \
      ...
      ~{if defined(name_fields_check) then ("-f " +  '"' + name_fields_check + '"') else ""} \
      ~{if defined(values_defining_select) then ("-u " +  '"' + values_defining_select + '"') else ""} \
  ...
  parameter_meta {
    ...
    name_fields_check: "The name of the fields to check for selection\\ncriteria. (default: None)"
    values_defining_select: "The values defining with records to select. A value\\nmay appear in any of the fields specified with -f.\\n(default: None)"
