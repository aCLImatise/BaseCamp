...
task NextflowbakKuberun {
  ...
  command <<<
    nextflow_bak kuberun \
      ...
      ~{if (main_script) then "-main-script" else ""} \
      ...
      ~{if (pod_image) then "-pod-image" else ""} \
      ...
      ~{if (volume_mount) then "-volume-mount" else ""} \
      ...
      ~{if (with_pod_man) then "-with-podman" else ""} \
  ...
  parameter_meta {
    ...
    dsl_two: "Execute the workflow using DSL2 syntax\\nDefault: false"
    ...
    main_script: "The script file to be executed when launching a project directory or\\nrepository"
    assign_mnemonic_name: "Assign a mnemonic name to the a pipeline run"
    namespace: "Specify the K8s namespace to use"
    offline: "Do not check for remote project updates\\nDefault: false"
    params_file: "Load script parameters from a JSON/YAML file"
    pod_image: "Specify the container image for the Nextflow pod"
    ...
    volume_mount: "Volume claim mounts eg. my-pvc:/mnt/path"
