...
- id: in_file_formatoptional_values
  ...
  inputBinding:
    prefix: -f
- id: in_filter
  doc: Store entries in <prefix>_good.txt and <prefix>_bad.txt files in the working
    directory. Entries with errors are stored in the bad file and entries without
    errors are stored in the good file. (optional)(default :false)
  type: File?
  inputBinding:
    prefix: -filter
- id: in_fix
...
- id: in_remote_run_ebioptional
  ...
  inputBinding:
    prefix: -r
- id: in_skip
  doc: ',<errorcode2>,... Ignore specified errors.(optional)(default:false)'
  type: long?
  inputBinding:
    prefix: -skip
- id: in_version
