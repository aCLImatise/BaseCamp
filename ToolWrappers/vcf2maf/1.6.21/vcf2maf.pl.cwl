...
- id: in_vep_forks
  ...
  inputBinding:
    prefix: --vep-forks
- id: in_vep_custom
  doc: String to pass into VEP's --custom option []
  type: boolean?
  inputBinding:
    prefix: --vep-custom
- id: in_vep_config
  doc: Config file to pass into VEP's --config option []
  type: boolean?
  inputBinding:
    prefix: --vep-config
- id: in_vep_overwrite
  doc: Allow VEP to overwrite output VCF if it exists
  type: boolean?
  inputBinding:
    prefix: --vep-overwrite
- id: in_buffer_size
...
- id: in_ncbi_build
  ...
  inputBinding:
    prefix: --ncbi-build
- id: in_cache_version
  doc: 'Version of offline cache to use with VEP (e.g. 75, 91, 102) [Default: Installed
    version]'
  type: boolean?
  inputBinding:
...
- id: in_retain_fmt
  ...
  inputBinding:
    prefix: --retain-fmt
- id: in_retain_ann
  doc: Comma-delimited names of annotations (within the VEP CSQ/ANN) to retain as
...
- id: in_remap_chain
  ...
  inputBinding:
    prefix: --remap-chain
- id: in_verbose
  doc: Print more things to log progress
  type: boolean?
  inputBinding:
    prefix: --verbose
- id: in_man
