...
task Rmatspy {
  ...
  command <<<
    rmats_py \
      ...
      ~{if (allow_clipping) then "--allow-clipping" else ""} \
      ~{if defined(fixed_event_set) then ("--fixed-event-set " +  '"' + fixed_event_set + '"') else ""}
  >>>
  runtime {
  ...
  parameter_meta {
    ...
    od: "The directory for final output from the post step"
    ...
    t_stat: "The number of threads for the statistical model. If\\nnot set then the value of --nthread is used"
    ...
    fixed_event_set: "A directory containing fromGTF.[AS].txt files to be\\nused instead of detecting a new set of events\\n"
  }
  output {
    File out_stdout = stdout()
    Directory out_od = "${in_od}"
    Directory out_tmp = "${in_tmp}"
    File out_specify_steps_run = "${in_specify_steps_run}"
  }
}