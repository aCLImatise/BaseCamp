...
- id: in_branch_lengths
  doc: "/ -a [r/n]       print branch lengths, the separator can be\ngiven as first\
...
- id: in_change_names
  doc: "/ -c [taxa]        change the name of tips. Tip names to be\nchanged should\
...
- id: in_cluster
  ...
  inputBinding:
    prefix: --cluster
- id: in_depth
  doc: "/ -D                      get the longest distance from the root to any\n\
    of the tips."
  type: boolean
  inputBinding:
    prefix: --depth
- id: in_distances_to_root
  doc: "/ -z [sep.]   output the number of nodes and branch length\ndistance to the\
    \ root for each tip. The\nseparator between tip name and value can be\nspecified,\
...
- id: in_drop_tips
  doc: "/ -d [taxa]           drop the given tips from the tree, e.g. -d\ntaxon1,taxon2,taxon3."
  type: boolean
  inputBinding:
    prefix: --drop_tips
- id: in_get_tip_names
  doc: "/ -t [sep.]       get the names of the tips in the tree, a\nseparator can\
...
- id: in_file
  doc: / -f [file]                give file name, e.g. -f file.tree.
  type: boolean
  inputBinding:
    prefix: --file
- id: in_format
...
- id: in_interval
  ...
  inputBinding:
    prefix: --interval
- id: in_inverse
  doc: "/ -i                    inverse the string of taxa, e.g. drop all tips\nbut\
    \ the given. E.g -d taxon1,taxon2,taxon3 -i"
  type: boolean
  inputBinding:
    prefix: --inverse
...
- id: in_ladder_ize
  doc: "/ -l                  laddrize the tree. If followed by l - left\nladderize,\
    \ if followed by r - right ladderize\n(default), e.g. -l r."
  type: boolean
  inputBinding:
    prefix: --ladderize
...
- id: in_mid_point_root
  doc: / -m             root the tree at the mid point.
  type: boolean
  inputBinding:
...
- id: in_no_branch_length
  doc: "/ -0           do not print branch lengths. If there are no\nbranch lengths\
    \ in input tree the default is to\nprint zero length branches in the out tree.\n\
...
- id: in_number_of_tax_a
  doc: / -n             get the number of tips/taxa in the tree.
  type: boolean
  inputBinding:
    prefix: --number_of_taxa
- id: in_out_group_root
  doc: "/ -o [taxa]         root using most recent common ancestor of given\ntaxa,\
    \ e.g. -o taxa1,taxa2."
  type: boolean
  inputBinding:
...
- id: in_patristic_distances
  doc: "/ -p [sep.] get the total patristic distance to all other\ntaxa in the tree\
    \ for each taxon, the separator\nbetween different taxa, and the separator\nbetween\
...
- id: in_random_tree
  doc: "/ -r                get a random topology (no branch lengths) with\ngiven\
    \ number of taxa, e.g. -r 20 (default 0).\nNumber of random trees can be given\
    \ behind a\ncolon (:), e.g. -r 20:100."
  type: boolean
  inputBinding:
    prefix: --random_tree
...
- id: in_set_branch_lengths
  doc: "/ -b [value] set all branches in the tree to the given\nvalue, e.g. 0.5 (default\
    \ 1.0)."
  type: boolean
  inputBinding:
...
- id: in_split_stop
  ...
  inputBinding:
    prefix: --split_stop
- id: in_rooted
  doc: "/ -R                     sets if the tree should be considered as\nrooted\
    \ or not (only matters when splitting\ntrees)."
  type: boolean
  inputBinding:
    prefix: --rooted
- id: in_sum_branch_length
  doc: "/ -s          get the sum of the branch lengths in the tree\n(including root\
