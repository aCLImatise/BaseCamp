...
- id: in_add_to_support
  doc: "add one to the value of the internal node for\neach tree that that split is\
    \ present in."
  type: boolean?
  inputBinding:
    prefix: --add_to_support
- id: in_calculate_support
  doc: "same as --add_to_support but divide the number\nby number of trees compared\
    \ against."
  type: boolean?
  inputBinding:
    prefix: --calculate_support
- id: in_compare
  doc: "[value]          output conflicting splits where at least one\nbranch support\
    \ the conflict with more than given\nsupport, e.g. -c 0.7."
  type: boolean?
  inputBinding:
    prefix: --compare
- id: in_database
  doc: "[file]          give a second file of trees to compare against\ninstead of\
    \ comparing within the ordinary input.\nIf - is given input will be taken from\
  ...
  inputBinding:
    prefix: --database
- id: in_decisiveness
  doc: "[genes]     calculates proportion of random trees for which\ngiven gene sampling\
    \ is decisive and the mean\nproportion of branches that are distinguished.\n(Sanderson\
    \ et al. 2010, BMC Evol. Biol. 19:\n155). The genes for each taxon are given as\
  ...
  inputBinding:
    prefix: --decisiveness
- id: in_iterations
  doc: "[number]      give numbers of iterations to do when\ncalculating decisiveness,\
    \ e.g. -i 1000."
  type: boolean?
  inputBinding:
    prefix: --iterations
- id: in_file
  doc: "[file]              give file name for trees, or decisiveness, e.g.\n-f file.tree."
  type: boolean?
  inputBinding:
    prefix: --file
- id: in_format
...
- id: in_rooted
  doc: "treat trees as rooted for --add_to_support and\n--calculate_support."
  type: boolean?
  inputBinding:
    prefix: --rooted
- id: in_verbose
