...
task Toilcwlrunner {
  input {
    ...
    File? beta_dependency_resolvers_configuration
  ...
  command <<<
    toil_cwl_runner \
      ...
      ~{job_store} \
      ~{provisioning_dot} \
      ~{if defined(not_strict) then ("--not-strict " +  '"' + not_strict + '"') else ""} \
      ...
      ~{if defined(default_preempt_able) then ("--defaultPreemptable " +  '"' + default_preempt_able + '"') else ""} \
  ...
  parameter_meta {
    not_strict: "Enable loading and running development versions of CWL"
    ...
    beta_dependency_resolvers_configuration: "Path prefix for temporary directories"
