...
- id: in_click
  ...
  inputBinding:
    prefix: --click
- id: in_force_norm
  doc: "Re-compute matrix normalization (balancing) and\noverwrite weights present\
    \ in the cool files instead\nof reusing them."
  type: boolean
  inputBinding:
    prefix: --force-norm
- id: in_inter
...
- id: in_path_contact_map
  ...
  inputBinding:
    position: 1
- id: in_output_directory_generated
  doc: output directory where files should be generated.
  type: string
  inputBinding:
    position: 2
- id: in_prefix
  doc: "Path prefix for config files. If prefix is a/b,\nfiles a/b.json and a/b.1.txt\
    \ will be generated.\nIf a given pattern has N kernel matrices, N txt\nfiles are\
    \ created they will be named a/b.[1-N].txt."
  type: string
  inputBinding:
    position: 0
outputs:
