...
task FilterSeqpyLength {
  ...
  command <<<
    FilterSeq_py length \
      ...
      ~{if defined(minimum_sequence_length) then ("-n " +  '"' + minimum_sequence_length + '"') else ""} \
  ...
  parameter_meta {
    ...
    minimum_sequence_length: "Minimum sequence length to retain. (default: 250)"
