...
task Rascaf {
  ...
  command <<<
    rascaf \
      ...
      ~{if defined(recommended_paths_raw) then ("-f " +  '"' + recommended_paths_raw + '"') else ""} \
      ...
      ~{if defined(size_kmer_you) then ("-k " +  '"' + size_kmer_you + '"') else ""} \
      ~{if (cs) then "-cs" else ""} \
  ...
  parameter_meta {
    ...
    recommended_paths_raw: "(recommended): the paths to the raw assembly fasta file(default: not used)"
    ...
    size_kmer_you: ": the size of a kmer(<=32; <=0 if you do not want to use kmer. default: 23)"
