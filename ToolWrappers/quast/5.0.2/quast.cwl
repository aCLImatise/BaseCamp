...
- id: in_output_dir
  ...
  inputBinding:
    prefix: --output-dir
- id: in_filename_reference_genome
  doc: <filename>      Reference genome file
  type: boolean?
  inputBinding:
    prefix: -r
- id: in_features
  doc: "[type:]<filename>  File with genomic feature coordinates in the reference\
...
- id: in_k_mer_size
  ...
  inputBinding:
    prefix: --k-mer-size
- id: in_gene_finding
  doc: "Predict genes using GeneMarkS (prokaryotes, default) or GeneMark-ES (eukaryotes,\
    \ use --eukaryote)\n--mgm                             Use MetaGeneMark for gene\
    \ prediction (instead of the default finder, see above)\n--glimmer           \
    \              Use GlimmerHMM for gene prediction (instead of the default finder,\
    \ see above)\n--gene-thresholds <int,int,...>   Comma-separated list of threshold\
    \ lengths of genes to search with Gene Finding module\n[default: 0,300,1500,3000]\n\
...
- id: in_conserved_genes_finding
  doc: "Count conserved orthologs using BUSCO (only on Linux)\n--operons  <filename>\
    ...
    \ TXT)\n--est-ref-size <int>              Estimated reference size (for computing\
    \ NGx metrics without a reference)\n--contig-thresholds <int,int,...> Comma-separated\
