...
inputs:
- id: in_bam
  doc: "BAM/CRAM file (expects co-located index)\n- if CRAM see '-ref'"
  type: boolean?
  inputBinding:
    prefix: -bam
- id: in_output
  ...
  inputBinding:
    prefix: -output
- id: in_loci
  doc: "Alternate loci file (just needs chr pos)\n- output is different, counts for\
    \ each residue"
  type: File?
  inputBinding:
    prefix: -loci
...
- id: in_gender
  ...
  inputBinding:
    prefix: -gender
- id: in_snp_six
  doc: "flag, presence indicates loci file is SNP6 format.\n- cannot be used with\
