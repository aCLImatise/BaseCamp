...
task Design.py {
  input {
    ...
    String? cluster_and_design_separately
    String? cluster_from_fragments
    String? filter_with_lsh_hamming
    String? filter_with_lsh_min_hash
    Boolean? cover_groupings_separately
    String? small_seq_skip
    String? small_seq_min
  ...
  command <<<
    design.py \
      ...
      ~{true="--cover-groupings-separately" false="" cover_groupings_separately} \
      ...
      ~{true="--use-native-dict-when-finding-tolerant-coverage" false="" use_native_dict_when_finding_tolerant_coverage} \
  ...
  parameter_meta {
    ...
    expand_n: "[EXPAND_N] Expand each probe so that 'N' bases are replaced by real bases; for example, the probe 'ANA' would be replaced with the probes 'AAA', 'ATA', 'ACA', and 'AGA'; this is done combinatorially across all 'N' bases in a probe, and thus the number of new probes grows exponentially with the number of 'N' bases in a probe. If followed by a command- line argument (INT), this only expands at most INT randomly selected N bases, and the rest are replaced with random unambiguous bases (default INT is 3)."
    limit_target_genomes: "(Optional) Use only the first LIMIT_TARGET_GENOMES target genomes in the dataset"
    limit_target_genomes_randomly_with_replacement: "(Optional) Randomly select LIMIT_TARGET_GENOMES_RANDOMLY_WITH_REPLACMENT target genomes in the dataset with replacement"
    cluster_and_design_separately: "(Optional) If set, cluster all input sequences using their MinHash signatures, design probes separately on each cluster, and combine the resulting probes. This can significantly lower runtime and memory usage, but may lead to a suboptimal solution. The value CLUSTER_AND_DESIGN_SEPARATELY gives the inter-cluster distance threshold to merge clusters (1-ANI, where ANI is average nucleotide identity); higher values result in fewer clusters, and thus longer runtime. Values must be in (0,0.5], and generally should be around 0.1 or 0.2. When used, this creates a separate genome for each input sequence -- it collapses all sequences, across both groups and genomes, into one list of sequences in one group. Therefore, genomes will not be grouped as specified in the input and sequences will not be grouped by genome, and differential identification is not supported"
    cluster_from_fragments: "(Optional) If set, break all sequences into sequences of length CLUSTER_FROM_FRAGMENTS nt, and cluster these fragments. This can be useful for improving runtime on input with especially large genomes, in which probes for different fragments can be designed separately. Values should generally be around 10,000. For this to be used, --cluster-and-design-separately must also be set."
    filter_with_lsh_hamming: "(Optional) If set, filter candidate probes for near- duplicates using LSH with a family of hash functions that works with Hamming distance. FILTER_WITH_LSH_HAMMING gives the maximum Hamming distance at which to call near-duplicates; it should be commensurate with (but not greater than) MISMATCHES. Using this may significantly improve runtime and reduce memory usage by reducing the number of candidate probes to consider, but may lead to a slightly sub-optimal solution. It may also, particularly with relatively high values of FILTER_WITH_LSH_HAMMING, cause coverage obtained for each genome to be slightly less than the desired coverage (COVERAGE) when that desired coverage is the complete genome; it is recommended to also use --print-analysis or --write-analysis-to-tsv with this to see the coverage that is obtained."
    filter_with_lsh_min_hash: "(Optional) If set, filter candidate probes for near- duplicates using LSH with a MinHash family. FILTER_WITH_LSH_MINHASH gives the maximum Jaccard distance (1 minus Jaccard similarity) at which to call near-duplicates; the Jaccard similarity is calculated by treating each probe as a set of overlapping 10-mers. Its value should be commensurate with parameter values determining whether a probe hybridizes to a target sequence, but this can be difficult to measure compared to the input for --filter-with-lsh-hamming. However, this allows more sensitivity in near-duplicate detection than --filter- with-lsh-hamming (e.g., if near-duplicates should involve probes shifted relative to each other). The same caveats mentioned in help for --filter-with-lsh- hamming also apply here. Values of FILTER_WITH_LSH_MINHASH above ~0.7 may start to require significant memory and runtime for near- duplicate detection."
    cover_groupings_separately: "Run a separate instance of set cover with the target genomes from each grouping and pool (union) the resulting probes. When set, the software will run faster than when not set, but it may yield more probes than when it is not set."
    small_seq_skip: "(Optional) Do not create candidate probes from sequences whose length is <= SMALL_SEQ_SKIP. If set to (PROBE_LENGTH - 1), this avoids the error raised when sequences are less than the probe length"
    small_seq_min: "(Optional) If set, allow sequences as input that are shorter than PROBE_LENGTH (when not set, the program will error on such input). SMALL_SEQ_MIN is the minimum sequence length that should be accepted as input. When a sequence is less than PROBE_LENGTH, a candidate probe is created that is equal to the sequence; thus, the output probes may have different lengths. Note that, when this is set, it might be a good idea to also set LCF_THRES to be a value smaller than PROBE_LENGTH -- e.g., the length of the shortest input sequence; otherwise, when a probe of length p_l is mapped to a sequence of length s_l, then lcf_thres is treated as being min(LCF_THRES, p_l, s_l) so that a probe is able to 'cover' a sequence shorter than the probe and so that a probe shorter than lcf_thres is able to 'cover' a sequence"
