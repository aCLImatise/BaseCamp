...
id: ../../../../home/ubuntu/BiocondaCli/methylpy_single_end_pipeline.cwl
inputs:
- id: read_files
  doc: 'list of all the fastq files you would like to run through the pipeline. Note
    that globbing is supported here (i.e., you can use * in your paths) (default:
    None)'
  type: string[]
  inputBinding:
    prefix: --read-files
...
- id: merge_by_max_mapq
  doc: 'Boolean indicates whether to merge alignment results from two converted genomes
    by MAPQ score. Be default, we only keep reads that are mapped to only one of the
    two converted genomes. If this option is set to True, for a read that could be
    mapped to both converted genomes, the alignment that achieves larger MAPQ score
...
- id: adapter_seq
  doc: "sequence of an adapter that was ligated to the 3' end. The adapter itself\
    \ and anything that follows is trimmed. (default: AGATCGGAAGAGCACACGTCTG)"
  type: string
  inputBinding:
    prefix: --adapter-seq
