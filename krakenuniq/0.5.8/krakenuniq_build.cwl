...
- id: download_library
  doc: Download partial library (TYPE = one of "refseq/bacteria", "refseq/archaea",
    "refseq/viral").  Use krakenuniq-download for more options.
  type: string
  inputBinding:
...
- id: min_contig_size
  doc: Minimum contig size for inclusion in database. Use with draft genomes to reduce
    contamination, e.g. with values between 1000 and 10000.
  type: string
  inputBinding:
