...
task CramFilter {
  input {
    ...
    Int? limit_output_overlapping
    Boolean? drop_quality_strings
    String? discard_comma_separated
  ...
  command <<<
    cram_filter \
      ...
      ~{if defined(limit_output_overlapping) then ("-r " +  '"' + limit_output_overlapping + '"') else ""} \
      ~{if (drop_quality_strings) then "-q" else ""} \
      ~{if defined(discard_comma_separated) then ("-t " +  '"' + discard_comma_separated + '"') else ""} \
  ...
  parameter_meta {
    ...
    limit_output_overlapping: "Limit output to containers overlapping 'range'.\\n'-r chr1' matches all of chr1.\\n'-r chr1:1000' is equivalent to '-r chr1:1000-1000'."
    drop_quality_strings: "Drop quality strings (CRAM QS)."
    discard_comma_separated: "Discard comma separated list of tag types."
