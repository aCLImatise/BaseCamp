...
task HtsboxPileup {
  ...
  command <<<
    htsbox pileup \
      ...
      ~{if defined(ignore_queries_float) then ("-V " +  '"' + ignore_queries_float + '"') else ""} \
      ...
      ~{if (use_mark_deleted) then "-e" else ""} \
      ...
      ~{if defined(soft_mask_force) then ("-D " +  '"' + soft_mask_force + '"') else ""} \
  ...
  parameter_meta {
    ...
    ignore_queries_float: "ignore queries with per-base divergence >FLOAT [1]"
    ...
    soft_mask_force: "soft mask if sumQ > avgSum+FLOAT*sqrt(avgSum) (force -F) [3.00]"
