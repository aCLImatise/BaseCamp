...
task Segmentationfold {
  ...
  command <<<
    segmentation_fold \
      ...
      ~{if defined(number_threads_maximum) then ("-t " +  '"' + number_threads_maximum + '"') else ""} \
  ...
  parameter_meta {
    ...
    number_threads_maximum: "Number of threads; 0 = maximum available,      [0,N}\\ndefault: 3"
