...
task FunnelWorkerRun {
  input {
    ...
    String? http_storage_dot_timeout
    ...
    String? logger_dot_output_file
    String? mongo_db_dot_addrs
    String? mongo_db_dot_database
    String? mongo_db_dot_timeout
    String? node_dot_resources_dot_cpus
    Float? node_dot_resources_dot_disk_gb
    Float? node_dot_resources_do_tram_gb
    String? node_dot_timeout
    String? node_dot_update_rate
    String? rpcclient_dot_max_retries
    String? rpcclient_dot_server_address
    String? rpcclient_dot_timeout
    ...
    String? worker_dot_log_update_rate
    String? worker_dot_polling_rate
    String? worker_dot_workdir
    String? config
    String? flags
  }
  command <<<
    funnel worker run \
      ~{flags} \
      ~{if defined(task_base_six_four) then ("--taskBase64 " +  '"' + task_base_six_four + '"') else ""} \
      ~{if defined(task_file) then ("--taskFile " +  '"' + task_file + '"') else ""} \
      ~{if defined(task_id) then ("--taskID " +  '"' + task_id + '"') else ""} \
      ~{true="--AmazonS3.Disabled" false="" amazons_three_dot_disabled} \
      ...
      ~{true="--GoogleStorage.Disabled" false="" google_storage_dot_disabled} \
      ~{true="--HTTPStorage.Disabled" false="" http_storage_dot_disabled} \
      ...
      ~{true="--LocalStorage.Disabled" false="" local_storage_dot_disabled} \
      ...
      ~{true="--Swift.Disabled" false="" swift_dot_disabled} \
      ~{if defined(swift_dot_max_retries) then ("--Swift.MaxRetries " +  '"' + swift_dot_max_retries + '"') else ""} \
      ~{true="--Worker.LeaveWorkDir" false="" worker_dot_leave_workdir} \
  ...
  >>>
  parameter_meta {
    task_base_six_four: "Task base64"
    task_file: "Task file"
    task_id: "Task ID"
