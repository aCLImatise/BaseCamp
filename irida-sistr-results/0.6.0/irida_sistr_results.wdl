...
task IridaSistrResults {
  ...
  command <<<
    irida-sistr-results \
      ...
      ~{true="--all-projects" false="" all_projects} \
      ~{if defined(to_tab_file) then ("--to-tab-file " +  '"' + to_tab_file + '"') else ""} \
      ~{if defined(to_excel_file) then ("--to-excel-file " +  '"' + to_excel_file + '"') else ""} \
      ~{true="--include-user-results" false="" include_user_results} \
      ~{true="--exclude-user-existing-results" false="" exclude_user_existing_results} \
      ~{if defined(reportable_serovars_file) then ("--reportable-serovars-file " +  '"' + reportable_serovars_file + '"') else ""} \
      ~{true="--exclude-reportable-status" false="" exclude_reportable_status} \
  ...
  parameter_meta {
    ...
    reportable_serovars_file: "The reportable serovars file [/tmp/tmpsawj3icy/lib/python3.8/site-packages/irida_sistr_results/data/reportable_serovars.tsv]."
    ...
    config: "Configuration file for IRIDA (overrides values in ['/tmp/tmpsawj3icy/lib/python3.8/site-packages/irida_sistr_results/etc/config.ini', '/home/ubuntu/.local/share/irida-sistr-results/config.ini'])"
