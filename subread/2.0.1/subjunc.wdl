...
task Subjunc {
  ...
  command <<<
    subjunc \
      ...
      ~{true="--allJunctions" false="" all_junctions} \
  ...
  parameter_meta {
    ...
    consensus_threshold_reporting: "Consensus threshold for reporting a hit (minimal number of subreads that map in consensus) . If paired-end, this gives the consensus threshold for the anchor read (anchor read receives more votes than the other read in the same pair). 1 by default."
    ...
    all_junctions: "Detect exon-exon junctions (both canonical and non-canonical junctions) and structural variants in RNA-seq data. Refer to Users Guide for reporting of junctions and fusions."
